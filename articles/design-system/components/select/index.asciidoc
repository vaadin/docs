...
// tag::description[]
Select is an input field component that allows the user to choose a single value from a set of options presented in a dropdown list.
// end::description[]
...
You create a Select component with the `vaadin-select` element.
...
Items can be set with `setItems()`, which can take a comma-separated list of strings or a collection of objects.
...
You need to use `addComponent()` to add a `Hr` component after a specific item.
...
The `disabled` attribute of an item controls whether it is shown as disabled.
...
You need to pass `setItemEnabledProvider()` a provider that tells whether a given item is disabled or not.
...
Placeholder is defined in an optional `placeholder` attribute for the `vaadin-select` e lement.
...
Use `setPlaceholder()` to set the placeholder.
...
Notice that you can not style the contents of the overlay in a `styles()` method as usual, as that would apply to the Select component, not the overlay, which is separate in the DOM.
...
Use the <<../menu-bar#, Menu Bar>> component to create dropdowns for actions.
