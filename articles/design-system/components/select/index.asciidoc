...
Items can be set with `setItems()`, which can take a comma-separated list or a collection of objects.
...
Placeholder is defined in an optional `placeholder` attribute for the `vaadin-select` element.
