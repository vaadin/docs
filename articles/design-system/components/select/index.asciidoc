...
Items can be disabled making it impossible to select them.
It's commonly used to show that an option, which under different circumstances would be available for selection, is unavailable at the moment:
...
This can be useful to provide information in a more legible fashion than just appending it to the item text.
...
|<<../radio-button#,Radio Button>>|Better accessibility than Select, as all options are visible at the same time.
