...
Separators can be used to group related options. Use separators sparingly to avoid creating unnecessary visual clutter.
...
[NOTE]
====
Please note that for large data sets it is preferable to use ComboBox instead of Select, as it allows users to filter the list of options.
====

== Disabled Items

Items can be disabled making them unselectable. It's commonly used to indicate that an option, which under different circumstances would be available for selection, is currently unavailable:
...
Items can be rendered with rich content instead of plain text. This can be useful to provide additional information in a more legible fashion than simply appending it to the item text.
...
[source,typescript]
----
include::{root}/frontend/demo/component/select/select-custom-item.ts[render,tags=snippet,indent=0,group=TypeScript]
----

[source, java]
----
include::{root}/src/main/java/com/vaadin/demo/component/select/SelectCustomItem.java[render,tags=snippet,indent=0,group=Java]
----
--
...
|<<../radio-button#,Radio Button>>|Better accessibility than Select, as all options are immediately visible.

|<<../combo-box#,Combo Box>>|Filterable list of options. Appropriate for large sets of options.
