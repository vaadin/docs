...
Items can be disabled making them unselectable.
It's commonly used to indicate that an option, which under different circumstances would be available for selection, is currently unavailable:
...
[source, typescript]
----
include::{root}/frontend/demo/component/select/select-custom-item.ts[render,tags=snippet;renderer,indent=0,group=TypeScript]
----
