...
You can use the https://www.openssl.org/[openssl] command line tool to create a secret. 
...
Create a new package `com.example.application.security`. In the new package, create a configuration file and read in the secret key:
...
Replace this annotation with `@PermitAll` to allow access to all logged-in users. 
The security configuration documentation page lists all the <<{articles}/fusion/security/configuring/#other-security-options,other security options>>. 
