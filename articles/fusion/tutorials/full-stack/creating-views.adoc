...
Vaadin Fusion uses the https://lit-element.polymer-project.org/[LitElement^] library for building views. 
...
LitElement is a lightweight and performant library for building reactive components with declarative templates. 

If you are not familiar with LitElement, please read the <<../../application/lit-element#, LitElement basics article>>.
...
- Utility CSS classes like `flex` and `h-full` help you to create layouts without having to write custom CSS. 
The next step is to import the CSS helpers.

== Importing CSS Helper Classes to the Theme

The Vaadin Lumo theme comes with a https://github.com/vaadin/vaadin-lumo-styles/pull/103[proposed^] set of CSS utility classes to simplify layouts and styling. 
Instead of having to write your own CSS for common tasks like adding margins, padding, setting sizes, and defining layouts. 

The utility classes are not yet in the official framework, so you need to import them with `npm`. 
...
It is important to remember to call `super.connectedCallback()` whenever you override `connectedCallback()` to ensure that LitElement initializes the component. 
