...
While Lit discourages manual DOM operations in favor of binding data to the component template
declaratively using reactive properties, sometimes it might be convenient to get a reference to
the component instance and use it to call methods.
...
Certain Vaadin components, namely Grid, Combo Box, CRUD, and Virtual List, support setting `items` property as an array of objects.
