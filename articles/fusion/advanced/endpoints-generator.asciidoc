...
It can be nullable, but it cannot be omitted.
If you have that parameter and want it to be `null` in Java, you must send an `undefined` there during the function call.
...
For the data class, one thing can be nullable:
...
For the collection types, one thing can be nullable:
