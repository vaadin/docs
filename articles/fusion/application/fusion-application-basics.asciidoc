...
The core of a Fusion application is the application shell, which consists of the following files:
...
`frontend/index.ts`::
The TypeScript entry point for the application.
...
In order to write the bootstrapping code for an application or create views in TypeScript, make sure that:
...
In the following, is an example `index.ts` and `index.html` files.
The  that adds a button click listener to load the full Vaadin application lazily, after the button is clicked.
