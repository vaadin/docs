...
It is almost like a framework built into the browser, so in most cases it is faster and smaller than any other framework, while still shares their advantages.

However, since the in-browser implementation of the web components is low-level, to make them more convenient, it is recommended to use a LitElement library.

LitElement provides a simple base class, and a set of tools to create reactive web components.
Under the hood, it uses link:https://lit-html.polymer-project.org[lit-html], a tiny library to render and re-render HTML templates in the most efficient and declarative way.

== Minimal LitElement Component

=== Templates

A class that inherits the LitElement base class must implement a method called `render()` which returns a `TemplateResult` instance produced by the `html` tag.
...
A class that pretends to be a web component must be added to the component registry and assigned with the HTML element name.
With LitElement, you may do it with the `@customElement` decorator.
It receives an HTML element name which should contain at least one dash `-`.
For example, `my-view`, `x-button` or `vaadin-checkbox`.
...
Since the web component is a simple JavaScript class, you may define any number of fields, methods, and accessors you need and call them as you are used to.
...
Changing those properties triggers asynchronous re-render to fill the template with the new data.

LitElement provides two decorators that can transform a regular class field to a component data property.

* `@property` - makes a field a data property that is allowed to receive data from the outside.
...
* `@internalProperty` - makes a field a data property that contains an internal state of the component.
...
**NOTE**: You don't need to bind `this` to the event listener method.
LitElement calls any listener with the correct `this` by design.
...
Each component has lifecycle methods that may be overridden to execute some code at a specific time.
To the standard web component lifecycle, LitElement adds its own methods to handle the asynchronous `render` calls.

Here is the list of LitElement lifecycle methods.
More about them you can read in the link:https://lit-element.polymer-project.org/guide/lifecycle[LitElement documentation].
...
**NOTE**: when the element is created, no render happens.
...
**NOTE**: since `render` is asynchronous it does not over when the `connectedCallback` finishes.
Use <<first-updated,`firstUpdated`>> callback to execute the code after the first render.

**NOTE**: you must call `super.connectedCallback()` in your `connectedCallback` method.
...
**NOTE**: this callback is invoked __after__ the element is disconnected from the DOM.
You do not have access to the DOM from within the component in this callback.

**NOTE**: you must call `super.disconnectedCallback()` in your `disconnectedCallback` method.
Otherwise, you may lose the default LitElement lifecycle.
...
You may use either a stylesheet imported from the `.css` file or define it in the component directly using the `css` tagged function.

You may specify multiple styles for the `styles` property.
Just use the array instead of a direct assigning.
...
It is recommended to use Shadow DOM only for design components (checkboxes, text fields, combo-boxes, etc.) and Light DOM for any other components like views or their parts.
It would allow avoiding possible performance issues that might happen due to multiple nesting of shadow trees.
