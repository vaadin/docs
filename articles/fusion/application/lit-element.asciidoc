...
It is almost like a framework built into the browser, and usually faster and smaller than any other framework, while still sharing their advantages.

The LitElement library is convenient way of authoring web components, and recommended over the low-level native browser APIs.
...
A class that inherits the LitElement base class must implement the `render()` method and return a `TemplateResult` instance produced by the `html` tag.
...
Each component has lifecycle methods that you can overridde to execute code at a specific time.
