...
To handle logging out, you can use the `logout()` helper defined in `auth.ts` above. You typically want to use a button for handling logout and not navigation and a route to avoid timing issues between rendering views and logging out. You can do e.g.
