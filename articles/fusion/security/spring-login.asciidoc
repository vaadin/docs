...
When you add `logging.level.org.springframework.security = DEBUG` to the `application.properties` file, the username and password are shown in the console when the application starts up.
...
Public views need to be added to the configuration before calling [methodname]#super.configure()#, e.g.
...
It utilises a Fusion [methodname]#login()# helper method for Spring-Security-based authentication.
...
For example, [methodname]#VaadinWebSecurityConfigurerAdapter.configure(http)# requires all requests but to be authenticated, except the Vaadin internal ones.
...
In an analogous way, the matcher for static resources to be ignored is available as [methodname]#VaadinWebSecurityConfigurerAdapter.getDefaultWebSecurityIgnoreMatcher()#:
...
The next example shows how to configure authentication by using an LDAP repository.
