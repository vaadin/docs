...
The access control for Vaadin views must be based on the actual Vaadin views, and not only on the URL filtering.
To protect a view from unauthenticated requests, that is, redirect unauthenticated requests to the login view, you can use route action.
