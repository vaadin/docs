...
Roles are, in the nutshell, a set of string attributes representing authorities that are assigned for a user.
...
After successful authentication, the `GrantedAuthority` objects returned `Authentication.getAuthorities()` represent those.
See the <<spring-login#, Authentication With Spring Security>> article for the configuration examples.

== Using Roles in TypeScript

For using roles for access control of TypeScript views, it is convenient add a Fusion endpoint, that obtains user information with roles from Java during authentication.
...
Then, change the authentication implementation in TypeScript to get the user information from the endpoint and store it in the browser’s `localStorage`.
...
.`auth.ts`
----
include::{root}/frontend/demo/fusion/authentication/auth.ts[tags=timestamp-userinfo;]
----

The above also defines `isUserInRole` helper, which enables role-based access control checks for the UI.
