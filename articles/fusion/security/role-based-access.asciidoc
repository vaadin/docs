...
Typically, roles are stored as string authorities prefixed with `ROLE_`.
...
Change the `auth.ts` defined in <<spring-login#client-configuration, Authentication With Spring Security>> as follows:
