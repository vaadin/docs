...
----
+
In a https://start.vaadin.com/[Vaadin Start] generated project, we can skip the version number here because it has already been declared by the `<selenium.version>` property in the `pom.xml` file.
...
As an example, the code snippet below tests whether the loaded [classname]`Button` text matches "Say Hello".
...
The test below demonstrates how a long Selenium test might look like.
