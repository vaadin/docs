...
<3> We wrap the test in a `try/finally` block so that the session closes even when the test fails.
...
We can use Selenium's https://www.selenium.dev/selenium/docs/api/java/org/openqa/selenium/support/ui/WebDriverWait.html[WebDriverWait] API to wait until the DOM has been _compiled_.
