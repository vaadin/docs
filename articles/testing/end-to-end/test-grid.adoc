...
TestBench is based on Selenium and does not contain any modifications to the grid hub/node part.
...
[NOTE]
The communication protocol used by the grid is standardized, so it should not be critical to match the Selenium version that TestBench is based on with the version of the grid.
