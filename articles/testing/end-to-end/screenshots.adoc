...
To enable the feature, you need to include the `ScreenshotOnFailureRule` JUnit4 rule with a member variable annotated with `@Rule` in the test case or in case of JUnit5 `ScreenshotOnFailureExtensions` with `@RegisterExtension`, as follows:
