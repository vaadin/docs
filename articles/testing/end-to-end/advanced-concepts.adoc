...
Using JUnit 5 it is possible to access additional test information in a method annotated with `@Test`, `@BeforeEach`, `@AfterEach`, `@BeforeAll`,
or `@AfterAll` by adding `BrowserTestInfo` method parameter, like on example:
...
`BrowserTestInfo` contains information about:

- `WebDriver` and browser capabilities used for current test execution
- hostname of the hub for remote execution
