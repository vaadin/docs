...
TestBench supports parallel tests execution using own test runner (JUnit 4) or native link:https://junit.org/junit5/docs/current/user-guide/#writing-tests-parallel-execution[JUnit 5 parallel execution].
...
To run tests in parallel you need to extend TestBench utility class `BrowserTestBase` or manually annotate test class with `@Execution(ExecutionMode.CONCURRENT)`.

To disable parallel execution annotate class with `@Execution(ExecutionMode.SAME_THREAD)`.
