...
[NOTE]
When running on a hub, you don't need a local web driver installed. The web driver must be installed on the node instead.
...
[NOTE]
You can also handle creation and destruction of the web driver manually when running on a hub if you don't want to use `ParallelTest` for one reason or another.
