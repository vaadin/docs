...
* run tests in parallel (JUnit 4, in JUnit 5 it depends from JUnit configuration)
...
You can make use of automatic `WebDriver` and browser information injection by extending `BrowserTestBase` class.

Parallel tests execution is handled by JUnit 5, read more at link:https://junit.org/junit5/docs/current/user-guide/#writing-tests-parallel-execution[JUnit 5 parallel execution].
