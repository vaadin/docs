...
1. Let TestBench take care of creating and destroying driver instances as needed by annotating test with `@BrowserTest` (JUnit 5, supported since Vaadin 24.0) or extending `ParallelTest` instead of `TestBenchTestCase` (JUnit 4).
...
You'll also need a test class annotated with `@BrowserTest` (JUnit 5, supported since Vaadin 24.0) or extending `ParallelTest` (Junit4) automatically to do the following:
