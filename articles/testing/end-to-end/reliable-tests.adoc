...
[NOTE]
`@RetryRule` is not available for JUnit 5 tests. Please use native test runners retry features, like `link:https://maven.apache.org/surefire/maven-surefire-plugin/test-mojo.html#rerunFailingTestsCount[Maven Surefire rerunFailingTestsCount]` or `link:https://github.com/gradle/test-retry-gradle-plugin[Gradle test-retry plugin]`.
