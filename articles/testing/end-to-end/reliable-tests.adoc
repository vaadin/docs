...
Ideally, the test would start the browser, execute the actions and terminate the browser nicely &ndash; always. In practice, there is potential for network problems, especially when using a cloud-based browser provider. There can also be browser problems causing randomness, or even browser crashes.
