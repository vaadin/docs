...
Another example is when the theme should be applied to an embedded Flow application or component which is going to be shown in an overlay inside another applications, and since the overlay cannot access the styles from web component's shadow DOM, styles also must be added in [filename]#document.css#, too.
