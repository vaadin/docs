...
Another example is when the theme should be applied to an embedded Flow application or component which is going to be shown in an overlay inside another applications, and since the overlay cannot access the styles from web component's shadow DOM, those styles must be added in a [filename]#document.css#, too.
