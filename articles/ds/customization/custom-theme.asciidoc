...
1. The file being removed is a stylesheet referenced from `my-theme/styles.css`, or from other stylesheet in the theme folder.
In that case, to avoid re-compilation errors, please remove the stylesheet import first, and then remove the file.
2. The file being removed is a component stylesheet in `my-theme/components` folder, or a whole components folder.
The files inside components folder are referenced from auto-generated file `theme-my-theme.generated.js` and, thus, re-compilation fails, because Webpack cannot find them.
To avoid errors, please remove your component stylesheet simultaneously (in one go) with the theme generated file.

[NOTE]
When you encounter a “no such file or directory” error reported by Webpack and shown on the overlay in your app page, please click on the overlay (or refresh the browser page), and it should disappear, so you can continue your work with application and theming.
If it doesn't help, please restart your app.
