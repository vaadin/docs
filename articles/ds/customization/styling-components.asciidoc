...
For example, we can style the Input Container component when it is invalid, using the `+[invalid]+` state attribute selector.

.`Styles included into the local scope of <vaadin-input-container>`
[source,css]
----
:host([invalid]) {
  box-shadow: inset 0 0 0 1px var(--lumo-error-color);
...
The customizations we did for the Input Container component end up affecting other components as well, which internally use Input Container.
For example, Text Field, Combo Box, Date Picker, Time Picker and Select all contain an Input Container, and those components will automatically "inherit" our style customizations.
