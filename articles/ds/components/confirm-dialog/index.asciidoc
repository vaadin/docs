...
Confirm Dialog can be closed by clicking one of its buttons (all buttons close the dialog by default), or by pressing kbd:[Esc] (which will trigger the action associated with the Cancel button if one exists). Closing on kbd:[Esc] can be disabled by setting the noCloseOnEsc property to false.
