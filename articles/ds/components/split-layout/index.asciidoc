...
Alternatively, allow the user to choose which orientation they wish to use.
...
The splitter will respect the minimum and maximum size of the content area components.
...
This is useful when the user wants to quickly toggle between certain positions.
...
There are two theme variants: `small` and `minimal`.
...
The `minimal` theme variant removes the divider completely.
Both variants only show the split handle on hover and are therefore not ideal for touch devices.
