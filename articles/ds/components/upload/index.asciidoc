...
We provide two default implementations of `Receiver`:

* `MemoryBuffer` for handling a single file upload at once
* `MultiFileMemoryBuffer` for handling multiple file uploads at once
...
[source,java]
----
include::{root}/src/main/java/com/vaadin/demo/component/upload/UploadMultiFileMemoryBuffer.java[tags=snippet,indent=0]
----
--

== Request Configuration (Web Component Only)

.Web component only
[NOTE]
====
This section is only relevant when using the web component directly, or through Fusion.
The Java/Flow component has its own set of APIs to handle file data directly, without having to set up an endpoint or a servlet.
====

When uploading, the Upload component sends a request with the method type `POST`, and the content type `multipart/form-data`.
By default, the request URL is the current browser location.
...
<vaadin-upload
  ...
  headers='{ "X-API-KEY": "df784hf83hj8fh39dfh34" }'>
</vaadin-upload>
----
--
