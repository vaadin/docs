...
* *Icon-only* buttons should be primarily used for *extremely common recurring actions* with highly standardized, universally understood icons (for example: a cross for close), and for actions that are *repeated e.g. in lists and tables*.
...
* Button groups representing options, such as the buttons of a <<../confirm-dialog#,Confirm Dialog>>, should clearly state what each option represents, such as _"Save changes"_ instead of simply _"Yes"_ (as the latter forces the user to carefully read the question being asked, and significantly increases the risk of selecting the wrong option).
