...
Buttons representing actions athat are not currently available to the user should be either hidden or disabled.
...
[source,html]
----
include::{root}/frontend/demo/component/button/button-disable-on-click.ts[render,tags=snippet,indent=0,group=TypeScript]
----
...
Buttons can be set to automatically get keyboard focus when the UI they're in is rendered using the "autofocus" feature.
...
Buttons can be assigned keyboard shortcuts that are either *global*, which work regardless of where focus is currently, or *scoped* to a certain component or container.

// TODO: here goes an example

== Best Practises

=== Button Labels

* The label should clearly indicate the action, preferably using active verbs, such as _"View details"_ rather than just _"Details"_.
* In cases where there is a risk for ambiguity, also specify the object of the verb, such as _"Save changes"_ instead of simply _"Save"_.
* Button groups representing options, such as the buttons of a <<../confirm-dialog#,Confirm Dialog>>, should clearly state what each option represents, such as _"Save changes"_ instead of simply _"Yes"_ (as the latter forces the user to carefully read the question being asked, and significantly increases the ris of selecting the wrong option).
* Labels should be kept short, ideally less than three words or 25 characters.
* Use ellipsis (...) to indicate that an action is *not immediate* but will require an additional step to perform.
This is especially useful for destructive actions like _"Delete..."_ when a <<../confirm-dialog#,Confirm Dialog>> is used to confirm the action before it's executed.

=== ARIA Labels

The `aria-label` attribute can be used to provide a separate label for accessibility technologies (AT) like screen readers.
This is especially important for icon-only buttons that lack a visible label.

Buttons with regular, visible labels can also benefit from separate `aria-label`s to provide additional context that may otherwise be difficult for the AT user to perceive. In the example below, the buttons’ `aria-label`s specify which email address will be removed.

// TODO: here goes an example of buttons with labels

=== Buttons in Forms

// TODO: here goes an example of buttons in a form
...
// TODO: here goes an example of buttons in a Dialog. No code sample needed.
...
* *Dangerous actions* should be aligned left, to avoid accidental clicks, especially if no confrmation step is included.
...
// TODO: here goes an example of buttons related to a Grid. No code sample needed
...
|<<../menu-bar#,Menu Bar>>
|Dropdown menus with items that trigger actions.
Can also be used for single "*menu buttons*" and "*button groups*" without dropdown menus.
|===
