...
Editable cells are indicated with a hover effect by default, but you can further help users distinguish these by highlighting either editable or read-only cells. This is recommended for grids with both editable and read-only cells.

=== Highlight editable cells

Editable cells can be highlighted by applying the `highlight-editable-cells` theme variant.

// Add highlight-editable-cells sample here
[.example]
--
...
// Add highlight-read-only-cells sample here
[.example]
--
...
You can also apply custom styling to either by targeting the `editable-cell` part name in CSS.
