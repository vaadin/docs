...
// tag::description[]
Badges are coloured text elements containing small bits of information. They are used for labelling content, displaying metadata and/or highlighting information.
// end::description[]
...
Badges should generally always contain a label. Labels should be clear, concise and written using sentence case.
Aim for 1-2 words.
...
[NOTE]
The benefit of icons in addition to a label should be weighed against the additional visual noise it creates.
...
You can combine any theme variants freely.

=== Size

There are two different sizes you can use for badges: the default (normal) and `small`.
...
In addition, they can be paired with the `primary` theme variant for additional emphasis.
...
|
|Default style.
Recommended for informational messages. Note that this style may be confused with a Button or link.

|Success
|`Success`
|Highlight positive outcomes, such as when a task or operation is successfully completed.
...
If you’re using colors and icons to convey information, provide the same info via `aria-label` to ensure screen readers can interpret the info correctly.
...
Badges and Buttons are very similar in appearance. This might lead users to think badges are interactive.
