...
=== Individual Menu Items Themes

Individual menu item's appearance can be adjusted by applying <<../../customization/custom-theme#, custom theme>> to it.

To style root level item theme should be created for `vaadin-menu-bar-button`.

To style sub-menu item theme should be created for `vaadin-context-menu-item`.
