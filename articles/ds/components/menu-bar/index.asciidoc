...
As only one primary action should be presented in the same part of the UI, this should be used only for <<drop-down-and-combo-buttons#,dropdown button use cases>>.
...
[TIP]
Note that the standard Menu Button styles can be adjusted using <<../../foundation/color#,theme features>>, so these variants should be used only to differentiate special instances of the component.
...
See <<../../customization/custom-theme#vaadin-component-styles, Vaadin Component Variants>> for details of how to add style sheets for these components.
