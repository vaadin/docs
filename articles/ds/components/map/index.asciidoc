...
It is possible to replace the background layer. Custom layers can be added to the map, for example to display overlays.
...
For more fine-grained control of the layer rendering order, a `z-index` can be defined for each layer.
...
Invisible layers are simply not rendered.
...
Note that some map services already use a lowered opacity in the map images that they provide, meaning that lowering the opacity of the layer is not needed in this case.
...
[source,java]
----
include::{root}/src/main/java/com/vaadin/demo/component/map/MapLayers.java[render,tags=snippet,indent=0,group=Java]
----
