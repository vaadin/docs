...
The OpenStreetMap tile servers are not intended to be used as map service for custom applications.
Please read and respect the https://operations.osmfoundation.org/policies/tiles/[OpenStreetMap tile usage policy,window=_blank].
====
...
By default, map coordinates, such as the view's center, are specified in https://en.wikipedia.org/wiki/Web_Mercator_projection[EPSG:3857 / Web Mercator Projection, window=_blank], unless the map service uses a custom projection.
...
=== Using the Vaadin debug tool

1. Click on the Vaadin debug tool in your running application
