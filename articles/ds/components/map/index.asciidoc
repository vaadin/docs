...
Map provides various sources for loading map data in a specific format from a map service.
...
Markers can be displayed on top of a map to indicate something of note.
That can be a point of interest, a vehicle, a building, or any other entity.
A marker is defined by a coordinate and an icon.
Icons can be configured either as a URL or a xref:{articles}/flow/advanced/dynamic-content#using-streamresource[StreamResource].
