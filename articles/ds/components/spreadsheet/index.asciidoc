...
However, if you make modifications to the workbook, you must allow the spreadsheet to update itself by calling appropriate update methods for the modified element or elements.
...
You can create popup buttons for a row of cells in a cell range by defining a table, as described in link:https://vaadin.com/docs/v8/spreadsheet/spreadsheet-table#spreadsheet.table[Tables Within Spreadsheets].
...
You can implement custom context menus either by implementing an action handler or extending the `SpreadsheetDefaultActionHandler`.
