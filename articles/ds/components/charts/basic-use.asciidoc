...
The [classname]`Chart` class is a regular component, which you can add to a
layout.
You can give the chart type in the constructor or set it later in the
chart model.
...
<<charttypes#charts.charttypes,"Chart Types">>. The
main parts of a chart are illustrated in <<figure.charts.overview.elements>>. Styling a
chart is discussed in <<css-styling#charts.styling,"Chart Styling">>
...
Most methods available in the [classname]`Chart` object handle its basic
...
<<configuration#charts.configuration,"Chart Configuration">>.
...
<<configuration#charts.configuration.plotoptions,"Plot Options">>.
