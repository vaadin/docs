...
Styling a chart is discussed in <<css-styling#css.styling,"CSS Styling">>.
...
A new data series is added to a chart with the [methodname]`addSeries()` method.
...
The result for both regular and stacked column charts is shown in <<figure.charts.basic-use.two-dimensional>>.
