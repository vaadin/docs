...
You can customise the form’s title and labels using internationalization.
...
[NOTE]
====
Login is incompatible with password managers if placed inside another component’s footnote:[When added to a web component that uses shadow root, password managers are unable to find the input fields and thus not work properly.] shadow root.
...
Use it to create simple login pages (that are fullscreen on mobile devices) or to handle authentication without a dedicated login page.
...
In addition to the login form, the overlay features an additional header.
...
The header is fully customisable.
...
Additional information can also be provided to the user, for example, by linking to a page with helpful material or by displaying contact information.
...
Customise the form’s title, input field and button labels.
...
You can add a listener to login events and/or define an action for which a POST request will be fired.
