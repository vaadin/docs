...
Cells can be focused by clicking on a cell or using the keyboard.
Using the keyboard the following shortcuts are available:

* +++<kbd>Tab</kbd>+++ - Switches focus between sections of the grid (header, body, footer)
* +++<kbd>←</kbd>+++, +++<kbd>↑</kbd>+++, +++<kbd>→</kbd>+++, +++<kbd>↓</kbd>+++ - Moves focus between cells within a section of the grid
* +++<kbd>Page Up</kbd>+++ - Moves cell focus up by one page of visible rows
* +++<kbd>Page Down</kbd>+++ - Moves cell focus down by one page of visible rows
* +++<kbd>Home</kbd>+++ - Moves focus to the first cell in a row
* +++<kbd>End</kbd>+++ - Moves focus to the last cell in a row

The cell focus event can be used to get notified when the user changes focus between cells.

[NOTE]
The focus outline will only be visible when using keyboard navigation.
