...
[role="since:com.vaadin:vaadin@V23.1"]#Freezing columns at the end of the grid# is useful, for example, for keeping row actions always visible.
...
While it is technically possible to freeze any column, this feature should primarily be used to freeze columns in the start or end of the grid, leaving remaining columns between them unfrozen.
