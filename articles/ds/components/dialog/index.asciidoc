...
[IMPORTANT]
The typescript examples in this page use an inline `.renderer` function, which is encapsulated by the `guard` directive in Lit. This is done for performance reasons. For more details on the `guard` directive, please refer to the https://lit.dev/docs/templates/directives/#guard[official documentation].
