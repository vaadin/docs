...
Modal Dialogs don’t benefit from being draggable as their modality curtain (the
dark overlay behind the dialog) obscures the underlying user interface.
...
You can choose whether or not to make the component’s content draggable as well,
...
[source, html]
----
include::{root}/frontend/demo/component/dialog/dialog-drag-handles.ts[render, tags=*, indent=0, group=TypeScript]
----
...
== Scrollable Dialog
Dialogs become scrollable when their content overflows.

See an example of how to create a scrollable Dialog with a fixed header and footer in the Scrollable https://cookbook.vaadin.com/scrollable-dialog[Dialog Cookbook] example.
