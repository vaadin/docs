...
A non-modal dialog, on the other hand, does not block interaction.
...
For example, a Dialog for taking notes or for adding widgets to a dashboard using drag and drop can offer a better experience by allowing the user to move the Dialog around.
...
Dialogs containing dynamic content and/or a lot of information, such as complex forms or Grids, may benefit from being resizable, as this offers the user some flexibility as to how much data is visible at once.
...
Do not use them to communicate non-essential information, such as success messages like “Logged in”, “Copied”, and so on.
...
Due to the <<../../../flow/advanced/server-side-modality#server-side-modality,server-side modality mechanism in Flow>>, this means that closing a modal Dialog automatically also closes all its children, that is, any subsequent Dialogs opened after the modal.
