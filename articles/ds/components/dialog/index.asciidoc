...
[IMPORTANT]
The TypeScript examples in this article use an inline `.renderer` function, which is encapsulated by the `guard` directive in Lit.
The examples do so for performance reasons.
For more details on the `guard` directive, refer to the https://lit.dev/docs/templates/directives/#guard[official Lit documentation].
