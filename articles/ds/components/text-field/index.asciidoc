...
In data entry forms they’re usually unnecessary, however.
...
.NOTE
NOTE: <<../number-field#,Number Field>> is usually a better choice for numeric fields like the examples above
...
In cases where the length requirements may not be obvious to the user, it is recommended to provide this information e.g. using a <<../input-fields#helper,helper>>.
...
In cases where the format requirements may not be obvious to the user, it is recommended to provide this information e.g. using a <<../input-fields#helper,helper>>.
...
.NOTE
NOTE: Size variants should only be used in special cases. See <<../../foundation/size-space#,Size and Space>> for details on how to change the default text field size.

== Autoselect

When a field is set to autoselect its contents are selected when the field is focused. Use autoselect when the user is likely to want to replace the entire value rather than tweak it.
...
*Consequent keyboard navigations result in the contents being selected until the selection is changed arrow navigation or mouse click.
