...
When a field is set to auto-select its contents are selected when the field is focused.
Use auto-select when the user is likely to want to replace the entire value rather than tweak it.

[options="header"]
|===
|Auto-select |Focused with |Result
