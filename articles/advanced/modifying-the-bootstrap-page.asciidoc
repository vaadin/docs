...
The servlet processes the template and injects the following information:
  ...
  It uses a pre-configured https://vitejs.dev/[Vite^] instance that is included together with `vaadin-maven-plugin` as a module bundler.
  Therefore, the [filename]#frontend/index.html# template does not need to include the [filename]#index.ts# (or [filename]#index.js#) script explicitly.
...
Webpack can be enabled using its associated feature flag. Learn more about how to enable it <<{articles}/configuration/live-relaod/index/#webpack-feature-flag, here>>.
