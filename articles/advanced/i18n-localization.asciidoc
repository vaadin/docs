...
The Spring add-on will automatically use it if it's available.
...
If that's empty, [methodname]`Locale.getDefault()` will be used.

[[provider-sample-for-translation]]
== Example of Using I18NProvider for Translation

For this example, we enable the use of Finnish and English, with Finnish being the *"default"* that's used if the user client doesn't specify English as an accepted language.
