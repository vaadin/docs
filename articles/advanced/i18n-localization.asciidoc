...
When <<Using Localization in the Application, calling for translation>>, or the I18NProvider is gotten for the first time, the folder resource `vaadin-i18n` will be checked if it contains any `translations.properties` or `translations_[langcode].properties` files.
