...
To use a custom I18N provider with more features than provided by the default provider, the application needs only to implement [interfacename]`I18NProvider` and define the fully qualified class name in the property `i18n.provider`.
...
Continuing from the previous examples, imagine that your application has now also been translated into a right-to-left Language, such as Arabic. As well as <<provider-sample-for-translation,following the I18NProvider example>>, in your main layout you can add code such as the following:
...
== Adding Right-to-Left Support to Custom Elements & Applications
...
First, if your element extends Vaadin's [classname]`ElementMixin`, no changes are needed.
