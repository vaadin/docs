...
- [methodname]`getOrCreate(Class<T>)` should always return a initialized class of type T where the class returned is a initialized one from the current scope or generate a new bean instance or if not a bean handle field injection.
- [methodname]`createComponent(Class<T>)` should always create a new component with field injection.

As an example the simplified instantiator for cdi could be as follows
