...
In the [classname]`DefaultErrorHandler` there is the possibility to enable transitioning to an [interfacename]`HasErrorParameter<T extends Exception>` error view on exceptions by setting the `enableErrorHandlerRedirect` parameter to `true`.

For more information on the [interfacename]`HasErrorParameter` see <<{articles}/routing/exceptions#, Error Resolving>>.

For a customized error handler the same can be done by using the [classname]`ErrorHandlerUtil` method [methodname]`handleErrorByRedirectingToErrorView`
...
Calling [methodname]`handleErrorByRedirectingToErrorView` will show the error view with the exact matching exception, so for [classname]`AccountingException` only the exact match `MyAccountingException implements HasErrorParameter<AccountingException>` will be accepted and rendered.
