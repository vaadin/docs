...
The TypeScript generator produces TypeScript files based on the information from an OpenAPI document that is generated from Java files in the `_src/main/java_` folder by default.
...
It is parsed as `_/<endpoint name>/<method name>_`, which is used as a parameter to call to Java endpoints in the backend.
...
This means that a method that does not have the `application/json` content type is considered to be one with no parameters.
====
...
[NOTE]
====
Although multiple tags do not break the generator, it might be confusing at development time if there are two identical methods in different TypeScript files.
