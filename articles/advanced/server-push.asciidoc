...
For example, you can use the following [classname]`SpringBootServletInitializer` class, which implements the [annotationname]`AppShellConfigurator` interface, to enable Push in your application by adding the [annotationname]`@Push` annotation to it.
