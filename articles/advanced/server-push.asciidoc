...
On server side the push endpoint will be mapped to `VAADIN/push`. This mapping will be added either under context root, context path, or the first url mapping of the Vaadin servlet, depending on application deployment configuration.
