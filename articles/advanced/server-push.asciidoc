...
On server side the push endpoint is mapped to `VAADIN/push`. This mapping will be added either under context root, context path, or the first URL mapping of the Vaadin servlet, depending on application deployment configuration.
