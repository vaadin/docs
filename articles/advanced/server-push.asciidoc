...
It's based on a client-server connection which the client establishes and the server can then use to send updates to the client.
...
It's also possible to configure the URL to use for push requests by setting the `pushURL` parameter.
