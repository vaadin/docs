...
With the Vaadin component <<../components/progress-bar#, ProgressBar>>, you can easily provide an indicator that a long running action is currently in progress.
...
<2> After configuring the [classname]`ProgressBar`, we need to set it to invisible.
<3> When the button is clicked, we show the [classname]`ProgressBar`.
...
image::images/async-progress-bar.gif[Long Running ProgressBar]
