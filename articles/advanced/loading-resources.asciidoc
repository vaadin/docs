...
|Static files such as images, icons (such as the favicon) and documents, directly requested from the browser
...
[[foot-2]]
. When using `Anchor` to provide a downloadable resource, the `router-ignore` attribute is required.
+
[source,java]
----
...
Components such as [classname]`Image` and [classname]`Anchor` allows to display or download dynamic content by providing a [classname]`StreamResource`.
The contents are fetched by an instance of [interfacename]`InputStreamFactory`, that is passed to the [classname]`StreamResource` as a constructor parameter.
The [interfacename]`InputStreamFactory` requires to implement the [methodname]`createInputStream()` method, and can be used to load for example images and documents shipped along the application.
Resources can be loaded from application classpath or from the web application.

The classpath resources are stored in the `/src/main/resources` folder and ca be loaded using [methodname]`Class.getResourceAsStream(name)` or [methodname]`ClassLoader.getResourceAsStream(name)` methods. The name is always consider to be an absolute path for [methodname]`ClassLoader.getResourceAsStream(name)`, whereas it can be an absolute path, starting with a `/` or relative to the class package for [methodname]`Class.getResourceAsStream(name)`.
...
Web application resources can be loaded through [methodname]`VaadinService.getResourceAsStream(path)` or directly from [methodname]`ServletContext.getResourceAsStream(path)`.
The Servlet specifications state that the provided `path` should be absolute, but some Servlet container (for example Tomcat) allows to pass a path without the starting `/`.
...
+
The "eager" mode is usually suitable.
...
When you configure an element, for example setting the `src` attribute for an `<img>`, you can't use the `context://` protocol.
