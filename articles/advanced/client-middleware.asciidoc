...
description: In Hilla, middleware intercepts the request and response for every call, so it is able to process requests and their responses.
order: 140
layout: page
---
...
The middleware in Hilla is a special TypeScript async callback function that is executed by the frontend client during calls to the backend.
...
====
The Hilla middleware does not invent a new data structure to represent the network request and response, but uses the interfaces declared by the https://fetch.spec.whatwg.org[Fetch API specification] instead.
...
To avoid issues connected with this, it is better to avoid modifying `middlewares`, or to modify `middlewares` only before the first call.
