...
The routed component instance is then the same server-side object that was created in the first request, with all its state preserved (member fields, subcomponent hierarchy, and so on).
...
The method [methodname]`isRefreshEvent` can be used to distinguish a refresh of the view so any data updates can be skipped.
