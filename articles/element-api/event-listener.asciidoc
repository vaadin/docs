...
[TIP]
The `filter` and `debounce` settings in the `@DomEvent` annotation can also be set through the [classname]`DomListenerRegistration` object. See <<../create-ui/creating-components/events#,Using Events with Components>> for more.
