...
This tutorial shows you how to deploy a Vaadin application to Microsoft's Azure cloud using the traditional link:https://learn.microsoft.com/en-us/azure/app-service/overview/[App Service], that works well for most typical Vaadin applications.
For different kind of requirements, Azure has several other options to host Vaadin applications, such as raw virtual machines, Container Apps, that we use in link:https://vaadin.com/docs/latest/tutorial/production-deployment[the tutorial], and a great link:https://vaadin.com/clustering[Kubernetes support].
