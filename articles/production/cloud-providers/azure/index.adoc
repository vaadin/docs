...
This tutorial shows you how to deploy a Vaadin application to Microsoft's Azure cloud using the traditional https://learn.microsoft.com/en-us/azure/app-service/overview/[App Service], that works well for most typical Vaadin applications. If you have specific requirements, Azure has several other options to host Vaadin applications, such as raw virtual machines, and Container Apps (which are used in link:{articles}/tutorial/production-deployment[the tutorial]). Azure also has great https://vaadin.com/clustering[Kubernetes support].
