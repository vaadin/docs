...
== Step 6 _(Optional)_: Override the Java Runtime Version

Google App Engine defaults to use the Java 11 runtime, so this step is only required if your Vaadin project needs to run on a different Java version.

[loweralpha]
. Create a directory called `appengine` under `src/main`.
. Create an empty file called `app.yaml` under `src/main/appengine`.
...
----
[NOTE]
You can change the version number (`17`) to a version number of your choosing.

== Step 7: Deploy to Google App Engine

You can build and deploy using a single command below:
