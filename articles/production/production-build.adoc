...
[NOTE]
The pre-compiled production bundle is not the same as a pre-compiled front-end bundle for development mode as it has debugging features enabled.

[NOTE]
The output from an optimized production bundle will not be used as a pre-compiled bundle for new later production builds.
		...
		<build>
			<plugins>
                <plugin>
                    ...
                    <configuration>
                        <!-- To always force an optimized production bundle set this configuration to 'true' -->
