...
By default, Vaadin uses `npm install` to install front-end packages.
This can be changed to `npm ci` by setting configuration parameter:
...
In this case Vaadin interrupts package installation if it spots any mismatch of package versions in `package.json` and `package-lock.json` and recommends to re-run `npm install`.
This makes the production builds in the CI pipelines be reproducible and spot the problems in advance.
...
Additionally, it reviews all resources used by the application and copies them under the `node_modules` folder, so they're available when `vite` builds the front-end. It also creates or updates the [filename]`package.json`, [filename]`vite.config.ts` and [filename]`vite.generated.ts` files.
...
Executing the `clean-frontend` goal removes the package lock file, the generated front-end folder (by default `frontend/generated`), and the `node_modules` folder, which might need to be deleted manually.
