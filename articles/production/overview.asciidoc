...
By default, Hilla applications are set up to run in the development mode.
...
The main difference between the development and production modes is that in the development mode Hilla uses webpack to serve JavaScript files to the browser, instead of the Java server the app is running on.
...
The `pom.xml` file in a Hilla project has the following built-in Maven configuration for creating a production mode build:
