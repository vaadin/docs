...
Due to the https://vaadin.com/docs/advanced/server-side-modality#server-side-modality[server-side modality mechanism in Flow], this means that closing a modal Dialog automatically also closes all its children, that is, any subsequent Dialogs opened after the modal.
