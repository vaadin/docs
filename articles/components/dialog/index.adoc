...
By default, the edges of dialogs can be dragged outside the viewport, which can make parts of a dialog inaccessible. To prevent this, set the `keepInViewport` property to `true` to keep the dialog fully within the viewport when dragging.
