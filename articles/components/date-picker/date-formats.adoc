...
Additionally, Date Picker allows you to provide a server-side parser that is used as a fallback when user input cannot be parsed using methods described in the previous sections. The parser is a function that receives the user-entered string and returns either a `Result<LocalDate>` object or an error message. The parsed date is then displayed like any other dates, using the date format pattern that was configured in the previous sections.

The server-side parser can be used, for example, to support shortcuts for quickly entering dates, such as `+1` or `tomorrow`. In the following example, try entering `+1`, `-2`, and `foobar` to see the server-side parser in action:
