...
Additionally, Date Picker allows you to provide a server-side parser that is used as a fallback when user input cannot be parsed using methods described in the previous sections. The parser is a function that receives the user-entered string and returns either a parsed date or an error message. When succesful, the parsed date is displayed like any other dates, using the date format pattern that was configured in the previous sections.
