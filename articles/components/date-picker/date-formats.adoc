...
By default, Date Picker displays and parses dates using the user's locale (<<{articles}/flow/advanced/i18n-localization#locale-selection-for-new-session,reference>>). Alternatively, setting a specific locale ensures that all users see the same format. 
