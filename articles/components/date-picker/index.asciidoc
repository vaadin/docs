...
Try clicking below on the small gray icon of a calendar. A calendar overlay will appear, spanning two months around the current date. Click on a date to see how it reacts.
...
Date Picker supports date formats with 2-digit years (such as `dd.MM.yy`). When parsing a 2-digit year, the component must decide as to which century to parse the year. For example, a year entered as `62` could be interpreted either as 1862, 1962, or 2062.
...
There are various ways you might want to validate a date entered or selected. For instance, you may want to allow only dates in a certain range, or you might not want to allow certain dates. Below are some of your options for date validation.
...
Date Picker supports custom validation, such as limiting the options to Monday through Friday. In the example below, try to select a date that's on a Sunday or a Saturday. See how it reacts.
...
Use this feature to minimize the need for unnecessary navigation or scrolling when the user's input is expected to be within a certain time. Click on the example below and notice how it has a pulsating, rounded box around a particular date in the future, at the end of a quarter.
...
You can create a date range picker using the Date Pickers twice. Imagine the example below is for an airline ticket booking page. It's asking the user for the date they want to depart and when they want to return. Try it: click on the calendar icon for the departure and select a date.  Then click on the icon for the return date. Notice how dates prior to the departure date you chose are now disabled.
...
Instead of a Date Picker, you can use individual input fields for day, month, and year, to improve usability on small touch devices. In the example below, click on the year box and start to type a year. As you enter each digit, notice the list of choices narrows in the pull-down list.
