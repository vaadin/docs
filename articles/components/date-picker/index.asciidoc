...
By default, the date picker displays and parses dates using the user's locale (<<{articles}/advanced/i18n-localization.asciidoc#locale-selection-for-new-session,reference>>).
...
The date picker allows you to configure a custom date format pattern that defines exactly how dates should be displayed and how user input should be parsed.
...
The Date Picker supports date formats with 2-digit years (such as `dd.MM.yy`).
...
From Vaadin 23.3 onward, 2-digit years are interpreted as being within a 100-year range whose midpoint is called the _reference date_.
As an example, a reference date of 2000-01-01 means that the date entered is interpreted as being within the range 1950-01-01 – 2049-12-31, so the year `50` is parsed to 1950 while `39` is parsed to 2049.

The reference date defaults to the current date, but can set to a different date through an <<datepicker-i18n,i18n>> property:
