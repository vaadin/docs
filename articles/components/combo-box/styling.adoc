...
The item elements are placed inside the Combo Box element, so a class name on the Combo Box can be used to style the items of that instance only. The following example reduces the spacing between items in both Aura and Lumo:

[source,css]
----
vaadin-combo-box.compact vaadin-combo-box-item {
  min-height: 0;
  padding-block: 2px;
}
----
