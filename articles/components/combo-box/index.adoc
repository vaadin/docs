...
By default, typing in the filter only selects an item on an exact match. If the filter matches an item's label partially, the user still has to pick that item manually, for example with the kbd:[Down] arrow key. Partial match handling removes this extra step: the component picks the best matching item automatically, so users can fill in long forms by typing a few characters and moving on.

While the user types, the item that will be selected is highlighted in the overlay. The selection is then applied when the value is committed with kbd:[Enter] or kbd:[Tab], or when the field loses focus.

Two modes are available: `first-match` selects the first item in the filtered results, whereas `only-match` selects an item only when the filter narrows the results down to a single item. Matching is case-insensitive, and an exact match always wins over partial matches, regardless of the mode.

By default, an item is automatically set to be selected only when the typed filter matches its label exactly: to commit a partial match, the user still has to select it, for example with the kbd:[Down] arrow key. The component can be configured to automatically set an item to be selected even when the filter only partially matches its label. The item that will be selected is highlighted in the overlay while the user types, and the selection is applied when the value is committed with kbd:[Enter] or kbd:[Tab], or when the field loses focus. This lets users fill in long forms by typing a few characters and moving on, without reaching for the arrow keys.

Two modes are available: `first-match` sets the first item in the filtered results to be selected, whereas `only-match` sets an item to be selected only when the filter narrows the results down to a single item. Matching is case-insensitive, and an exact match is always set to be selected and takes precedence over partial matches, regardless of the mode.
...
[source,html]
----
include::{root}/frontend/demo/component/combobox/combo-box-partial-match-mode.ts[render,tags=snippet,indent=0,group=Lit]
----
endif::[]
...
[source,java]
----
include::{root}/src/main/java/com/vaadin/demo/component/combobox/ComboBoxPartialMatchMode.java[render,tags=snippet,indent=0,group=Flow]
----
endif::[]
...
[source,tsx]
----
include::{root}/frontend/demo/component/combobox/react/combo-box-partial-match-mode.tsx[render,tags=snippet,indent=0,group=React]
----
endif::[]
--

[NOTE]
A partial match isn't applied when <<#custom-value-entry,custom values>> are allowed, since a partial match is also a valid custom value. It's also only applied while the overlay is open: with <<#auto-open,auto-open>> disabled, typing alone doesn't highlight a match or set it to be selected until the overlay is opened.
...
By default, the overlay always opens at the top of the list. For long lists with the selected item far down, this makes it hard to see what's selected without scrolling. To address this, you can enable [methodname]`setFocusSelectedItem(true)` so the overlay opens scrolled to and focused on the selected item instead.

[.example,themes="lumo,aura"]
--
[source,typescript]
----
include::{root}/frontend/demo/component/combobox/combo-box-basic.ts[preimport,hidden]
----

[source,java]
----
include::{root}/src/main/java/com/vaadin/demo/component/combobox/ComboBoxFocusSelectedItem.java[render,tags=snippet,indent=0]
----
--

For <<#lazy-loading,lazy data providers>>, the lazy data view must also have an [classname]`ItemIndexProvider` configured so the selected item's index can be resolved against the current sorting and filter. In-memory data works out of the box.
...
[source,java]
----
include::{root}/src/main/java/com/vaadin/demo/component/combobox/ComboBoxFocusSelectedItemLazy.java[render,tags=snippet,indent=0]
----
--

See <<../../flow/binding-data/data-provider#callback-for-an-item-index, Callback for an Item Index>> for more on [classname]`ItemIndexProvider`.
