...
Pressing kbd:[Enter] or blurring the field (for example, by using kbd:[Tab]) while filtering commits an item only when the typed filter matches its full label. If the filter matches the label only partially, the item has to be focused in the overlay first, for example with the kbd:[Down] arrow key.
