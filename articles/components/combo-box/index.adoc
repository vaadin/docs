...
== Auto Focus Partial Match

By default, typing a filter only narrows down the list of options: the user still has to select an item, for example with the kbd:[Down] arrow key, before committing it. The component can be configured to automatically focus an item whose label partially matches the typed filter. The focused item is highlighted in the overlay while the user types, and is selected when the value is committed with kbd:[Enter] or kbd:[Tab], or when the field loses focus. This lets users fill in long forms by typing a few characters and moving on, without reaching for the arrow keys.

Two modes are available: `first-match` focuses the first item in the filtered results, whereas `only-match` focuses an item only when the filter narrows the results down to a single item. Matching is case-insensitive, and an item whose label matches the filter exactly is always focused, regardless of the mode.
...
[source,html]
----
include::{root}/frontend/demo/component/combobox/combo-box-auto-focus-partial-match.ts[render,tags=snippet,indent=0,group=Lit]
----
endif::[]
...
[source,java]
----
include::{root}/src/main/java/com/vaadin/demo/component/combobox/ComboBoxAutoFocusPartialMatch.java[render,tags=snippet,indent=0,group=Flow]
----
endif::[]
...
[source,tsx]
----
include::{root}/frontend/demo/component/combobox/react/combo-box-auto-focus-partial-match.tsx[render,tags=snippet,indent=0,group=React]
----
endif::[]
--

[NOTE]
A partial match isn't focused when <<#custom-value-entry,custom values>> are allowed, since the typed text could be intended as a new value. It's also not focused while the overlay is closed: with <<#auto-open,auto-open>> disabled, typing alone doesn't focus or select a match until the overlay is opened.
