...
Pressing kbd:[Enter] or blurring the field (for example, by pressing kbd:[Tab]) while filtering commits an item only when the typed filter matches its full label. If the filter matches the label only partially, the item has to be focused in the overlay first, for example with the kbd:[Down] arrow key.

To skip this step and speed up data entry, Combo Box can be configured to also commit partially matching items using the [methodname]`setPartialMatchMode` method. This method supports the following modes:
...
|`NONE`
|Commits an item only when the filter matches its label exactly (default).

|`FIRST_MATCH`
|Commits the first item in the filtered results.

|`ONLY_MATCH`
|Commits the item only when it's the only one left in the filtered results.
|===
...
The following example uses the `FIRST_MATCH` mode. Try typing `gre` and pressing kbd:[Enter]: this commits _Greece_, the first matching country, even though _Greenland_ and _Grenada_ also match the filter.
