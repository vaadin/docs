...
By default, an item can be committed with kbd:[Enter] only after typing out its full label. If the filter matches the label only partially, the item has to be focused in the overlay first, for example with the kbd:[Down] arrow key.

To skip this step, Combo Box can be configured to also commit partially matching items using the [methodname]`setPartialMatchMode` method. This method supports the following modes:
