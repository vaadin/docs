...
By default, the overlay always opens at the top of the list. For long lists with the selected item far down, this makes it hard to see what's selected without scrolling. With [methodname]`setFocusSelectedItem(true)`, the overlay opens scrolled to and focused on the selected item instead.
