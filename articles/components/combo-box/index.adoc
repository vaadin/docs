...
For long item lists, opening the overlay at the top isn't always the most useful place to land. In Flow, a one-line opt-in scrolls to and focuses the selected item when the overlay opens. On the client side, the web component's imperative [methodname]`scrollToIndex` method achieves the same effect.

[role="since:com.vaadin:vaadin@V25.2"]
=== Focus Selected Item [badge-flow]#Flow#

The overlay can be configured to scroll to and focus the selected item when it opens. This is off by default. Opt in with [methodname]`setFocusSelectedItem(true)`.
...
The [methodname]`scrollToIndex(index)` method scrolls the overlay so the item at the given index is in view and focused. To replicate the Flow opt-in behavior on the client side, listen for the overlay to open and pass the index of the selected item. The call is safe to make before the overlay opens or while a data provider is loading -- it is queued and executed once the overlay is open and ready.
...
[source,typescript]
----
include::{root}/frontend/demo/component/combobox/combo-box-focus-selected-item.ts[render,tags=snippet,indent=0,group=Lit]
----
endif::[]
...
[source,tsx]
----
include::{root}/frontend/demo/component/combobox/react/combo-box-focus-selected-item.tsx[render,tags=snippet,indent=0,group=React]
----
endif::[]
--
