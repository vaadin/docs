...
layout: tabbed-page
title: Combo Box
meta-description: The Vaadin ComboxBox component is accessible, themable, and enterprise-grade. Made for Java and Typescript, use it with Flow or a web frontend of your choice.
page-links:
...
As the user is typing, the Combo Box component filters out the options that don't match. Once the correct value has been found, the user can use the Up/Down arrow keys to navigate the list and the Enter key to set the value, essentially using the Combo Box as an autocomplete field.
