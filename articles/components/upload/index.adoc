...
Upload also allows to drag & drop folders. This recursively uploads all files from the folder and its sub-folders. Note that the folder structure will be flattened and the names of uploaded files will not indicate in which folder they were in. Uploading folders is currently only supported when using drag & drop, but not when using the file picker.
