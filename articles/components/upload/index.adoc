...
In order to conserve resources the files themselves are not part of the component server state. This has significant implications in component behavior. Removing the Upload component from the layout will mean that files in browser memory will be lost. The same applies to refreshing of the page. Check more details about [file handling](https://vaadin.com/docs/latest/components/upload/file-handling) on a separate document page.

The UI including Upload component should take this into account. For example if refreshing of the page is expected, you can display a message to the user that files needs to be reselected if this was not the first attach.
