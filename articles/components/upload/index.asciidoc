...
The Java/Flow component has its own set of APIs to handle file data directly, and does not require the request to be handled manually.
====
...
Exceptions that arise from the user violating any of the imposed restrictions are not shown in the UI by default.
...
By default, Upload does not limit the number of files that can be uploaded.
If you set the maximum to one, the native file browser prevents multiple files from being selected.

.Java Flow-specific
[NOTE]
====
When using a [classname]`Receiver` that does not implement the [interfacename]`MultiFileReceiver` interface, such as [classname]`MemoryBuffer` or [classname]`FileBuffer`, the Upload component automatically limits the number of files to one.
...
|_File Rejected_
|Sent when the file selected for upload does not meet the constraints, for example file size
...
For example, if the user is to upload a single PDF file, it is better to have the button label say "Upload PDF&hellip;" instead of "Upload File&hellip;".
