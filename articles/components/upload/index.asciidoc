...
[NOTE]
====
File format restrictions set with [methodname]`setAcceptedFileType` are checked on the client side. Uploaded file's format is not restricted on the server side by default. Upload doesn't have API to restrict uploaded files by file format or content in the server side. It's however possible to implement application specific restriction also on the server side in one or more event listeners when restriction is required.
====
