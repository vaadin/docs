...
It shows the upload progression and status of each file. Files can be uploaded using the *Upload* button or by dragging them onto the component.
...
|Handles multiple file uploads at once. For each, saves a file on the system.
...
For more advanced use cases, you can provide custom implementations for [classname]`Receiver` or [classname]`MultiFileReceiver`. You might do this, for example, to save files into a specific directory, or upload them to cloud storage.
...
Upload allows the user to drag files onto the component to upload them. Multiple files can be dropped simultaneously. By default, this is enabled on desktop and disabled on touch devices. Explicitly setting it to enabled or disabled, though, affects both desktop and mobile devices.
...
Exceptions that arise from the user violating any of the imposed restrictions aren't shown in the UI by default. Use a <<#listeners, File Rejected listener>> to catch those exceptions and, for example, a <<../notification#, Notification>> to inform the user of the problem, together with any potential solutions.

The user should be informed upfront, though, about any file upload restrictions. Limitations on the maximum number of files allowed, file size, and format should all be communicated clearly, to avoid exceptions.
...
.Revalidate Size Limit in Server
[NOTE]
====
...
When a file is queued (i.e., auto-upload disabled) it has a *Start* Button that the user must press to begin the upload process.
...
|_Finished_
|Sent when Upload receives a file, regardless of whether the upload was successful. To distinguish results, use either Succeeded or Failed listeners.
...
A "Server Unavailable" might suffice for tech-savvy users, but for some it might not be helpful. Error messages should be written with your users in mind.
