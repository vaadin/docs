...
The uploading of files may be handled either with Vaadin Flow using Java, or with Hilla using Lit and React.
...
The [classname]`MemoryBuffer` can handle a single file upload at once. It writes the file data into an in-memory buffer.
...
The [classname]`MultiFileMemoryBuffer` is the same as [classname]`MemoryBuffer`, but handles multiple file uploads at once. It writes the file data into a set of in-memory buffers.
...
The [classname]`FileBuffer` handles a single file upload at once. It saves the file on the system, in the current working directory of the Java application.
...
The [classname]`MultiFileBuffer` is the same as [classname]`FileBuffer`, but it can handle multiple file uploads at once. For each, it saves a file on the system, in the current working directory of the Java application.
