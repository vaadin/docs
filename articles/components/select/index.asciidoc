...
The drop-down can be opened with a click, up and down arrow keys, or by typing the initial character of one of the options.
...
For large data sets, it's preferable to use Combo Box instead of Select. It allows users to filter the list of options.
...
include::{articles}/components/_input-field-common-features.adoc[tags=basic-intro;label;helper;tooltip;clear-button;prefix]
