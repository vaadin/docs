...
The dropdown can be opened with a click, up/down arrow keys, or by typing the initial character of one of the options.
...
[NOTE]
For large data sets, it is preferable to use Combo Box instead of Select, as it allows users to filter the list of options.
...
Use the placeholder feature to provide an inline text prompt for the field.
Do not create, or use, a separate item for this purpose.
