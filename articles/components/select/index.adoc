...
The width of the overlay is, by default, the same width as the input field. The overlay width can be overridden to any fixed width in cases where the default width is too narrow.
...
Like other field components with overlays, Select provides dedicated API to set a CSS class name on the overlay for styling.
