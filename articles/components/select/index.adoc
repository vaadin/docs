...
By default, Select overlay overlaps the input-field when opened. In order to change this and make the drop-down menu look more like ComboBox, use `noVerticalOverlap` API.
...
Use the [methodname]`setOverlayWidth()` Java API or `--vaadin-select-overlay-width` custom CSS property on the web component to customize the overlay width.
...
Use the [methodname]`addOverlayClassName()` Java API or `overlayClass` web component property to set CSS class names on the select overlay for styling.
