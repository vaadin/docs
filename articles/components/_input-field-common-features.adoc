...
Note that prefix and suffix elements typically do not work well with assistive technologies like screen readers, so the information conveyed by them should also be conveyed through other means, such as in the <<#label,Label>>, a <<#helper,Helper>> or through ARIA attributes on the field itself.
====
...
Note that prefix elements typically do not work well with assistive technologies like screen readers, so the information conveyed by them should also be conveyed through other means, such as in the <<#label,Label>>, a <<#helper,Helper>> or through ARIA attributes on the field itself.
====
// end::prefix[]
...
Note that suffix elements typically do not work well with assistive technologies like screen readers, so the information conveyed by them should also be conveyed through other means, such as in the <<#label,Label>>, a <<#helper,Helper>> or through ARIA attributes on the field itself.
====
// end::prefix[]
