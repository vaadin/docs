...
[%collapsible]
====
The optional clear button, displayed when the field is not empty, clears the field's current value. Although the button itself is not keyboard focusable, the clear action can be triggered with the `Esc` key when the field has focus. Clear buttons can be especially useful in search and filter fields, where users often need to clear the value. They're less useful in regular forms.
====
// end::clear-button[]
