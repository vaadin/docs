...
[%collapsible]
====
The optional clear button, displayed when the field is not empty, clears the field's current value. Although the button itself is not keyboard focusable, the clear action can be triggered with the kbd:[Esc] key when the field has focus. Clear buttons can be especially useful in search and filter fields, where users often need to clear the value. They're less useful in regular forms.
====
// end::clear-button[]
...
// tag::aria-labels[]
[#aria-labels]
.[since:com.vaadin:vaadin@V24.1]#External and Invisible Labels (ARIA)#
[%collapsible]
====
...
[%collapsible]
====
The small variant can be used to make individual fields more compact. The default size of fields can be customized with <<{articles}/lumo/lumo-style-properties#,style properties>>.
====
//end::small-variant[]
