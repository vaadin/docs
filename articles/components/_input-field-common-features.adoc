...
Suffix elements typically don't work well with assistive technologies like screen readers, so the information conveyed by them should also be conveyed through other means, such as in the <<#label,Label>>, a <<#helper,Helper>> or through ARIA attributes on the field itself.
====
// end::suffix[]
...
Fields that are currently _unavailable_ should be *disabled*. The reduced contrast of disabled fields makes them inappropriate for displaying information. They canìt be focused or display tooltips. They're invisible to screen readers, and their values cannot be selected and copied.
...
[%collapsible]
====
The minimum and maximum value length constraints dictate the smallest and largest number of characters a field accepts. It limits text entry to the maximum length, and triggers a validation error if a value shorter than the minimum length is entered. They can be used to enforce specific formats, or to cap the value to the length supported by the underlying database schema.
