...
[%collapsible]
====
The placeholder is a text that is displayed when the field is empty. Its primary purpose is to provide a short input hint (e.g., the expected format) in cases where a <<#helper,Helper>> cannot be used.
...
[%collapsible]
====
The minimum and maximum input length value constraints dictate the smallest, and the largest number of characters a field accepts. It riggers a validation error if a value shorter than the minimum length is entered, and limits text entered to the maximum length. They can be used to enforce specific formats, or to cap the value to the length supported by the underlying database schema.
...
Right-alignment is recommended for numerical values when presented in vertical groups. It tends to aid interpretation and comparison of values.
====
//end::text-alignment[]
