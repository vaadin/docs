...
Also, MenuBar is an interactive component. Using other interactive components like ComboBox as menu items is not adviced as this may produce conflicts in keyboard navigation and interaction. Although items are children of MenuBar, the MenuBar is not supposed to act as layout component.
