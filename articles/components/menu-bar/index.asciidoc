...
[TIP]
Note that the standard Menu Button styles can be adjusted using <<{articles}/components/ds-resources/foundation/color#,theme features>>, so these variants should be used only to differentiate special instances of the component.

=== Styling Menu Items

An individual menu item can be styled using a <<{articles}/components/ds-resources/customization/component-variants#custom-variants, custom variant>>.
...
See <<{articles}/components/ds-resources/customization/custom-theme#vaadin-component-styles, Vaadin Component Variants>> for details of how to add style sheets for these components.
