...
Tooltips can be configured on menu items, including both top-level items and items nested inside sub-menus, to provide additional context, such as labels for icon-only menu buttons. Use the `tooltipPosition` property to control where the tooltip appears relative to its item. When an item is disabled, its tooltip isn't shown.
