...
Tooltips can be configured on any menu item -- top-level or nested in a sub-menu -- to provide additional information, especially for icon-only items. Use the `tooltipPosition` property to control where the tooltip appears relative to its item. When an item is disabled, its tooltip isn't shown.
