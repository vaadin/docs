...
Individual menu items can be styled by [since:com.vaadin:vaadin@V24.3]##applying custom class names to them##, and writing CSS style blocks targeting those class names. Note that root-level menu items in the Menu Bar are wrapped inside `vaadin-menu-bar-button` elements which inherit the class names from the items inside them.
...
[source,css]
----
include::{root}/frontend/themes/docs/menu-bar-styling.css[]
----
