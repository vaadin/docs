---
title: Login
page-title: Login component | Vaadin components
...
meta-description: Vaadin Login component contains a log-in form. You can use it to authenticate the user with a username and password.
page-links:
...
[source,html]
----
include::{root}/frontend/demo/component/login/login-event.ts[render,tags=snippet,indent=0,group=Lit]
----
endif::[]
...
[source,java]
----
include::{root}/src/main/java/com/vaadin/demo/component/login/LoginEventExample.java[render,tags=snippet,indent=0,group=Flow]
----
endif::[]
...
[source,tsx]
----
include::{root}/frontend/demo/component/login/react/login-event.tsx[render,tags=snippet,indent=0,group=React]
----
endif::[]
--
...
Login features its own modal overlay which is also <<../../flow/advanced/server-side-modality#server-side-modality,modal on server side>>. Use it to create simple log-in pages -- which are full-screen on mobile devices -- or to handle authentication without a dedicated log-in page. You can also use it to handle re-authentication when the user's session has expired.
