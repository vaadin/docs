...
layout: tabbed-page
title: Login
description: How to use the Login component to authenticate the user.
page-links:
...
.Password Managers
[NOTE]
Login is incompatible with password managers if placed inside another component's footnote:[When added to a web component that uses shadow root, password managers are unable to find the input fields and therefore won't work.] shadow root. This isn't an issue when using Login's modal overlay.
...
Overlay provides a custom form area for adding more fields in addition to username and password. This area is placed above the "Submit" button. Use `name` attribute to ensure custom field `value` is submitted with the form.
...
Footer can be used for placing additional custom content, such as text, buttons, etc. Adding components after the overlay is opened is not supported.
...
Login shows an error message when authentication fails. The error message consists of a title and the message. It's displayed directly below the title of the form.
...
Login's error message is shown when authentication fails. It consists of a title and the message.
