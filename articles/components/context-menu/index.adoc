...
In order to prevent duplicate clicks while the server is processing a request, you can use `setDisableOnClick` to immediately disable the button on the client-side.
