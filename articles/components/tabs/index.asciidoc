...
=== Autoselect (Flow Only)

If autoselect is enabled, selection occurs when a Tab is added to an empty component.
Removing the selected tab will select the next available Tab if autoselect is true.
Autoselect can be toggled and is enabled by default.
...
[source,java]
----
include::{root}/src/main/java/com/vaadin/demo/component/tabs/TabsAutoselect.java[render,tags=snippet,indent=0,group=Java]
----
--

If autoselect is enabled, selection change listeners should be added before adding the first Tab in order to be triggered with the initial selection.
