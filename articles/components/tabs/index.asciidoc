...
Using the integrated content areas in [since:com.vaadin:vaadin@V23.3]#Tab Sheet# is the easiest way to switch between different content for each tab.
In some cases, such as when the tabs need to be structurally separated from their content areas, it may be necessary to use the standalone Tabs component and implement content switching manually.
