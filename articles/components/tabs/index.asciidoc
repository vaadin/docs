...
Sometimes, such as when the tabs need to be structurally separated from their content areas, it may be necessary to use the standalone Tabs component and implement content switching manually.
...
Sometimes it can be desirable to initialize the contents for a tab lazily, in other words delay its initialization until the tab is selected.
