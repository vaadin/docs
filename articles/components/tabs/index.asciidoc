...
Tabs are most conveniently used as part of a [since:react-dom@v23.3]#Tab Sheet# that includes automatically switched content areas for each tab.

////
TODO: Add basic TabSheet example here
////
...
Tabs support two different orientations: horizontal (default) and vertical (not currently supported in Tab Sheets).
...
In horizontal orientation, scroll buttons are displayed by default in aid scrolling the tabs. These can be disabled by applying the `hide-scroll-buttons` theme variant.
...
Vertical tabs can be a better choice for a large number of items, as it is easier for the user to scan a vertical list of options.
However, they may be less easy to understand and associate with their contents.
...
Vertical orientation is not available for Tab Sheets.
...
Icon-only labels should provide a textual description using a <<../tooltip#,tooltip>>, or an `aria-label` attribute for assistive technologies.
...
Note that this disables the ability to scroll, as the content never overflows.
...
* Use with caution, as this reduces the visual distinction of selected tabs to color only, which can be difficult to discern for many users.

=== Small

The `small` theme variant can be used to make the Tabs smaller, for use when space is limited.
...
Using the integrated content areas in [since:react-dom@v23.3]#Tab Sheet# is the easiest way to switch between different content for each tab. In some cases, such as when the tabs need to be structurally separated from their content areas, it may be necessary to use the standalone Tabs component and implement content switching manually.
...
In some cases it can be desirable to initialize the contents for a tab lazily, in other words delay its initialization until the tab is selected.

////
TODO: Add lazy initialization sample here (see TabSheet documentation plan for details)
////
