...
Below is a simple example of tabs with the labels, `Details`, `Payment`, and `Shipping`:
...
Horizontal tabs may be easier for users to understand and associate with the content. They're best suited for a small number of items, but provide scrolling on overflow. Try making the tabs in the example here scroll horizontally: you'll see that there are more than the initially visible three tabs.
...
You would use this variant to suit your visual and stylistic preference. It's typically used for top-level navigation. However, use it only where appropriate since by default left-aligned tabs are more discoverable and what most users expect.
...
You would generally use this to reduce visual clutter. However, use it with caution as it reduces the visual distinction of selected tabs to color only. This can be difficult to discern for many users.
...
The `bordered` theme variant adds a border around the Tab Sheet component. You can see there's a long rounded-rectangle box around the Tabs and Tab Sheet here.
...
Custom content can be placed before or after the tabs in a Tab Sheet by placing that content in the `prefix` and `suffix` slots. Notice in the example here that there are three tabs like the previous example, but there's a button labeled "Close all" before the tabs and another labeled with a plus-sign after them, far to the right.
...
Tab focus is rendered differently when focused by the keyboard. Once a tab is focused, arrow keys can be used to move the focus between tabs. Try pressing kbd:[Enter] or kbd:[Space] to select the tab on which to be focus -- or click on one tab with your mouse.  Then use the left and right arrow keys to shift the focus.
...
Try clicking on each tab here. Notice how only the content, seemingly one word changes depending on which you select.
