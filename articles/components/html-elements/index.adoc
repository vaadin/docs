...
| `NativeTable` | `table` | The https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/table[table element] represents data presented in a two-dimensional table comprised of rows and columns of cells containing data. For most use cases, prefer the more advanced Vaadin <<{articles}/components/grid#,Grid>>, <<{articles}/components/grid-pro#,Grid Pro>> or <<{articles}/components/tree-grid#,Tree Grid>>. *Deprecated:* use <<table#,`Table`>> instead.
| `NativeTableBody` | `tbody` | The https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/tbody[table body element] encapsulates a set of table rows (`NativeTableRow`). *Deprecated:* use <<table#,`TableBody`>> instead.
| `NativeTableCaption` | `caption` | The https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/caption[caption element] specifies the caption (or title) of a table, providing the table an accessible description. *Deprecated:* use <<table#,`TableCaption`>> instead.
| `NativeTableCell` | `td` | The https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/td[table data cell element] defines a cell of a table that contains data and may be used as a child of the `NativeTableRow`. *Deprecated:* use <<table#,`TableDataCell`>> instead.
| `NativeTableFooter` | `tfoot` | The https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/tfoot[table footer element] encapsulates a set of table rows (`NativeTableRow`), typically used to provide a summary for the columns. *Deprecated:* use <<table#,`TableFoot`>> instead.
| `NativeTableHeader` | `thead` | The https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/thead[table head element] encapsulates a set of table rows (`NativeTableRow`), indicating that they comprise the head of a table with information about the table's columns. *Deprecated:* use <<table#,`TableHead`>> instead.
| `NativeTableHeaderCell` | `th` | The https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/th[table header element] defines a header cell, which can be used inside a `NativeTableRow`, typically within a `NativeTableHeader`. *Deprecated:* use <<table#,`TableHeaderCell`>> instead.
| `NativeTableRow` | `tr` | The https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/tr[table row element] defines a row of cells in a table. The row can contain either `NativeTableHeaderCell` or `NativeTableCell` elements. *Deprecated:* use <<table#,`TableRow`>> instead.
...
| [since:com.vaadin:vaadin@V25.3]#`Table`# | `table` | The https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/table[table element] represents data presented in a two-dimensional table comprised of rows and columns of cells containing data. See the <<table#,Table>> page for the whole family. For most use cases, prefer the more advanced Vaadin <<{articles}/components/grid#,Grid>>, <<{articles}/components/grid-pro#,Grid Pro>> or <<{articles}/components/tree-grid#,Tree Grid>>.
| [since:com.vaadin:vaadin@V25.3]#`TableBody`# | `tbody` | The https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/tbody[table body element] encapsulates a set of table rows (`TableRow`). A table may have several bodies.
| [since:com.vaadin:vaadin@V25.3]#`TableCaption`# | `caption` | The https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/caption[caption element] specifies the caption (or title) of a table, providing the table an accessible description.
| [since:com.vaadin:vaadin@V25.3]#`TableColumn`# | `col` | The https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/col[table column element] defines one or more columns within a `TableColumnGroup`, so that attributes can be applied to whole columns at once.
| [since:com.vaadin:vaadin@V25.3]#`TableColumnGroup`# | `colgroup` | The https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/colgroup[table column group element] defines a group of columns within a table. It either carries a span or contains `TableColumn` children, but not both.
| [since:com.vaadin:vaadin@V25.3]#`TableDataCell`# | `td` | The https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/td[table data cell element] defines a cell of a table that contains data, and may be used as a child of the `TableRow`.
| [since:com.vaadin:vaadin@V25.3]#`TableFoot`# | `tfoot` | The https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/tfoot[table foot element] encapsulates a set of table rows (`TableRow`), typically used to provide a summary for the columns.
| [since:com.vaadin:vaadin@V25.3]#`TableHead`# | `thead` | The https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/thead[table head element] encapsulates a set of table rows (`TableRow`), indicating that they comprise the head of a table with information about the table's columns.
| [since:com.vaadin:vaadin@V25.3]#`TableHeaderCell`# | `th` | The https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/th[table header element] defines a header cell, which can be used inside a `TableRow`, typically within a `TableHead`.
| [since:com.vaadin:vaadin@V25.3]#`TableRow`# | `tr` | The https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/tr[table row element] defines a row of cells in a table. The row can contain either `TableHeaderCell` or `TableDataCell` elements.
...
The `Table` components form a family of their own: a `Table` holds a caption, column groups, and the head, body, and foot sections, each of which holds rows of header and data cells. The types accept only the children the HTML specification allows, and keep them in the required order. See the <<table#,Table>> page for the full documentation.

The older `NativeTable` family is deprecated and is scheduled for removal in Vaadin 26. See <<table#migrating-from-native-table,Migrating from Native Table>> for the replacements.
...
Regarding the `Html` Component, there are a few things to consider. First, remember that developers are responsible for sanitizing the HTML content passed to the `Html` component. Failure to do so may lead to cross-site scripting (XSS) vulnerabilities, as the raw HTML is sent directly to the client. The component can also [since:com.vaadin:vaadin@V25.2]#do the sanitization itself#: the constructors that take a [classname]`SerializableSupplier<Safelist>` clean the content with the supplied jsoup [classname]`Safelist`, for example `new Html(untrustedHtml, Safelist::relaxed)`. Pass a method reference or a lambda that builds the [classname]`Safelist`, rather than capturing a pre-built instance, because [classname]`Safelist` isn't serializable. Note that the safelist must permit the root element of the fragment. See <<{articles}/flow/security/advanced-topics/vulnerabilities#xss,Cross-Site Scripting>> for more information.
