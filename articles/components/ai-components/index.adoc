...
[classname]`SpringAILLMProvider` supports both streaming and non-streaming Spring AI models.
...
// From a ChatClient (memory managed externally)
var provider = LLMProvider.from(chatClient);
----
...
[classname]`LangChain4JLLMProvider` supports both streaming and non-streaming LangChain4j models. The mode is determined by the model type passed to the factory method:

[source,java]
----
// Streaming mode
var provider = LLMProvider.from(streamingChatModel);
...
[TIP]
Non-streaming mode does not require push. If push is not available in your environment, disable streaming on the provider. A warning is logged at runtime if push is not enabled when using streaming mode.
