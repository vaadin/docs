...
* While the component is serializable, Apache POI prevents using the `@PreserveOnRefresh` annotation for preserving objects, and therefore also blocks session replication with Kubernetes Kit.
