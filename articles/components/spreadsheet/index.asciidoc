...
[NOTE]
Vaadin Spreadsheet is a Flow-only component. It's not available for Hilla applications.
...
Install Spreadsheet by adding the dependency to the project. The example below is as a Maven dependency in [filename]`pom.xml`:
...
|Rows and columns headings display the row and column indexes, and allow selecting and resizing of the rows and columns.
...
Cells can be styled with different fonts. A font definition not only includes a particular typeface, but also the weight (e.g., bold or normal), emphasis, underlining, and other such font attributes.
...
Cells may include comments, which are shown when the mouse is hovered over them. They're indicated by ticks in the corner of the cells. The [classname]#SpreadsheetDefaultActionHandler# enables adding comments from the context menu.
...
Components in a spreadsheet must be generated by a [classname]#SpreadsheetComponentFactory#, which you need to implement.
