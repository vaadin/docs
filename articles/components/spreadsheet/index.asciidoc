...
A cell may contain a variety of types, like string, number, and date. 
To set a cell value programmatically, the reference needs to be retrieved from the `getCell()` method. 
If a cell with the given coordinates exists, then use the `setCellValue()` method to update the cell value.
Otherwise, it can be created from the `createCell()` method, which takes the row and column coordinates and the cell value.

.Don't forget to refresh the cells
[NOTE]
In case of the cells changed are part of the active sheet that is currently being displayed on the browser, you need to call `refreshCells()` passing the cells changed to get them updated on the client side as well.
...
** As a workaround, you can manually convert tables to ranges. Open the spreadsheet in Excel, right-click on the table, select the [guilabel]*Convert to Range* action, and then save the spreadsheet.
