...
By default, items only match the exact path of the current URL. To enable matching of nested paths / routes, set the `matchNested` property to `true`. With nested matching, an item with the path `/path` not only matches `/path`, but also `/path/child`, `/path/child/grandchild`, etc.
