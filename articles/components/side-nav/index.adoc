...
The navigation item matching the current URL is automatically highlighted as active.

[role="since:com.vaadin:vaadin@V24.4"]

If the item’s path contains query parameters, only URLs containing those parameters are considered a match. Additional parameters in the URL not specified in the item path are ignored (i.e. the URL is considered a match).
