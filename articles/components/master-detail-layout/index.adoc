...
The default `masterSize` is `30rem`. If `detailSize` is not set, it is automatically determined based on the detail content's minimum width.
...
A placeholder component can be shown in the detail area when no detail content is set. Unlike detail content, the placeholder is not rendered as an overlay.
