...
|Large
|For especially important call-to-action buttons, where more emphasis is needed.
...
[TIP]
The standard Button colors can be adjusted using <<{articles}/styling/lumo/design-tokens/color#,theme features>>, so these variants shouldn't be used to replace standard buttons only to achieve a different color.
...
* *Use icons sparingly*.
Most actions are difficult to represent reliably with icons, and the benefit of icons plus text should be weighed against the additional visual noise this creates.
* *Icon-only* buttons should be primarily used for *common recurring actions* with highly standardized, universally understood icons (for example, a cross for *close*), and for actions that are *repeated* (for example, in lists and tables).
