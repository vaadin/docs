...
See <<{articles}/components/ds-resources/foundation/size-space#size-and-space,Size and Space>> for details on how to change the default button size.
...
[TIP]
Note that the standard Button colors can be adjusted using <<{articles}/components/ds-resources/foundation/color#,theme features>>, so these variants should not be used to replace standard buttons just to achieve a different color.
