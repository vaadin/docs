...
Button groups representing options, such as the buttons of a <<../confirm-dialog#,Confirm Dialog>>, should state what each option represents (e.g., _"Save Changes"_). Don't label the button _"Yes"_ since that requires the user to read the question being asked. It'll increase the risk of selecting the wrong option.
