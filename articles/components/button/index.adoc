...
=== Prevent Multiple Clicks [badge-flow]#Flow#
...
In most cases, the button only needs to be disabled while the server handles the click. Use the [since:com.vaadin:vaadin@V25.4]#`UNTIL_RESPONSE` mode# for this. The button is enabled again automatically once the click has been handled, so no additional code is needed in the click listener.
...
[source,java]
----
include::{root}/src/main/java/com/vaadin/demo/component/button/ButtonDisableOnClick.java[render,tags=snippet,indent=0,group=Flow]
----
endif::[]
--

When the action continues after the click listener returns, for example in a <</building-apps/business-logic/background-jobs#,background job>>, use the [since:com.vaadin:vaadin@V25.4]#`UNTIL_ENABLED` mode# instead. The button then stays disabled until the application enables it again:
...
Updating the button from a background thread requires <</building-apps/server-push#,server push>>.
