...
Form Item only supports placing a single field inside. In the case you need to place multiple fields, <<../custom-field#,Custom Field>> should be used as a wrapper:
...
However, keeping fields in individual Form Items is preferable. Wrapped fields can be hard to visually distinguish since they usually have no individual label except for a placeholder which is only visible when the field has no value.
