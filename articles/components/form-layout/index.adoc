...
image::form-rows.png["A form where the first name and last name fields share one Form Row, the email address field stands alone, and the password and confirm password fields share another Form Row", width=440, align=left]
...
Alternatively, you can write custom CSS. When labels are rendered on the side, Form Layout sets the `has-labels-aside` attribute on its host element and the `data-form-layout-has-labels-aside` attribute on children. You can target these attributes to adapt custom components to labels-aside mode, for example:

[source,css]
----
/* To define a two-column layout with labels aside: */

custom-component[data-form-layout-has-labels-aside] {
  display: grid;
  grid-template-columns: var(--vaadin-form-layout-label-width) 1fr;
  column-gap: var(--vaadin-form-layout-label-spacing);
}

/* or, instead, to push the whole component to the input column: */
...
Form Item is a wrapper that provides a field-like layout with a label. Use it for native HTML `<input>` elements or custom web components that have no label of their own:
...
Form Item connects the label to the wrapped element with the https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Reference/Attributes/aria-labelledby[`aria-labelledby`] attribute to make it available to screen readers. This works with HTML input elements out of the box. Custom web components, on the other hand, need to define an `ariaTarget` property that returns an appropriate element inside the component that should receive the `aria-labelledby` attribute.

To combine multiple inputs in the same form field, use <<../custom-field#,Custom Field>> instead.

Form Item can also be used without a label to move a non-field component, such as Button, from the label column into the input column in <<#label-position,labels-aside mode>>:
