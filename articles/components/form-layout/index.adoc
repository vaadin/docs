...
tab-title: Usage
title: Form Layout
description: Using Form Layout to build responsive forms with multiple columns and input labels.
page-links:
...
For example, if you have a Form Layout with three columns and a component's `colspan` is set to 3, it therefore takes the entire width of the Form Layout.
...
The positioning of labels on the sides is also useful when there's a need to compare numeric data.
