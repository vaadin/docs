...
The width of side-positioned labels can be adjusted using the `--vaadin-form-item-label-width` CSS property on the Form Layout element, or [since:com.vaadin:vaadin@V24.5]#the [methodname]`setLabelWidth()` method# on the [classname]`FormLayout` instance in Flow.
