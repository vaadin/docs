...
The width of side-positioned labels can be adjusted using the `--vaadin-form-item-label-width` CSS property on the Form Layout element.

[NOTE]
[role="since:com.vaadin:vaadin@V24.5"]
Since Vaadin 24.5, you can use the [methodname]`setLabelWidth()` method on the [classname]`FormLayout` instance in Flow.
