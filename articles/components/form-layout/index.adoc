...
Components that are not fields, such as buttons, span from the label column when labels are aside. To align such a component with the inputs, wrap it in a Form Item, as in the example above with the Register button. The Form Item reserves space for the label, so the component moves to the input column.
