...
Wrapping an element in a Form Item provides it with a label, which is useful for elements that have no label of their own, such as native html `<input>` elements or custom components. In labels-aside mode, a Form Item can also be used without a label to move a non-field component, like Button, from the label column to the input column.
