...
[NOTE]
.Side-Positioned Label Width
Starting from Vaadin 24.5, you can control the width of side-positioned labels. In Lit and React, use the `--vaadin-form-item-label-width` CSS property on the Form Layout element. In Flow, use the [methodname]`setLabelWidth()` method on the [classname]`FormLayout` instance.
...
Form Layout provides CSS properties for configuring the spacing between columns, rows, and between the label and input field when labels are positioned aside.
