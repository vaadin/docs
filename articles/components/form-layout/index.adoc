...
Starting from Vaadin 24.5, you can control the width of side-positioned labels using the `--vaadin-form-item-label-width` CSS property on the Form Layout element. In Flow, use the [methodname]`setLabelWidth()` method on the [classname]`FormLayout` instance.
