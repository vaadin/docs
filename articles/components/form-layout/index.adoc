...
Form Item is a wrapper that gives the element a field-like layout with a label. Use it to provide a label for native HTML `<input>` elements or custom input web components that have no label of their own:
...
Form Item connects the label to the wrapped element with the https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Reference/Attributes/aria-labelledby[`aria-labelledby`] attribute to make it available to screen readers. HTML input elements support this attribute out of the box. Custom web components, on the other hand, need to define an `ariaTarget` property that returns an appropriate input element inside the component that should receive the `aria-labelledby` attribute.

To combine multiple inputs in the same form field, use <<../custom-field#,Custom Field>> instead.

Form Item can also be used without a label to move any other element from the label column into the input column in <<#label-position,labels-aside mode>>:
