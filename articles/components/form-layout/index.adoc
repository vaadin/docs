...
All Vaadin field components placed directly in the Form Layout support rendering their label next to the field out of the box. Any other components, such as buttons or custom components, are placed at the start of the label column and can take up both columns. How they should be laid out in this mode is left to the developer. There are two ways to handle this.

The first option is to wrap the component in a Form Item. This lets you either define a custom label for the component, or simply move it to the input column so that it aligns with the other fields, as the Form Item reserves space for the label:
...
----
<source-info group="Flow"></source-info>
formLayout.addFormItem(new Input(), "Name");
formLayout.add(new FormItem(new Button("Submit")));
----
...
The second option is to write custom CSS. When labels are rendered next to fields, Form Layout sets the `has-labels-aside` attribute on its host element and the `data-form-layout-has-labels-aside` attribute on fields. You can use these attributes to style custom components for this mode.
