...
In cases where vertical space needs to be conserved, horizontal orientation can be used. However, no more than three options are recommended:
...
|<<../select#,Select>>|A field for selecting an item from a list of options which are presented in an overlay. Recommended when there is insufficient space for a Radio Button Group.
