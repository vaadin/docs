...
Placeholders are not universal substitutes for labels, as they are only visible while the field is empty.
...
This example invalidates any value that doesn't consist exclusively of numbers, parenthesis, dashes, and the plus sign. In cases where the format requirements may not be clear to the user, it is recommended to provide this information, for example by using a <<../input-fields#helper,helper>>.
