...
This example invalidates any value that doesn't consist exclusively of numbers, parenthesis, dashes, and the plus sign. In cases where the format requirements may not be clear to the user, it's recommended to provide this information, for example by using a <<../input-fields#helper,helper>>.
