...
When a field is set to `autoselect`, its content is selected when the field is focused. Use `autoselect` when the user might to want to replace the entire value rather than tweak it.
