...
Text Area can't be resized by the user by default. The CSS `resize` property adds a resize handle to the component. It only takes effect when the `overflow` property is set to a value other than `visible`. Use `vertical`, `horizontal`, or `both` to control the direction in which the component can be resized. Resizing sets an explicit size on the component, which replaces the automatic height adjustment.
...
----
<source-info group="Lit"></source-info>
<vaadin-text-area style="resize: vertical; overflow: auto;"></vaadin-text-area>
----
endif::[]
...
----
<source-info group="React"></source-info>
<TextArea style={{ resize: 'vertical', overflow: 'auto' }} />
----
endif::[]
--
