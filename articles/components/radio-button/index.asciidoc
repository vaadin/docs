...
It's generally recommended to set the most common option as the default value for Radio Button Groups.
...
If the Checkbox doesn't represent a simple yes/no choice, and its label cannot clearly communicate the meaning of its unchecked state, it's better to use a Radio Button Group with two options:
