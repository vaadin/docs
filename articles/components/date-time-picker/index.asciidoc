...
Please note that they can only be displayed when the first day of the week is Monday.
...
Date Time Picker provides an API for internationalization.
...
The calendar overlay is useful when the user needs to choose a day that is close to the current date or when information such as day of the week, week number, valid dates, and so on, can aid in choosing the best option.
...
Use a helper text to indicate the expected date and time formats, and placeholders to help users identify the two sub-fields correctly.
