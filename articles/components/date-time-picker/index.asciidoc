...
[#aria-labels]
.[since:com.vaadin:vaadin@V24.1]#Invisibe Labels (ARIA)#
[%collapsible]
====
If the built-in label cannot be used, an invisible label can be provided for users of assistive technologies like screen readers. The date and time sub-fields can additionally be assigned invisible suffixes, such as `date` and `time`, that are appended to their invisible labels.
