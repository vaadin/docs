...
Grid supports programmatic navigation to a specific row. This is particularly useful when dealing with large data sets, as it saves users from having to manually scroll through potentially hundreds or thousands of rows.
...
Providing multiple indexes allows you to navigate to a specific row in the item hierachy. Each given index represents the row number on an individual item hierarchy level.
...
[source,java]
----
include::{root}/src/main/java/com/vaadin/demo/component/tree-grid/TreeGridScrollToIndex.java[render,tags=snippet,indent=0,group=Flow]
----
endif::[]
...
[source,tsx]
----
include::{root}/frontend/demo/componenttree-grid/react/tree-grid-scroll-to-index.tsx[render,tags=snippet,indent=0,group=React]
----
endif::[]
--
