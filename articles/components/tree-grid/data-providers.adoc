...
* The full size and structure of the tree stay unknown since determining them would require loading the entire hierarchy into memory, which isn't efficient for this format. As a result, Tree Grid can't restore the scroll position automatically after [methodname]`HierarchicalDataProvider#refreshAll()` because it resets the cached hierarchy state.
...
class FolderDataProvider implements AbstractHierarchicalDataProvider<Folder, Void> {
    ...
    @Override
    public int getChildCount(HierarchicalQuery<Folder, Void> query) {
        // Return the size of the flattened sub-tree of the requested parent folder
    ...
    private Stream<Folder> flatten(Folder parent, Set<Object> expandedFolderIds) {
        // Build the parent folder's sub-tree based on the provided expanded folders. The
