...
When a filter is set, Tree Data Provider evaluates it against every item in the hierarchy, including items in collapsed branches. A parent item is kept in the result whenever any of its descendants match, so the chain of ancestors leading to a match is never filtered out — even when those ancestors don't match the filter themselves. The match itself only appears in Tree Grid once all of its ancestors are expanded, since setting a filter doesn't change Tree Grid's expansion state.
