...
- <<#setting-items,Setting items>> from a collection of root items and a callback that returns their children, for quick setup.
...
The quickest way to populate Tree Grid from existing objects is to add them to a [classname]`TreeData` structure with the [methodname]`TreeData#addItems(rootItems, childItemProvider)` method, and then bind the structure to Tree Grid through a [classname]`TreeDataProvider`. The [methodname]`addItems` method takes a collection of root items and a callback that returns the children of each item:
...
The [methodname]`addItems` method traverses the root items and their descendants eagerly: it calls the child item provider on each root item, then on each returned child, and continues until no more children are returned.

The example passes [enumname]`HierarchyFormat.FLATTENED` to the [classname]`TreeDataProvider` constructor. This is the recommended format for [classname]`TreeData`, as explained in the next section.

.Deprecated Shorthand Methods
[NOTE]
====
[deprecated:com.vaadin:vaadin@V25.3]#The [methodname]`setItems(rootItems, childItemProvider)` and [methodname]`setTreeData(treeData)` shorthand methods, and the single-argument [classname]`TreeDataProvider` constructor, are deprecated.# They create a data provider with [enumname]`HierarchyFormat.NESTED`, but use [enumname]`HierarchyFormat.FLATTENED` starting from the next major version. This changes the behavior of Tree Grid methods that depend on the hierarchy format, such as [methodname]`scrollToIndex()`. Create the [classname]`TreeDataProvider` with an explicit hierarchy format instead, as shown above. To keep the current behavior, pass [enumname]`HierarchyFormat.NESTED`. See <<{articles}/upgrading/25-2-to-25-3#deprecations-to-address-before-vaadin-26,Deprecations to Address>> in the upgrade guide for a before-and-after example.
====

The next section explains the [classname]`TreeData` and [classname]`TreeDataProvider` concepts in more detail, and how to use them to modify, sort, and filter hierarchical data. Keep references to both instances if the application needs to change the data later.
...
To reveal all matching items, expand the filtered tree after setting the filter. The [methodname]`expandRecursively` method fetches the items through the data provider, so it only visits the matching items and their ancestors, and expands the paths leading to the matches:
...
searchInput.addValueChangeListener(event -> {
    ...
    if (!searchTerm.isEmpty()) {
        treeGrid.expandRecursively(treeData.getRootItems(), Integer.MAX_VALUE);
    }
});
----

Expanding recursively traverses the whole filtered tree, so consider limiting the depth for large hierarchies.
