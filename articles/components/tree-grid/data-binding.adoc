...
To fill the viewport, Tree Grid sends separate requests to the data provider for each expanded item within the visible range. Each request comes with a [classname]`HierarchicalQuery` object that contains the following fields:
...
Each request comes with a [classname]`HierarchicalQuery` object that contains the following fields:

|===
|Method |Description

|`HierarchicalQuery#getParent()`
|The parent item whose sub-tree should be returned as a flat list. Its direct children should always be included in the result. The `null` value means the entire tree from the root.

|`HierarchicalQuery#getExpandedItemIds()`
|The set of item IDs that are descendants of `getParent()` and whose children need to be included in the flat list as well.
...
The data provider must also implement [methodname]`HierarchicalDataProvider#getDepth(T item)` to return the depth for each item in the hierarchy. Tree Grid uses this information to indent items visually.
