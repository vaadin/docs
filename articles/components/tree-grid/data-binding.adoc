...
Whenever the user changes the sort order on a column or a filter is set on the data provider, Tree Grid passes the updated information through the [classname]`HierarchicalQuery` on the next data load. The data provider can then read these properties and apply them when fetching items:
...
|`Query#getFilter()`
|An optional filter value of the type defined by the data provider's filter type parameter. The filter is set externally via [methodname]`HierarchicalDataProvider#withConfigurableFilter()` and [methodname]`setFilter`.
|===

The example below extends the nested data provider shown earlier with column sorting and a string filter. The data provider's filter type parameter is changed from [classname]`Void` to [classname]`String` so that the search term entered by the user can be passed in:

[source,java]
----
public class FolderDataProvider extends AbstractHierarchicalDataProvider<Folder, String> {
    public FolderTreeData folderTreeData = new FolderTreeData();
    ...
    @Override
    public boolean hasChildren(Folder folder) {
        // For accuracy under filtering, this should also check whether any
        // descendant matches the active filter — omitted here for brevity.
        return folderTreeData.getChildren(folder).size() > 0;
    }

    @Override
    public Stream<Folder> fetchChildren(HierarchicalQuery<Folder, String> query) {
        Stream<Folder> children = filteredChildren(query);

        // Apply the in-memory comparator built from the active column sort
        // orders, if any.
        children = query.getSortingComparator()
                .map(children::sorted)
                .orElse(children);

        return children.skip(query.getOffset()).limit(query.getLimit());
    }

    @Override
    public int getChildCount(HierarchicalQuery<Folder, String> query) {
        // Sorting doesn't affect the count, so only filtering needs to match
        // fetchChildren.
        return (int) filteredChildren(query).count();
    }

    private Stream<Folder> filteredChildren(HierarchicalQuery<Folder, String> query) {
        Stream<Folder> children = folderTreeData.getChildren(query.getParent())
                .stream();
        String term = query.getFilter().orElse("").toLowerCase();
        if (term.isEmpty()) {
            return children;
        }
...
To enable column sorting in Tree Grid, mark the columns sortable. Setting a column key is required when the data provider reads [methodname]`Query#getSortOrders()` to map columns to backend fields, but isn't needed for in-memory sorting that uses [methodname]`getSortingComparator()`, as in the example above:
...
[NOTE]
====
Sort orders must be applied separately at each hierarchy level so that parents continue to precede their children in the result. With nested data providers, this is implicit because each [methodname]`fetchChildren` call returns the children of a single parent. With flattened data providers, sorting must be applied independently to each level when constructing the flat list.
====

==== Setting a Filter

The filter type parameter of a hierarchical data provider can be any type that fits the data source, such as a search term or a more complex filter object. To pass a filter value to the data provider, wrap it using [methodname]`HierarchicalDataProvider#withConfigurableFilter()` and call [methodname]`setFilter` on the result.

The example below uses a Text Field component to filter folders by name in real time:
...
HierarchicalConfigurableFilterDataProvider<Folder, Void, String> filterableDataProvider =
        folderDataProvider.withConfigurableFilter();

treeGrid.setDataProvider(filterableDataProvider);

TextField searchInput = new TextField("Search folders", "Enter folder name");
searchInput.setValueChangeMode(ValueChangeMode.EAGER);
searchInput.addValueChangeListener(event -> {
    filterableDataProvider.setFilter(event.getValue());
});
----

The [methodname]`setFilter` method re-renders the Tree Grid automatically, so calling [methodname]`refreshAll` isn't necessary.

[NOTE]
====
Unlike [classname]`TreeDataProvider`, custom hierarchical data providers don't automatically include the parent chain of a matching item. If a deeply nested item matches the filter but its ancestors don't, the matching item won't be visible in Tree Grid. To handle this, treat an item as matching in [methodname]`fetchChildren` and [methodname]`getChildCount` when either its own data or any of its descendants match the filter.
====
