...
To pass a filter value to a hierarchical data provider, you need to wrap it with [methodname]`withConfigurableFilter()` and call [methodname]`setFilter` on the resulting [classname]`HierarchicalConfigurableFilterDataProvider`. The filter value is then available through [methodname]`HierarchicalQuery#getFilter()` on every load.
