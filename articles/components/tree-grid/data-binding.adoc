...
For the nested hierarchy format, the implementation will be pretty much the same, except that [methodname]`HierarchicalQuery#getExpandedItemIds()` is always empty, which means the recursive logic that walks expanded descendants is no longer needed.

=== Filtering

To pass a filter value to a hierarchical data provider, you need to wrap it with [methodname]`withConfigurableFilter()` and call [methodname]`setFilter` on the resulting [classname]`HierarchicalConfigurableFilterDataProvider`. The filter value is then passed to the data provider on every fetch, where it can be accessed via [methodname]`HierarchicalQuery#getFilter()`.
