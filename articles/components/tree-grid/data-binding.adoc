...
To fill the viewport, Tree Grid makes a single data provider request covering the entire visible range. Requests to individual levels can still be made for specific operations that require it, such as recursive tree expansion.
...
|`HierarchicalQuery#getParent()`
|The parent item whose sub-tree should be returned as a flat list. The `null` value means the entire tree starting from the root items.

|`HierarchicalQuery#getExpandedItemIds()`
|The set of IDs identifying expanded items within the requested sub-tree. Their children should be included in the flat list in addition to the direct children of `getParent()`.
