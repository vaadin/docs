...
Tree Grid supports connecting to various types of data sources through hierarchical data providers. For simple use cases, it offers a built-in [classname]`TreeData` structure along with a ready-made [classname]`TreeDataProvider`, which together provide an easy way to create and manage hierarchies in memory. It also supports custom hierarchical data providers to fetch data from other sources, e.g. a database.

== Setting Items

The simplest way to populate Tree Grid is the [methodname]`setItems(rootItems, childItemProvider)` method. It takes a collection of root items and a callback that returns the children of any given item:
...
// First argument: root-level items shown at the top level of the tree.
// Second argument: a callback that returns the children of any folder.
treeGrid.setItems(List.of(work, family), Folder::children);
----

[methodname]`setItems` walks the hierarchy eagerly: it calls the child item provider on each root item, then recursively on each returned child, until no more children are produced. The full hierarchy is loaded into an in-memory [classname]`TreeData` structure wrapped in a [classname]`TreeDataProvider`, both created internally.

A [methodname]`setItems(Stream, ValueProvider)` overload is also available for populating the tree from a stream of root items.

To modify the hierarchy after [methodname]`setItems` has been called, retrieve the underlying [classname]`TreeData` instance with [methodname]`HasHierarchicalDataProvider#getTreeData()` and update it directly. Tree Grid doesn't pick up such changes automatically, so call [methodname]`DataProvider#refreshAll()` afterwards to update the UI:

[source,java]
----
// Add a "Reports" folder under "Work"
treeGrid.getTreeData().addItem(work, new Folder("Reports", List.of()));

// Refresh the Tree Grid to reflect the changes in the UI
treeGrid.getDataProvider().refreshAll();
----

For more control over the data provider — for example, to use the [enumname]`HierarchyFormat.FLATTENED` format — construct a [classname]`TreeDataProvider` explicitly as shown in the next section.

[NOTE]
====
Because the entire hierarchy is walked and held in memory upfront, [methodname]`setItems` is best suited for hierarchies that fit comfortably in memory. For larger or backend-driven hierarchies, use a custom hierarchical data provider as described in the <<#custom-data-providers,Custom Data Providers>> section.
====
