...
Tree Grid supports several ways to bind hierarchical data, depending on where the data lives and how much control is needed:

- Convenience method <<#setting-items,setItems>> for quickly populating Tree Grid from an in-memory collection.
- More general <<#tree-data-provider,TreeData and TreeDataProvider>> for storing and managing hierarchies in memory.
- <<#custom-data-providers,Custom hierarchical data providers>> for connecting to external data sources such as a database.
...
The underlying [classname]`TreeData` structure and its associated [classname]`TreeDataProvider` are then available via the `TreeGrid#getTreeData` and `TreeGrid#getDataProvider` methods:
...
The next section explains the [classname]`TreeData` and [classname]`TreeDataProvider` concepts in more detail, and how to use them to modify, sort, and filter the hierarchical data.
