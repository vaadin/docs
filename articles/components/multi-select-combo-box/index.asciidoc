...
The component can be configured to auto-expand in order to accommodate chips for selected items.
It is possible to expand both horizontally (until max-width is reached) and vertically (in which
case the field with chips wraps in multiple lines). These modes can be also used together.
...
The component can be configured to group selected items at the top of the overlay. When selection
changes, a set of items to be shown in the top group is only updated after the overlay is closed.
