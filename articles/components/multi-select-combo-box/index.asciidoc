...
Use the `selected-items-changed` event (Web component) or `addValueChangeListener` (Flow component) to react to the user changing the selection:
...
A read-only Multi-Select Combo Box still allows opening the overlay, which then only shows the selected values, instead of all options.
...
The string must contain a `\{count}` placeholder that will be replaced with the actual count of selected items by the component.
...
For consistency, the default width of the Multi-Select Combo Box matches that of other Vaadin input fields.
We recommend increasing the width of the component when using items with long labels, or if you expect users to select several items, in order to avoid collapsing selected items into the overflow chip.
