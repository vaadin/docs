...
== Slot-based Horizontal Alignment

Individual components can be aligned by assigning the corresponding slot. By default, start alignment is used. Set the slot to `middle` or `end` in order to change its alignment.
...
Slot-based alignment does not provide any special logic for placing items responsively. It is recommended to wrap items in nested layouts when necessary, e.g. to create columns.
