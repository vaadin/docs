...
Instead of adjusting individual components, you can enable the `layoutComponentImprovements` <<{articles}/flow/configuration/feature-flags#,feature flag>>. With this flag, the Flow APIs `setWidthFull`, `setHeightFull`, and `setSizeFull` are rewired to automatically apply `flex: 1` to the component. This prevents fixed-size components from shrinking and makes the full-size component take up the remaining space in the layout.
