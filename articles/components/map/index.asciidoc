...
Markers can be configured to also show a text, which by default is displayed below the marker icon.
The position and the style of the text can be customized by setting a custom text style on the marker.
...
Markers can be configured to be draggable, which allows the user to move them around on the map.
Use the `MapFeatureDropEvent` to get notified when a marker is dropped on the map, it includes the marker that was dropped and the start and end coordinates of the drag operation.
The marker's coordinates are updated automatically.
