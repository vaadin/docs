...
The https://operations.osmfoundation.org/policies/tiles/[tile usage policy for OpenStreetMap] only allows for _light usage_, which means that it *shouldn't be used for production*.
...
You should read and respect the https://operations.osmfoundation.org/policies/tiles/[OpenStreetMap tile usage policy].
...
The event is triggered only after the user has finished manipulating the viewport, for example after letting go of the mouse button after a mouse drag interaction.
...
Internally, the component converts all coordinates into the projection that's used by the map's view, which is called the view projection.
...
As this documentation has already been updated to 23.2, all documentation sections see the new coordinate system, and examples have been adapted to create coordinates using latitude and longitude.

If you are still using 23.1, the default coordinate system is EPSG:3857, and all coordinates provided to the map must be in that projection.
