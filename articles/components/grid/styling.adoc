...
- *Drag source*: the row that is being dragged by the user. This part doesn't visually move during the drag.

- *Ghost*: a copy of the drag source (i.e., dragged row), which follows the user's pointer during the drag operation.

- *Drop target*: a row or a space between rows that the ghost can be dropped onto.

// separate lists

Cell in the row being dragged (the “ghost”):: `vaadin-grid+++<wbr>+++**::part(dragstart-row-cell)**`
Cell in a drag-source row:: `vaadin-grid+++<wbr>+++**::part(drag-source-row-cell)**`
Cell in a row that ghost hovers above, with drop-target above:: `vaadin-grid+++<wbr>+++**::part(dragover-above-row-cell)**`
Cell in a row that ghost hovers above, with drop-target below:: `vaadin-grid+++<wbr>+++**::part(dragover-below-row-cell)**`
Cell in a drop-target row that ghost hovers above:: `vaadin-grid+++<wbr>+++**::part(dragover-on-top-row-cell)**`
Cell in a row that isn’t draggable:: `vaadin-grid+++<wbr>+++**::part(drag-disabled-row-cell)**`
Cell in a row that isn’t a drop-target:: `vaadin-grid+++<wbr>+++**::part(drop-disabled-row-cell)**`
Row being dragged (the “ghost”):: `vaadin-grid+++<wbr>+++**::part(dragstart-row)**`
Row that ghost hovers above, with drop-target above:: `vaadin-grid+++<wbr>+++**::part(dragover-above-row)**`
Row that ghost hovers above, with drop-target below:: `vaadin-grid+++<wbr>+++**::part(dragover-below-row)**`
Drop-target row that ghost hovers above:: `vaadin-grid+++<wbr>+++**::part(dragover-on-top-row)**`
A row that isn’t draggable:: `vaadin-grid+++<wbr>+++**::part(drag-disabled-row)**`
A row that isn’t a drop-target :: `vaadin-grid+++<wbr>+++**::part(drop-disabled-row)**`
