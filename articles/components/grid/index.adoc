...
ifdef::flow[]
In Flow, columns created from a value provider -- for example, `grid.addColumn(Person::getAge)` -- are sorted by the provided value when using in-memory data. Columns created with a renderer, such as [classname]`LitRenderer`, [classname]`ComponentRenderer`, or [classname]`NumberRenderer`, don't have a value to sort by. They don't sort items unless you define a comparator with [methodname]`setComparator()`. Similarly, a column that renders numbers as formatted strings sorts them alphabetically unless you define a comparator:

[source,java]
----
grid.addColumn(item -> "Value: " + item.getValue())
        .setComparator(Item::getValue)
        .setHeader("Value");
----
endif::[]
...
You can sort a Grid programmatically using the [methodname]`sort()` method with [classname]`GridSortOrder`. This is useful when you want to set an initial sort order or change the sorting based on application logic:
...
To sort by multiple columns, use [methodname]`GridSortOrder.asc()` or [methodname]`GridSortOrder.desc()` and chain with [methodname]`thenAsc()` or [methodname]`thenDesc()`:
...
.GridSortOrder vs. QuerySortOrder
[NOTE]
[classname]`GridSortOrder` is used with [methodname]`grid.sort()` and references Grid columns. Don't confuse it with [classname]`QuerySortOrder`, which is used in data provider callbacks and references property name strings. The Grid automatically translates between the two when fetching data.
...
See the <<../grid/data-binding#custom-data-providers,Data Binding>> page for more information about data providers in Flow.
...
[NOTE]
====
Using Markdown is discouraged if accessibility of the tooltip content is essential, as semantics of the rendered HTML content (headers, lists, ...) won't be conveyed to assistive technologies.
====
...
|Accessible name template for each row's selection checkbox. The `{rowHeader}` placeholder is replaced with the row's row-header cell text, or the 1-based row index when there's no row-header column.

|`sorter`
|`Sort by {column}`
|Accessible name template for column sorters. The `{column}` placeholder is replaced with the column's header text.
|===
