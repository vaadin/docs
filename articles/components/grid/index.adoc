...
[NOTE]
Although most code examples define columns explicitly, the Flow component can generated them automatically based root-level properties on the bean class if you pass it as an argument to the constructor: `Grid<Person> grid = new Grid<>(Person.class);`
