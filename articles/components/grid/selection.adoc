...
=== Selecting Rows with Arrow Keys [badge-flow]#Flow#

By default, the arrow keys only move the focus between cells, and the user selects the focused row with kbd:[Space]. To make the selection follow the focus, select the item of the focused row in a cell focus listener. Disallowing deselection keeps the row selected when the user clicks it again. Click a row, and then use kbd:[Up] and kbd:[Down] to change the selection.

[.example,themes="lumo,aura"]
--
[source,typescript]
----
include::{root}/frontend/demo/component/grid/grid-select-on-focus.ts[preimport,hidden]
----

[source,java]
----
include::{root}/src/main/java/com/vaadin/demo/component/grid/GridSelectOnFocus.java[render,tags=snippet,indent=0]
----
--

See <<index#cell-focus,Cell Focus>> for more information about the cell focus event.
