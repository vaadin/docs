...
[NOTE]
====
`Grid` is a component with complex server - client interactions. For example when you update a `Grid` by using [methodname]`Grid.getDataProvider().refreshAll()`, the `Grid` will report this back to the server. Hence there will request to the server. For example if you periodically update a `Grid` from a background thread, the application will not be idling, and the session will not time out.
====
