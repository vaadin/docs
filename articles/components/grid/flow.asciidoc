...
You can use a [classname]`LitRenderer` to create and display new properties (i.e., properties the item didn't originally contain).
...
* The column uses the `age` string to build a [classname]`QuerySortOrder` that's sent to the [classname]`DataProvider` to do the backend sorting.
