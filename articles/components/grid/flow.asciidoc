...
The method can not be used to hide columns. You can hide columns with `Column()`, as described later.
////
...
[WARNING]
All properties of the bean, even ones which are not used in the template, are sent to the client, so use this feature with caution.
