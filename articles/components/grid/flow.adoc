...
title: Flow Usage
order: 50
description: Shows how to Grid in a Vaadin Flow application, and explains unique features available with Grid.
---


= Using Grid in Flow

Grid is a feature-rich, complex component. Due to the server-driven nature of this component, there are some exclusive features available for the Java API, and when using Grid together with Vaadin Flow. This page shows how to use Grid in a Vaadin Flow application. It also explains the unique features, such as configuring Lit renderers instead of generic component renderers and styling the component with the Theme property.
...
Obtaining a reference to the [classname]`SingleSelectionModel` allows you access to a fine-grained API for the single-selection use case. You can use the [methodname]`addSingleSelect(SingleSelectionListener)` method to access [classname]`SingleSelectionEvent`, which includes additional convenience methods and API options.

In single-selection mode, it's possible to control whether the empty (i.e., null) selection is allowed. This is enabled by default. Disallowing empty or null selection, for example, using the [methodname]`setDeselectAllowed()` method would look like this:
...
In multi-selection mode, a user can select multiple items by selecting checkboxes in the left column. Obtaining a reference to the [classname]`MultiSelectionModel` allows you access to a fine-grained API for the multi-selection use case.

You can use the [methodname]`addMultiSelectionListener(MultiSelectionListener)` method to access [classname]`MultiSelectionEvent`, which includes additional convenience methods and API options. For example, you could use the [methodname]`addMultiSelectionListener()` method to access selection changes like this:
...
This constructor only adds columns for the direct properties of the bean type. The values are displayed as strings. You can add columns for nested properties by using the dot notation with the [methodname]`setColumn(String)` method.
...
You can also use the [methodname]`setColumns()` method to reorder the columns you have already.
...
Many data types, such as [classname]`String`, [classname]`Number`, primitive types and [classname]`Date`/[classname]`LocalDate`/[classname]`LocalDateTime` are [interfacename]`Comparable`, and therefore are also sortable by default.
...
You can disable sorting for a specific column by using the [methodname]`setSortable()` method. Below is an example of this, disabling sorting on the `address.postalCode` column:
...
You can also define a list of columns as sortable by using the [methodname]`setSortableColumns()` method. This makes all other columns not sortable. The example here shows how to set defined columns as sortable:
...
A hidden column still sends the data required to render it, to the client side. The best practice is to remove -- or not add -- columns if the data isn't needed on the client side. This reduces the amount of data sent, and thereby lessens the load on the client.
...
You can set the column width either relatively, using flex grow ratios by using the [methodname]`setFlexGrow()` method, or you can set them explicitly by using a CSS string value with [methodname]`setWidth()` -- with flex grow set to `0`.
...
When columns are frozen, user reordering is limited to the frozen columns. Below is an example of setting a column as frozen:
...
You can use several basic renderers to configure grid columns. They're described in the sub-sections that follow.
...
```
Depending on how the column was created, it may need additional configuration. In order to determine this, you'll need to consider the two sorting mechanisms supported by Grid: in-memory sorting; or backend sorting.
...
- For columns created from a `ValueProvider`, only in-memory sorting is configured automatically, where the value provider is used to build a comparator. Backend sorting is not configured automatically, as the column doesn't know which property name to pass to the data provider.
...
This next example customizes the in-memory sorting for a column created from a value provider by implementing a comparator that is not case-sensitive:
...
This simple example uses the [methodname]`setMultiSort()` method to enable multi-sorting:
...
You can use normal CSS to style the content in grid cells. Although the `Grid` component itself is in the shadow DOM, the actual values (i.e., cell contents) are in slots and therefore in the light DOM.
...
You can also use a class to apply styles to a specific component instance. For example, to apply rounded borders and center images in a grid with a _styled_ class name, you would do something like this:
