...
[NOTE]
Because the `em` length unit is relative to the font size of the element, don't use it for column widths. If the font sizes of the header, body or footer cells differ, the resulting column widths will be inconsistent, leading to misaligned content. When length relative to the font size is needed, use instead the `rem` length unit -- or make sure that the font size is consistent across the cells.
