...
.About the `em` length unit
[NOTE]
Because the `em` length unit is relative to the font size of the element, it's not recommended to use it for column widths. If the font size of the header, body or footer cells differs, the resulting column widths will be inconsistent, leading to misaligned content. If length relative to the font size is needed, use the `rem` length unit instead, or make sure that the font size is consistent across the cells.
