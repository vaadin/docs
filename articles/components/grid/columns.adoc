...
ifdef::flow[]
=== Responsive Columns [badge-flow]#Flow#

Column visibility can also be adjusted based on the width of the browser window. For example, on narrow screens you can hide the individual columns and show a single column that combines their content. The example below checks the window width when the grid is attached, and again whenever the browser window is resized:

[source,java]
----
include::{root}/src/main/java/com/vaadin/demo/component/grid/GridResponsiveColumns.java[tags=snippet,indent=0]
----
endif::[]
