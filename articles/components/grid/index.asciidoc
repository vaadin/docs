...
This mode virtualizes horizontal scrolling and only renders body cells when their corresponding columns are inside the visible viewport.
...
- *No performance improvement if all columns are visible:* The lazy column rendering mode can only improve the rendering performance of when a significant portion of the columns are outside of the Grid's visible viewport. It has no effect on Grids where all columns are visible without horizontal scrolling.
- ```
[.example]
--
