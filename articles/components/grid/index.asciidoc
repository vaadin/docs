...
Cells can be styled dynamically, based on application logic and the Grid's data, though custom part names.
This can be used, for example, to highlight specific rows and apply custom styling to specific columns.

In the example below, bold font weight is applied to the _Rating_ column with a `font-weight-bold` part name, and the rows are colored red and green, based on their rating, with `low-rating` and `high-rating` part names, respectively. The styling itself is applied in a stylesheet with `vaadin-grid::part(custom-name-here)` selectors.
