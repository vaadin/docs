...
When dealing with a large number of columns in the Grid, performance can become a concern.
To optimize the rendering of cells in the grid for such scenarios, Vaadin Grid provides the "column rendering" feature with a special "lazy" rendering mode.
