...
Grids with large numbers of columns can exhibit performance issues.
If many of the columns are typically outside the visible viewport, rendering performance can be optimized by using a "lazy column rendering" mode.

This mode enables virtual scrolling horizontally and only renders body cells when their corresponding columns are inside the visible viewport.

Using "lazy" rendering should be used only if you're dealing with a large number of columns and performance is your highest priority.
For most use cases, the default "eager" mode is recommended.
...
- *No performance improvement if all columns are visible:* The lazy column rendering mode can only improve the rendering performance when a significant portion of the columns are outside of the Grid's visible viewport. It has no effect on Grids where all columns are visible without horizontal scrolling.
