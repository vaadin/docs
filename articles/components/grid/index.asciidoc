...
Lit templates still enable event handling on the server side but you can't, for example, disable or change the text of a button from the event handler.
...
In addition to setting any fixed or relative value, the height of a grid can be set by the number of items in the dataset, meaning that the grid grows and shrinks based on the row count.
...
Although it's technically possible to freeze any column, this feature should primarily be used to freeze columns at the start or end of the grid, leaving the remaining columns between them unfrozen.
...
Sorting helps users find and analyze the data, so it's recommended to enable it for all applicable columns, except in cases where the order of items is an essential part of the data itself (such as prioritized lists).
...
It's useful for displaying more information on-screen without having to scroll.
