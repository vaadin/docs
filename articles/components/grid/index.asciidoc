...
* For providing additional details on the contents of a cell (if an <<item_details,item details panel>> would be overkill or otherwise undesirable)
* For providing the full text of a cell if it's too long to feasibly fit into the cell itself (if <<wrap_cell_content,wrapping the cell contents>> is insufficient or otherwise undesirable)
* For providing textual explanations for non-text content such as status icons

////
TODO: Add grid tooltip generator example:

A Grid with 3 columns:
* Name
* Date of birth, tooltip: “Age: nn”
* Notes (clipped to just 10em or so), tooltip: full text (long lorem ipsum that wouldn’t make sense in a cell)
* Status, an icon representing one of 3 states: Unconfirmed (question-circle), Active (check-circle), Inactivated (close-circle)), tooltip: name of status
////
