...
A separate multi-sort on shift-click mode [since:com.vaadin:vaadin@V23.3] combines the two by multi-sorting only when the column header is clicked while holding shift.
The order in which multi-sort criteria (columns) are evaluated is determined by the multi-sort priority setting.

// TODO: Add live sample of Grid with mutiSort and multiSortPriority="append"
