...
Vertical Layout places components, top-to-bottom in a column. By default, it has a 100% width and an undefined height. Its width is constrained by its parent component and its height is determined by the components it contains.
...
It's also possible to align horizontally individual components, overriding the alignment set by the parent layout.
