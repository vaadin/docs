...
Successful operations shouldn't routinely be declared using notifications, as these can be more distracting than useful for the user.
...
Warning notifications should be persistent, and provide the user with a button that closes the notification and/or allows the user to take appropriate action.
...
For simple, one-off notifications, it's convenient to use the static [methodname]`show()` helper method.
