...
page-title: Rich Text Editor component | Vaadin components
description: Rich Text Editor allows the user to author text that has rich formatting.
meta-description: The Vaadin Rich Text Editor component allows users to author text that has rich formatting. Discover how to implement and customize it in your application.
page-links:
...
[NOTE]
Only Base64-encoded data URLs are supported for images. Web URLs are not supported.
...
The JSON-based Delta format consists of an array of operations to apply to a document. Rich Text Editor specifically only uses insert operations, each operation sequentially adding content to the document.
Operations can have attributes, such as whether to render a piece of content with a specific text style, or as a link. For the full specification of the format, see the https://github.com/quilljs/delta[Quill Delta GitHub repository].
...
|[.text-color.rte-button]#A#
|Text color
|Changes text [.red]#color#.

|[.text-background.rte-button]#A#
|Text background
|Changes text [.yellow]#background#.
|===
...
<style>
  ...
  .icon > .content {
    ...
    height: calc(var(--docs-space-s) * 6.5);
  }
  ...
  .block-quote {
    display: block;
    line-height: calc(var(--docs-line-height-s) / 0.7);
    font-size: calc(var(--docs-font-size-m) * 2);
  }

  .code-block {
    line-height: calc(var(--docs-line-height-s) * 1.9);
    font-size: calc(var(--docs-font-size-m) * 1.2);
  }
</style>
++++
