...
[.sr-only]
<<form_inputs, Visualization and interaction components>>
...
=== Accessibility
image::{articles}/components/_images/a11y.png[opts=inline, role=banner]
Designed for compliance to WCAG 2.1 level AA. Check the current status of components and read our accessibility statement.
