...
The inline [methodname]`.renderer()` function is encapsulated by the https://lit.dev/docs/templates/directives/#guard[`guard` directive] to avoid reassigning the renderer property every time the view is rendered.
Basically, the renderer function is cached, improving performance.
