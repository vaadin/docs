...
It's more likely to be visible at small viewport sizes when the field is focused, and screen reader users can read the instructions before entering the field.
...
It's recommended to provide hint text to let users know how required fields are indicated:
...
In some cases, the user isn't allowed to edit the value of certain fields.
