...
Vaadin icons are defined on a 16&times;16 pixel canvas without whitespace around the icon, but they're rendered by default in 24&times;24 pixel size. Many components, though, apply padding around the icons to shrink them to a size matching the Lumo icons.
