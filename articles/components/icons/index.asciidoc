...
Apart from the built-in Lumo and Vaadin icons, you can also use icons from other icon sets.
However, icon sets come in many different formats, which means that there is no standard way to integrate and use them with Vaadin that would work for all icon sets.
Unfortunately, this implies that there are no standard, simple steps to follow that would work for all icon sets.
Nevertheless, as an example, you can consider how projects generated from https://start.vaadin.com[Vaadin Start] use the icons from the https://icons8.com/line-awesome/[Line Awesome] icon set.
