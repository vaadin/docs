...
Two icon collections are available through Vaadin dependencies: Vaadin Icons and Lumo Icons. The Flow API provides enums for these collections that makes them easy to use.
...
Thanks to inline SVG rendering, the icon can be styled with CSS. For example, you could change its fill color or stroke.
...
- Icon-specific CSS class, such as `fa-calendar`, which requires a stylesheet defining CSS classes for each icon, usually bundled with the icon font;
- Ligature, such as `calendar`, which requires an icon font that supports ligatures; and
- Character code, such as `f199`, corresponding to the code point of the icon's glyph in the font.
...
You may configure a few properties or styles of the icons.
...
In situations where icons are used to convey information on their own (e.g., in a table column to convey a value in a graphical manner), they need to be given an accessible name and an ARIA image role attribute in order to be correctly announced by screen readers.
