...
By default, the editor is opened by clicking the edit Button in the last column. However, this button column can be removed if you want to provide an alternative way to engage the editor. For example, you set it to open using a double-click like so:
...
When using a bottom-positioned editor, make sure there's enough vertical space to fit both the grid and the editor comfortably. Incidentally, a bottom-positioned editor is generally an inappropriate option for longer forms.
