...
Overlays aren't constrained to the CRUD's size, which makes them ideal for complex forms.
...
Use this position when there is sufficient horizontal space to accommodate both the grid and the editor, and it's beneficial for the user to be able to view and interact with the grid while the editor is open.
...
This is useful when you wish to customize the Grid, for example to place the edit Button in the first column, or to apply <<../grid#tooltips,tooltips>>.
...
.Edit Column
[NOTE]
You need to explicitly add an edit column to the replacement Grid to edit items.
In addition, Grid doesn't have sorting and filtering enabled by default.
...
For more information about sorting and filtering, see the <<../grid#,Grid documentation>>.
