...
Note that, unlike <<../number-field#,Number Field>>, Time Picker accepts values that do not align with the specified step.
...
For example, _"15 minutes"_, _"30 minutes"_ and _"2 hours"_ are valid steps, whereas _"42 minutes"_ is not.
...
[NOTE]
The overlay does not appear for steps less than 900 seconds (15 minutes), in order to avoid showing an impractical number of choices.
...
You can disable this, so as to open the overlay only when the toggle button or kbd:[Up]/kbd:[Down] arrow keys are pressed.
...
If the minimum and maximum values are not sufficient for validation, you can also apply custom validation.
