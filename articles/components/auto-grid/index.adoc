...
- `Pageable.getOffset` is the offset of the first item to fetch.
- `Pageable.getPageSize` is the maximum number of items to fetch.
- `Pageable.getSort` is the sort order of the items. Each sort order contains the property by which to sort (`Sort.Order.getProperty()`), and the direction (`Sort.Order.getDirection()`)
- `Filter` is the filter to apply to the items. 

The last one, the filter is a tree structure of `Filter` instances, where each `Filter` instance is either an `AndFilter`, an `OrFilter`, or a `PropertyStringFilter`:
