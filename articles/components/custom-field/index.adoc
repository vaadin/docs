...
The value propagation between Custom Field and its child components heavily depends on its structure, contained components, and their types. For this reason, no default implementation is provided. To set up the value propagation, Custom Field provides two methods that should be implemented manually: `generateModelValue` and `setPresentationValue`.
...
Custom Field only supports string values, but it provides control over the format of the value with a parser and formatter. The parser and formatter define how Custom Field's value should be split to child component values and vice versa. When a child component emits a change event or Custom Field's value changes programmatically, Custom Field automatically propagates values based on the result of the parser or formatter.

The default value format is a string concatenation of the child component values, separated by the `\t` character.
