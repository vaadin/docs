...
Since Custom Field can be organized in many different ways, there is no default strategy for the value propagation between the Custom Field and its child components. Instead, Custom Field provides two methods that should be implemented manually: `generateModelValue` and `setPresentationValue`.
