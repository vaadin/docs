...
- The `generateModelValue` method defines how to generate a value of the Custom Field type from the child components values. Custom Field automatically triggers this method to update its value when a child component emits a change DOM event on the client-side. In this method, you typically need to collect values from all child components and return a single value of the Custom Field type based on those values.
- The `setPresentationValue` method receives a value of the Custom Field type and defines how to distribute it to the child components. Custom Field automatically triggers this method to update child component values when its value changes programmatically on the server-side. In this method, you typically need to split the given value into parts and apply them to each individual child component respectively.
