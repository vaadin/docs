...
The value propagation between Custom Field and its child components heavily depends on its structure and components it contains. For this reason, no default implementation is provided. To set up the value propagation, Custom Field provides two methods that should be implemented manually: `generateModelValue` and `setPresentationValue`.

- The `generateModelValue` method defines how to generate a Custom Field value from the child components values. Custom Field automatically triggers this method to update its value when a child component emits a change DOM event on the client-side. In this method, you typically need to collect values from all child components and return a single value of the Custom Field type based on those values.
- The `setPresentationValue` method receives a Custom Field value and defines how to distribute it to the child components. Custom Field automatically triggers this method to update child component values when its value changes programmatically on the server-side. In this method, you typically need to split the given value into parts and apply them to each individual child component respectively.
...
Custom Field only supports string values, but it allows customizing the format of the value by providing a parser and formatter. The default value format is a string concatenation of the child component values, separated by the `\t` character.

==== Parser

The parser is used for updating child component values when Custom Field's value is changed programmatically.
