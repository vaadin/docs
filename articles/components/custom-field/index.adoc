...
Custom Field doesn't offer a default strategy for value propagation between the Custom Field and its child components, as the child components can vary widely in type and may not be direct children of the Custom Field in the DOM. Instead, Custom Field provides two methods that should be implemented manually: `generateModelValue` and `setPresentationValue`.

The `generateModelValue` method defines how to convert child components' values to a value of the Custom Field type. This method is automatically triggered whenever a change event is detected on a DOM element within the Custom Field.

The `setPresentationValue` method defines how to distribute the Custom Field's value to the child components. This method is triggered when the Custom Field's value changes programmatically on the server-side.
