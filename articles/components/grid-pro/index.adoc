...
- kbd:[Enter] and kbd:[Shift + Enter] save changes and exits edit mode.
- kbd:[Tab] and kbd:[Shift + Tab] save changes and shifts focus to the next or previous editable cell, respectively, while remaining in edit mode.


== Modes

Grid Pro has a few modes:  edit on single-click; single cell edit; enter next row; and edit column. These are described in the sub-sections that follow.
...
By default, when in edit mode, kbd:[Tab] and kbd:[Shift+Tab] move the focus to the next or previous editable cell, respectively, while remaining in edit mode.

Single Cell Edit is a mode that allows tabbing from one cell to another without exiting from edit mode.
...
Pressing kbd:[Enter] and kbd:[Shift+Enter] saves any changes and exits edit mode, by default. However, kbd:[Enter] and kbd:[Shift+Enter] can be made to shift focus to the editable cell in the next or previous row, respectively, by using the Enter Next Row mode.
...
When you've entered incorrect or invalid data, you can rollback changes.
...
Editable cells are indicated with a hover effect, by default. You can also distinguish them by highlighting either editable or read-only cells. This is good for grids containing both editable and read-only cells.
...
Inline editing is recommended when the user typically needs to make many small changes to different items, and when quick editing is important.
...
Use non-inline editing when any of the editors need to be larger than a simple field (e.g., Text Area or multi-select fields), or when fields alone may be insufficient (e.g., when helpers, validation errors or other features are needed). Also, use it when explicit save or cancel actions are beneficial, for example to prevent accidental edits.
