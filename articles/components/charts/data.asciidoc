...
parameter is [literal]#++false++#, the chart isn't updated immediately. This is
...
Turbo mode only works with specific types of series, and other series that aren't compatible will not render correctly when their number of data items exceeds the configured threshold.
