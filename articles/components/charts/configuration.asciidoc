...
`LOGARITHMIC`:: For numerical values, as in the linear axis, but the axis is scaled in the logarithmic scale. The minimum for the axis __must__ be a positive non-zero value ( [methodname]`log(0)` isn't defined, as it has limit at negative infinity when the parameter approaches zero).
