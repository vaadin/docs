...
The Markdown syntax enables rich text formatting with headings, lists, links, and more.
HTML tags are also supported in Markdown.
The Message List component sanitizes all content internally to prevent potentially dangerous HTML from being rendered.
...
Message List combined with Message Input creates an effective user interface for AI chat applications. Implementing an AI chat interface involves:

* A `MessageList` with Markdown formatting to display the conversation history
* A `MessageInput` component for user input
* A backend service connection for AI interactions

To ensure a smooth user experience, consider the following best practices:

* Appending tokens to the assistant message in real time as they are received
* Disabling user input during processing and showing visual indicators to reflect system activity
* Preserving conversation history by loading previous messages and maintaining state across sessions
