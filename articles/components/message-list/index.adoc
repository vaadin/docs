...
The Markdown syntax allows you to create headings, lists, links, and other formatting options.
HTML tags can also be used in Markdown.
All content provided to the component is internally sanitized to prevent potentially dangerous HTML from being rendered.

See the standalone <<../markdown#,Markdown component>> documentation.
...
Message List combined with Message Input creates an effective user interface for AI chat applications. To implement an AI chat interface:

* Use a `MessageList` with Markdown formatting to display the conversation history
* Add a `MessageInput` component for user input
* Connect to a backend service for AI interactions

For the best user experience:

* Stream responses by creating an empty assistant message and appending tokens as they arrive
* Manage loading states by disabling input during processing and adding visual indicators
* Maintain conversation context by loading previous messages on initialization and preserving history between sessions
