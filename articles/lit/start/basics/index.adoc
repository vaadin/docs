...
description: Learn the basics of Hilla application development by creating a project, defining and accessing Endpoints, and building views.
page-links:
...
HTML, and CSS. You don't need to be an advanced developer, but understanding the syntax and basic concepts makes it easier to follow along.


== Requirements

This tutorial generally takes from ten to twenty minutes, depending on whether you decide to do each part step-by-step. As for software, you need the following:

include::{articles}/lit/start/quick.adoc[tag=requirements]

https://code.visualstudio.com/[Visual Studio Code] is used in this tutorial. For help setting up VS Code for TypeScript and Java, see the related https://youtu.be/G_aJONwi0qo[YouTube] video. You can use any IDE that supports Java, TypeScript, HTML, and CSS.


== Create a Hilla Project

First, you'll need to use the Hilla CLI to create a new project. You can do that by executing the following from the command-line:
...
Alternatively, you can https://start.vaadin.com/dl?preset=hilla-tutorial&projectName=hilla-todo[download the starter zip-file] and extract it.
endif::[]
ifdef::react[]
[source,terminal]
----
...
Alternatively, you can https://start.vaadin.com/dl?preset=react-tutorial&projectName=hilla-todo[download the starter zip-file] and extract it.
endif::[]

After you unpack the downloaded file, import the project into the IDE you chose.

If you open one of the `.ts` files in the `frontend/views` folder, you might see red underlines for unresolved imports. They'll be resolved when you start the development server for the first time, when all dependencies are downloaded.

.Importing into VS Code
[%collapsible]
====
Import the project by either navigating to the project folder and running `code .` (note the dot), or by choosing menu:File[Open...] in VS Code and selecting the _project folder_.
...
Incidentally, VS Code should suggest these for you when you open the project.
====
...
`frontend`::
The folder where your views and front-end code are located.

`src`::
The folder where your Java back-end code is located.
...
Begin by setting up the data model and services for accessing the database. You can do this defining an entity, and then creating a repository for accessing the database.

This tutorial shows how to use an in-memory H2 database and JPA for persistence. The starter you downloaded includes the needed dependencies in the `pom.xml` file.
...
Next, create a _repository_ for accessing the database. You only need to define an interface with type information: Spring Data handles the implementation. Create a new file, `TodoRepository.java`, in `src/main/java/com/example/application`, with the following content:
...
You now have all of the necessary back-end code in place to start building a UI. Hilla can generate TypeScript versions of the Java files you created. To do this, run the project from the command-line:

include::{articles}/lit/start/quick.adoc[tag=run]

The first time you run the application, it may take up to a few minutes because Hilla downloads all of the dependencies and builds a front-end bundle.
Subsequent builds won't download dependencies, so they'll run much faster.

When the build is finished, you should see the application running on http://localhost:8080.
...
One of the key features of Hilla is type-safe server access through _endpoints_. When you define an `@Endpoint`, Hilla creates the needed REST-like endpoints, secures them, and generates TypeScript interfaces for all of the data types and public methods used. Having full-stack type safety helps you to stay productive through autocomplete and helps to guard against breaking the UI when you change the data model on the server.
...
<2> By default, endpoint access requires an authenticated user. `@AnonymousAllowed` enables access for anyone. See <<{articles}/lit/guides/security/configuring#,Configuring Security>> for more information on endpoint security.
<3> Use Spring to inject automatically the `TodoRepository` dependency for database access.
...
ifdef::lit[]
Hilla uses the https://lit.dev/[Lit] library for client-side views. Lit is a lightweight and highly performant library for building reactive components with declarative templates.
endif::[]
ifdef::react[]
Hilla can use React for client-side code. You can use your React skills and favorite libraries, and you'll have first-class support for Vaadin components.
endif::[]

Next, you'll create a view for adding and viewing to-do items. You can choose to build it step-by-step, learning some concepts along the way, or to copy the complete view implementation. When you're ready, you can learn more about <<{articles}/lit/components/create#,creating components>>.

ifdef::react[]
In this example, you'll use Formik to create the form. You'll need to add it to the project:
...
<2> Register the new component with the browser. This makes it available as `<todo-view>`. The routing in `index.ts` is already set to show it when you navigate to the application.
...
----
<1> The list of `Todo` items is private and decorated with `@state()`, so Lit watches it for changes.
<2> A Hilla `Binder` is used to handle the form state for creating new `Todo` objects. `TodoModel` is automatically generated by Hilla. This describes the data types and validations that `Binder` needs. Read more about forms in <<{articles}/lit/guides/forms/binder#,Binding Data to Forms>>.


[discrete]
=== Define Styles

You'll need to define some padding on the view. Web Components have a default `display` value of `inline`, which is rarely preferred. Set it to `block` instead by adding the following code to the `themes/hilla-todo/styles.css` file. This is where you define styles in a Hilla application, as explained in <<{articles}/lit/guides/styling#,Styling>>.
...
----
<1> The Text Field component is bound to the `task` property of a `Todo` using `${field(this.binder.model.task)}`. You can read more about forms in <<{articles}/lit/guides/forms/binder#,Binding Data to Forms>>.
<2> The click event of the _Add_ button is bound to the `createTodo()` method.
...
[discrete]
=== Update View State & Call the Backend
...
<3> The `getTodos()` method is automatically generated by Hilla based on the method in `TodosEndpoint.java`. The method was imported in the head of the file. Because of the `await` keyword, it waits for the server response without blocking the UI.
...
Now you're ready to run the project. Do so by executing following from the command-line:

include::{articles}/lit/start/quick.adoc[tag=run]

[.secondary-text.small]
The first time you run the application, it may take up to a few minutes because Hilla downloads all the dependencies and builds a front-end bundle. Subsequent builds won't download dependencies, so they'll run much faster.

When the build is finished, if successful, you'll see the application running on http://localhost:8080.

You should now have a fully functional to-do application. Notice that when you refresh the browser, it keeps the same `todo` items, as they're persisted in the database.
