...
[source,terminal]
----
npx @vaadin/cli init --preset hilla-react-tutorial hilla-todo
----
endif::[]
...
+
ifdef::react[]
* https://plugins.jetbrains.com/plugin/9997-styled-components--styled-jsx[Styled Components & Styled JSX]
endif::[]
...
Hilla can generate TypeScript versions of the Java files you create. To achieve this, run the project from the command line with the following command:

include::{root}/articles/lit/start/quick.adoc[tag=run]
...
Define a `render()` method in the To Do view that returns an `html` template literal inside the class.
...
import { Button, Checkbox, TextField } from '@hilla/react-components'; // <1>
import { TodoEndpoint } from 'Frontend/generated/endpoints';
