...
Hilla is developed using Java as reference language for backend. As it parses `.class` files, it is able to accept those coming from other languages, but support from some features may vary according to the language of origin. For example, in Java an annotation is used to mark a field as not null, while in Kotlin that's part of the language syntax.
