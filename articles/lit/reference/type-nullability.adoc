...
Along with `@Nonnull` annotations you could also use package-level `@NonNullApi` annotations. It makes all the nullable types in package non-nullable by default. All nested types: `List` and `Map` items, etc, - are also affected.
...
There could be a case where several annotations are applied to the same type. To resolve this collision, each annotation has its priority; among all applied annotations the one with the highest priority is selected.

* `@NonNullApi` annotation has priority of `10` which makes it the weakest. This annotation applies only to types without any nullability annotation.
* `@Nullable` annotation has priority of `20`. It makes any type that affected by `@NonNullApi` annotation nullable.
* `@Nonnull` annotation has priority of `30` which makes it the strongest. It makes any type non-nullable again.
