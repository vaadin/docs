---
title: UI Component type definitions
...
The Lit syntax for binding listeners doesn't let TypeScript know what type of component the listener belongs to, so this needs to be handled manually by specifying the correct type:
...
This API isn't compatible with Lit templates.
...
However, this approach isn't clean, as it requires us to write `as ColorItem[]` every time the method is called.
