...
[NOTE]
If the endpoints external to the main application have autowired Spring dependencies, make sure that Spring can find them. Otherwise, Hilla will try to instantiate them using a default no-arguments constructor, which will not trigger dependency injection.
