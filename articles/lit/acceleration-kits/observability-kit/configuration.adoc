...
- `service.name` is an attribute that's used to distinguish a service by name. The default value is `vaadin`. It's the only attribute with a default value and the only one required.
