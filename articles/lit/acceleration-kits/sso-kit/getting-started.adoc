...
Add the `sso-kit-starter` module and other required dependencies to the [filename]`pom.xml` file of a Vaadin application like so:
...
By default, the package name in generated Hilla projects is `com.example.application`. SSO Kit uses the package name `dev.hilla.sso.starter`. To be able to find the annotated endpoints in SSO Kit that are required, you have to allow-list its package name in your Spring Boot application. To do this, open your [filename]`Application.java` file and add these packages to the `@SpringBootApplication` annotation like so:
