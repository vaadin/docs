...
You can create a Hilla application without auuthentication by entering the following from the command-line:
...
By default, the package name in generated Hilla projects is `com.example.application`. SSO Kit uses the package name `dev.hilla.sso.starter`. To be able to find the annotated endpoints in SSO Kit that are required, you have to whitelist its package name in your Spring Boot application. To do this, open your [filename]`Application.java` file and add these packages to the `@SpringBootApplication` annotation like so:
