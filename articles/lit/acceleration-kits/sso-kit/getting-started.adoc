...
The SSO Kit Client provides the `User` class which contains information about the authenticated user. You can get the user information by calling the asynchronous `SingleSignOnContext.getUser()` function. You can implement yours if you want to customize the returned object and its fields.
