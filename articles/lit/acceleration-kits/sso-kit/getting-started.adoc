...
Then add the state and the to `App.tsx` and wrap the `RouterProvider`:
...
ifdef::lit[]
You can protect your views one by one by verifying that authentication has happened before loading the view. To do that, open the [filename]`frontend/routes.ts` file and replace the `component` property with a conditional action as follows:
...
In this example we assume that views without a `component` property are protected. In a real-world application, you should add explicit properties to handle access according to authentication and user roles. See <<{articles}/lit/guides/security/role-based-access# Role-based access control>> for an example.

With the assumption just made, it is enough to filter the menu excluding unauthorized views by amending the view filter in [filename]`main-layout.ts`:
...
ifdef::react[]
You can protect your views one by one by verifying that authentication has happened before loading the view. To do that, open the [filename]`frontend/routes.tsx` and add the `requiredLogin` parameter to a view:
...
// Enrich the ViewMeta with AccessProps
...
// Add requiresLogin to the about view
{
  path: '/about',
  element: <AboutView />,
  handle: { icon: 'la la-file', title: 'About', requiresLogin: true }
},
----
...
// Store the authenticate user
const [user, setUser] = useState<User | undefined>();

// Fetch the authenticate user from the server
useEffect(() => {
