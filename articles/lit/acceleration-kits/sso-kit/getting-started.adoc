...
For Hilla to be able to parse and generate endpoints from an application and from SSO Kit for the frontend, you have to add a configuration to the Hilla Maven plugin with your application and SSO Kit package names like this:
...
Start the application by executing `./mvnw` (`.\mvnw` on Windows). Then try the application in the browser. It should work correctly, except that when you click on the `Say hello` button, nothing happens. This is because the endpoint is no longer accessible without authentication.
...
The SSO Kit provides a default endpoint to get information about the authenticated user. You can implement it if you want to customize the returned object and its fields.

As the About page is now protected, this is an excellent place to show some information about the current user:
...
SSO Kit provides two methods for logging out a user. They're defined by the OpenID Connect specification like so:
...
RP-Initiated Logout (i.e., Relaying Party, the application) enables the user to logout from the application, ensuring the connected provider session is terminated.


=== Back-Channel Logout

Back-Channel Logout is a feature that allows the provider to close user sessions from outside the application. For example, it can be done from the provider's user dashboard or from another application.

To enable this feature in the application, you need to set the `hilla.sso.back-channel-logout` property to `true`. You would do this as you see here:
