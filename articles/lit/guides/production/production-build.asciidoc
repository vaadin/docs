...
The `prepare-frontend` goal validates whether the `node` and `npm` tools are installed and aren't too old (i.e., `node` version 10 or later and `npm` version 5.6 or later). It also installs them automatically, if they are missing, to the `.vaadin` folder in the user's home directory. If they're installed globally, but too old, there's an error message suggesting that you install newer versions. `Node.js` is needed to run `npm` to install front-end dependencies and `Vite`, which bundles the front-end files served to the client.
