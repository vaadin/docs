---
title: Frontend
description: How to setup and write front-end tests in TypeScript.
order: 10
---
:lit:
...
For writing unit tests in TypeScript, several tools are recommended. https://karma-runner.github.io[Karma] is good for running tests. Use https://vitejs.dev[Vite] for building tests to run in the browser.
...
Before writing and running tests, you'll need to install some basic packages and necessary plugins. This setup assumes that you already have `Vite` installed and configured.
...
Additionally, you have to install browser launchers. You can do this by executing the following from the command-line:
...
----
ifdef::lit[]
For Lit, install Open WC library with testing helpers. It provides useful tools for testing web components. To install it, execute the following:
...
ifdef::react[]
For React, install the React Testing Library. It contains useful tools for testing react components. To install it, execute the following:
...
Create `karma.config.js`, or `karma.config.cjs` if you use ESM. However, Karma does not yet support ESM.
...
[NOTE]
For converting from TypeScript to JavaScript, Karma uses `ts-node` internally -- it doesn't allow you to change this. Since it could cause various issues, it's recommended that you don't use it.
...
<2> If in the CI environment, some unnecessary options can be omitted to speed up testing, slightly.
...
<6> For coverage, this uses a combination of `karma-vite` and `karma-coverage`. They have the built-in integration.
<7> Generates human-readable code coverage as HTML. Since it's unnecessary in the CI environment, you can drop it.
...
To learn how to customize Karma configuration, please read the https://karma-runner.github.io/6.4/config/configuration-file.html[Karma documentation].
