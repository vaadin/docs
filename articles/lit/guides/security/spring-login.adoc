...
To implement your own security configuration, create a new configuration class that extends the [classname]`HillaWebSecurityConfigurerAdapter` class, and annotate it to enable security.
[classname]`HillaWebSecurityConfigurerAdapter` is a helper which extends from the link:https://docs.spring.io/spring-security/site/docs/5.2.0.RELEASE/api/org/springframework/security/config/annotation/web/configuration/WebSecurityConfigurerAdapter.html[WebSecurityConfigurerAdapter] class.
It takes care of the configuration for Hilla requests, so that you can concentrate on your application-specific configuration.
...
----
  @Override
  protected void configure(HttpSecurity http) throws Exception {
    http.authorizeRequests().requestMatchers("/public-view").permitAll(); // custom matcher
    super.configure(http);
  }
----
