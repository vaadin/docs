---
title: Endpoints
description: A server-side Java endpoint is a back-end method that's exposed to be able to call from client-side TypeScript code.
order: 30
---
:lit:
// tag::content[]
...
Hilla bridges Java back-end endpoints and a TypeScript frontend. It generates TypeScript clients to call the Java backend in a type-checkable way. The <<../reference/endpoint-generator#,Endpoint generator>> reference page contains details about the generator itself.
...
An endpoint method can return or receive a parameter as an entity (i.e., a non-primitive type). In this case, the generator also creates a TypeScript interface for the entity.
...
The following example creates two buttons: one that makes requests to a slow endpoint method, and another that triggers the cancellation of previously created requests. If a request is subject to cancellation, errors must be handled as the endpoint method will throw `AbortError`.
...
Any public method in any Hilla endpoint can be accessed with the `\http://${base_url}/${prefix}/${endpoint_name}/${method_name}` URL format.
...
And the `${method_name}` is the public method name from the Java class.

For an application started locally with the [classname]`CounterEndpoint` endpoint defined as shown, the endpoint URL will be: `\http://localhost:8080/connect/counterendpoint/addone`
...
Since all of the dependencies needed for validating beans and methods are present, you can reuse them in any part of your project -- not just in the endpoint methods. For example:
...
If required, you can throw an [classname]`EndpointValidationException` from an endpoint method. This exception will be caught by TypeScript and the corresponding [classname]`EndpointValidationError` will be raised.
