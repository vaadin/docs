...
// TODO remove mention of webpack, replace with Vite
Hilla has this path alias in <<../reference/configuration#ts-compiler-options,the default TypeScript compiler configuration>> ([filename]`tsconfig.json`); the webpack configuration file ([filename]`webpack.generated.js`) respects the `tsconfig` aliases by default.
