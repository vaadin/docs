...
Hilla has this path alias in <<{articles}/lit/reference/configuring#,the default TypeScript compiler configuration>> ([filename]`tsconfig.json`); the webpack configuration file ([filename]`webpack.generated.js`) respects the `tsconfig` aliases by default.
...
Please refer to <<{articles}/lit/reference/endpoint-generator#type-nullability, Type Nullability>> for more information about how the nullability algorithm works and how to make types non-nullable.
...
Also, see documentation about <<{articles}/lit/guides/client-caching#,caching endpoint data in local storage>> using a generic wrapper.
