...
Any public method in any Hilla endpoint can be accessed with the `\http://${base_url}/${prefix}/${endpoint_name}/${method_name}` URL format. Don't use the URL directly to access the endpoint, instead use the TypeScript methods, i.e., `CounterEndpoint.ts`.
