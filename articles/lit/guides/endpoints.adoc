...
Any public method in any Hilla endpoint can be accessed with the `\http://${base_url}/${prefix}/${endpoint_name}/${method_name}` URL format. Use TypeScript methods instead of the URL when calling it, i.e., `myurl = new URL("http://");`.
