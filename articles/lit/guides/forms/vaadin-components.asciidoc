...
Client <<binder.adoc#,Form binder>> supports out of the box the set of form elements present in Hilla components.
...
You also need to perform the endpoint call to get the entity value, as explained in the tutorials <<binder.adoc#,Binding Data to Forms>> and <<binder-load#,Loading from and Saving to Business Objects>>.
