...
function FullNameComponent({ fullNameModel }: FullNameProps) {
  ...
  const { model, field, required, errors, invalid } = userFormContext<string, StringModel>(fullNameModel);
...
export default function Main() {
  const { model } = userForm(PersonModel);
...
function Person({ model }: PersonProps) {
  const {field, model, value} = userFormContext(model);
...
[source,tsx]
----
const { model, value } = userForm(PersonModel);
...
return (
  <>
    {value.name.toString()}
