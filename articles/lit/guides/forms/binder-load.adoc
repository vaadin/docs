...
export default function PersonView() {
  ...
  useEffect(() => {
    PersonService.loadPerson().then(read);
  }, [])
...
For example, you can define a [methodname]`submit` behavior to submit to a browser-callable service method, as follows:
...
For example, you can use [methodname]`submitTo()` to submit to an browser-callable service method, as follows:

[source,typescript]
----
await this.binder.submitTo(viewService.savePerson);
----
...
private binder = new Binder(this, PersonModel, {
  onSubmit: viewService.savePerson
});
