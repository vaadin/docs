...
export default function PersonView() {
  const { read } = userForm(PersonModel);
...
export default function PersonView() {
  const { reset } = userForm(PersonModel);
...
export default function PersonView() {
  const { clear } = userForm(PersonModel);
...
ifdef::react[]
You can define a [methodname]`submit` callback when calling [methodname]`userForm` to configure the [methodname]`onSubmit` behavior of the binder.
...
export default function PersonView() {
  const { model, submit, field } = userForm(PersonModel, {
