...
import { FieldDirectiveResult, userForm } from '@hilla/react-form';

import EmployeeModel from 'Frontend/generated/com/example/application/EmployeeModel';
// other imports

export default function EmployeeView() {
  const { model, field } = userForm(EmployeeModel);
...
export default function ProfileView() {
  const { model, submit, field } = userForm(EmployeeModel, {
      onSubmit: async (e) => {
...
[source,tsx]
----
import { userForm, userFormNode } from '@hilla/react-form';
import * as owasp from 'owasp-password-strength-test';

import { useEffect } from 'react'

export default function ProfileView() {
  const { model } = userForm(EmployeeModel);

  const passwordBinderNode = userFormContext(model.password);
...
export default function ProfileView() {
  ...
  const { model, field, addValidator } = userForm(EmployeeModel);
...
import { userForm, userFormContext } from '@hilla/react-form';

export default function ProfileView() {

  const { model, field, addValidator } = userForm(PersonModel);

  const nameBinderNode = userFormContext(model.password);
  const usernameBinderNode = userFormContext(model.username);
  const ageBinderNode = userFormContext(model.age);
  const emailBinderNode = userFormContext(model.email);
