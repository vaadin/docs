...
export default function ProfileView() {
  ...
  const passwordBinderNode = useFormContext(model.password);
...
import { useForm, useFormContext } from '@hilla/react-form';

export default function ProfileView() {

  const { model, field, addValidator } = useForm(PersonModel);

  const nameBinderNode = useFormContext(model.password);
  const usernameBinderNode = useFormContext(model.username);
  const ageBinderNode = useFormContext(model.age);
  const emailBinderNode = useFormContext(model.email);
