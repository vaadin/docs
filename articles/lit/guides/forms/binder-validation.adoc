...
When creating forms in TypeScript with {teckstack} and the `Binder` API, all data model constraints are automatically applied to your form fields.
...
export default function EmployeeView() {
  const { model, field: originalField } = useBinder(EmployeeModel);
...
export default function ProfileView() {
  const { model, submit, field: originalField } = useBinder(EmployeeModel, {
      onSubmit: async (e) => {
...
export default function ProfileView() {
  const { model } = useBinder(EmployeeModel);

  const passwordBinderNode = useBinderNode(model.password);
...
----
import { AbstractModel } from '@hilla/form';
import { useBinder, useBinderNode, FieldDirectiveResult } from '@hilla/react-form';
import { useEffect } from 'react'

export default function ProfileView() {

  const { model, field: originalField, addValidator } = useBinder(EmployeeModel);
...
[source,typescriptjsx]
----
import { AbstractModel, Email, Min, NotEmpty, Size } from '@hilla/form';
import { useBinder, useBinderNode, FieldDirectiveResult } from '@hilla/react-form';

export default function ProfileView() {

  const { model, field: originalField, addValidator } = useBinder(PersonModel);

  const nameBinderNode = useBinderNode(model.password);
  const usernameBinderNode = useBinderNode(model.username);
  const ageBinderNode = useBinderNode(model.age);
  const emailBinderNode = useBinderNode(model.email);
