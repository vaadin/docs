...
The [classname]`Validator` instance holds the name of the validator, which can be used to look up a translation for the message.
...
Binder.interpolateMessageCallback = (message, validator, binderNode) => {
  ...
  if (validator.name !== undefined) { // validator.name is not empty for built-in validators
