...
.Defining a Validator for Custom [annotationname]`@StrongPassword` Constraints
[source,java]
----
...
=== Adding Single Field Validators [[single-field-ts-validators]]
...
In this example, we use a third-party library to measure password strength in order to implement the custom validator. Add a dependency to your [filename]`package.json`:
