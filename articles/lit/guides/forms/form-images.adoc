...
[source,java]
----
include::{root}/src/main/java/com/vaadin/demo/fusion/forms/ContactService.java[]
----
...
The [classname]`Contact` instance is first submitted to the [methodname]`saveContact()` service.
...
Only then does the server-side service implementation receive the image string.
The server can then choose to recode the image for more efficient storage, if necessary.

The advantage of using the string type is simplicity; you can use the built-in serialization mechanism of Hilla's form binder and services.
