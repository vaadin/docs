...
- The [methodname]`userForm` React Hook returns a [classname]`BinderControls` object that is responsible for keeping track of the form state, the default and current values, and validation of the data.
endif::[]
ifdef::lit[]
The form binding API consists of three key concepts:
...
ifdef::react[]
. Import the [methodname]`userForm` hook from the `@hilla/react-form` package.
...
. Acquire a [classname]`BinderControls` instance for your view by calling the [methodname]`userForm`:
+
[source,tsx]
----
export default function PersonView() {
  ...
  const { model, field } = userForm(PersonModel);
...
To access the actual current or default value of the form, you can acquire their respective reference by destructing the [classname]`BinderControls` instance as `const { value, defaultValue, ... } = userForm(...)` when calling the [methodname]`userForm` hook.
