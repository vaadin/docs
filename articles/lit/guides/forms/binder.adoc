...
- The [methodname]`useForm` React Hook returns a [classname]`BinderControls` object that is responsible for keeping track of the form state, the default and current values, and validation of the data.
endif::[]
ifdef::lit[]
The form binding API consists of three key concepts:
...
. Acquire a [classname]`BinderControls` instance for your view by calling the [methodname]`useForm`:
+
[source,tsx]
----
...
To access the actual current or default value of the form, you can acquire their respective reference by destructing the [classname]`BinderControls` instance as `const { value, defaultValue, ... } = useForm(...)` when calling the [methodname]`useForm` hook.
