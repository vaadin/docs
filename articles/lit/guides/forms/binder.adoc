...
ifdef::react[]
. Import the [methodname]`useBinder` hook and the [classname]`FieldDirectiveResult` field directive from the `@hilla/react-form` package, and [classname]`AbstractModel` from `@hilla/form` package.
...
export default function PersonView() {
  ...
  const { model, submit, field: originalField, read } = useBinder(PersonModel, {
    onSubmit: async (person) => {
      PersonEndpoint.savePerson(person);
    }
  });
