...
The client-side [classname]`Binder` supports Java browser-callable services for loading and saving the form data, and reuses the metadata from Java Bean validation annotations for client-side validation.
...
- The generated TypeScript models for POJO classes used in browser-callable services, which are used as field references and provide the necessary metadata
...
For example, let us consider a Java browser-callable service with methods for loading and saving a [classname]`Person` bean:

[source,java]
----
/**
 * A Hilla browser-callable service for the person-view.ts form view.
 */
@BrowserCallable
...
Import your [classname]`PersonService` data service and the generated [classname]`PersonModel` from the `frontend/generated` folder:
+
[source,tsx]
----
import { useForm } from '@hilla/react-form';

import { PersonService } from 'Frontend/generated/PersonService';
