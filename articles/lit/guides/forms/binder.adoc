...
- The [methodname]`useBinder` React Hook returns a [classname]`BinderControls` object that is responsible for keeping track of the form state, the default and current values, and validation of the data.
endif::[]
ifdef::lit[]
The form binding API consists of three key concepts:
