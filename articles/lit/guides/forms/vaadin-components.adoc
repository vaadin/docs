...
== Configuring Server Data & Browser-callable Service

When defining Bean objects in Java, apart from the type, you can define validation and error messages.

For this article, the code on the client side is based on the data and services defined in the following Java classes:
...
You also need to perform the service call to get the entity value, as explained in the tutorials <<binder#,Binding Data to Forms>> and <<binder-load#,Loading from and Saving to Business Objects>>.
...
Since the call to the service is asynchronous, one easy way is to
ifdef::react[]
use the Typescript [mathodname]`map` on the list received from the services.
endif::[]
ifdef::lit[]
...
useEffect(() => {
  MyService.getMyOptions().then(entities => setOptions(options))
}, []);
...
return (
  <>
...
...
useEffect(() => {
  MyService.getMyOptions().then(myEntities => setMyOptions(myEntities));
}, []);
...
return (
  <>
