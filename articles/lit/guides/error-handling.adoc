...
// TODO seems like most of the content here could/should be moved under Endpoints?
...
Also see <<{articles}/lit/guides/cache-client-side-data#,this article>> on caching endpoint data in local storage using a generic wrapper.
...
This happens only when you explicitly cancel a request, as explained in <<endpoints/cancelling-requests#,Cancelling Endpoint Requests>>.
