...
First, create a new endpoint in a package of your choice and name it `ReactiveEndpoint`. Add these two methods to it:
...
Now you get to use these methods in your client views.
...
ifdef::react[]
Create the [filename]`frontend/views/reactive/ReactiveView.tsx` file to add a new view.
This looks like the default "Hello world" example, customized with the code that follows.
...
Your new view should now be available in the application.
