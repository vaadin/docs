...
Next you will need to create a server-side endpoint. To do this, open your application in your favorite IDE or editor.
...
Next, you will need to create a view to show the messages. You'll use these methods in your client views.
...
Then, add the new view to the router by importing it in [filename]`frontend/routes.tsx` with:
...
Your new view should now be available in the application. When the `Toggle server clock` button is clicked, the time will appear in the text field and will update each second. You can stop it by clicking again.
