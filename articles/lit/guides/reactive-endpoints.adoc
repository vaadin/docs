...
= [since:com.vaadin:vaadin@v1.2]#Reactive endpoints#

.Experimental feature
[IMPORTANT]
Reactive endpoints are an experimental feature, which means that the behavior, API, and look and feel might still change.
You need to enable the <<feature-flag>> to use reactive endpoints.
...
Note the presence of the `--push` flag; this enables push support, which is an experimental feature in Hilla.
You can also enable it later, for example if you want to add it to an existing application (see <<feature-flag>>).
...
To use reactive endpoints, you need to enable a feature flag.
...
. Enable the [guilabel]*Push support in Hilla* feature
. Restart the application.

=== Using the feature flags properties file

. Create a [filename]`src/main/resources/vaadin-featureflags.properties` file in your application folder
. Add the following line: `com.vaadin.experimental.hillaPush=true`
. Restart the application.
