...
[source,terminal]
----
npx @hilla/cli init --react --push flux-clock
----
endif::[]
...
Then, add the new view to the router by importing in [filename]`frontend/routes.ts` it with:
...
Then, add the new view to the router by importing in [filename]`frontend/routes.tsx` it with:
