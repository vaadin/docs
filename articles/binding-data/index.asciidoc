...
[.example.show-code]
--
This example has a view with two editors, one writable, one read-only.
When the user clicks *Save*, the form data is written to the [classname]`BusinessPerson` object, which is reflected in the read-only form.
