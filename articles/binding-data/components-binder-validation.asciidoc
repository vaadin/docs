...
When a component such as DatePicker (or any other component that accepts a formatted text as input) is used as an optional field, what happens if the user provides some invalid value for it and tries to save the form?
Since the provided value cannot be parsed correctly, a `null` will be provided to the binder and, as the field is optional, the binder will not complain and the validation status would be `true`.
...
Components that implement the [interfacename]`HasValidator` interface and override the default implementation of [methodname]`addValidationStatusChangeListener()` to fire the [classname]`ValidationStatusChangeEvent` properly will always benefit automatically from an up-to-date validation status of their associated binding.
This is because the associated binding instance (upon creation) will register itself for changes in the component's validation status and revalidate itself accordingly.
