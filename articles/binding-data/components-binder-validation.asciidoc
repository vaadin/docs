...
When a component such as DatePicker (or any other component that accept a formatted text as input) is used as an optional field, what happens if the user provide some invalid value for it and try to save the form?
Since the provided value cannot be parsed correctly, a `null` will be provided to the binder, and as the field is optional, then the binder will not complain and the validation status would be `true`.
This behavior can create an illusion for the user that they could save an invalid value!
Thus, there must be a way to prevent the form submission until that invalid value is either cleared out or fixed, no matter the field is a required or not.

There might be different workarounds for such cases, but the preferred solution is the one that keeps Binder Validation Status as the _single source of truth_ when it comes to check the sanity of the data in the form.
That is why [methodname]`addValidationStatusChangeListener` method exists in [interfacename]`HasValidator` interface.

Components that implement [interfacename]`HasValidator` interface and override the default implementation of [methodname]`addValidationStatusChangeListener` to fire the [classname]`ValidationStatusChangeEvent` properly,
will automatically benefit from an up-to-date validation status of their associated binding all the time, since the associated binding instance (upon creation) will register itself for the changes in component's validation status and revalidates itself accordingly.

The following code snippet shows how a component can enable the binding instance to subscribe itself to [classname]`ValidationStatusChangeEvent`:
[source,java]
----
...
----
<1> The validator instance returned by [methodname]`getDefaultValidator` gets called everytime the binding instance validates/revalidates (as a part of the validator chain of the binding).

For a complete implementation, please refer to https://github.com/vaadin/flow-components/blob/master/vaadin-date-picker-flow-parent/vaadin-date-picker-flow/src/main/java/com/vaadin/flow/component/datepicker/DatePicker.java[DatePicker's source code] as an example.
