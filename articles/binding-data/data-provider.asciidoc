...
It's important to understand how lazy data binding works. Here's how it functions:

. The user performs an action that requires the component to display more data. For example, the user might scroll down the list of items in a `Grid` component.
. The component automatically detects that more data is needed, and it passes a link:https://vaadin.com/api/platform/{moduleMavenVersion:com.vaadin:vaadin}/com/vaadin/flow/data/provider/Query.html[`Query`] object as a parameter to your callback methods. This object contains the necessary information about the data that should be displayed next to the user.
. Your callback methods use this [classname]`Query` object to fetch only the required data -- usually from the backend -- and return it to the component, which automatically displays it once the data is available.
