...
== Ensuring Items Identities are Stable and Unique
...
To ensure that, your POJO class should implement [methodname]`hashCode()` and [methodname]`equals()` methods based on the invariant properties of the POJO.
For example, the [classname]`Person` class in the example below have a `final long id` property that is used in the [methodname]`hashCode()` and [methodname]`equals()` methods as the identity of the [classname]`Person` object.
...
You can do that by setting `onlyExplicitlyIncluded = true` for [annotationname]`@EqualsAndHashCode`, and then annotate the entity's `id` property with [annotationname]`@EqualsAndHashCode.Include`.
