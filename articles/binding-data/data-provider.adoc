...
The example below sets the item index provider that uses a service which uses the Spring Data repositor. It fetches all persons and finds the index of the matching item in the list. This is not an optimal solution with a large data set, but it shows how to implement the callback. Callback should always make sure that the data set used to find the item index matches the component's data set with the same sorting and filtering:
...
The index is inconsistent if the data set for the returned index is different from the component's data set. Changing the data set of either side during this call may cause as a result an inconsistent index.
