...
Another open path for navigator migration is to wrap the existing `View` classes
...
and an `AfterNavigationListener` for the `afterViewChange` to the Flow UI. See https://github.com/vaadin/flow-and-components-documentation/blob/master/documentation/routing/tutorial-routing-lifecycle.asciidoc[Routing lifecycle] documentation.
