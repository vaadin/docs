...
The Vaadin 10 Spring add-on doesn't have a feature comparable with `@ViewScope`, so when using MPR with Spring, that scope is not supported.
...
When MPR is on the classpath, it's currently not possible to use hot swap of code for fast deployment of the application during development time. See https://github.com/vaadin/multiplatform-runtime/issues/19[this issue] for details.
