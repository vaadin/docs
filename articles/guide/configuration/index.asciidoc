...
`devmode.serializeSession`::
Determines whether the `VaadinSesion(s)` of the users are serialized on server shutdown when running in development mode. This is set to `false` by default so that only other HTTP session data is serialized/deserialized on development server restart. This allows e.g. access control information to be retained during development and does not require logging in again for each change.
