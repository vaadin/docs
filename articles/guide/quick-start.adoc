...
To start, you'll need JDK 17 or higher (e.g., https://adoptium.net/[Eclipse Temurin JDK]). Next, download the quick start project by clicking on the button here:
...
Once you've done that, unpack the downloaded `ZIP` file into a folder on your computer. Then <<./step-by-step/importing#, import the project into an IDE>>.


== Add Code

Next, you'll add code. Open the [filename]`MainView.java`, located in the  `src/main/java/com/example/application/views/main/` directory. Replace the code in that file with the following:
...
That may not seem like much, but enough to be able to run the application. To run it in your IDE, launch [filename]`Application.java`, located under `src/main/java/org/vaadin/example` directory.
...
Continue learning Vaadin Flow by reading and watching the <<../tutorial/overview#, in-depth 2-hour course>> -- or create another application with custom views with Vaadin Start.
