...
Normally at least the repository holding Vaadin libraries is needed.
...
With `20.+` version specification, you choose to use the latest version of Vaadin 14, but you can also give exact version.
...
`vaadinClean`::
  Cleans the project completely and removes `node_modules`, `package-lock.json`, `webpack.generated.js`, `tsconfig.json`, `types.d.ts`, `pnpm-lock.yaml` and `pnpmfile.js`.
...
The application is run with the `appRun` task:
...
[[pre-release]]
== Using Plugin pre-release Version

There is a snapshot version of the plugin that is pushed to the pre-release repository.
...
For production, the front-end is transpiled for older browsers and optimized, as described in <<{articles}/guide/production#,Deploying to Production>>.
...
Defaults to `null` which will use the auto-detected value of of the main SourceSet, usually `build/resources/main/META-INF/VAADIN/webapp/`.
...
Generate embeddable web components from WebComponentExporter inheritors.
...
It will then be used instead of globally or locally installed installed 'node'.

`useDeprecatedV14Bootstrapping: Boolean = false`::
Defines if we should run in legacy V14 bootstrap mode.
Defaults to false.
...
Folder where flow will put TS API files for client projects.
...
Usually needed for firewalled corporate environments where the `node.js` download can be provided from an intranet mirror.
