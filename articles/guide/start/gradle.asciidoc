...
`gradlew` and `gradlew.bat`::
  ...
  So `gradlew` and `gradle` commands can be used interchangeably if you have Gradle installed already, and you prefer to use your installed Gradle.
...
Obviously, you must replace it with `gradlew.bat` if you are on a Windows machine.
