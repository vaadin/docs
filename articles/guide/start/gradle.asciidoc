...
`vaadinBuildFrontend`::
  This builds the frontend bundle with the `Vite` utility. Vaadin frontend resources (e.g., HTML, JavaScript, CSS, and images) are bundled to optimize loading the front-end. This task isn't executed automatically on the `build` and other targets, so you'll need to run it, explicitly.
