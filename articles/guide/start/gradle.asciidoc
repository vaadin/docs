...
Vaadin uses https://docs.gradle.org/current/userguide/incremental_build.html[Gradle Incremental Builds feature] for `vaadinPrepareFrontend` task to avoid `Node.js` installation and front-end files generation that has already been done and not changed since the previous build.
This saves time when building and running applications in development mode.

If none of these has been changed since the previous build, Gradle skips prepare frontend task, giving an `UP-TO-DATE` state:

- Inputs:
   * installed `Node.js` executable,
   * `npm/pnpm` executable,
...
This allows to force the prepare frontend task as a fallback in case of issues in inputs or outputs definition, while it is being fixed.
