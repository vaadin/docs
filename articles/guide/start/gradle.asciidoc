...
Otherwise, the project is built with the standard `build` task.
...
One way to enable the Gretty plugin is in the `plugin` section of the `gradle.build` file, as is done in the starter project:
...
Defaults to `null` which uses the autodetected value of the main SourceSet, in most cases `build/resources/main/META-INF/VAADIN/webapp/`.
...
Mostly needed in corporate environments where the `node.js` download is provided from an intranet mirror.
