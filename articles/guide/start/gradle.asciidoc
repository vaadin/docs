...
[NOTE]
To avoid any inconsistencies, don't use any prerelease versions in your production environment, especially snapshots.
...
This section is about trying the prerelease and snapshot versions of the Vaadin Gradle plugin itself, not Vaadin.

To use the prerelease plugin, add the `vaadin-prereleases` repository to the project [filename]`settings.gradle` file.
...
In some cases, when running a Spring Boot application, the resource loader doesn't load the classpath packages correctly from the manifest.
