...
Vaadin uses https://docs.gradle.org/current/userguide/incremental_build.html[Gradle Incremental Builds feature] for `vaadinPrepareFrontend` task to not run it when project's configuration hasn't been changed and necessary front-end files have already been generated and not changed since the previous build.
...
- Inputs:
   * installed `Node.js` and `npm/pnpm` versions,
...
This allows to force the `vaadinPrepareFrontend` task execution as a fallback in case of issues in inputs or outputs definition, while it is being fixed.
