...
In addition to <<flow-unit-testing#,unit tests>>, we want to ensure that a given functionality works correctly in an application.
This is needed for features and bugs that depend on the browser functionality.
For this, we create a View & Integration Test combination.
...
Descriptions about integration test modules are inside link:https://github.com/vaadin/flow/blob/master/flow-tests/README.md[the readme file in /flow-tests].

The integration testing is done with link:https://vaadin.com/testbench[TestBench], for information see <<{articles}/tools/testbench/overview#, Vaadin TestBench>>.
...
The integration test class should be named the same as the `View` class that it tests e.g. `PageView` gets the test class `PageIT`.
...
There are also some `Abstract*` classes that provide common functionality to be reused in the tests.
...
To reduce the chance your IT test is flaky, we recommend running it several times before publishing it out.

[NOTE]
In order to run the integration tests locally, you should have the Chrome Web Driver installed and configured properly, as described in <<{articles}/tools/testbench/installing-webdrivers#,Installing Web Driver>>
