...
=== Spring Security Configuration Helpers

Two new helper methods, `VaadinSpringSecurity::configure(HttpSecurity)` and `VaadinSpringSecurity::configure(WebSecurity)`, are available for configuring Spring Security.
Use the methods as a starting point for your own configuration:
...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        // app's own HttpSecurity configuration...
    }

    @Override
    protected void configure(WebSecurity web) throws Exception {
        VaadinSpringSecurity.configure(web);
        // app's own WebSecurity configuration...
    }
}
----

`VaadinSpringSecurity::configure(HttpSecurity)`  configures authentication for all routes by default.
Modify this behavior with your own followup configuration as needed.
The helpers also bypass framework internal and static resources (`/VAADIN/**`, `sw.js` ...).
...
- In Vaadin 15+ with client-side bootstrapping the `index.html` page includes only the basic HTML markup and links to the TypeScript UI code. When <<{articles}/fusion/routing/defining#,adding routes in TypeScript>>, the `UI` is not guaranteed to be created, thus is optional. It will be only available after the user navigates to a server-side route.


== Earlier Versions

- link:../../../v14/flow/migration/1-migrating-v8-v10/[Upgrading from Vaadin 8 to Vaadin 14, role=skip-xref-check]
- link:../../../v14/flow/v14-migration/v14-migration-guide/[Upgrading from Vaadin 10–13 to Vaadin 14, role=skip-xref-check]
