...
`VaadinWebSecurityConfigurerAdapter` also configures Spring CSRF token for login and Fusion endpoint calls, so you no longer need to ignore Spring CSRF protection for them like before with `http.csrf().ignoringAntMatchers("/login", "/connect/**");`
...
You can update your login view configuraiton with the `setLoginView()` helper, which sets up the login success handler automatically.
