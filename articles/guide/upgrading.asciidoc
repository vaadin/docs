...
[.lead]
This article gives instructions for upgrading to the latest Vaadin version.
...
. Delete `node_modules` folder and either lock file: `package-lock.json` (with npm) or `pnpm-lock.yaml` (with pnpm).
...
----
_To view the complete example `pom.xml` file, click the "Expand code" button._
...
Previously, all the Java types were generated as required in TypeScript.
