...
These instructions apply when upgrading from a version before Vaadin 21.

=== Lit Templates are using Lit 2

Previously, lit templates were bases on LitElement 2.x and lit-html 1.x.
The new Lit 2.0 is used for lit templates in the latest Vaadin version.
Some changes are required to be able to use your existing template with Lit 2.0.
Refer to the https://lit.dev/docs/releases/upgrade/#update-packages-and-import-paths[Upgrade guide]
to find all necessary changes.
In most cases you should be aware of changes in the imports:
...
and directive imports if you are using them:
...
Also be aware about change in the renamed API, most important change is
renamed `@internalProperty` decorator. Now it's `@state`.
