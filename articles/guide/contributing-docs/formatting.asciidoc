...
Validity of cross-references is checked during Docs compilation, and reported in the test builds at GitHub.

[[xrefs.within-a-page]]
=== Within a Page

A referred section or figure needs an ID.
IDs should only have lower-case letters, numbers, dash, and dots.
Please follow a dot-separated hierarchical order.
...
The file reference must be terminated with dash (+++#+++), which can be followed by a sub-target within the page.
...
[source, AsciiDoc]
----
See <<{articles}/guide/contributing-docs/style-guidelines#, Style Guidelines>>.
----
...
For example, examples about Java annotations like [classname]#@Theme# that can only be defined for a single class in an application can't be inline.
