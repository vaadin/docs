...
However, if we were interested in only very particular types of updates and didn't need to change the state, we could implement the [interfacename]`PortletModeHandler`, [interfacename]`WindowStateHandler`, or [interfacename]`EventHandler` interfaces
...
The classes contain code that isn't directly related to the portlet implementation and we skip those parts.
