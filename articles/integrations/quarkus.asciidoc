...
To start a new project with Quarkus and Vaadin, you can download a project base from https://github.com/vaadin/base-starter-flow-quarkus/[Quarkus base starter]. It's a project template with the necessary configuration and dependencies for building an application.

It's also available with Gradle configuration in the https://github.com/vaadin/base-starter-flow-quarkus/tree/gradle[Gradle] branch.
...
Quarkus’s dependency injection solution is based on CDI. Therefore, it's possible to use all CDI features.
...
The Quarkus base starter already includes the necessary Maven configuration to run the application in production mode. If you use a project not based on the starter, this needs the configuration described in <<{articles}/production#enabling-the-production-mode, Deploying to Production>>.
...
When running in development mode (i.e., `quarkus:dev`), changes in Java or front-end files compile after saving and appear after the browser page is refreshed. For front-end changes the browser page is automatically reloaded. However, for Java changes a manual refresh is required. Furthermore, Java hot reload may sometimes break front-end live reload. If this happens, the server needs to be restarted.
