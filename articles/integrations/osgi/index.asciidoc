...
It's the developer's responsibility to register a `VaadinServlet` in the servlet container, inside the OSGi container.
...
When you have more than one bundle created by Vaadin, you shouldn't have multiple `VaadinServlet` registrations with the same servlet pattern.
...
Many transitive dependencies also need to be deployed.
The bundle isn't activated unless all its dependencies are deployed and activated.
...
This isn't a full list of all the required bundles.
...
If your OSGi container doesn't have it out of the box, you need to deploy an implementation bundle to support the Compendium API.
...
See the https://github.com/vaadin/base-starter-flow-osgi/blob/feature/osgi-bnd/README.md[README.md] file in the project for details.
...
All frontend resources get built into a bundle along with the Vaadin `WAB`.
As a result:
...
* You can't use OSGi declarative services with Vaadin components.
You can't inject a service declaratively in Vaadin classes (using annotations), because UI objects aren't managed by OSGi.
However, you can still call OSGi services programmatically.

* Automatic servlet registration is unavailable.
The web application bundle needs to register the servlet itself.

* Push doesn't work with WebSockets.
It isn't clear how to enable WebSockets for a pure OSGi container.
