...
You cannot use Declarative Services directly, because Vaadin components, including navigation targets, aren't declared as services.
However, it's possible to use services through an OSGi API that allows you to use Declarative Services indirectly.
...
You can call the API from any class, be it a component, a navigation target, or a helper class.
...
There are several reasons why this isn't supported:

* It cannot work with OSGi Declarative Services.
