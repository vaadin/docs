...
If you want to have two versions of the web component, one that's preserved and one that isn't, you can simply export the same [classname]`Component` twice under different tags:
...
In order to guarantee that a web component instance is uniquely preserved and its state doesn't leak to other instances of that web component, the generated ID is combined with the window name, and the state is stored to the user's session.
