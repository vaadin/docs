...
To ensure the beans are recognized, they should be qualified by the `@VaadinServiceEnabled` annotation. This is required because it marks a bean which is used as an I18N provider by the service. If there are any other I18N beans, the one that's also used by the service is used.
