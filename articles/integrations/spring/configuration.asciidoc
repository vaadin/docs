...
Sometimes when using Spring Boot Development Tools while automatic restart is enabled, more than one restarts can trigger in a row. It depends on how many files changed at once and how IDE is changing monitored files. As a result, it may slow down the overall restart time.

Spring Development tools has two properties that can be adjusted in [filename]`application.properties` to help with this and to speed up restart time: `spring.devtools.restart.poll-interval` and `spring.devtools.restart.quiet-period`.
...
For example, in small project developed with Eclipse, using the following smaller values can speed up the restart time when changing one or a few classes:
...
As another example, in a project developed with IntelliJ IDEA, increasing values can ensure that restart happens only once after changing one or few classes:
...
For larger applications, try to increase the values for these properties to avoid multiple restarts. One second more for the poll interval doesn't matter much if everything else takes more than ten seconds to restart.

One value isn't best for all development environments. The examples here are given as suggestion of how to adjust -- not as recommended values.
