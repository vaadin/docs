...
[.example]
--
In server-side views (Java), the standard way is to organize style sheets as an <<application-theme, application theme>> then use `@Theme` annotation to apply the application theme.
...
NOTE: Using an <<application-theme, application theme>>, component-specific style sheets are imported and will be applied to components that have their tag names matches with the CSS file names. For example, `frontent/theme/my-theme/components/vaadin-button.css` will be the same as `@CssImport` with `themeFor="vaadin-button"`.
...
There are three ways to import style sheets to the global scope:
  ...
  . Using <<application-theme#global-styles, Application Theme global style>> (recommended).
...
In server-side views, when an application theme is used, <<application-theme#components-styles, components styles>> are loaded automatically.
