...
WARNING: The static resource should be in the same theme with the CSS file. Otherwise, it will be served as external resources by the web server instead of webpack.
...
NOTE: Global styles can override <<theme-variants#global-variants, global variants>> styles. For example, `Lumo.DARK` variant defines a `--lumo-base-color` which can be overrided by `global.css` file.
...
You must set the annotation on an https://vaadin.com/api/platform/com/vaadin/flow/component/page/AppShellConfigurator.html[AppShellConfigurator] class, or an error "Found app shell configuration annotations in non `AppShellConfigurator` classes." will be thrown.
