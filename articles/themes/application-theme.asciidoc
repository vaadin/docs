...
Assets like fonts and images from an npm dependency can be used in the theme after mapping them from the dependency to local URIs.
That can be done by adding an `assets` block in `theme.json`.
