...
The [filename]`useAuth.tsx` created before needs more functionalities to support back-channel logout, as there is a chance that the user doesn't want to leave the application abruptly when a logout notification arrives, for example to avoid losing ongoing work. That's why more state management is needed.

Replace that file with this version:
...
The state can be put in [filename]`App.tsx`:
...
Add a dialog to ask the user if he wants to log in again:
...
Implement the two functions used by the dialog:
