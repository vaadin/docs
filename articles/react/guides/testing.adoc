...
Save the test so the Vitest can run it in the browser, verifying that the interaction works as expected.
...
The actual test does the same user interaction as before, but this time you're verifying that the service is called with the correct argument. 
...
For more information about Vvitest and React Testing Library, refer to the official documentation:
