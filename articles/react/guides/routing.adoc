...
By default, this will push a new entry to the browser's navigation history. If you want to replace the current entry instead, you can pass `{ replace: true }` as the second argument like so:
...
- First, create a new component for your error page (e.g., `ErrorView.tsx`). This component will be displayed when no other route matches:
+
[source,tsx]
----
...
- Next, in your `routes.tsx` file, add a wildcard route at the end of your `routes` array. This route will match any unknown routes and display the error page.
+
[source,tsx]
----
