...
Individual columns can be customized using the `columnOptions` property. This takes an object where each key is the name of a data property, and the value is a props object that is passed when rendering the column for that property.
