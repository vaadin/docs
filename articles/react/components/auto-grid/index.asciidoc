...
The following snippet shows how to convert the parameters into a query string that is then used to call an imaginary REST API to retrieve a list of paginated, sorted and filtered products:
[source,java]
----
import com.vaadin.flow.server.auth.AnonymousAllowed;
import dev.hilla.BrowserCallable;
...
public class ProductListService implements ListService<Product> {
    ...
    public ProductDtoService(ProductApiClient productApiClient) {
