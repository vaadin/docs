...
Note that Auto Grid automatically expands all `@OneToOne` relationships, in this case the `manufacturer` property, and displays its properties as columns in the grid.
...
[NOTE]
When trying to access nested properties, use the Dot Notation, as shown in the example above. For example, to access the `name` property of the `manufacturer` property, use `manufacturer.name`.
...
Sometimes, it is needed to create a custom column, for example, by joining the value of two columns. This can be done by using the `customColumns` property to add one or more custom columns in the grid. The `customColumns` property takes an array of `GridColumn` instances to render custom content. Please refer to the <<../grid/#,Grid>> documentation page for more information on the `GridColumn` type.
