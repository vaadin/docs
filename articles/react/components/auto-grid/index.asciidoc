...
You can define the order of appearance for custom columns also using the `visibleColumns` property. This means, custom columns can be freely placed before, in between, or after other columns. To achieve this, you need to provide the `key` property of the custom column in the `visibleColumns` property. The following example shows a custom column with _"supplierInfo"_ as the `key` placed between two automatically rendered columns:
