...
[NOTE]
If the `visibleColumns` property is present, only the custom columns that their keys are mentioned in the `visibleColumns` are rendered according to the specified order.
...
[source,tsx]
----
include::{root}/frontend/demo/component/auto-grid/react/auto-grid-refresh.tsx[render,tags=snippet,indent=0,group=React]
----
