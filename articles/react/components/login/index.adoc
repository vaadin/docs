...
.Password managers
[NOTE]
Login is incompatible with password managers if placed inside another component's footnote:[When added to a web component that uses shadow root, password managers are unable to find the input fields and thus not work.] shadow root. This isn't an issue when using Login's modal overlay.
