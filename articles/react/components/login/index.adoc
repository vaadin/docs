...
You can use it to authenticate the user with their username and password.
...
.Password Managers
[NOTE]
Login is incompatible with password managers if placed inside another component's footnote:[When added to a web component that uses shadow root, password managers are unable to find the input fields and thus won't work.] shadow root. This isn't an issue when using Login's modal overlay.
