...
[NOTE]
====
Grid Pro is an extension of the <<../grid#,Grid>> component, so that all Grid's features are also applicable to Grid Pro.
====


== Usage

Begin editing by double-clicking on the editable cell and then pressing kbd:[Enter], kbd:[Space], or typing an alphanumeric character when an editable cell is focused.

When editing, a few keys and key combinations are available to the user:

- kbd:[Esc] discards the changes and exits edit mode.
- kbd:[Enter] and kbd:[Shift + Enter] save any changes and the user exits edit mode.
- kbd:[Tab] saves any changes and shifts focus to the next editable cell. kbd:[Shift + Tab] does the same, but shifts the focus to the previous editable cell. The user remain in edit mode for either one.



== Modes

Grid Pro has a few editing modes. These are described in the sub-sections below.
...
By default, when in edit mode, kbd:[Tab] moves to the next editable cell, and kbd:[Shift+Tab] moves to the previous one, while remaining in edit mode.
...
By using the Enter Next Row mode, kbd:[Enter] will shift the focus to the editable cell in the next row, while kbd:[Shift+Enter] will shift the focus to the one in the previous row.
...
- There are plenty of columns or fields;
