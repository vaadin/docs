...
[NOTE]
When the Auto Form component is rendered based on its default settings similar to the example above, it uses the <<../form-layout#,Form Layout component>>'s  default responsive breakpoints. If you want to customize the responsive breakpoints, you can see how it is done in the <<customizing-form-layout,Customizing Form Layout>> section.

== Customizing Visibility & Order

As you can see in the example above, the Auto Form component by default renders input components for all the data properties in a Form Layout with a two-column layout, except for the fields annotated by `@Id` and `@Version`. You can customize the visibility and order of the fields using `visibleFields` property:
...
Alternatively, you can create input fields as needed, and use the `form` binder instance to take control over the bindings. The following example uses a custom renderer and manually does the form binding for the fields:
