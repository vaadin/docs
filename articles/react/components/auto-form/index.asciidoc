...
As the children property is a read-only list of bound fields, you can use them right away in your custom renderer. The following example shows how to use the `children` property to render the fields in separate sub-form sections using the Vertical and Horizontal layouts:
...
Auto Form uses the same validation mechanism as <<{articles}/react/guides/forms/binder-validation#,default Hilla Forms>>. That means you can add Bean validation annotations to your entity classes and the form will automatically validate the input values on both client and server.

The following example shows how to add annotations to the `Employee` class in order to ensure that `firstName` is neither `null` nor an empty string:
...
For further information on using validation please refer to the <<{articles}/react/guides/forms/binder-validation#,Forms>> documentation.
