...
For the `EmployeeService` example, you can import the generated TypeScript object in your React view, and use it as a value for the `<AutoForm service={}/>` property to display the fields:

[source,tsx]
----
include::{root}/frontend/demo/component/auto-form/react/auto-form-basic.tsx[tags=snippet,indent=0]
----
...
[NOTE]
When the Auto Form component is rendered based on its default settings similar to the example above, it uses the <<../form-layout#,FormLayout component>>'s  default responsive breakpoints.
...
.Default Responsive Breakpoints
[NOTE]
When the Auto Form component is rendered based on `string[][]` template similar to the example above, it uses the the following responsive breakpoints:
`[
