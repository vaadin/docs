...
Auto Form supports working with plain, non-JPA classes. This can be useful, for example, if you want to use the Data Transfer Object (DTO) pattern. See the <<working-with-plain-java-classes, Working with Plain Java Classes>> section.

.Default Responsive Breakpoints
[NOTE]
Auto Form uses the default responsive breakpoints for the <<../form-layout#,Form Layout component>>. To customize them, see the <<customizing-form-layout,Customizing Form Layout>> section.
...
As Auto Form component uses the Form Layout component as the layout, you can customize the `colspan` property of the fields rendered by Auto Form by using the `fieldOptions` property like this:
...
To render a form in a completely different layout, you can use a custom renderer to define any arbitrary layout using the `layoutRenderer` property.
...
Since the children property is a read-only list of bound fields, you can use them right away in your custom renderer. The following example shows how to use the `children` property to render the fields in separate sub-form sections using the Vertical and Horizontal layouts:
...
The following example shows how to use the `onSubmitError` and `onDeleteError` callbacks to inform the user about their respective errors:
...
Auto Form uses the same validation mechanism as <<{articles}/react/guides/forms/binder-validation#,default Hilla Forms>>. You can add Bean validation annotations to your entity classes, and the form automatically validates the input values on both client and server.
...
For further information on using validation, refer to the <<{articles}/react/guides/forms/binder-validation#,Forms>> documentation.
