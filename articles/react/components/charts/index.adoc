...
Charts come with over a dozen types of charts. They're listed alphabetically in the table here:
...
The sections that follow provide examples of charts.


=== Column Chart

One of the most common type of chart is a column chart. The image below shows an example of one, followed by the code that generates this chart.
...
Another commonly used type of chart is an area chart. Here's an example of one in the image here, followed by its code.
...
For some developers and for some situations, the pie chart is preferred. The image below shows an example of one. It also is followed by the TypeScript code used to generate it.
...
Not as popular, but sometimes perhaps best for displaying more complex data is the polar chart. The image that follows is an example of one, followed by the related code.
