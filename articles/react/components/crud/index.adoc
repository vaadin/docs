...
By default, the editor is opened by clicking the [guibutton]*Edit* button in the last column. However, this button column can be removed if you want to implement another way to open the editor. For example, you can have it open using a double click like so:
...
The `aside` position displays the editor as an overlay next to the grid. Use this position when there is enough horizontal space to accommodate both the grid and the editor. Use it also when it's beneficial for the user to see and interact with the grid while the editor is open. Aside positioning is also a good fit for <<../form-layout#,single-column forms>>.
...
* [guibutton]*Delete* deletes the item, permanently. The user is asked to confirm, after clicking this button, if they want to delete the item.
* [guibutton]*Cancel* closes the editor unless there are unsaved changes. When there are, the user is asked to either discard the changes or go back to editing.
* [guibutton]*Save* saves the changes and closes the editor. This is disabled until there is a change.



== Grid Replacement

CRUD's default Grid is replaceable. This is useful when you wish to customize the Grid. An example might be if you want to place the edit Button in the first column, or to apply <<../grid#tooltips,tooltips>>. See <<../grid#,Grid documentation>> for details on configuring grids.
...
.Edit Column
[NOTE]
You need to add explicitly an edit column to the replacement Grid to edit items. Also note that Grid doesn't have sorting and filtering enabled by default.



== Toolbar

The [guibutton]*New item* Button in CRUD's toolbar creates new items. Both the toolbar and its Button are customizable. For example, you can use the toolbar to display statistics such as the size of the dataset or the number of search results.
