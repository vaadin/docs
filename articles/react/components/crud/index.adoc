...
The overlay position, which is the default, renders the editor in a modal overlay.
...
Use this position when there is enough horizontal space to accommodate both the grid and the editor. Use it also when it's beneficial for the user to see and interact with the grid while the editor is open.
...
Grid also doesn't have sorting and filtering enabled by default.

== Toolbar

The [guibutton]*New item* Button in CRUD's toolbar creates new items.
