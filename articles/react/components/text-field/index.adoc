---
title: Text Field
description: Text Field allows the user to input and edit text.
---
...
The optional clear button clears the field's current value. It's hidden while the field is empty. Clear buttons are useful for search and filter fields, where it's common for the user to want to clear the field value. However, in data entry forms, they're typically unnecessary.
...
Right-aligned values are recommended for numerical values when presented in vertical groups. This makes it easier to interpret and compare values.
