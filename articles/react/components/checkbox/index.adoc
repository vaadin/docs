...
Checkboxes have a couple of possible states. They're set by adding them within the Checkbox tag: `disabled`, and `indeterminate`. These states are described in the following sub-sections.


=== Disabled

Disable a field to mark it as unavailable currently. This is done by adding `disabled` to the tag, as shown in the example that follows. 
