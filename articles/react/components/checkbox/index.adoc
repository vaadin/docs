...
Checkboxes have a couple of states possible. They're set by adding them within the Checkbox tag: `disabled`, and `indeterminate`. They're described in the following sub-sections.


=== Disabled

Disable a field to mark it as unavailable currently. This is done by adding _disabled_ to the tag, as shown in the example that follows. 
