...
Use the <<../details#,Details>> component to allow multiple, simultaneously expanded sections.

=== Summary

The summary is the part that's always visible, and typically describes the contents, for example, with a title.
...
Set the theme attribute for each panel, not on Accordion itself.
...
* Accordions are better suited than Tabs for long labels.
However, Accordions can feel “jumpy” as panels are toggled -- in particular if there are several panels or the panel content is long.
...
* The expandable and collapsible nature of accordions can sometimes be difficult for users to discover.
Use the <<filled-panels,filled variant>> and apply [since:com.vaadin:vaadin@V23.3]##<<../tooltip#,tooltips>>## on the panels to make this more discoverable.
