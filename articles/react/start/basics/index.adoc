...
Hilla is a full-stack web framework for Java. It can help you build better business applications faster by combining a Spring Boot backend with a React frontend, and more than forty UI components. 

In this tutorial, you'll learn the basics of Hilla development by building a full-stack CRM application. The code snippets are optimized for a smooth _copy and paste_ experience to ensure you'll have a working application up and running quickly. 
...
There are two ways to run the application: you can use your IDE to run `Application.java` (recommended, supports debugging); or you can run the default Maven goal from the command line: `mvn`.
...
With the backend completed, you can start building the UI. Update the contents of `ContactsView.tsx` to the following:
