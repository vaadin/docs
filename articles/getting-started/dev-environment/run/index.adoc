...
There's also a third option: the <<{articles}/flow/configuration/live-reload/dev-loop-cli#,Dev Loop CLI>> runs your application under a daemon that owns the edit-to-running-app loop, and gives you a single command that makes your edits live and reports whether they are. It's especially useful when an AI coding agent is doing the editing; the <<{articles}/building-apps/agentic-development/dev-loop-tutorial#,Dev Loop Tutorial>> shows that end to end.
