...
The plugin creates a debug configuration named `Application [Hotswap]` that runs the application on JetBrains Runtime with the bundled HotswapAgent and the required JVM parameters. It loads its own copy of HotswapAgent with `-javaagent`, so those parameters aren't quite the same as the manual ones in <<debug-configuration,Debug Configuration>>, and you don't have to add any of them yourself. You can reuse that configuration later from the *Debug* button in the toolbar.
...
`-XX:+ClassUnloading`::
Lets the JVM unload the classes that hotswap replaces, so that repeated reloads don't leak memory.
