...
The easiest way of running a Vaadin application with hotswap in Eclipse IDE is by installing the link:https://marketplace.eclipse.org/content/vaadin-tools[Vaadin plugin] from the Eclipse Marketplace. If you haven't done so already, do it now. Use the Eclipse IDE for Enterprise Java and Web Developers package. The same plugin also integrates Eclipse IDE with Vaadin Copilot, so that changes you make in the browser are written through the IDE and can be undone there.

The plugin bundles HotswapAgent and installs it for you, so the only thing you have to install yourself is JetBrains Runtime. The plugin picks up any JetBrains Runtime that Eclipse IDE knows about. If it can't find one, add it in menu:Preferences[Java > Installed JREs].
...
The plugin creates a debug configuration named `Application [Hotswap]` that runs the application on JetBrains Runtime with the bundled HotswapAgent and the required JVM parameters. You can reuse that configuration later from the *Debug* button in the toolbar.
...
3. Create a debug configuration for Eclipse IDE that runs the application with JetBrains Runtime and additional JVM parameters.
