...
The Vaadin Grid supports sorting out of the box, but it has to be enabled explicitly on the columns. How you do this depends on whether you're using in-memory data or a paginated data provider that fetches data from a backend. 
...
// end::snippet[]
// The rest of the imports have been omitted for brevity
...
Next, add a `ValueChangeListener` to the `searchField` that refreshes the grid's data provider whenever the value changes. Since this listener will reference the `grid` variable, it has to be added after the grid is created. A good practice is to add the listeners after setting up the components but before laying out the view:
...
The complete updated constructor now looks like this:
...
With sorting and filtering implemented, your product catalog view is now much more user-friendly. In the next step, you'll add a form for creating and editing products in the catalog. Proceed to the <<modify-data#,Add and Edit Products>> step to continue the tutorial.
