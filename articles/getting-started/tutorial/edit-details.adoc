...
// tag::snippet[]
    public Optional<ProductDetails> getFormDataObject() {
        if (binder.getBean() == null) {
            binder.setBean(new ProductDetails()); // <1>
        }
...
----
<1> Create a new FDO if there is none. This makes it possible to use the form for both editing and adding items.
<2> Return the FDO if the form is valid.
<3> Return an empty `Optional` if there are validation errors.
...
You can trigger the same error by selecting an existing product, clearing the Price field, and saving it.
...
1. The edit drawer sometimes tries to _insert_ new products.
...
Right now it is possible to use the drawer to add items if the form is empty. The easiest way to fix this is to hide the drawer when it does not contain any details, and handle adding items in another way.
...
Now try the application. The drawer should only show up when you have selected a product in the grid. You'll implement support for adding new products in the next step of the tutorial.
...
// end::snippet[]
    ...
    public Optional<ProductDetails> getFormDataObject() {
        if (binder.getBean() == null) {
            binder.setBean(new ProductDetails());
        }
        if (binder.validate().isOk()) {
