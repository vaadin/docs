...
Next, try to insert a product with the following SKU: `NBK-A5-DOT`
...
You have now discovered a mismatch between how Spring Data and Vaadin assume data objects should be used. These mismatches are prone to happen whenever you're dealing with mutable data object, like `ProductDetails` in this case.
...
Since you'll be working around a limitation in your persistence layer, you want to hide it from the user interface. You'll therfore introduce an <</building-apps/business-logic/add-service#,application service>> that acts as an intermediary between the user interface and your repositories.
