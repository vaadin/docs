...
To create a layout for the views, start with a file named `@layout.tsx` in the `src/main/frontend/views` directory. Then copy-paste the following code into it:
...
Start by updating the imports at the top of the `@layout.tsx` file as follows:
...
<2> `createMenuItems` is a function provided by Hilla that returns a list of views that the user can navigate to.
...
To handle this situation, you have to declare a signal that will contain the current view title. Add the following to `@layout.tsx`:
...
Finally, you need to update the lobby and channel views to set their page titles. Start with the lobby view. Open `views/@index.tsx` and add the following:
...
Then, do the same with the channel view. Open `views/channel/{channelId}/@index.tsx` and add the following:
...
----
<1> In the previous version, this line set `document.title` to the channel name. Now when that is handled by the layout, it is enough to set the value of the `pageTitle` signal to the channel name.
