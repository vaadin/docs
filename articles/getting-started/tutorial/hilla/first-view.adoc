...
Finally, you are going to make the message input take up the full width of the screen and remove the extra padding that the component has by default. There are CSS utility classes for this: `w-full` and `p-0` (for setting the padding to 0):
...
export default function ChannelView() {
    ...
    return <VerticalLayout theme="padding spacing" className="h-full">
        <MessageList className="h-full w-full border"/>
        <MessageInput className="p-0 w-full"/>
    </VerticalLayout>
}
----
...
export default function ChannelView() {
    ...
    return <VerticalLayout theme="padding spacing" className="h-full">
        <MessageList className="h-full w-full border"/>
        <MessageInput className="p-0 w-full" onSubmit={e => postMessage(e.detail.value)}/>
    </VerticalLayout>
}
----
...
On the server side, `ChatService` has a method called `liveMessages(..)` that returns a `Flux`. A `Flux` is a reactive stream that can emit zero or more elements. In `ChatService`, it is a _hot_ stream, which means that subscribers will only receive elements that are emitted after they subscribed. Hilla makes this stream available on the browser side as well.

Because you are going to subscribe to a stream, you will need to manage the subscription itself. When the channel ID changes, you want to start by cancelling any existing subscription. After that, you want to subscribe to the new channel.
