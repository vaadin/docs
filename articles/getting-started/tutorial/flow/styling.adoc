...
In the next section, you'll be adding style for the channel list. Let's make it so that the channels [classname]`VirtualList` has these new styles applied. Update the constructor for [classname]`LobbyView` to look like this:
...
...
----
<1> This adds the `channel-list` class to the [classname]`VirtualList` component. This class will be used to style the channel list.
