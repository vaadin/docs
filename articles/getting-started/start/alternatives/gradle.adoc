...
Only the contents of `base-starter-gradle` -- a simple web application starter without Spring Boot -- are shown in the preceding figure. However, the Spring Boot-based starter project is a little different, except that it has an additional class annotated with `@SpringBootApplication`. If this is the first time you've tried Vaadin with Spring Boot, see <<{articles}/flow/integrations/spring/spring-boot#,Using Vaadin with Spring Boot>> for more information.
...
If you want to try Vaadin pre-release versions, you can also add the following repository:
...
image::images/gradle-eclipse-tasks.png[Gradle Tasks tab in eclipse]
