...
To compile your project, You can run `./gradlew` (Linux/Mac) or the `gradlew` (Windows) from the terminal / command line tool on your machine. If you are not using a starter project, see the <<{articles}/flow/configuration/gradle#,configuration page>> to determine what's necessary to have in your build file.
...
When running the compilation for the first time, you should invoke `vaadinPrepareFrontend` task. For more information see <<{articles}/flow/configuration/gradle#compiling.tasks,compiling tasks>> section.

[source,terminal]
----
./gradlew vaadinPrepareFrontend vaadinBuildFrontend build
----
