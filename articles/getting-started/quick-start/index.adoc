...
link:https://start.vaadin.com/skeleton?platformVersion=v24.9&javaVersion=21&groupId=com.example&artifactId=app&frameworks=none&aiTool=NONE[Download Project, role="button primary water"]   

After downloading the project, unzip it and <<../import#,open it in your favorite IDE>>.

.This is a Spring Boot Project
[NOTE]
The downloaded project skeleton is a Spring Boot project. The <<../start/alternatives#,Alternatives>> documentation page describes *how to create Vaadin projects using other frameworks*.
...
You can run the application <<../run#,directly from your IDE>> or from the command line. To run it from the command line, open a terminal in the project directory and run the following command:
...
If you started the application from your IDE with <<../run#,hotswap (automatic code reload) enabled>>, the changes should be reflected automatically. If not, restart the application and refresh the browser. Click the button to see what happens:
...
Enter your name in the text field and click the button. The added text greets you using the name you provided.
...
[.device]
image::images/fourth-view.png[The Vaadin view with extra space between the text field and button]
...
Execute the Maven `package` goal with the `production` profile active to create a production build. You can do it from the command line like this:
