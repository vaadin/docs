...
The `spotless-maven-plugin` is used to format the Java and TypeScript source files. This is needed when you generate code with <<{articles}/tools/copilot#,Vaadin Copilot>>, since it does not format the code itself. You'll learn more about Copilot on the <<debug#,Debug>> page.
...
The `production` profile triggers a production build, and is deactivated by default. You'll learn more about making a production build on the <<deploy#,Deploy>> page.
