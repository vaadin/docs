...
Note that it doesn't expose properties such as `required`, `invalid` and `errorMessage`, which is why the default field strategy that works for all Vaadin components would not work here.
