...
See the <<{articles}/flow/security/intro#, corresponding article for Hilla Flow>>.

== Architecture

In contrast to Hilla Flow applications, where all application logic (including authentication and authorization) is processed on server side, Hilla applications involve the orchestration of server and client-side security.
