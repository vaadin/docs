...
To access the authenticated user’s information (e.g., name, email and roles), Flow provides the [classname]`AuthenticationContext` class that can be used to retrieve this information. [classname]`AuthenticationContext` is a Spring bean that can be injected as a view constructor argument. [classname]`AuthenticationContext` can be useful for rendering the UI differently based on the user’s roles. 
