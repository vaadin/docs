...
Before Vaadin 24.3, Vaadin shows [classname]`RouteNotFoundError` view instead of [classname]`RouteAccessDeniedError` view. Both views looks same as they extend [classname]`AbstractRouteNotFoundError` class.

[role="since:com.vaadin:vaadin@V24.3"]

== Customizing Error Messages for Unauthorized Views

Vaadin shows the [classname]`RouteAccessDeniedError` view for unauthorized views by default. This can be customized in following ways:

- By providing custom implementation by overriding [classname]`RouteAccessDeniedError` class.
- By providing custom implementation by implementing [interfacename]`HasErrorParameter<AccessDeniedException>` interface.
- By rerouting to different error type with [annotationname]`@AccessDeniedErrorRouter` annotation

Examples of these approaches are provided below.

=== Customizing error by overriding a class
[source,java]
----
...
=== Customizing error by implementing an interface
[source,java]
----
@Tag(Tag.DIV)
...
Following example reroutes unauthorized admin view to [classname]`RouteNotFoundError` view which is default view for [classname]`NotFoundException` type.
