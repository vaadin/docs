...
Log-in view should always be accessible by anonymous users.
...
Log-in view should always be accessible by anonymous users, so it should have the [annotationname]`@AnonymousAllowed`.
This is specially important if using the [methodname]`setLoginView` variant providing the route path, although this signature is meant to be used with Hilla views, not with Flow views. 
