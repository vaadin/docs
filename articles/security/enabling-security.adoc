...
== Navigation Access Control And Spring's URL-Pattern-Based HTTP Security
...
URL-based security can be enable by activating the out-of-the-box [classname]`RoutePathAccessChecker` component.
To activate it you need to define a [classname]`NavigationAccessControlConfigurer` bean in your Spring configuration, creating a new instance of that class and calling the [methodname]`withRoutePathAccessChecker()` method.
...
<2> Activation of of Spring URL-pattern-based checker
