...
Do not forget to import the `login-view` component, otherwise the login view is not visible.
...
The Hilla view templates are always in the bundle and can be accessed by anybody, so it is important not to store any sensitive data in the view template.
