...
Navigation Access Control is an improvement and a replacement of the [classname]`ViewAccessChecker` mechanism, that is deprecated since Flow 24.3. It provides annotation-based view security. It also expands the capabilities of the former access checker by introducing a new path-based check and by allowing custom rules, which are set by implementing the [interfacename]`NavigationAccessChecker` interface.
...
image::images/navigation_access_control_2.svg[Navigation Access Control Architecture]
