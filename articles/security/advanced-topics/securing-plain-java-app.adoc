...
[NOTE]
====
The former [classname]`ViewAccessChecker` has been replaced as default security mechanism by [classname]`NavigationAccessControl` since Vaadin 24.3.
[classname]`ViewAccessChecker` can still be transparently used in place of [classname]`NavigationAccessControl`, but it is deprecated and will be removed in a future version.
It is recommended to not mix both the access control mechanism in the same project.
====
...
In project base on earlier Vaadin version, view security can be configured in the same way but using the [classname]`ViewAccessChecker` component:
...
The same instructions applies for [classname]`ViewAccessCheckerInitializer`, if using the deprecated [classname]`ViewAccessChecker`.
