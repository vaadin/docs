...
== Access Annotations
Before we see some examples of access annotations, it's worth having a closer look at the annotations, and their meaning when applied to a view:
...
However, we don't recommend specifying more than one of the above access annotations on a view class.
