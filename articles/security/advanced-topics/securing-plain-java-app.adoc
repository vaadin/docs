...
[NOTE]
The former [classname]`ViewAccessChecker` has been replaced as default security mechanism by [classname]`NavigationAccessControl` since Vaadin 24.3. [classname]`ViewAccessChecker` can still be transparently used in place of [classname]`NavigationAccessControl`, but it's deprecated and it'll be removed in a future version. It's recommended to not mix both the access control mechanism in the same project.
