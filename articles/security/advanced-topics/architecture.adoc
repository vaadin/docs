...
- This click is caught with framework JavaScript and translated into these two entities that are sent to the server: the unique ID of the `Button` instance (previously assigned sequentially by the framework); what was done with the button, in this case a click, with associated mouse details (what type of click it was, and the associated cursor position).
