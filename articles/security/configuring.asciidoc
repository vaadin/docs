...
When access to the [classname]#UserPrincipal# is required in an endpoint, call the [methodname]#HillaRequest.getCurrent()# method to access the `HttpServletRequest` API.
