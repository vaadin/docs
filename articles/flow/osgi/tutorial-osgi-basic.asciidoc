...
----
+
1. Change the scope for all Vaadin dependencies from default to `provided`, like this:
+
[source, XML]
----
    <dependency>
...
----
+
1. Add OSGi-related dependencies to the project
+
[source, XML]
----
    <groupId>com.vaadin</groupId>
...
----
+
1. Setup necessary plugins for building the project:
+
[source, XML]
----
 <build>
...
----
+
1. Add bundle script (`bnd.bnd`) into the project root folder:
+
[source]
----
...
Make a production build of the project (see <<../../guide/production/tutorial-production-mode-basic.asciidoci#,Preparing your application for Production>>) which means that frontend resources are compiled into a bundle (using `build-frontend` Vaadin maven plugin goal) and packaged together with Java code into the resulting project JAR.
