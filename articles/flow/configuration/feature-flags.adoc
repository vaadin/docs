...
`ssePushTransport`::
Enables `Transport.SERVER_SENT_EVENTS`, which streams server push messages to the browser as server-sent events. Selecting the transport while the flag is off throws a `DisabledFeatureException`.
