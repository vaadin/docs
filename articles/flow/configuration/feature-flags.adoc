...
`accessibleDisabledMenuItems`::
Make disabled <<{articles}/components/context-menu#, Context Menu>> and <<{articles}/components/menu-bar#, Menu Bar>> sub-menu items focusable and hoverable.
...
`aiComponents`::
Enables <<{articles}/flow/ai-support#, AI components>> such as `AIOrchestrator`.

`messageListTypingIndicator`::
Enables the experimental typing indicator API of Message List, `MessageList.setTypingUsers()`.

`ssePushTransport`::
Enables <<{articles}/flow/advanced/server-push#push.configuration.transport.sse, Server-Sent Events>> as a server push transport.

`messageListTypingIndicator`::
Enables the experimental typing indicator API of <<{articles}/components/message-list#, Message List>>. This API is also enabled by the `aiComponents` flag.
...
A <<{articles}/flow/testing/browserless/test-configuration#, browserless test>> is the exception: it can enable a flag for a single test class or test method, overriding all of the sources below.
...
[source,terminal]
-Dvaadin.defaultAutoResponsiveFormLayout=true
