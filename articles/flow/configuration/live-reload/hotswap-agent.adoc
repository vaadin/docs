...
Depending on project setup and used IDE, Copilot can download proper version of JDK and suggest Vaadin plugin for your IDE.

When debugging application using *Debug using HotSwap Agent* from Vaadin IDE, plugin automatically checks if you are using proper JDK, if HotSwap Agent is available and applies additional required JVM arguments.
