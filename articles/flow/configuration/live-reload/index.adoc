...
[TIP]
The <<dev-loop-cli#,Dev Loop CLI>> is an alternative to everything on this page: instead of a save-triggered watcher, it gives you a command that makes your edits live and reports whether they actually are. It's aimed particularly at AI coding agents, which need a definitive answer rather than a screenshot to reason about.
