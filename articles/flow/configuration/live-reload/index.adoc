...
From a deployment and reloading point of view, there are two types of files in a Vaadin project: Java classes and resources, and frontend files. Both have a slightly different solution for updating the browser whenever a file changes. The frontend reloading is setup automatically out of the box while Java updates can be handled in several different ways, as described on this page.

Hot deploy and live reload are only used during development. They are disabled in production builds.

[TIP]
The <<dev-loop-cli#,Dev Loop CLI>> is an alternative to everything on this page: instead of a save-triggered watcher, it gives you a command that makes your edits live and reports whether they actually are. It's aimed particularly at AI coding agents, which need a definitive answer rather than a screenshot to reason about. The <<{articles}/building-apps/agentic-development/dev-loop-tutorial#,Dev Loop Tutorial>> walks through it with a starter application.
...
The <<dev-loop-cli#,Dev Loop CLI>> is a fourth option, and a different kind of one. Rather than watching for saved files, it owns the whole loop: a daemon runs your application, and one command compiles your edits, hot-swaps or restarts, and reports the result. It uses HotswapAgent underneath, so it needs neither Spring Boot Developer Tools nor a separate HotswapAgent setup, and the application must be started through the CLI rather than through Maven or your IDE.
