...
|ci.build |
**Available since Vaadin 14.10**, when set to `true`, the build will install the exact npm package versions defined in the package lock file. In practice `npm ci` will be run instead of `npm install`, or if pnpm is used the install command will be run with `--frozen-lockfile` parameter. Defaults to `false`.
|===
