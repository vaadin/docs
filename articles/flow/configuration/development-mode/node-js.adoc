...
[NOTE]
The official Node.js 26 binaries link against `libatomic.so.1`, which Node.js 24 didn't require. On minimal Linux images, such as slim Docker base images, the downloaded Node.js fails to start with an error like "error while loading shared libraries" unless the `libatomic1` package (or equivalent for your distribution) is installed.
