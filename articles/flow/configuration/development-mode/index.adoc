...
When customizing a link:/docs/v24/styling/application-theme[Vaadin 24-style theme folder] in this mode, the following points should be taken into account:
...
This mode is recommended if components or views use <</flow/component-internals/templates#,Templates>> and are edited often. This mode is used when the configuration parameter `frontend.hotdeploy` is set to `true` (see <<{articles}/flow/configuration/properties#,Configuration Properties>> for more information).
...
Set `vaadin.npm.excludeWebComponents` <<../properties.adoc#,configuration property>> to `true` to exclude professional and core components, including `hilla-auto-crud`, from [filename]`package.json`.
