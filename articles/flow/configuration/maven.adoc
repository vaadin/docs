...
`cleanFrontendFiles`::
Clears the generated frontend files after building a project for production. Always keeps the generated files if they did exist before the build, or if this parameter is set to `false`. When building a bundle in development mode, the generated files are always removed unless they exist before the build. Default to `true`.
