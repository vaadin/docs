...
The Vaadin Trial experience lets you evaluate all Premium commercial features - such as Vaadin Copilot, commercial components (e.g., Charts), Acceleration Kits and Control Center - with minimal effort:

- 7‑day Trial (no registration needed): Start instantly by clicking a button in the development tooling of a started Vaadin application - available since Vaadin 24.9,
...
Production builds do not enable commercial features if you don’t have a valid license. During a commercial trial, you can opt‑in to enable commercial features in production builds by adding a `-Dvaadin.commercialWithBanner` build flag that shows a visible persistent banner on the bottom of the page in runtime.
