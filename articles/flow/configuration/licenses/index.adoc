...
Each offline license key is tied to the machine ID, in the form of `mid-xxxxxxxx-xxxxxxxx` (License Checker version 2 or earlier) or `mid2-xxxxxxxx-xxxxxxxx` (since License Checker version 3). You'll have to submit that to `https://vaadin.com/myaccount/licenses`, so as to download an offline license key.
...
----
<source-info group="Windows"></source-info>
mvn dependency:get -Dartifact=com.vaadin:license-checker-machineid:3.0.0
java -jar %userprofile%\.m2\repository\com\vaadin\license-checker-machineid\3.0.0\license-checker-machineid-3.0.0.jar
----

[source,terminal]
----
<source-info group="macOS/Linux"></source-info>
mvn dependency:get -Dartifact=com.vaadin:license-checker-machineid:3.0.0 && java -jar ~/.m2/repository/com/vaadin/license-checker-machineid/3.0.0/license-checker-machineid-3.0.0.jar
----
--

If you need the version 1 machine ID, replace all occurencies of `3.0.0` with `2.0.0`
