...
A notification can display additional information, such as an icon, custom actions in a dropdown, or an arbitrary data object. The available options are documented https://developer.mozilla.org/en-US/docs/Web/API/ServiceWorkerRegistration/showNotification[here]. Notification options can be provided to a [classname]`WebPushMessage` as a Java class object or record, or as a Jackson's `ObjectNode` representing the options in JSON format.
...
The example here is a web push notification with an icon, a data object, and a custom action.
...
The listener above opens the Dashboard view or focuses on the already open browser when a user clicks the 'Open Dashboard' action in a notification's dropdown. If a user clicks on the notification area instead, it opens a default page with an empty URL mapping. Additionally, it demonstrates how to access custom data in a notification.
