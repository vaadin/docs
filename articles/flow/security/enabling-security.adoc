...
A common use-case for secured applications is when a user has an issue and the admin/super users log in as the troubled user to see their issue to be able to assist them. This can be helpful with customer support analysis where the analyst can access the system as the actual user.

An insecure or security breach solution would to ask the customer to provide their password, or for the administrator to retrieve it from the database. Neither of these should be used. Instead, Spring Security has an impersonation feature.

By using impersonation, the system knows who has really logged in. It can be used to track an impersonator's action if there's an audit log in place.
...
If the [classname]`SwitchUserFilter` is initialized first, the wrong security holder is used and the feature won't work.
...
This has the admin impersonate the `user` account without having to know the password.
