...
- Log-out capability;
- Security configuration class that uses [classname]`VaadinSecurityConfigurer`; and
...
The next step is to have a Spring Security configuration class that uses [classname]`VaadinSecurityConfigurer`. There's no convention for naming this class, so here it's named [classname]`SecurityConfiguration`. However, take care with Spring Security annotations.
...
The [classname]`VaadinSecurityConfigurer` handles all of the Vaadin-related configuration. For example, it ignores static resources, enables `CSRF` checking while ignoring unnecessary checking for Vaadin internal requests, and configures proper exception handling for Vaadin applications.
...
The most important configuration in the previous example is the call to [methodname]`configurer.loginView(LoginView.class)`. This is how the view-based access control mechanism knows where to redirect users when they try to navigate to a protected view.

The log-in view should always be accessible by anonymous users, so it should have the [annotationname]`@AnonymousAllowed` annotation. This is especially important when using the variant of the [methodname]`loginView` method where you provide the route path -- although this signature is meant to be used with https://vaadin.com/hilla[Hilla] views, not with Flow views.
...
The component-based security configuration shown in the `SecurityConfiguration` example above is the recommended approach. Read more about  https://spring.io/blog/2022/02/21/spring-security-without-the-websecurityconfigureradapter[updating from WebSecurityConfigurerAdapter to component-based security configuration].
...
The same applies to custom [classname]`RouteNotFoundError` subclasses -- they also need a security annotation to be accessible. Note that, for a not-found view, the annotation alone isn't enough: requests for non-existing pages are denied at the HTTP layer unless the catch-all request rule is also relaxed. See <<{articles}/flow/routing/exceptions#,Router Exception Handling>>.
...
[NOTE]
The exception class given to `rerouteToError` is instantiated reflectively, so it needs a public no-arg constructor. If the class only declares a constructor that takes a message, it loses its implicit no-arg constructor and navigation fails with an internal server error instead of showing the error view. The exception doesn't need to carry the denial reason itself: the reason is made available to the error view separately, as [methodname]`ErrorParameter#getCustomMessage()`.
...
[IMPORTANT]
If you add the bean definition method in the same configuration class where you use [classname]`VaadinSecurityConfigurer`, the method must be declared `static`, to prevent bootstrap errors because of circular dependencies in bean definitions.
...
To add impersonation for a Vaadin application, create the [classname]`SwitchUserFilter` bean as follows:
