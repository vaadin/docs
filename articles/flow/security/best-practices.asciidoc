...
The way how to get the authenticated user and how to login/logout the user may vary from application to application.
Vaadin View-based access control implementation uses [methodname]#HttpServletRequest::getUserPrincipal()# and [methodname]#HttpServletRequest.isUserInRole(String)# to check whether the currently authenticated user has access to the view.
Thus, you have to use a solutions, which put the user information into request.
The example below uses currently active Vaadin servlet request and shows how to:

- Check whether the user is currently authenticated to the application or not.
The request is not available in background threads, so this method shows authentication state only in Vaadin request processing threads, otherwise it would always return `false`.
- Authenticate the user with the given credentials.
When this method is called from background threads, the authentication would always fail.
