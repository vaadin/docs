...
<1> `ViewAccessChecker`, this the core of this access control, which is instantiated and enabled.
<2> `LoginView` class is set to the `viewAccessChecker` instance, now it knows where to redirect unauthenticated users.
<3> `viewAccessChecker` instance is set as the `BeforeEnterListener` in the overridden `serviceInit` method.
