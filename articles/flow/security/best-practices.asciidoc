...
[NOTE]
If your Vaadin Flow application is based on Spring Boot, we strongly recommend following the dedicated documentation on using <<{articles}/flow/integrations/spring/view-based-access-control#, View-Based Access Control in Vaadin Flow Spring Boot applications>>.
...
[NOTE]
In this example, Vaadin's `LoginForm` component is used for the sake of brevity.
