...
public class LoginView extends VerticalLayout implements BeforeEnterObserver,
    ...
    public void onComponentEvent(AbstractLogin.LoginEvent loginEvent) {
        ...
        if (authenticated) {
            UI.getCurrentOrThrow().getPage().setLocation(LOGIN_SUCCESS_URL);
        } else {
            login.setError(true);
        }
    }
...
To restrict access to views, a [classname]`VaadinServiceInitListener` must be registered for the [classname]`VaadinService` to initialize and enable the [classname]`NavigationAccessControl` for all UIs:
