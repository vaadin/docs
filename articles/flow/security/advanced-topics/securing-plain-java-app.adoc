...
As described in the <<{articles}/flow/security/enabling-security#, enabling security>> chapter, Vaadin Flow comes with built-in security helpers that are most convenient to implement with Spring Boot and Spring Security.
While recommended, using Spring Security is not mandatory, however.
...
[NOTE]
====
If yours is a Spring Boot project, then it is highly recommended to follow the instructions provided in the <<{articles}/flow/security/enabling-security#, enabling security>> chapter.
====
