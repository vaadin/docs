...
This enables the web applications to run as _Progressive Web Applications_ (PWA) that aim to give the same experience as native applications.
...
PWAs combine new technologies with established best practices. They allow you to create reliable, accessible, and engaging applications that give users a native-like experience, with a user-friendly opt-in installation flow, and a capability to work offline.
...
[NOTE]
For more information about these properties, see https://developer.mozilla.org/en-US/docs/Web/Manifest[Web Application Manifest] in the MDN web docs. 
