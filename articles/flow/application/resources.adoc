...
The path for [methodname]`forClassResource()` is relative to that path.
...
[methodname]`forClassResource()` can also be used when the image is in the same path under resources as the class. For instance for the class `com.example.ui.MyData` the image can be below `src/main/resources/com/example/ui`
