...
The path for [methodname]`forServletResource()` is relative to that path.
...
The images can also be loaded using the [methodname]`forClassResource()` but then the image needs to be in the same path under resources as the class. For instance for the class `com.example.ui.MyData` the image needs to be below `src/main/resources/com/example/ui`
