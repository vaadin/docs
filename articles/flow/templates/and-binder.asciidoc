...
See <<../binding-data/flow-components-binder#,Binding Data to Forms>> for more.
...
First, we create the `MainView` LitElement template component. This component displays a grid of users and our new `UserForm` component. For the grid, we use the Vaadin <<../components/flow-grid#,`Grid` component>>
