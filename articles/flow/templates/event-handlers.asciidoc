...
`this.$server` and a `@ClientCallable` method. See <<tutorial-bindings#,PolymerTemplate, Binding Model Data>>) for how to get the model value directly from the server-side model.
