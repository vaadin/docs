...
With signals, the button text updates automatically. In this version, each time the button is clicked, the [classname]`Integer` value in the `count` [classname]`ValueSignal` is updated by adding 1 to it, and because the signal is bound to the text of the button, it tells the button to update itself with the new value:
...
For custom logic, you can create effects directly. In this example, the effect is a lambda callback passed into [methodname]`Signal.effect` that prints a message to the console whenever `firstName` or `lastName` changes:
...
| `SharedValueSignal`, `SharedNumberSignal`, `SharedListSignal`, `SharedMapSignal`, `SharedNodeSignal`
|===
