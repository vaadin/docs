...
[since:com.vaadin:vaadin@V25.3]#A value that arrives from the client isn't guaranteed to have the type of the bound signal -- the client is free to set a property to anything. A value the signal can't hold is rejected: a warning is logged, and the property is reverted on the client to the value the signal holds. The write callback isn't expected to handle this itself, so a binding needs no defensive code for it.#
