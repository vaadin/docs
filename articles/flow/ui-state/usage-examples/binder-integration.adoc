...
For simpler cross-field validation that doesn't need signals, use [classname]`Binder.Binding.valueSignal()` - binding validator re-runs always when a binding signal value changes:
...
You can mix [methodname]`binding.valueSignal()` and [methodname]`signal.get()` in the same validator - Binder re-validates whenever any referenced binding signal or outer signal value changes. Use signals when you need to share validation state across multiple components or derive complex logic from multiple sources.
