...
For simpler cross-field validation that doesn't need separate signal variables, use [methodname]`Binder.Binding.valueSignal()` — the binding validator re-runs whenever a binding signal value changes:
