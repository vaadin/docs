...
For simpler cross-field validation that doesn't need separate signals, use [classname]`Binder.Binding.valueSignal()` - the binding validator re-runs whenever a referenced binding value changes:
...
The [methodname]`valueSignal()` method returns a [classname]`ValueSignal` holding the current converted and validated value of the binding's field. When called inside a validator with [methodname]`get()`, Binder automatically tracks the dependency and re-validates when the referenced field changes.

You can mix [methodname]`binding.valueSignal().get()` and [methodname]`signal.get()` in the same validator - Binder re-validates whenever any referenced binding or signal value changes. Use signals when you need to share validation state across multiple components or derive complex logic from multiple sources.
...
The [methodname]`validationStatusSignal()` method returns a signal that updates whenever validation state changes. The [methodname]`map()` method transforms it to a boolean that controls the button's enabled state.
