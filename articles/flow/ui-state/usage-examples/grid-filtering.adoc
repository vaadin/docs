...
Connect the grid to the computed signal using [methodname]`ComponentEffect.effect()`:
...
*Component effects for grid updates*:: Use [methodname]`ComponentEffect.effect()` to connect the grid to the filtered data signal. The effect automatically tracks signal access via `signal.get()` and reruns whenever those signals change, updating the grid's items.
