...
Connect the grid to the computed signal using [methodname]`Signal.effect()`:
...
*Component effects for grid updates*:: Use [methodname]`Signal.effect()` to connect the grid to the filtered data signal. The effect automatically tracks signal access via `signal.get()` and reruns whenever those signals change, updating the grid's items.
...
- <<{articles}/components/grid#,Grid>> - Reference documentation for Grid component
