...
=== Session-Scoped Signals (Per User)

Use `@VaadinSessionScope` to create one signal instance per user session. Each logged-in user gets their own independent signal.
...
*Use case*: User-specific data like authentication state, preferences, or shopping cart contents.

*Result*: User A with ID 123 has their own `CurrentUserSignal` instance, completely separate from User B's instance.
...
public class DashboardView extends VerticalLayout {
    ...
    public DashboardView(CurrentUserSignal currentUserSignal) {
        Button increment = new Button("Increment");
        increment.addClickListener(e ->
            counterSignal.set(counterSignal.get() + 1));

        add(increment, VSpan.builder()
            .text(counterSignal.map(count -> "Count: " + count))
            .build());
    }
}
----
...
|User session lifetime, one per logged-in user
