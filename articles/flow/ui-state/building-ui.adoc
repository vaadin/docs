...
description: Using signals to build reactive user interfaces with component bindings in Vaadin Flow.
...
Vaadin components provide direct binding methods for common properties. These bindings create a reactive connection between your data and the UI:
...
This section covers the component-level binding APIs that most developers use. For lower-level Element bindings, see <<element-bindings#,Element Bindings>>.


== Signal Basics

Signals hold reactive values. The most common type is [classname]`ValueSignal`:

[source,java]
----
ValueSignal<String> name = new ValueSignal<>("initial value");
name.value("new value");  // Write
String current = name.value();  // Read
----

Use [methodname]`map()` to transform a signal's value:
...
For complete coverage of signal types and operations, see <<local-signals#,Local Signals>> and <<shared-signals#,Shared Signals>>.
...
[source,java]
----
ValueSignal<String> message = new ValueSignal<>("Welcome");
...
message.value("Hello!");
// Automatically updates to "Hello!"
----
...
Use [methodname]`Signal.computed()` to combine values from multiple signals:
...
termsCheckbox.addValueChangeListener(e -> termsAccepted.value(e.getValue()));
...
Signal<Boolean> isFormValid = Signal.computed(() -> {
...
submitButton.bindEnabled(isFormValid);
...
[source,java]
----
ValueSignal<Boolean> isLocked = new ValueSignal<>(false);

TextField field = new TextField("Document title");
field.bindReadOnly(isLocked);
// Field is editable initially

Button lockButton = new Button("Lock", e -> isLocked.value(true));
...
[source,java]
----
ValueSignal<Boolean> isDarkMode = new ValueSignal<>(false);

VerticalLayout layout = new VerticalLayout();
layout.getThemeList().bind("dark", isDarkMode);
// Dark theme applied when isDarkMode is true

Button toggleTheme = new Button("Toggle Dark Mode");
toggleTheme.addClickListener(e -> isDarkMode.update(v -> !v));
----
...
[source,java]
----
ValueSignal<Boolean> isCompact = new ValueSignal<>(false);
ValueSignal<Boolean> isBordered = new ValueSignal<>(true);

Grid<Person> grid = new Grid<>();
grid.getThemeList().bind("compact", isCompact);
grid.getThemeList().bind("row-stripes", isBordered);
----
...
ComponentEffect.bindChildren(todoList, todos, todoSignal -> {
    ...
    row.add(checkbox, text, deleteButton);
    return row;
});
----
