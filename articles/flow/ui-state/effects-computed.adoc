...
meta-description: Learn how to use Effect, computed signals, and standalone effects for reactive programming.
order: 30
---
...
The [methodname]`Effect.effect()` method creates an effect that runs immediately and re-runs whenever its signal dependencies change.
...
[source,java]
----
Registration effectRegistration = Effect.effect(() -> {
    // effect logic
});
