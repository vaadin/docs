...
However, local signals are scoped to a single server instance and cannot be used in clustered deployments. If you need state that works across a cluster or is shared between users, use <<shared-signals#,Shared Signals>> instead.
