...
By default, signals use [methodname]`Objects.equals()` to determine whether a value has changed. You can provide a custom equality checker to control when updates are skipped:

[source,java]
----
// Skip updates when strings are equal ignoring case
...
For [classname]`ListSignal`, the equality checker is applied to all entry [classname]`ValueSignal` instances within the list:

[source,java]
----
// All entries use the custom equality checker
ListSignal<String> items = new ListSignal<>(
    (a, b) -> a != null && a.equalsIgnoreCase(b));

ValueSignal<String> entry = items.insertLast("Hello");
entry.set("hello"); // No update triggered
----


== Reading and Writing Values

Signals provide two methods for reading values:

* `get()` - reads the value and registers a reactive dependency. This must only be called inside a reactive context such as an effect, a computed signal, or a transaction. Calling `get()` outside a reactive context throws [classname]`IllegalStateException`.
* `peek()` - reads the current value without registering a dependency. Use this for one-time reads in click listeners, initialization code, or any code outside a reactive context.

Use `set()` to write a new value:

[source,java]
----
ValueSignal<String> signal = new ValueSignal<>("Hello");

// Read the current value (outside reactive context)
...
Use [methodname]`moveTo()` to move an entry to a new position within the list without removing and re-inserting it:

[source,java]
----
ListSignal<String> items = new ListSignal<>();
ValueSignal<String> a = items.insertLast("A");
items.insertLast("B");
items.insertLast("C");
// List is: ["A", "B", "C"]
...
This is more efficient than removing and re-inserting, and preserves the entry's signal identity. Components bound to the moved entry are not recreated — they are repositioned in the DOM.


=== Reading List Contents

Use [methodname]`get()` to get a snapshot of the list:
...
List<ValueSignal<String>> entries = items.peek();

// Read values from entries
entries.forEach(entry -> System.out.println(entry.peek()));
----
...
Use [methodname]`asReadonly()` to create a read-only view of a [classname]`ValueSignal`. The returned [classname]`Signal` reflects all changes to the original signal, but cannot be written to:

[source,java]
----
ValueSignal<String> editable = new ValueSignal<>("Hello");
Signal<String> readOnly = editable.asReadonly();

// Changes to the original are visible through the read-only view
editable.set("World");
// readOnly now returns "World"
----

This is useful for exposing signal values to other parts of the application without allowing modifications:

[source,java]
----
public class UserService {
    private final ValueSignal<User> currentUser = new ValueSignal<>(null);

    // Expose a read-only signal to the UI
    public Signal<User> currentUser() {
        return currentUser.asReadonly();
    }

    // Only the service can modify the signal
...
The read-only [methodname]`map()` method described above creates a derived signal that updates when the source changes, but changes to the derived signal don't propagate back. For two-way binding scenarios where you need changes to flow in both directions, use [methodname]`bindValue()` with separate getter and setter arguments.
...
For immutable values like Java records, use [methodname]`map()` for reading and [methodname]`updater()` for writing:
...
The [methodname]`updater()` method takes a wither function that receives the current parent value and the new child value, and returns a new parent value with the child property updated. Using wither methods like `withDone()` is a common pattern for records.


=== Mapping Mutable Values (Beans)

For mutable beans with setters, use [methodname]`map()` for reading and [methodname]`modifier()` for writing:
...
The [methodname]`modifier()` method takes a setter function that receives the parent object and the new value, and mutates the parent in place. The signal framework handles change notification automatically.
