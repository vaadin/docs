...
For immutable values like Java records, use [methodname]`bindValue()` with [methodname]`map()` and [methodname]`updater()`:
...
The [methodname]`updater()` method creates a setter function that takes a new value and returns an updated parent object. Using wither methods like `withDone()` is a common pattern for records.


=== Mapping Mutable Values (Beans)

For mutable beans with setters, use [methodname]`bindValue()` with [methodname]`map()` and [methodname]`modifier()`:
...
The [methodname]`modifier()` method creates a setter function that takes a new value and mutates the parent object in place. The signal framework handles change notification automatically.

[NOTE]
====
Prefer immutable records over mutable beans when possible. Records are easier to reason about, naturally thread-safe, and work better with reactive patterns. Use [methodname]`modifier()` only when working with existing mutable bean classes.
====
...
| `bindValue(signal, signal::set)`
| Two-way binding to a simple signal
| `textField.bindValue(nameSignal, nameSignal::set)`

| `bindValue(signal.map(getter), signal.updater(wither))`
