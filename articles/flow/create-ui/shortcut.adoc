...
*Example*: Triggering a button click with the kbd:[Enter] key:
...
*Example*: Focusing a text field with the kbd:[Alt+F] key combination:
...
*Example:* Calling the `openCustomerCreation()` method with the kbd:[Ctrl+Alt+N] key combination:
...
*Example*: Clearing the input fields when kbd:[Esc] is pressed, but only when the focus is inside the `Scope` component:
...
*Example*: Removing a focus shortcut:
...
*Example*: Using the shorthand methods to assign key modifiers:
...
*Example*: Allowing default browser behavior for a focus shortcut:
...
*Example*: Allowing event propagation for a focus shortcut:
...
*Example*: Ensuring that any changes to the `description` field are submitted to the server before saving if kbd:[Ctrl+Enter] is pressed while the field is focused:
...
*Example*: Binding a shortcut to the lifecycle of a `Paragraph` component:
...
*Example*: Binding the lifecycle of a button click shortcut to a `Grid` component:
...
*Example*: Using a single listener for multiple shortcuts:
