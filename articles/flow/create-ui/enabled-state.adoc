...
*Example*: Overriding the default disabled behavior:
...
*Example*: Setting a child component as `disabled` by overriding the [methodname]`onEnabledStateChanged()` method.
...
*Example*: Implicitly enabled and disabled components
...
*Example*: Using `addPropertyChangeListener()` to synchronize the `prop` property of a disabled component:
...
*Example*: Using the `@Synchronize` annotation to synchronize the `prop` property of a disabled component:
...
*Example*: Unblocking a DOM event for a disabled element using the [methodname]`addEventListener()` method:
...
*Example*: Unblocking a DOM event for a disabled component using the `@DomEvent` annotation:
