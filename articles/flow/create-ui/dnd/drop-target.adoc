...
*Example*: Making two [classname]`VerticalLayout` components drop targets in different ways:
...
*Example*: Making a custom `Column` component an active drop target by default:
...
*Example*: Listening to drop events and moving the dragged component inside the drop target:
...
*Example*: Painting a light green outline around the drop target when a draggable element hovers over it:
