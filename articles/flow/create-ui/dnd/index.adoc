...
description: Dragging and dropping between components as well as between the UI and the operating system.
...
Vaadin supports generic drag and drop for any component. A *drag source* is a component that the user can drag, and a *drop target* is a component that the user can drop things on.

Drag and drop, or "DnD", support is based on the link:https://developer.mozilla.org/en-US/docs/Web/API/HTML_Drag_and_Drop_API[HTML5 drag and drop API]. Vaadin supports a subset of the API's features that make sense for server-side component development.

== Topics

section_outline::[]

// TODO Is the information below still accurate? Framework team needs to verify.
