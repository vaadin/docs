...
*Example*: Making two [classname]`Div` components draggable in different ways:
...
*Example*: Painting a dotted outline around the dragged component with some transparency:
...
*Example*: Making a custom `CardComponent` draggable by default:
...
*Example*: Setting different types of drag data on two draggable `CardComponent` instances:
...
*Example*: Highlighting suitable drop targets when dragging starts:
...
*Example*: Making only an icon within a custom `RouteItem` component draggable:
...
*Example*: Setting an image of the Ace of Spades as the drag image:
...
*Example*: Positioning the cursor 20 pixels from left edge, aligned with top edge:
...
*Example*: Using a [classname]`Span` as a drag image by adding it off-screen:
