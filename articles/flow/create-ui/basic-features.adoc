...
The component isn't removed from the DOM. Invisible components' DOM elements do not receive updates from the server, and server-side components do not react to RPCs (Remote Procedure Calls) if the component is set to invisible.
