...
*Example*: Calling the `clearSelection()` JavaScript function on the root element from the server side:
...
*Example*: Calling the `expand(otherComponentElement)` JavaScript function on the root element from the server side:
...
*Example*: Calling `MyModule.complete(true)` on the client side:
...
*Example*: Checking for support of Constructable Stylesheets in the browser:
