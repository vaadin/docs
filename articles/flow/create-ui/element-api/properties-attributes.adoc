...
You can use different variations of the [methodname]`Element.setProperty()` method to set a property value as a `String`, `boolean`, `double` or `JsonValue`.
...
Similarly, you can use different variations of the [methodname]`Element.getProperty()` method to retrieve the value of a property as a `String`, `boolean`, `double` or `JsonValue`.
...
*Example:* Using a NO-OP listener to synchronize the `value` property of a text input element whenever the `change` event occurs:
