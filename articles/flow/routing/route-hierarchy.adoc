...
- [methodname]`getRouteParent(Class, RouteParameters)` resolves the logical parent of a navigation target, returning an [classname]`Optional<RouteReference>`.
...
public class Breadcrumbs extends Div {
    ...
    private void update(RouterState state) {
        ...
        for (RouteReference entry : configuration.getRouteHierarchy(
