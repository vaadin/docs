...
You can define a parent layout by adding the [annotationname]`@Layout` annotation on the class, or by using the optional element `layout` from the `@Route` annotation.
...
This example renders [classname]`CompanyComponent` inside [classname]`MainLayout` by defining the layout for the route:
...
[NOTE]
When using the <<route#,`@Route("path")`>> annotation to define a route, the component by default renders in the `<body>` tag on the page when there is no `@Layout` RouterLayout. This is because the element returned by [methodname]`HasElement.getElement()` is attached to the `<body>` tag.


=== Configuring Layout Matching Routes

To configure which routes a [annotationname]`@Layout` annotated [classname]`RouterLayout` handles the value can be given as routes starting with.
The default value is `/` which matches all paths.

A value of `/user` would only match routes starting with a route path `/user/`, so for instance `/user/info`.
