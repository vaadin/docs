...
*Example*: Render all routes inside [classname]`MainLayout`
...
*Example*: Render [classname]`CompanyComponent` inside [classname]`MainLayout` by defining the layout for the route:
...
[NOTE]
When using the <<route#,`@Route("path")`>> annotation to define a route, the component by default renders in the `<body>` tag on the page when there is no `@Layout` RouterLayout.
