...
If the application is using access protection the view access is checked for the Layout as well as the view itself.
To make the layout accessible add [annotationname]`@AnonymousAllowed` (or other access annotation) on the `MainLayout` so that the request is not denied.
