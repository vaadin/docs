...
In addition to registering routes and route templates using the <<routing-annotation#,`@Route` annotation>>, you can add and remove routes dynamically during runtime. This is useful when a route should be added or removed based on changed business data or application configuration at startup, for example.
...
* <<router-templates#,Route Templates>>
* <<router-url-parameters#,Typed URL Parameters>>
