...
After log-in, a new route is added, depending on the access rights of the user.
...
The [classname]`LoginPage` class dynamically registers either the `AdminView` or `UserView` as a new route for a particular user session as follows:
...
private void handleLogin(
    ...
    UI.getCurrentOrThrow().getPage().reload();
}
----
