...
[since:com.vaadin:vaadin@V25.2]#Signals offer a reactive alternative#: `UI` and `VaadinSession` expose the current locale as a read-only signal through `localeSignal()`. Reading it with `get()` inside `Signal.effect()` re-runs the effect on every locale change, which fits the signal-based UI style and avoids implementing `LocaleChangeObserver`:
...
----
<1> Reading the signal registers it as a dependency, so the effect re-runs whenever the locale changes. Use `getLocale()` instead for a non-reactive read.
...
A locale set with `setLocale()` lasts only as long as the session. To remember the user's choice across sessions, store it in a cookie and apply it when a new session starts.

Write the cookie when the user changes the language, for example in the value change listener of the language switcher:
...
addValueChangeListener(event -> {
    ...
    Cookie cookie = new Cookie("app-locale", locale.toLanguageTag());
...
Read the cookie in a session init listener, registered through a `VaadinServiceInitListener`. Session init listeners run after Vaadin has picked the initial locale from the browser, so the locale from the cookie takes precedence:

[source,java]
----
@Bean
public VaadinServiceInitListener localeFromCookie() {
    return serviceInit -> serviceInit.getSource()
            .addSessionInitListener(sessionInit -> {
        Cookie[] cookies = sessionInit.getRequest().getCookies();
        if (cookies == null) {
            return;
        }
        List<Locale> providedLocales = sessionInit.getService()
                .getInstantiator().getI18NProvider().getProvidedLocales();
        Stream.of(cookies)
                .filter(cookie -> "app-locale".equals(cookie.getName()))
                .map(cookie -> Locale.forLanguageTag(cookie.getValue()))
                .filter(providedLocales::contains) // <1>
                .findFirst()
                .ifPresent(sessionInit.getSession()::setLocale);
    });
}
----
<1> The cookie is client-controlled, so only locales supported by the application are accepted.

This example uses a Spring `@Bean`. Without Spring, register the listener as described in <<{articles}/flow/advanced/service-init-listener#,VaadinServiceInitListener>>.
