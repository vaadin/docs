...
You can still use it on top of the latest stable version Vaadin 22.
...
Classic Components comes with `HorizontalLayout` and `VerticalLayout` designed to work and behave as close as possible to the corresponding components present on the Vaadin 7 or 8 versions.
