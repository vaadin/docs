...
To enforce nullability at compile time, use https://github.com/uber/NullAway[NullAway] with https://errorprone.info/[Error Prone]. This requires JDK 22 or later.

=== Adding the JSpecify Dependency

Add JSpecify annotations as a dependency:
...
The plugin configures Error Prone and NullAway for you. By default, it enables JSpecify mode and only checks classes in packages that are explicitly marked with `@NullMarked`.
...
=== Marking Your Code as @NullMarked

After setting up the tooling, annotate your own code. Apply `@NullMarked` at the package level by creating a `package-info.java` file:
...
You can also apply `@NullMarked` to individual classes if you prefer a gradual rollout.
