...
The binding between state and UI is done using a [classname]`ComponentEffect.format` helper that receives an owner component, the method reference that in turn plays a role of a signal effect function and the formatted string with the corresponding signals holding the values:
...
The [classname]`ComponentEffect.format` internally initializes effect function:
[source,java]
----
Signal.effect(() -> button.setText(String.format("Clicked %.0f times", counter.value())));
----

and automatically enables/disables it when a component is attached or detached from the UI.
...
Few helper methods are available for easier binding of signals to components that automatically enable/disable effect functions when a component is attached or detached from the UI:

[source,java]
----
// Bind an effect function to a component:
import java.util.Locale;ComponentEffect.effect(myComponent, () -> {
