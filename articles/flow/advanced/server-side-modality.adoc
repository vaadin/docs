...
Calling [methodname]`setChildComponentModal` with `true` equals [enumname]`ModalityMode.STRICT` and `false` equals [enumname]`ModalityMode.MODELESS`. More about modality mode in the next section.
...
* [enumname]`ModalityMode.VISUAL` — Modal only on the client. UI is visually blocked, but server-side interactions remain active.
