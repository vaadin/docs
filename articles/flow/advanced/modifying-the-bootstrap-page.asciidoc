...
[CAUTION]
Starting from Vaadin 15, Vaadin uses the <<../upgrading/changes#bootstrapping-changes, client-side bootstrapping>> by default, which means that the `UI` is not available in `AppShellSettings` unless the <<../guide/configuration#properties, `useDeprecatedV14Bootstrapping` flag>> is used.
