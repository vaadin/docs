...
[CAUTION]
Starting from Vaadin 15, Vaadin uses the client-side bootstrapping by default, which means that the `UI` is not available in `AppShellSettings` unless the <<{articles}/flow/configuration/properties, `useDeprecatedV14Bootstrapping` flag>> is used.
