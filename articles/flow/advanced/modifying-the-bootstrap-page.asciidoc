...
By default, during the first request, Vaadin parses the [filename]#frontend/index.html# template file and delivers it with a few modifications, as described in <<{articles}/fusion/application/starting#default-bootstrap-template-and-entry-point,Default Bootstrap Template and Entry Point>>.
