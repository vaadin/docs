...
[CAUTION]
Starting from Vaadin 15, Vaadin uses the <<../guide/upgrading#bootstrapping-changes, client-side bootstrapping>> by default, which means that the `UI` is not available in `AppShellSettings` unless the <<../guide/configuration/#configuration-properties, `useDeprecatedV14Bootstrapping` flag>> is used.
To configure the `loading indicator`, `reconnection dialog` or `push`, use an <<./ui-init-listener/#, UIInitListner>> instead.

[source,java]
----
include::{root}/src/main/java/com/vaadin/demo/flow/advanced/bootstrap/ServiceListener.java[]
----
