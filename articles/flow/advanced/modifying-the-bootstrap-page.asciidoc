...
The servlet processes the template and injects the following information:
  ...
  It uses a pre-configured https://webpack.js.org/[webpack^] instance that is included together with `vaadin-maven-plugin` as a module bundler.
...
.You can change the [filename]#frontend# diretory path
[TIP]
