...
[CAUTION]
Starting from Vaadin 15, Vaadin uses the <<{articles}/flow/upgrading/changes#bootstrapping-changes, client-side bootstrapping>> by default, which means that the `UI` is not available in `AppShellSettings` unless the <<{articles}/flow/configuration/properties, `useDeprecatedV14Bootstrapping` flag>> is used.
To configure the `loading indicator`, `reconnection dialog` or `push`, use a <<./session-and-ui-init-listener#, UIInitListener>> instead.
