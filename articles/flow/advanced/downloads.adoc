...
Since detaching the owner component doesn't interrupt an ongoing download, listeners are also notified for a download that outlives its owner: the UI used for the `UI.access` call is the one that was active when the download started, and it's kept attached to its session until the download has completed, even if it's closed while the download is ongoing.
A download does not, however, outlive its session: if the session is invalidated, either explicitly or by timing out, the ongoing download is terminated and the listener is notified through `onError` rather than `onComplete`.
...
[discussion-id]`C82C3953-6480-493C-A5D6-E397BC072DBA`
