...
The following tables show how to load various resources and where the resource files should be placed in a project, depending on packaging.
...
|CSS files
|`@StyleSheet("styles.css")`<<foot-1,[1]>>
...
.In projects with JAR packaging (e.g. Spring Boot self-contained executable JAR)
|===
|File type |Import |File location
...
Components such as [classname]`Image` and [classname]`Anchor` allow you to display or download dynamically generated content by using [classname]`DownloadHandler`. <<{articles}/flow/advanced/downloads#,Handle Downloads>> section describes how to use [classname]`DownloadHandler` in more detail.
...
+
Lit templates should be imported using `@JsModule` (see <</flow/component-internals/templates/basic#,Creating a Simple Component Using the Template API>> for more information).
...
+
By default, this renders a classic `<script>` element. Set the `type` attribute to `JavaScript.Type.MODULE` to render a `<script type="module">` element instead. A `type = JavaScript.Type.MODULE` file is always loaded at runtime and never bundled, even for a bare relative path, which is then resolved against the servlet context root. Use it for hand-authored or CDN-hosted ES modules that shouldn't go through Vite; for build-time bundled ES modules, use `@JsModule` instead. `LoadMode.INLINE` isn't supported in combination with `JavaScript.Type.MODULE`.

`@CssImport`::
Imports a style sheet from an npm package, which it can be included in the frontend bundle.
+
See <<{articles}/styling/advanced/npm-assets#styles-from-npm, Importing Stylesheets from npm Packages>>.

`@StyleSheet`::
Imports a local, external or linked style sheet.
+
See <<{articles}/styling/stylesheets#, Stylesheets>>.
...
You can use the [methodname]`addJavaScript(String url)` method in [classname]`Page` to load JavaScript. To load an external JavaScript module instead of a classic script, use the [methodname]`addJavaScript(String url, LoadMode loadMode, JavaScript.Type type)` overload with `type` set to `JavaScript.Type.MODULE`. This is the recommended replacement for the deprecated [methodname]`addJsModule(String url)` method, which serves the same purpose but doesn't accept a load mode.

The following example uses [methodname]`addJavaScript(String url)` to import a classic script, and the `type`-taking overload to import an external module:

[source,java]
----
UI.getCurrentOrThrow().getPage().addJavaScript("/js/script.js");
// external JavaScript module
UI.getCurrentOrThrow().getPage().addJavaScript(
        "https://unpkg.com/lodash@4.17.15", LoadMode.EAGER,
...
----
  public MainLayout() {
      UI.getCurrentOrThrow().getPage().addStyleSheet(
