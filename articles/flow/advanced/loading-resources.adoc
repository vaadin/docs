...
The classpath resources are stored in the `/src/main/resources` folder and can be loaded using this. If the resource name starts with `/`, it is considered to be an absolute path (underneath `/src/main/resources`). Otherwise, it will relative to the class package.
