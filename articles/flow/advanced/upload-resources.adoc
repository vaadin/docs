...
The existing [classname]`UploadHandler` implementations are accessible through the static methods in [classname]`UploadHandler`, e.g. `UploadHandler.toTempFile(FileUploadCallback successCallback)`
...
An upload is received independently of the owner component's lifecycle: once handling has started, detaching the owner, navigating to another view, or closing the browser tab doesn't abort it on the server.
The UI that owns the upload is kept attached to its session for as long as the upload is being received, even if that UI is closed in the meantime, so progress listeners and the upload callback still run.
The upload doesn't, however, outlive its session: if the session is invalidated, either explicitly or by timing out, the ongoing upload is terminated, its callback isn't run, and the partially received file is discarded.
