...
The returned [classname]`ExtendedClientDetails` also includes
[methodname]`isScreenOrientationSupported()`, which tells you whether the
browser exposes the Screen Orientation API. That is useful when you want to
decide synchronously whether to show an orientation-lock control before wiring
[methodname]`ScreenOrientation.orientationSignal()`.
