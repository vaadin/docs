...
The JavaScript runs on nothing in particular, since page-level JavaScript works on globals rather than on an element. A method that declares a return value can use the returned [classname]`PendingJavaScriptResult` the same way [methodname]`executeJs(String, Object...)` does.
