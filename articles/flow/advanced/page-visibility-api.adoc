...
The signal is seeded from the initial bootstrap, so reading it from [methodname]`onAttach` returns the real state instead of a placeholder. Subsequent changes are pushed from the browser's https://developer.mozilla.org/en-US/docs/Web/API/Page_Visibility_API[Page Visibility API] combined with [code]`focus`/[code]`blur` events.
...
`VISIBLE`:: The tab is shown and focused. The user is actively looking at the page ([code]`!document.hidden && document.hasFocus()`).
`VISIBLE_NOT_FOCUSED`:: The tab is shown but another window or application has focus. The page is on screen but is unlikely to be receiving the user's attention ([code]`!document.hidden && !document.hasFocus()`).
`HIDDEN`:: The tab is in the background, the window is minimized, or the tab is on a different virtual desktop ([code]`document.hidden`). Browsers also throttle timers and `requestAnimationFrame` in this state.
`UNKNOWN`:: No value has been reported yet. Only observed briefly between server attach and the first client handshake; after bootstrap, the signal transitions to one of the other values and never returns to [code]`UNKNOWN`.
