...
= [since:com.vaadin:vaadin@V25.2]#Page Visibility API#
:toc:
...
When the reaction is more than setting one property -- starting and stopping a task, sending a notification, updating several things at once -- subscribe with [methodname]`Signal.effect()` instead:

[source,java]
----
Signal<PageVisibility> visibility = UI.getCurrent().getPage()
...
The effect is bound to a component owner ([code]`this` above), so it stops automatically when the component detaches. Most applications never need explicit cleanup. For a one-shot read outside a reactive context, call [methodname]`peek()`.
