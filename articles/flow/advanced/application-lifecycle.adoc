...
page-title: How to manage the Vaadin application lifecycle
description: The more technical details of application deployment, user sessions, and the UI instance lifecycle.
meta-description: Understand the lifecycle of a Vaadin application by reading through the technical details of deployment, user sessions, and the UI instance lifecycle.
order: 10
---
...
If a UI is serving an upload or download when it's closed, it stays attached to its session until that transfer has been handled, so the transfer progress listeners and completion callbacks bound to it keep running even though there's no client left to receive UI changes.
The request that serves the transfer detaches the UI once it's done.
...
An upload or download doesn't outlive the session of the UI it's serving.
If the session is invalidated, whether explicitly or by timing out, while a transfer is still ongoing, that transfer is terminated: the stream carrying it stops transferring bytes and throws an [classname]`IOException` instead, so an ongoing upload is discarded before its callback runs and an ongoing download is left truncated.
An application with a transfer that should survive the session can keep the session alive while the transfer is ongoing, or serve it through a servlet of its own that isn't tied to a UI and a session.
