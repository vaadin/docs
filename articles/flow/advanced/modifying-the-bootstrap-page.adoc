...
If the XHR response cannot be parsed as JSON, Vaadin looks for a Vaadin-Refresh string anywhere inside the response text. If it is present, then Vaadin reloads the page instead of showing an error message. The refresh token can be added as meta tag.
