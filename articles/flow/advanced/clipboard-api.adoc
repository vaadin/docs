...
The Clipboard API requires every write to happen inside a fresh user gesture, which is why [methodname]`Clipboard.onClick(component)` is the only entry point. A few consequences:
...
- If the application performs a long server round-trip before the write call, the browser may consider the gesture stale and reject the write. Keep the action attached to the click rather than scheduling it through [methodname]`ui.access()`.
