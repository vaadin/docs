...
*Thread-Safe*:: Shared signals handle concurrent access from multiple users safely.
...
Effects are callbacks that re-run automatically when any signal they read changes. Dependencies are tracked automatically based on which signals are accessed during execution.
