...
[NOTE]
====
For most applications, use the component-level binding methods covered in <<building-ui#,Building UI Using Signals>>. Component methods like `bindText()`, `bindVisible()`, and `bindEnabled()` provide a simpler API for common use cases. Use Element bindings when:
...
[source,java]
----
ValueSignal<Boolean> isVisible = new ValueSignal<>(true);
...
[source,java]
----
ValueSignal<Boolean> isEnabled = new ValueSignal<>(true);
...
Form field bindings provide two-way synchronization between fields and signals. For comprehensive examples including all supported field types, see <<building-ui#two-way-form-field-binding,Two-Way Form Field Binding>> in Building UI Using Signals.
...
[source,java]
----
ValueSignal<Boolean> isReadOnly = new ValueSignal<>(false);
