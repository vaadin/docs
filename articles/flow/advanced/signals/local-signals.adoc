...
meta-description: Learn how to use ValueSignal from com.vaadin.signals.local for managing UI-only state.
order: 20
---
...
The [classname]`ValueSignal` class from `com.vaadin.signals.local` is a writable signal that holds a reference to a value. Unlike shared signals, it doesn't provide thread safety for concurrent reads and writes, making it suitable for state that is only used by a single UI instance.
...
== Best Practice: Store Signals as Fields
