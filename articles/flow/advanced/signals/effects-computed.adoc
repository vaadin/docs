...
The [methodname]`ComponentEffect.bind()` method is a convenience helper for the common pattern of binding a signal value to a component property:
...
ComponentEffect.bind(ageLabel, userSignal.map(u -> String.valueOf(u.age())), Span::setText);
