...
For binding signals directly to component properties like text, visibility, and form field values, see <<building-ui#,Building UI Using Signals>>. This section covers effects for custom logic and computed signals for derived values.


== When to Use Effects

Use direct binding methods (covered in <<building-ui#,Building UI>>) when:
...
See <<building-ui#,Building UI Using Signals>> for available binding methods.
