...
[NOTE]
====
The annotation is applied while the UI is being created, before a <<session-and-ui-init-listener#,`UIInitListener`>> runs. If a `UIInitListener` also sets the push mode or transport on the UI's `PushConfiguration`, the listener's settings take precedence over the `@Push` annotation.
====
