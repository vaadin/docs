...
[NOTE]
====
The loading of shadow DOM stylesheets from the `components` sub-folder isn't compatible with the [since:com.vaadin:vaadin@V24]#<<{articles}/flow/configuration/development-mode#precompiled-bundle, pre-compiled frontend bundle>>#. To use this mechanism for loading shadow DOM styles, you need to <<{articles}/flow/configuration/development-mode#disable-precompiled-bundle, disable the pre-compiled bundle>>.
====
