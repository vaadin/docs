...
A ComboBox differs in its databinding for Grid in two ways: it doesn't have UI controls for defining the sort order; and it has a UI control for defining a filter string.
...
A service used for a ComboBox always has at least two parameters: a `Pageable` instance to define which page to load; and a `String` that provides the input the user has typed into the `combobox` field -- which is empty by default.
