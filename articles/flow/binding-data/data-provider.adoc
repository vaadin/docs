...
Use [methodname]`DataView.getItems(int offset, int limit)` for efficient paginated access to displayed items. This returns a filtered and sorted stream of items from the specified range:

[source,java]
----
GridDataView<Person> dataView = grid.getGenericDataView();

// Get items 20-29 (page 3, page size 10)
try (Stream<Person> page = dataView.getItems(20, 10)) {
    List<Person> pageItems = page.toList();
    // Process the page...
}
----

[IMPORTANT]
The returned stream must be used with try-with-resources to ensure proper resource cleanup. The method throws [classname]`IndexOutOfBoundsException` if offset or limit is negative.
