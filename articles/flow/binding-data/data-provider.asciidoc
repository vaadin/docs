...
If you are using DTOs or otherwise want to customize binding to Spring Data based backend, the VaadinSpringDataHelpers class also contains `toSpringPageRequest` and `toSpringDataSort` methods to convert Vaadin query hints to related Spring Data relatives. 
