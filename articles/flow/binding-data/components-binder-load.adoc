...
By default, any change to a binding's value marks that binding as changed, even if the initial value is restored for said binding. However, if [methodname]`setChangeDetectionEnabled(true)` is called, initial value of the binding is compared to the new value. The `hasChanges()` method returns true only if the new value doesn't match the initial value. Otherwise, the binding is not marked as "changed," thereby ensuring that `changedBindings` only includes entries where the current value differs from the initial value. When this feature is enabled, Binder will use [methodname]`equals` to check equality of values, which can be overridden with a custom equality predicate for each binding via [methodname]`withEqualityPredicate(SerializableBiPredicate<TARGET, TARGET> equalityPredicate)`.

When there are status changes, the Binder fires a StatusChangeEvent. You can use this event to enable and disable the form buttons, depending on the current status of the Binder.
