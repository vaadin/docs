...
After each value is changed, the initial value is compared to the new value. The `hasChanges()` method returns true only if the new value doesn't match the initial value. Otherwise, the binding is not marked as "changed," thereby ensuring that `changedBindings` only includes entries where the current value differs from the initial value. 

When there are status changes, it fires an event. You can use this event to enable and disable the form buttons, depending on the current status of the form.
