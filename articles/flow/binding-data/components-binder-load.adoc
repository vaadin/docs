...
[classname]`Binder` keeps track of which bindings have been updated by the user and which bindings are in an invalid state. When the `readBean` method is invoked, the initial value of each field is stored, and after each value change, the initial value is compared to the new value. The `hasChanges()` method returns true only if the new value does not match the initial value.
Else, the binding is not marked as "changed," thereby ensuring that `changedBindings` only includes entries where the current value differs from the initial value. It fires an event when there are status changes. You can use this event to enable and disable the form buttons, depending on the current status of the form.

It fires an event when there are status changes. You can use this event to enable and disable the form buttons, depending on the current status of the form.
