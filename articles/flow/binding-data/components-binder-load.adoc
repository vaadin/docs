...
For writing the record, the `writeRecord` method should be used. Calling this method will apply field and binder level validators, and either return a new record instance with the current state or throw a `ValidationException`.
