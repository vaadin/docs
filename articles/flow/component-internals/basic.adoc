...
=== Attach-Scoped Setup and Cleanup

When the cleanup in [methodname]`onDetach()` only undoes what [methodname]`onAttach()` did, the two halves can be kept together with [methodname]`whenAttached()`. The handler runs every time the component is attached -- immediately, if the component is already attached -- and the [classname]`Registration` it returns runs when the component is detached again.

.Subscribing to an event bus in [methodname]`whenAttached()` and unsubscribing in the returned registration
[example]
====
[source,java]
----
include::{root}/src/main/java/com/vaadin/demo/reference/componentinternals/basics/CartSummaryWhenAttached.java[indent=0]
----
====

Because the cleanup closes over the local variables of the handler, no field is needed to carry state from an attach to the matching detach. The handler receives the [classname]`UI` the component was attached to, and it can return `null` when there is nothing to clean up.

Removing the returned registration removes the handler and also runs any cleanup still pending from the latest attach. An exception thrown by the handler is propagated to the caller, whereas one thrown by the cleanup is passed to the session error handler, so that a failing cleanup does not prevent the rest of the detach handling from running.

The same method is available on the element level as [methodname]`Element.whenAttached()`. Both are public, so they can also be used from the outside to hook into the life cycle of another component.
