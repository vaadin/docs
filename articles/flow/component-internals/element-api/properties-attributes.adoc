...
You can use different variations of the [methodname]`Element.setProperty()` method to set a property value as a `String`, `boolean`, `double`, or `JsonNode`.
...
Similarly, you can use different variations of the [methodname]`Element.getProperty()` method to retrieve the value of a property as a `String`, `boolean`, `double`, or `JsonNode`.
...
For complex property values such as objects or arrays, use the [methodname]`Element.setPropertyJson()` method with Jackson's `JsonNode` types.
