...
The arguments passed to the expression must be a type supported by the communication mechanism. The supported types are `String`, `Integer`, `Double`, `Boolean`, `JsonNode`, `Element`, and `JsFunction` (see <<#js-function,Passing JavaScript Functions>>).
...
Captures may be any value supported as an [methodname]`executeJs()` parameter, including `Element`. An attached element arrives on the client as the corresponding DOM node; a detached element arrives as `null`.
...
Use [methodname]`withArguments()` to declare named parameters that the materialized function accepts at call time. The body references them by name, and the caller supplies them positionally after the captured values:
...
Inside the body, `this` follows normal JavaScript call semantics &ndash; it isn't bound to the host element. The caller of the materialized function chooses the receiver:
...
To use a specific element from within the body regardless of how the function is called, pass it as a capture instead of relying on `this`.
