...
As with [methodname]`executeJs()`, values passed after the expression are available as `$0`, `$1`, and so on. Here the event type (`"click"`) and the log label (`"clicked"`) come from the server rather than being hard-coded in the expression.
