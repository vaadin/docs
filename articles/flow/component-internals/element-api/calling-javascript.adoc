...
The return value of the expression is read synchronously. A function is treated as the cleanup callback; returning nothing (no `return`, or `undefined`) means there's no cleanup. Returning any other value &ndash; including a `Promise` resolving to a function &ndash; is logged as an error on the client.

If setup is asynchronous, don't return the `Promise`. Do the async work inside the body, keep a reference to whatever needs to be torn down, and return a synchronous cleanup function that uses it:

[source,java]
----
getElement().addJsInitializer("""
        let widget;
        import('./my-widget.js').then((module) => {
            widget = module.create(this);
        });
