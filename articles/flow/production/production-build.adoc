...
Note that Vaadin automatically clears the generated frontend files after building a project for production, if they did not exist before the build. See `vaadin.clean.build.frontend.files` and `cleanFrontendFiles` in <<{articles}/flow/configuration/properties#vaadin-plugin-properties,Vaadin Plugin Properties>> for more information.
