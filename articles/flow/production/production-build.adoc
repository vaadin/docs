...
The content depends on the environment in which your application is running, but all variations call the Maven goal `vaadin:build-frontend`. Once the `vaadin:build-frontend` goal is added, you can call the production build command.
...
Transpilation in Vaadin means converting all TypeScript files to JavaScript. The build targets the browsers listed under <<{articles}/compatibility#browser-compatibility, Browser Compatibility>> -- evergreen Chrome, Edge and Firefox, Firefox ESR, and the latest minor of Safari 17 -- rather than a JavaScript language year, so CSS and JavaScript are downleveled only as much as those browsers actually need. Targeting real browsers instead of a language year keeps CSS features such as `light-dark()` intact, since a language year maps to browsers old enough that Lightning CSS would otherwise rewrite it into something the browser only resolves once for the whole page.

During the build, minimization is done to make the files smaller. When minifying code, it's often obfuscated, which makes it more difficult to read. Hence, this isn't done for development builds. To keep client-side stack traces readable in a production build, generate source maps for the bundle -- see <<source-maps#, Source Maps in Production>>.
...
== Pre-Compressed Static Resources

Vaadin has always served a pre-compressed `.br` or `.gz` sibling of a static resource when one exists, and Vite writes those siblings for the assets that go into the frontend bundle. Static resources served straight from [filename]`META-INF/resources`, however -- a stylesheet referenced with `@StyleSheet`, a JavaScript file, an SVG -- never enter that bundle, and so had no compressed variant to serve. A production build now compresses them too, writing a https://github.com/google/brotli[Brotli] (`.br`) and a gzip (`.gz`) sibling next to each compressible file. The server then sends whichever variant the browser accepts, and the uncompressed file to a browser that accepts neither.

Files with these extensions are compressed, provided they're at least 1 KB -- below that, the compressed variant tends to be no smaller than the original: `.css`, `.js`, `.mjs`, `.cjs`, `.json`, `.map`, `.svg`, `.html`, `.htm`, `.xml`, and `.txt`.

Brotli compression needs Node, which the JDK can't provide. Node is used only when it's already available, so it's never downloaded merely to compress resources; without it, the build writes the gzip variants only. Compression is a best-effort optimization: a failure is logged as a warning and never fails the build, since the uncompressed resource is always there to serve.

Serving the Brotli variants is controlled by the `brotli` <<{articles}/flow/configuration/properties#,configuration property>>. Set it to `false` to stop serving them; the gzip variants are used regardless.
...
Vaadin clears the generated frontend files after building a project for production, if they didn't exist before the build. See `vaadin.clean.build.frontend.files` and `cleanFrontendFiles` in <<{articles}/flow/configuration/properties#vaadin-plugin-properties,Vaadin Plugin Properties>> for more information.
