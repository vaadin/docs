...
Vite compresses the assets that go into the frontend bundle, but static resources served straight from [filename]`META-INF/resources` -- a stylesheet referenced with `@StyleSheet`, a JavaScript file, an SVG -- never enter that bundle. A production build therefore compresses them separately, writing a https://github.com/google/brotli[Brotli] (`.br`) and a gzip (`.gz`) sibling next to each compressible file. The server then sends whichever variant the browser accepts, and the uncompressed file to a browser that accepts neither.
