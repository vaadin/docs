...
The same attention should be kept when using POJOs with [classname]`Element` <</flow/component-internals/element-api/properties-attributes#,Properties & Attributes>> or <</flow/component-internals/element-api/calling-javascript#,JavaScript invocation>>; every used custom class must be registered for reflection.
An exception are classes used in <</flow/component-internals/client-server-rpc#,Remote Procedure Calls>> API, that are automatically detected and registered for reflection by Vaadin, but only when used with Spring Boot or Quarkus.
...
Another use case that requires manual registration of reflection hints is the usage of [methodname]`rerouteToError()` method in [classname]`BeforeEvent`. The framework automatically registers hints for exceptions handled by [interfacename]`HasErrorParameter<T extends Exception>` implementors, but it does not detect direct usage with [methodname]`rerouteToError()`. Exception used with that method must be manually registered as explained above.
