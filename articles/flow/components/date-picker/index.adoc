...
By default, Date Picker displays and parses dates using the user's locale (<<{articles}/flow/advanced/i18n-localization.asciidoc#locale-selection-for-new-session,reference>>). Alternatively, setting a specific locale ensures that all users consistently see the same format. In the example here, the Date Picker is set to the date format used in Finland.
...
include::{articles}/flow/components/_input-field-common-features.adoc[tags=basic-intro;label;helper;placeholder;tooltip;clear-button;prefix;aria-labels]
...
include::{articles}/flow/components/_input-field-common-features.adoc[tag=readonly-and-disabled]
...
include::{articles}/flow/components/_input-field-common-features.adoc[tags=styles-intro;text-alignment;small-variant;helper-above-field;borders]
