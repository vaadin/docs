...
By default, Date Picker displays and parses dates using the user's locale (<<{articles}/flow/advanced/i18n-localization.adoc#locale-selection-for-new-session,reference>>). Alternatively, setting a specific locale ensures that all users consistently see the same format. In the example here, the Date Picker is set to the date format used in Finland.
