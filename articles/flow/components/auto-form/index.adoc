...
include::{articles}/hilla/react/components/_shared.asciidoc[tag=scaled-examples]
include::{articles}/hilla/react/components/_shared.asciidoc[tag=scaled-examples-responsive]
...
Auto Form uses the same validation mechanism as <<{articles}/hilla/react/guides/forms/binder-validation#,default Hilla Forms>>. You can add Bean validation annotations to your entity classes, and the form automatically validates the input values on both client and server.
...
For further information on using validation, refer to the <<{articles}/hilla/react/guides/forms/binder-validation#,Forms>> documentation.
