...
The component class included with the https://github.com/vaadin/addon-starter-flow/archive/master.zip[Vaadin Add-on Starter], for example `PaperSlider.java`, is only a stub that handles the imports (see <<index#,Integrating a Web Component>> for more). In this section, we demonstrate how to add functionality to the included stub class to transform it into a functioning Java API.
