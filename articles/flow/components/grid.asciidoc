...
The `withFunction()` method can also receive additional parameters.
The additional parameters can be accessed in the handler as the second parameter (of type `JsonArray`).
...
grid.addColumn(LitRenderer.<Person>of(
    ...
    .withFunction("changed", (person, params) -> {
        String profession = params.getString(0);
