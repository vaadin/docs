...
Behind the scenes `Grid` uses the `DataProvider` interface to communicate with the backend.  The `setItems` method is a shorthand to create a `ListDataProvider`. For a large amount of data or other advanced use cases you should probably use the `DataProvider` interface and lazy loading. See <<../binding-data/tutorial-data-provider#,Data Providers>> for more.
...
See <<../binding-data/tutorial-data-provider#,Data Providers>> for more.
...
* *Backend sorting* is applied by providing a list of `QuerySortOrder` objects to your `DataProvider`, that typically passes the sort hints to the backend code, and in some cases all the way to database queries. See <<../binding-data/tutorial-data-provider#Sorting,Data Providers>> for more.
...
To learn how to customize the styles inside a component’s shadow DOM, see <<{articles}/theming/styling-components#,Styling Components>>.
