...
The `paper-slider` component allows the user to input a single value. To make it work automatically as a field,  this kind of component should implement the `HasValue` interface. See in <<../binding-data/flow-components-binder#,Binding Data to Forms>> for more.
