...
When making a production-ready build, the Vaadin Gradle Plugin transpiles the client-side dependencies to legacy browsers, as described in <<{articles}/flow/production/overview#, Deploying to Production>>.
...
Defaults to false.
For production, the frontend is transpiled for older browsers and optimized, as described in <<{articles}/flow/production/overview#,Deploying to Production>>.
