...
The file and folder hierarchy defines the website URL and navigation structure directly. Each <<asciidoc,AsciiDoc>> file (`.adoc` or `.adoc`) inside the content folder corresponds to a page on the website.
...
| `articles/flow/components/button/style.adoc`
...
.`articles/flow/components/accordion/index.adoc`
[source,yaml]
----
---
title: Accordion
...
Files whose names start with an underscore (e.g., `_shared.adoc`) and which don't define any <<metadata,page metadata>>, don't appear as pages in the website. Those files can be used to share content using AsciiDoc's https://docs.adoctor.org/asciidoc/latest/directives/include/[include directive].
...
The content of each page is written in the AsciiDoc syntax, which has some https://docs.adoctor.org/asciidoc/latest/syntax-quick-reference/#markdown-compatibility[similarities with Markdown]. See the https://docs.adoctor.org/asciidoc/latest/syntax-quick-reference/[AsciiDoc Syntax Quick Reference] for a comprehensive list of AsciiDoc features.
...
Use https://docs.adoctor.org/asciidoc/latest/verbatim/source-blocks/[source code blocks] to display code or command-line examples. Syntax highlighting for various languages is supported. The most commonly used ones are `java`, `typescript`, `html`, and `css`. For example:
...
By default, the source code language is used as the title of the block. You can define a https://docs.adoctor.org/asciidoc/latest/blocks/add-title/#add-a-title-to-a-block-with-attributes[custom title], as well. For example, you could define a file name to illustrate the location of the code like so:
...
You can include code examples from any file in the documentation project, by using the AsciiDoc https://docs.adoctor.org/asciidoc/latest/directives/include/[include directive] -- primarily from files in the `frontend` and `src/main/java` source code folders, but practically from any folder in the project. The benefit of including them from the source code folders is that those files are compiled during the build. This ensures that any compilation errors in the code examples are detected early.
...
Snippets -- known as https://docs.adoctor.org/asciidoc/latest/directives/include-tagged-regions/[Tagged Regions] in AsciiDoc -- are segments of code examples. They're displayed by default, instead of the entire source code of an included file.
...
Add dependencies for custom components to the [filename]`pom.xml` file the same way as a <<{articles}/flow/styling/advanced/multi-app-themes#,custom theme JAR>>. Components and themes can be included in the same dependency.
