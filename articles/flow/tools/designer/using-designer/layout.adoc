...
Vaadin supports many pre-built <<{articles}/flow/components#layouts, layout components>> for you to quickly start constructing a view.
...
|<<{articles}/flow/components/app-layout#, App Layout>>
...
[NOTE]
When the Lumo theme is in use, the margin, padding and spacing properties accept t-shirt sizes which translate to the corresponding <<{articles}/flow/styling/lumo/lumo-style-properties/size-space#space, spacing properties>>. Otherwise, the property values map directly to the corresponding CSS properties.
