...
The default component theme is `Lumo`. The `Material` component theme is also available. Both themes have "light" and "dark" variants (see <<{articles}/flow/styling/lumo/lumo-variants/#light-and-dark-variants, Dark Mode>>).
...
Vaadin Designer supports theming in the same way as Flow (see <<{articles}/flow/styling/application-theme#, Application Theme>> for more information). When a design is opened, Vaadin Designer does a few things.
