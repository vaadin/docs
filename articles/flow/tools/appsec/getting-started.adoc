...
AppSec Kit isn't enabled in production mode. You'll need to start your application in <<{articles}/flow/configuration/development-mode#, development mode>> -- which is the default mode in which applications start. You don't need to enable this, explicitly.
