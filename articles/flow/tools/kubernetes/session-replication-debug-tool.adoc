...
When the HTTP Session Replication Tester tries to serialize and deserialize the HTTP session, the serialization process may take longer for larger sessions. The Debug Tool uses a timeout to wait for this process to finish. If the serialization process takes longer than the timeout, it aborts the waiting and continues with the next steps and logs the results. In such a case, serialization timeout is logged and appears in the results to notify the user. To avoid the timeout, configure the serialization `timeout` property. The value should be in milliseconds.

In SpringBoot applications, it's available in the `SerializationProperties` configuration properties. It can be set through application properties like so:
