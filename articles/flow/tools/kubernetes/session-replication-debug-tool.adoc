...
== Configuring Debug Tool

=== Session Serialization Timeout

When the HTTP Session Replication Tester tries to serialize and deserialize the HTTP session the serialization process may take longer time in case of larger sessions. The Debug Tool uses a timeout to wait for this process to finish. If the serialization process takes longer than the timeout then it aborts the waiting and continues with the next steps and logs the results. In such a case, the serialization timeout is logged, that also appears in the results to notify the user. To avoid the timeout, there is a serialization `timeout` property, that can be configured. The value should be defined in milliseconds.

In SpringBoot applications it is available in the `SerializationProperties` configuration properties. It can be set through application properties:
...
In non SpringBoot applications, a  system property is available to set it:
...
If not configured, it defaults to 30 seconds.
