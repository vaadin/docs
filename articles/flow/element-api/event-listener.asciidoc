...
[TIP]
The `filter` and `debounce` settings in the `@DomEvent` annotation can also be set through the `DomListenerRegistration` object. See <<../creating-components/tutorial-events#,Using Events with Components>> for more.
