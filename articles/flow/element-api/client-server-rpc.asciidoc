...
In client-side Polymer template code, `this` refers to the corresponding element, so that the calling convention is [methodname]#is.$server.serverMethodName(args)#.
...
*Example*: Using [methodname]#is.$server.getGreeting()# to call a server-side method and `await` the result.
...
*Example*: Using [methodname]#is.$server.getGreeting()# to call a server-side method and wait for the result in a callback.
