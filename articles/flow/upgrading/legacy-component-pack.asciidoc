...
The Label component included in the legacy components pack supports the same API as its counterpart in Vaadin 7/8. The only unsupported API is `setIcon`.

Same as Vaadin 7/8, it's possible to change how the component interprets its contents. The content mode can be `ContentMode.HTML`, `ContentMode.PREFORMATTED` and `ContentMode.TEXT` which is the default.
For the caption to be interpreted as HTML `setCaptionAsHtml` method can be used.

==== Deviations

Unlike the Label component in Vaadin 7/8, the wrapper element is present at all times, regardless of whether a caption is set or not. Note that this does not affect the layout, but it might break some CSS selectors. For example when you use CSS direct child selector, e.g. `.my-class > .v-label`.

Another deviation from Vaadin 7/8 is when the content mode is set to `ContentMode.HTML`. Although the anchor and the image tags are still going to work, contrary to Vaadin 7/8 the script tags are completely removed from the content. The same applies to when the caption is interpreted as HTML.
