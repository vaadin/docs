...
The Legacy Component Pack is licensed under Commercial Vaadin Developer License 4.0 (CVDLv4) and part of the https://vaadin.com/pricing[Prime Subscription].
...
If you're upgrading from Vaadin 7 you can configure the Legacy Component Pack components to use the Vaadin 7 defaults with the system property `vaadin.legacy-pack.vaadin7defaults`. By introducing the property (no value needed), the Vaadin 7 defaults are used.
...
While most of the API comes from the legacy API, there are a few methods that are either not implemented or have their signature changed.
...
The Label component included in the legacy components pack supports the same API as its counterpart in Vaadin 7/8.
