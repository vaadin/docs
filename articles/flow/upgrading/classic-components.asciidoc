...
Classic Components includes the following: `HorizontalLayout`, `VerticalLayout`, `Panel`, `Label`, `GridLayout`, and `FormLayout`. These components either lack counterparts in Flow entirely, or have significantly different APIs, requiring a lot of modifications when migrating. 

Additionally, it contains a number of abstract components from Vaadin 8 on which these components were based including `AbstractComponent`, `AbstractOrderedLayout`, and others. By using Classic Components, only the imports for the supported components need to be changed to be able to use almost all of the same component features. 
...
Using Classic Components in combination with MPR is not recommended for the current versions since it can lead to styling conflicts between the two.
