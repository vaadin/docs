...
The Classic Components is an add-on that provides a set of backwards-compatible "classic components" that ease migrating from Vaadin 7 or 8 to the latest Vaadin version.
...
By using the classic components, only the imports for the supported components need to be changed to be able to use almost all of the same component features.

== License

The Classic Components is licensed under Commercial Vaadin Developer License 4.0 (CVDLv4) and part of the https://vaadin.com/pricing[Prime Subscription].
...
To use the classic components, you first need to add the dependency to your project's `pom.xml` or `build.gradle` as follows:
...
The classic `FormLayout` resembles, as close as possible, the Vaadin 7 or 8 version of `FormLayout`.
...
The Classic Components is shortened to _LCP_ in the tables below.
