...
These alternatives are covered in this documentation, but if some information is missing, please https://github.com/vaadin/docs/issues/new[create an issue] so we may add the necessary information.
...
The `@PreserveOnRefresh` annotation in Vaadin 8 and 7 was added to the `UI` subclass. In Vaadin 10+, the  `@PreserveOnRefresh` annotation should be added to the view class or router layout class. As a consequence, a typical pattern of storing data via reference to the UI class can not be used with Vaadin 10+. For details see <<{articles}/flow/advanced/tutorial-preserving-state-on-refresh#,preserving state on refresh>>.
