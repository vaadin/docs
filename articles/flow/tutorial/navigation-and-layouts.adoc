...
+
`MainLayout`: application Layout with header and navigation:
+
...
Lastly, in [classname]#ListView#, update the `@Route` mapping to use the new `MainLayout` and delete the `@Theme` annotation.
...
[NOTE]
====
<<{articles}/ds/components/charts#,Vaadin Charts>> is a collection of data visualization components that is a part of the https://vaadin.com/pricing[Vaadin Pro subscription].
