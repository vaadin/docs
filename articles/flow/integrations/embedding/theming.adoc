...
=== CssImport in Embedded Application

<<../../../styling/legacy/css-import#,`@CssImport` annotation>> with just a `value` attribute can be injected automatically to embedded application's shadow root by using `autoInjectGlobalCssImports` theme property in `theme.json` in a following way. Property is `false` by default.
...
This is useful when embedded application uses a component that uses `@CssImport` annotation to import global styles. `autoInjectGlobalCssImports` ensures that global styles are also applied inside the embedded application's shadow root.
