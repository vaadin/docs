...
The `@Push` annotation declaration has the same limitation as the <<tutorial-theming#,@Theme` annotation>>: it is only possible to configure Push for one exporter. Declaring different `@Push` annotations for different exporter classes will result in an exception during startup. 
