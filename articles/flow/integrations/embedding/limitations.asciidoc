...
* Theme resources should be available on the server of the embedding page or inlined in css.
+
When embedding the application on a different server any relative css url will target the embedding server and not the embedded application server.
