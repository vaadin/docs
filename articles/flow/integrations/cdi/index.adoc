...
The https://github.com/vaadin/cdi[Vaadin CDI] add-on allows you to use Vaadin with https://tools.jboss.org/features/cdi.html[CDI].
...
Import the Vaadin Bill of Materials (BOM), and add the `vaadin-cdi` add-on and the Jakarta EE API with the `provided` scope to your [filename]`pom.xml`:
...
Use <<{articles}/building-apps/testing/browserless/setup-cdi#,Weld-backed browserless tests>> to exercise CDI-managed Flow views without deploying the application.
The <<{articles}/flow/testing/browserless/cdi#,testing reference>> describes lifecycle ordering and the boundary between the CDI and Vaadin contexts.
