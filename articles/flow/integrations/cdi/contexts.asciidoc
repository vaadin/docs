...
Vaadin CDI contexts are conceptually similar to <<../spring/spring-scopes#,Vaadin Spring scopes>>.
...
See <<../../advanced/push-access#asynchronous-updates,Asynchronous Updates>> for more about using push.
...
See <<../../advanced/application-lifecycle#loading-a-ui,Loading a UI>> for more about the `UI` lifecycle.
...
See <<../routing/routing-annotation#,Defining Routes With @Route>> and <<../routing/router-layout#,Router Layouts and Nested Router Targets>> for more about route targets, route layouts, and the route chain.
