...
The root Vaadin component of your Vaadin Portlet application is the one passed as a type parameter to the subclass of `CdiVaadinPortlet` (`BookListView` above).
...
You can have your own `I18NProvider` subclass automatically instantiated as a managed bean when using Vaadin Portlet CDI.
