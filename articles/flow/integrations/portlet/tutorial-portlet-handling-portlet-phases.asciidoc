...
As described in <<tutorial-portlet-creating-vaadin-portlets#,Creating Vaadin
Portlets>>, the Vaadin Portlet consists of two parts: a class extending `VaadinPortlet` (or `VaadinLiferayPortlet` for Liferay) and a portlet view class extending Vaadin `Component`.
...
The portlet view is instantiated automatically from the actual type parameter passed to `VaadinPortlet` (or `VaadinLiferayPortlet` for Liferay) when subclassed into your own portlet class.
...
- `PortletModeListener`
- `WindowStateListener`
- `PortletEventListener` (part of the <<tutorial-portlet-inter-portlet-communication.asciidoc#,Inter-Portlet Communication>>)
