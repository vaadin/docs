...
The classes ending in `Portlet` are the classes extending `VaadinLiferayPortlet` and classes ending in `View` are the view component classes for the portlets.
...
The [methodname]`onPortletViewContextInit(PortletViewContext)` in the [interface]`PortletView` interface provides the implementing class a reference to a [classname]`PortletViewContext` object, which allows us to register listeners and change the portlet's state.
