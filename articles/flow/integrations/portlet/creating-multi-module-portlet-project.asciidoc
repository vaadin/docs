...
order: 5
description: Shows how to create a multi-module Vaadin Portlet Maven project with each portlet in its own module, and frontend assets are in one module
layout: page
page-links:
...
This allows each portlet to be placed in its own module, making for greater separation between the portlets.
...
The project consists of _1+N_ modules, where _N_ is the number of portlets in the project; _1_ stands for the Vaadin frontend asset module.
...
.Add module as dependency to static assets [filename]#pom.xml#
[source,xml]
----
<dependency>
...
The significant part of the project structure, in the context of add-ons, is the `frontend` resource folder.
...
For portlets running in Liferay 7, the JSON file should contain a segment "o":
...
[NOTE]
The relative URL is mapped to port `8080`, so the above partial URL would map to `http://127.0.0.1:8080/vaadin-portlet-static/VAADIN/config/stats.json`.
...
The deployment of a multi-module Vaadin Portlet project is very similar to that of a single-module project: deploy all the portlet `WAR` files and the static assets `WAR` file to your web server.
