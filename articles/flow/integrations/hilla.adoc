...
It is possible to use a Flow server side main layout for both server views and Hilla client views.
...
This will tell the builder that `Flow` should be used as a main layout for Hilla views where `ViewConfig` contains `flowLayout: true`.
...
[NOTE]
If the `flowLayout` is not defined or set to false then the view will not ask the server for a route layout to embed into.
