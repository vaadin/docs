...
As described in <<../routing/page-titles.adoc#Updating Page Title during Navigation,Updating Page Title during Navigation>>, page title for a route can be updated with an annotation and with an interface. Page title can show up anywhere in hilla main menu by using Signal called `window.Vaadin.documentTitleSignal`. As long as the signal is initialized in the client side, server will keep signal's value in sync with the server.

Following example shows how to use `window.Vaadin.documentTitleSignal` to show page title defined with `PageTitle` annotation in a server side route in the Hilla main menu. Example includes only relevant parts that need to be added for the functionality:
