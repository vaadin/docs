...
Hilla is part of the Vaadin Platform. It's used for building Reactive web applications on Java backends. It integrates seamlessly a https://reactjs.org/[React] TypeScript frontend with a https://spring.io/projects/spring-boot[Spring Boot] backend.
...
Mode is configurable with [interfacename]`MenuAccessControl` interface with the `PopulateClientMenu` enumerated choice.
