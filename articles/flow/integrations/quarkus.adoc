...
Hilla is currently not supported by Vaadin Quarkus add-on, because of its Spring requirement.
Adding Quarkus Spring extensions does not allow Hilla to work correctly, because they do not provide a complete Spring implementation,
as explained in the https://quarkus.io/guides/spring-di#important-technical-note[Important Technical Note] paragraph of Quarkus Spring DI documentation.
