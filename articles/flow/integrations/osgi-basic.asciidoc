...
Make a production build of the project (see <<../../guide/production/production-mode-basic#,Preparing your application for Production>>) which means that frontend resources are compiled into a bundle (using `build-frontend` Vaadin maven plugin goal) and packaged together with Java code into the resulting project JAR.
...
For example, classes annotated with `@Route` annotation are used in the routing functionality (see <<../routing/routing-annotation#,Defining Routes with @Route>>).
Many other classes require discovery as well (see also
<<../routing/routing-exception-handling#,Router Exception Handling>>,
<<../pwa/pwa-pwa-with-flow#,Creating PWA with Flow>>).
