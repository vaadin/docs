...
However, there are several reasons why this is not supported:

* This cannot properly work with OSGi Declarative Services.
The `MainView` would need to be a service by itself, which requires quite error prone boilerplate code, such as: `@Component(scope=ServiceScope.PROTOTYPE, service=SomeDedicatedService.class)`
