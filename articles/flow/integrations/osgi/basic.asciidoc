...
Make a production build of the project (see <<{articles}/flow/production/overview#,Deploying to Production>>) which means that frontend resources are compiled into a bundle (using `build-frontend` Vaadin maven plugin goal) and packaged together with Java code into the resulting project JAR.
...
<<{articles}/flow/configuration/pwa#,PWA Configuration>>).
