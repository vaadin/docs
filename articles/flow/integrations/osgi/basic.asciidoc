...
Make a production build of the project (see <<{articles}/flow/guide/production#,Deploying to Production>>) which means that frontend resources are compiled into a bundle (using `build-frontend` Vaadin maven plugin goal) and packaged together with Java code into the resulting project JAR.
...
For example, classes annotated with `@Route` annotation are used in the routing functionality (see <<{articles}/flow/routing/overview#,Defining Routes with @Route>>).
Many other classes require discovery as well (see also
<<{articles}/flow/routing/exceptions#,Router Exception Handling>>,
<<{articles}/flow/guide/configuration/pwa#,PWA Configuration>>).
