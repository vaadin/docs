...
Following a server restart, the application refrains from opening a new browser tab until a 30-minute interval has elapsed to avoid excessive tab proliferation. This delay is recalculated upon each server restart. In the event of a restart occurring before the interval lapses, a new tab will be opened 30 minutes post-restart. To trigger the tab opening immediately, you can perform a project cleanup (e.g., mvn clean or gradle clean) or delete the [filename]`tab.launch` file in the build directory (e.g., `target`` for Maven or `build`` for Gradle).
