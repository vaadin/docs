...
If your Vaadin views are served under a custom mapping, such as `http://localhost:8080/my_mapping/`, you need to ensure that the correct path is provided for the images. To reference the image from the application root, you can use a relative path, such as `new Image("../images/logo.png", "Company Logo")`.
