...
[NOTE]
The [classname]`BeanStore` is stored in the session. This means in case you persist sessions, e.g. when using Kubernetes Kit, the beans needs to be serializable.
...
[NOTE]
Unlike with earlier Vaadin versions 7 and 8, the `UI` and thus the `UIScope` beans aren't preserved when the `@PreserveOnRefresh` annotation is used and the browser is refreshed. To preserve the beans on refresh, you need to use `@RouteScope` instead, as described in <<routescope.preserve, the next chapter>>.
...
public class UserProfileView extends VerticalLayout {
    ...
    public void redirectToLogin() {
        ...
        UI.getCurrentOrThrow().navigate("login");
    }
}
