...
This approach is also based on Vaadin's Spring Boot and Spring Security autoconfiguration.
...
As mention in the <<Introduction,Introduction>>, this mechanism is based on Spring Security and Vaadin's Spring Boot autoconfiguration.
...
Therefore, none of the views are accessible, until one of the above annotations (except the `@DenyAll`) applied to them.
