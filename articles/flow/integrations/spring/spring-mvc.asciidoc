...
[NOTE]
See <<basic#,Using Vaadin with Spring Boot>> to use Vaadin with https://spring.io/projects/spring-boot[Spring Boot].
...
To use <<spring-scopes#,Vaadin Spring scopes>> you need to register the `VaadinScopesConfig` configuration class. As an alternative, you can add the `@EnableVaadin` annotation to your configuration class to import `VaadinScopesConfig`.
...
To handle URLs, you need at least one Vaadin component, annotated with `@Route`. See <<basic#handling-urls,Handling URLs>> for an `@Route` annotation example.
