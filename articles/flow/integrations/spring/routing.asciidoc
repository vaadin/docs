...
Routing works the same way with Spring as it does in plain Vaadin applications. See <<../routing/routing-annotation#,Routing and Navigation>> for more.

This content applies to using Vaadin with both <<basic#,Spring Boot>> and <<spring-basic-mvc#,Spring MVC>>.
...
<<../routing/routing-annotation#,Defining Routes with @Route>> for more about using the `Router` class.
...
[NOTE]
You do not need to use the `@EnableVaadin` annotation at all with <<basic#,Spring Boot>>. This annotation is intended to be used with <<spring-basic-mvc#,Spring MVC>> to enable the Vaadin configuration. In Spring Boot, auto-configuration is available which makes it work out of the box. The only reason to use `@EnableVaadin` is to specify the packages to scan with Spring MVC.
