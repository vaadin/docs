...
However, there are some benefits to extending from `VaadinWebSecurityConfigurerAdapter`, for example, the *view-based access control mechanism is enabled automatically*, and no further configurations needed to enable it.
Other of benefits are covered as follows.
