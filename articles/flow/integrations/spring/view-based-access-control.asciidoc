...
The configuration steps for a Vaadin Flow Spring application are quite similar to those of a Vaadin Flow application which is based on Spring Boot.

- The application should obviously have a login view; you can see an example login view <<Login View,here>>.

- The Spring Security dependencies are as follows:

.Direct Spring Security dependencies that should be added to the `pom.xml` file:
[source,XML]
----
<dependencies>
    <!-- other dependencies -->
...
Again, this is very similar to the <<Annotating the View Classes,provided examples>> for a Vaadin Flow application based on Spring Boot.
