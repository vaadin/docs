...
[TIP]
Although the same set of annotations is used to define the security access control rules on endpoints of a <<{articles}/fusion/overview#,Vaadin Fusion>> application, this document does not describe how to secure client-side views.
This is described in <<{articles}/fusion/security/configuring#,Configuring Security>> in Fusion documentation.
