...
Having an owner view class as a parameter in the `@RouteScopeOwner` of a model class ties application's view layer to a model layer.
It can be decoupled by using a model's interface in a view class and marking a concrete model's implementation class with `@RouteScopeOwner`.
