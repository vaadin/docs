...
Having an owner view class as a value in the `@RouteScopeOwner` of a model class ties application's view layer to a model layer.
It can be decoupled by splitting the model into interface and concrete class, and then using the model's interface in the view class and marking a concrete model's implementation class with `@RouteScopeOwner`.
