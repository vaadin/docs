...
More information on beans and scopes in Spring can be read from the https://docs.spring.io/spring-framework/docs/current/reference/html/core.html#beans-definition[Spring project documentation].
...
IMPORTANT:
By default, all Flow routing components (`@Route`, `RouterLayout` or `HasErrorParameter`) are Spring-initialized and act like a bean in _prototype_ scope even without explicitly adding the `@SpringComponent` and `@Scope("prototype")` annotations.
You can inject other beans to the components, but note that while `@PostConstruct` methods work, `@PreDestroy` methods do not.
It is possible to widen the scope of the routing components with `@UIScope` and `@RouteScope` as described later.
