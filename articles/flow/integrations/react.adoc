...
You can use these methods to implement Java component property getters and setters, and event APIs. In our example, we are going to use the synchronized state for the [methodname]`getColor()` and [methodname]`setColor(RgbaColor)` accessor methods.
...
Let us invoke [methodname]`setColor(RgbaColor)` from the constructor to define the initial state.
...
In addition, add the [methodname]`addColorChangeListener(Consumer<T>)`  state change callback.
