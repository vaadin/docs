...
Add the `search` method to the contacts repository to provide the service class with the needed method for filtering contacts.
...
Build the project and verify that you can now see contacts in the grid. 
Try filtering the contents by typing into the filter text field.

image::images/database-access/contacts-grid-filtering.png[The grid dispays 4 contacts after filtering with the string "ja"]
