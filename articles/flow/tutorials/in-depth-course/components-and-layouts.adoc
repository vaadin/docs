...
description: Flow uses a event-driven component model for building UIs. This tutorial covers the basics.
---
...
== Quick Introduction to Vaadin Flow
...
The first view is the Contact list view. This view lists all the contacts in the system. 
...
<8> Configure the search field to only fire value change events when the user stops typing. This way you avoid unnecessary database calls.
...
There are only a couple of cases where you need to restart the server, those will be highlighted in the instructions. 
