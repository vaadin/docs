...
== Running Vaadin TestBench Tests

TestBench tests are run by executing `./mvnw -Pit verify` from the command line, or by selecting the Maven `it` profile and running the verify goal from the Maven menu in IntelliJ.
