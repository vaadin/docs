...
The plugin records browser traffic from TestBench tests, converts it into k6 scripts, and handles Vaadin-specific details such as session management, CSRF tokens, and push communication. The generated scripts can then be run with k6 against any server.
...
. *Record* -- A TestBench test runs through a recording proxy that captures all HTTP traffic as a HAR (HTTP Archive) file.
...
The plugin analyzes HAR content to identify user actions (clicks, text input) and page loads, and inserts appropriate delays. If the recorded HAR already contains large gaps (for example, from `Thread.sleep()` or TestBench wait methods), no additional delay is added for that action.
...
Disable think times when:

- You want to stress test the server at maximum request rate.
- Your TestBench tests already include realistic pauses, and you prefer not to add extra delays.


[[error-handler]]
== Error Visibility in Load Tests

By default, Vaadin catches server-side exceptions during request processing and shows a notification in the browser. The HTTP response remains a 200 with valid UIDL, which means k6 cannot detect that an error occurred.

To make server-side errors visible to k6, add the load test error handler dependency to your application:

[source,xml]
----
<dependency>
    <groupId>com.vaadin</groupId>
    <artifactId>loadtest-error-handler</artifactId>
    <scope>runtime</scope>
</dependency>
----

This registers an error handler via ServiceLoader that re-throws exceptions, causing Vaadin to return an error response that k6 checks can detect. No code changes are needed.

[NOTE]
Only include this dependency in builds used for load testing. It changes error handling behavior, which is not desirable in production.


[[full-example]]
== Full Maven Configuration Example

Below is a complete example that records two TestBench scenarios and runs them as a combined load test:
