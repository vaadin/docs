...
Create a base class for your load test scenarios that handles proxy configuration. The following base class checks whether the `k6.proxy.host` system property is set. When it is, it replaces the default driver with one configured to route traffic through the recording proxy. When the property is not set, the tests run normally without a proxy:
