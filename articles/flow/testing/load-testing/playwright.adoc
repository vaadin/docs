...
| Extra test boilerplate
| None -- `PlaywrightHelper` only
| Proxy driver swap in `@BeforeEach` via `LoadTestItHelper.openWithProxy()`
...
=== "Session needs to be refreshed" errors

This typically means the server ran out of session capacity.
