...
If you're migrating from JUnit 5 (`vaadin-testbench-unit-junit5`), no other code changes are needed. The test API -- including [methodname]`navigate()`, [methodname]`test()`, and the [methodname]`$()` query methods -- remains the same.
