...
[since:com.vaadin:vaadin@V25.3]#A location string may also carry a query string and a fragment. The query string is parsed into the [classname]`QueryParameters` of the resulting location, so that a view reading them sees what it sees in the browser, and the fragment is kept on the location:#

[source,java]
----
OrderView view = navigate("orders/1?tab=history#totals", OrderView.class);
----
