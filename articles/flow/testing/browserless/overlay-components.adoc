...
[NOTE]
Earlier versions let a test click the items of a closed menu. See <<migration#menu-testers-require-an-open-menu, Menu Testers Require an Open Menu>> for updating existing tests.
...
.Behavior Changed in 25.3
[NOTE]
[methodname]`test(gridContextMenu)` previously returned a [classname]`ContextMenuTester`. See <<migration#grid-context-menus-have-their-own-tester, Grid Context Menus Are Opened on a Row>> for updating existing tests.
