...
If you've added custom components (not just text items) to the menu, use the tester's [methodname]`find()` method on the wrapped component (e.g., [methodname]`test(menu).find(...)`) instead of the top-level [methodname]`find()` query. Call [methodname]`open()` first if you need the components to be in an attached state.
