...
| One [classname]`UI` instance -- a single browser window. Provides the testing DSL ([methodname]`navigate`, [methodname]`$`, [methodname]`test`, ...).
|===
...
Routes are discovered with [classname]`Routes.autoDiscoverViews(...)`, passing the packages that contain [annotationname]`@Route`-annotated views.

.Plain Java
[source,java]
----
Routes routes = new Routes()
        .autoDiscoverViews("com.example.shop");
...
class MultiUserSecurityTest {
    ...
    void setUp() {
        Routes routes = new Routes()
                .autoDiscoverViews("com.example.security");
...
[methodname]`BrowserlessApplicationContext.builder(routes)` exposes the same configuration knobs as the framework-specific factories. Call [methodname]`build()` directly for an unsecured context, or chain [methodname]`withSecurityContextHandler(...)` to transition to a typed [classname]`SecuredBrowserlessApplicationContext.Builder<C>` whose [methodname]`build()` returns the credential-aware variant.
...
| [methodname]`withServletFactory(BiFunction<Routes, UIFactory, VaadinServlet>)`
