...
Since Selenium 4.6, the built-in https://www.selenium.dev/documentation/selenium_manager/[Selenium Manager] automatically downloads and manages browser drivers. There's no need to install or configure drivers manually -- instantiating a driver such as [classname]`ChromeDriver` takes care of everything. This works both when running tests locally and in CI/CD pipelines.
