...
The LLM can read the chart's current configuration and queries through a state tool. The configuration is returned without series data, but its x-axis categories and series names are derived from the query results, so those values are visible to the LLM. See <<controllers#database-provider,Database Provider>>.
