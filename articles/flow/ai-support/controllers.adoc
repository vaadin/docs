...
* <<ai-powered-form#,[classname]`FormAIController`>> -- fills the fields of any form layout from natural-language input or attached files.
...
* [methodname]`onRequest()` -- runs on the UI thread just before the LLM stream opens. Use it to lock UI surfaces, snapshot state the tool definitions depend on, or otherwise prepare for the turn.
