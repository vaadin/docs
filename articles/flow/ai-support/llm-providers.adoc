...
In synchronous mode, the whole exchange runs in the request that triggered it and blocks the UI until the response is complete. See <<threads-and-ui-updates#,Threads & UI Updates>> for keeping the UI responsive during long prompts.
...
Synchronous mode blocks the UI for the duration of each exchange; see <<threads-and-ui-updates#,Threads & UI Updates>>.
