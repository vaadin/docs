...
[TIP]
Model Context Protocol (MCP) support lives in the AI framework, not in the orchestrator. To give the orchestrator an MCP server's tools, configure Spring AI's MCP client, build a [classname]`ChatClient` that carries the resulting tool callbacks, and wrap that client in a [classname]`SpringAILLMProvider`. See <<mcp-servers#,MCP Servers>>.
