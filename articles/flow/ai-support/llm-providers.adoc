...
[TIP]
A Model Context Protocol (MCP) server's tools reach the orchestrator through Spring AI's own MCP client rather than through the orchestrator: build a [classname]`ChatClient` that carries the MCP tool callbacks, and wrap that client in a [classname]`SpringAILLMProvider`. See <<mcp-servers#,MCP Servers>>.
