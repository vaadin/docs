...
.MCP Needs the Spring AI Provider
[NOTE]
[classname]`LangChain4JLLMProvider` is built from a [classname]`ChatModel` or [classname]`StreamingChatModel` and runs the tool-calling loop itself, so LangChain4j's [classname]`McpToolProvider` -- which attaches to an AI service built with [classname]`AiServices` -- has nowhere to plug in. Use [classname]`SpringAILLMProvider` for an assistant that takes its tools from an MCP server.
