...
Each field gets an opaque identifier that the controller uses on the LLM side. The identifier is stored on the field component, so it stays the same when a field is detached and re-attached within a session.
...
For every named binding (`bind("propertyName")`, `bindInstanceFields(this)`, or `@PropertyId`), the bean property name is used as a default field description, so the LLM can refer to the field by its bean-side name. An explicit [methodname]`describe()` call always overrides the default. Lambda-bound bindings have no property name and contribute no default.
