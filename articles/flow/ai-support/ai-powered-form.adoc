...
The controller walks the container's component tree on every LLM turn, so fields added or removed between turns are picked up automatically. Any component that implements the [classname]`HasValue` interface is treated as a field; nested layouts are walked recursively.
