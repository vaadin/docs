...
[classname]`GridAIController` populates a [classname]`Grid` with data from the application database based on natural-language requests. Given a <<controllers#database-provider,[classname]`DatabaseProvider`>>, the controller exposes three tools to the LLM:
