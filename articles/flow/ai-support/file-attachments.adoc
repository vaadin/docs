...
[methodname]`saveAttachments` and [methodname]`showAttachment` are placeholders for application code. The first persists the [classname]`AIAttachment` data under the request's message ID -- to filesystem, database, object storage, or wherever fits. The second loads it back when the user clicks an attachment in the message list.
