...
[NOTE]
The following MIME type categories are supported: images (`image/*`), text (`text/*`), PDF (`application/pdf`), audio (`audio/*`), and video (`video/*`). Attachments with other MIME types are silently dropped and not sent to the LLM. To send such a file anyway, convert it to a supported format with a <<request-interception#,request interceptor>>.
