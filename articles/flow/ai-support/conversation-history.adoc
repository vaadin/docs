...
[methodname]`saveToDatabase` is a placeholder for application code. Write the [classname]`ChatMessage` list to whichever storage the application uses, keyed by a session identifier so the history can be loaded again later.
...
[source,java]
----
// provider, tools, controller, and savedAttachments are application-supplied:
// providers and controllers are not serialized and must be recreated.
