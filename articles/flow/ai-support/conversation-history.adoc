...
[IMPORTANT]
With a streaming provider, or when <<background-execution#,background execution>> is enabled, the listener runs on a background thread, where blocking I/O (such as database writes) is safe. With a synchronous provider in the default execution mode, the whole turn -- this listener included -- runs in the request that triggered the prompt, so blocking work extends that request. To update Vaadin UI components from this callback, wrap the update in `ui.access()`.
...
orchestrator.reconnect(provider)
        ...
        .withBackgroundExecution(executor)       // optional
        .apply();
----

A custom background executor is transient like the provider; pass it to the reconnector to keep running turns on it. Without it, an orchestrator built with background execution falls back to the shared pool. See <<background-execution#,Background Execution>>.
