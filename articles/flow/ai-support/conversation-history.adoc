...
[IMPORTANT]
The thread the listener runs on depends on the provider mode -- see <<threads-and-ui-updates#,Threads & UI Updates>>. To update Vaadin UI components from this callback, wrap the update in `ui.access()`.
...
Provider settings such as the streaming mode and <<threads-and-ui-updates#background-execution,background execution>> live on the provider and are not serialized; apply them to the new provider instance before passing it to [methodname]`reconnect()`.
