...
The builder accepts either a <<{articles}/components/message-list#,[classname]`MessageList`>> directly or any [classname]`AIMessageList` implementation. With a [classname]`MessageList`, the orchestrator adds and removes only its own entry among the list's <<{articles}/components/message-list#typing-indicator,typing users>>. When the application has bound the typing users to a signal, they can't be set, and the orchestrator leaves the indicator to the application.
