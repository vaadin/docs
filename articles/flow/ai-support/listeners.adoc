...
A turn that [classname]`LangChain4JLLMProvider` ends because the model requested more tool calls than allowed fails with a [classname]`ToolCallLimitExceededException` as its error; see <<tool-calling#tool-call-limits,Tool Call Limits>>. Spring AI stops its loop at the same limits without failing the turn and reports the finish reason `toolCallLimitExceeded` instead.
