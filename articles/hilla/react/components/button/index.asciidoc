...
[TIP]
Size variants should only be used in special cases. See <<https://vaadin.com/docs/styling/lumo/lumo-style-properties/size-space#,Size and Space>> for details on how to change the default button size.
...
[TIP]
The standard Button colors can be adjusted using <<https://vaadin.com/docs/styling/lumo/lumo-style-properties/color#,the Lumo color properties>>. Therefore, these variants shouldn't be used to replace standard buttons only to achieve a different color.
