...
Custom Flow components can be embedded into Hilla views by implementing a [classname]`WebComponentExporter` and using it in the view. A [classname]`WebComponentExporter` can target any Flow Component. Below is an example for the component that follows it:
...
It would be made into a [classname]`WebComponent` like so:
...
If the component needs attributes or other custom parts in `React.createElement`, instead of importing `createWebComponent`, import `loadComponentScript` and create the element in the function.
