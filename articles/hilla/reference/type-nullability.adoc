...
For a service function, nullable elements are as follows: *Function Parameter Type* or *Function Return Type*.

For Function Parameter Types, arguments cannot be omitted, even when the parameter types are nullable. To receive a `null` parameter value in Java, send an `undefined` argument in the service function call.

.Original Java service class
[source,java]
----
...
.Generated TypeScript service functions
[source,typescript]
----
export async function setName(
  ...
  return client.call('PersonService', 'setFullName', {firstName, lastName, middleName});
}

export async function getFullName(): Promise<string> {
  return client.call('PersonService', 'getFullName');
}

export async function getConnections(): Promise<Record<string, string> | undefined> {
  return client.call('PersonService', 'getConnections');
}
----
