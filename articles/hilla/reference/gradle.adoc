...
. https://start.spring.io/[Spring Initializr] which enables you to configure a bare-bones Vaadin project by adding the `Vaadin` dependency. Add the Hilla Spring Boot starter to it as described in <<../guides/endpoints#enabling, Enabling Endpoints in a Vaadin Project>>.
