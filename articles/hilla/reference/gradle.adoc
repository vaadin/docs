...
<<{articles}/getting-started#, Getting Started Guide>>.
...
. https://start.spring.io/[Spring Initializr] which enables you to configure a bare-bones Vaadin project by adding the `Vaadin` dependency. Add the Hilla Spring Boot starter to it as described in <<../guides/browser-callable-services#enabling, Enabling Browser-Callable Services in a Vaadin Project>>.
...
.Hilla Lit Applications
[NOTE]
In a Hilla Lit application, the view is a [filename]`HelloWorldView.ts` file, and the [filename]`frontend` folder also contains [filename]`index.ts` and [filename]`routes.ts`.
...
`hillaGenerate`::
  This task reads the configuration file created by the configure task, and then parses the classes annotated by [annotationname]`@BrowserCallable` to generate an [filename]`openapi.json` file. Then the [filename]`openapi.json` file is loaded and passed to a process that generates or updates the TypeScript stubs for calling the backend services. `hillaGenerate` can be executed independently of the startup process as a standard Gradle task:
...
The following options are provided by the Hilla Gradle plugin and can be used while configuring a Vaadin project:

`exposedPackagesToParser`::
  By default, the classes annotated by [annotationname]`@BrowserCallable` in the [filename]`src` of the current Gradle module are parsed by Hilla to generate TypeScript code for calling the services. If you have browser-callable services in a dependency or in another module of a multi-module Gradle project, you need to explicitly expose their package to Hilla. You can achieve this like so:
...
.Exposing Services from Current Module or Project
[NOTE]
...
`productionMode`::
  By default, the Vaadin Gradle plugin builds the project in production package when run with `bootJar` or `bootBuildImage` task. If you plan to build the project for production some other way, configure the [filename]`build.gradle` file as follows:
