...
Although getting comfortable with creating streaming, non-blocking applications requires some effort, Hilla allows you to take advantage of Reactor in minutes. To learn the basics of reactive services, read the https://vaadin.com/blog/interactive-applications-with-server-push[blog article] on the subject.


== Familiarizing

Before looking at the API details, you might familiarize yourself with the basics of reactive services by writing a simple application that can stream the current time of the server to the client. The task of this example is to send the server time to all subscribed clients, with an interval of one second between each message.

Start with a Vaadin project that has browser-callable services enabled: see <<{articles}/getting-started#,Getting Started>> for creating a project, and <<{articles}/hilla/guides/browser-callable-services#enabling,Enabling Browser-Callable Services in a Vaadin Project>> for the dependency they need.


=== Server-Side Services

Next you'll need to create a server-side service. To do this, open your application in your favorite IDE or editor.

Create a new browser-callable service in a package of your choice and name it `ReactiveService`. Add these two methods to it:

[source,java]
----
include::{root}{root-fix}/src/main/java/com/vaadin/demo/fusion/reactive/ReactiveService.java[tags=snippet,indent=0]
----
...
import { getClockCancellable } from 'Frontend/generated/ReactiveService';
...
The generated TypeScript service methods return a [classname]`Subscription<T>` object, which offers these methods:
