...
== Configuring Server Data & Service

When defining Bean objects in Java, apart from the type, you can define validation and error messages.

For the examples here, the code on the client side is based on the data and services defined in the following Java classes:
...
Before using form binders, you need to import your entity models and instantiate the binder. You also need to perform the service call to get the entity value, as explained in the articles <<binder#,Binding Data to Forms>> and <<binder-load#,Loading from and Saving to Business Objects>>.
...
async firstUpdated(arg: any) {
  ...
  this.binder.read(await MyService.getMyEntity());
}
----
endif::hilla-lit[]
...
Options for these components can be set by calling a server-side service that provides the list of strings. Since the call to the service is asynchronous, one easy way is to
ifdef::hilla-react[]
use the Typescript [methodname]`map` on the list received from the service.
endif::hilla-react[]
ifdef::hilla-lit[]
...
useEffect(() => {
  MyService.getMyOptions().then(opts => options.value = opts)
}, []);
...
return (
  <>
...
render() {
  return html`
  ...
    ${until(MyService.getMyOptions().then(opts => repeat(opts, (item) => html`
      <div>${item}</div>
    `)))}
  ...
  `;
...
...
useEffect(() => {
  MyService.getMyOptions().then(myEntities => myOptions.value = myEntities)
}, []);
...
return (
  <>
...
render() {
  return html`
    <vaadin-combo-box
      ...
      .items="${until(MyService.getMyOptions())}"
    ></vaadin-combo-box>
    ...
    >
      ${until(
        MyService.getMyOptions().then((opts) =>
          repeat(
            opts,
            (item) => html`
