...
See the <<{articles}/hilla/guides/forms/vaadin-components#, Binding Data to Hilla Components>> for more, related information.
...
Hilla automatically generates Model classes for server-side services from Java beans. There's usually no need to define models, manually.
...
The following TypeScript interfaces are generated to type-check the services:
...
ifdef::hilla-lit[]
  ...
  The service method to submit the form data to.
