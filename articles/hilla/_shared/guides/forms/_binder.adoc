...
The client-side [classname]`Binder` supports browser-callable Java services for loading and saving the form data, and reuses the metadata from Java Bean validation annotations for client-side validation.
...
For example, let us consider a browser-callable Java service with methods for loading and saving a [classname]`Person` bean:
