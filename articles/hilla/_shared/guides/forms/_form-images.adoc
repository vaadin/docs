...
It also assumes that the server exposes a browser-callable service for saving updated `Contact` instances:

[source,java]
----
include::{root}{root-fix}/src/main/java/com/vaadin/demo/fusion/forms/ContactService.java[]
----
...
export default function ContactForm() {
  ...
  useEffect(() => {
    ContactService.loadContact().then(form.read);
  }, []);
...
The [classname]`Contact` instance is first submitted to the [methodname]`saveContact()` service method. This happens through the
ifdef::hilla-react[]
...
Only then does the server-side service implementation receive the image string. The server can then choose to recode the image for more efficient storage, if necessary.

The advantage of using the string type is simplicity; you can use the built-in serialization mechanism of Hilla's form binder and services. This approach isn't suitable for larger files. A streamed upload may be more appropriate.
