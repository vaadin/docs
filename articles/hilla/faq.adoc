...
It seamlessly integrates a reactive TypeScript frontend -- https://reactjs.org/[React], or https://lit.dev/[Lit] in the deprecated Hilla Lit version -- with a https://spring.io/projects/spring-boot[Spring Boot] backend.
...
- Simplifies security with browser-callable services that are secured by default and validation that ensures data integrity across the server and client; and
...
Hilla isn't an alternative to React: it uses React for the frontend and puts the Java backend in the same project. Hilla includes everything you need to build an application in one package: UI components, routing, forms, security, build tooling. You can get first-party support for using Hilla from Vaadin, the company that built it.
...
Hilla builds on web standards: its UI components are web components, used from React.
...
Yes. You can use standard Maven modules in your application, or even external dependencies, as there is no need for the service and entity classes to be in the same project as the Hilla application. See <</hilla/reference/configuration#java-compiler-options,Configuration>> for details.
...
Yes. Hilla only requires a thin service layer to communicate with the frontend. The rest of the backend can employ a microservice setup with which the browser-callable services would communicate. See the https://vaadin.com/blog/using-microservices-from-hilla/["Using microservices from Hilla"] article for more details.
