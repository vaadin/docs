...
When generating the corresponding TypeScript code, Hilla maps the domain object to its raw value type. For example, if the domain object is a single-value object that wraps a string, the TypeScript type will be `string`.
