...
You can create a production build by running the following command on the command-line:
...
The production build command works out-of-the-box for starter projects, which come with the necessary Maven configuration. If you've manually created your project's [filename]`pom.xml` file, add the following `vaadin-maven-plugin` to enable production builds:
...
If you don't have the `build-frontend` goal in your `POM` file, add the `vaadin-maven-plugin` configuration shown above.
