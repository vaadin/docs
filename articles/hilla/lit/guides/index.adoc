...
Guides that don't depend on the frontend framework are documented only in the main <<{articles}/hilla/guides#, Hilla Guides>> section: <<{articles}/hilla/guides/client-caching#, Client-Side Data Caching>> and <<{articles}/hilla/guides/client-middleware#, Client Middleware>>. Upgrading is covered there as well, by the <<{articles}/hilla/guides/upgrading#, Upgrading Guide>>: its step from 24 to 25 applies to Lit applications, too, while the steps from earlier Hilla versions have parts that are specific to React.
