...
// Hilla views connect with browser-callable Java services that can provide business data and logic, and can connect further with backend services.
...
Guides that don't depend on the frontend framework are documented only in the main <<{articles}/hilla/guides#, Hilla Guides>> section: <<{articles}/hilla/guides/browser-callable-services#, Browser-Callable Services>>, <<{articles}/hilla/guides/client-caching#, Client-Side Data Caching>>, and <<{articles}/hilla/guides/client-middleware#, Client Middleware>>. Event handling is documented for React views in <<{articles}/hilla/guides/event-handling#, Event Handling>>, which also describes how the same events are bound in Lit templates. Upgrading and production deployment aren't specific to Hilla, either: they're documented for all of Vaadin in the <<{articles}/upgrading#, Upgrading Guide>> and in <<{articles}/flow/production#, Deploying to Production>>.
