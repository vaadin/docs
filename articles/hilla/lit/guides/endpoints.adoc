...
[NOTE]
====
The `'Frontend/'` path prefix is an alias for the `{project.basedir}/src/main/frontend` directory in your project. If you are using the default convention from before Vaadin 24.4, it refers to `{project.basedir}/frontend`.
...
Even though you can access any public method in any Hilla endpoint with the `\https://${base_url}/${prefix}/${endpoint_name}/${method_name}` URL format, don't use those URLs directly. Instead use the TypeScript methods.
...
All regular Hilla requests support some options as the last parameter.

=== Abort Signal

You can pass an https://developer.mozilla.org/en-US/docs/Web/API/AbortController/signal[AbortSignal] object to the call to cancel the request. This is useful when you want to cancel a request that's taking too long or is no longer needed.
...
By default, when a request takes too long, Hilla shows a progress bar to the user at the top of the page. You can suppress this behavior by setting the `mute` option to `true`.
