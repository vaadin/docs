...
== Implement stateful authentication for both Hilla and Flow

Vaadin applications, that have both Hilla and Flow views, can be configured for using stateful authentication, which basically requires <<spring-login#, steps for Hilla>> and <<{articles}/flow/security/enabling-security#, steps for Flow>>.
...
Here's a list of steps that explains this example:

- Add <<Dependencies, Spring Security dependency>>,
- Set up <<Server Configuration, Security Configuration>>,
- Implement <<Implement Client-Side Security, Client-Side Security>>, including user information endpoint, `useAuth` and `AuthProvider` objects and their use,
- Implement <<Create a Login View, Login View>>. Note that the browser page needs to be reloaded after log in and, if you want to exclude the `LoginView` from automatically generated menu, you need to set:
...
- Protect the views with login and roles:

Add the annotations to the server-side views, as described in <<{articles}/flow/security/enabling-security#annotating-the-view-classes, Annotating View Classes>>.
Add the `ViewConfig` object to the client-side views, as shown below:
...
// nav component inside <AppLayout>
<nav>
    {
...
Or add the menu items manually and specify the access options:

.`frontend/MainLayout.tsx`
[source,typescript]
----
include::{root}/frontend/demo/auth/MainLayout.tsx
----

- Add a custom configuration for routes (optional)

Routes configuration is usually present in `routes.tsx` file, which is generated by Vaadin and this should be enough for common cases:
...
Note that the client-side views are protected by default with `protect()` function.
If a custom routing is desired, the generated file [filename]`Frontend/generated/routes.tsx` should be copied to [filename]`Frontend/routes.tsx` and modified.

Examples:

You may want to change the login url:
...
Disable server-side views or add a fallback component with `withFallback` function, e.g. 404 page that will be shown, if no client-side view is found for a given url.
