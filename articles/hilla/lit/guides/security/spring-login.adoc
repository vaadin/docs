...
For this example, you would add <<Dependencies, Spring Security dependency>> and then set up <<Server Configuration, Security Configuration>>.

Next, implement <<Implement Client-Side Security, Client-Side Security>>, including user information endpoint, `useAuth` and `AuthProvider` objects and their use. Also, implement <<Create a Login View, Login View>>. 

Note that the browser page needs to be reloaded after log in and, if you want to exclude the `LoginView` from automatically generated menu, you need to set:
...
Then you can add a custom configuration for routes, but this is optional. Routes configuration is usually present in [filename]`routes.tsx` file, which is generated by Vaadin. This should be enough for common cases:
...
Note that the client-side views are protected by default with `protect()` function. If a custom routing is desired, the generated file [filename]`Frontend/generated/routes.tsx` should be copied to [filename]`Frontend/routes.tsx` and modified.
