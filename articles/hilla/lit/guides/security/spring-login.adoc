...
Vaadin applications, that have both Hilla and Flow views, can be configured for using stateful authentication, which basically combines <<spring-login#, steps for Hilla>> and <<{articles}/flow/security/enabling-security#, steps for Flow>>.
...
Here's a list of steps that summarise this example:

- Add <<Dependencies, Spring Security dependency>>,
- Set up <<Server Configuration, Security Configuration>>,
- Implement <<Implement Client-Side Security, Client-Side Security>>, including user information endpoint, `useAuth` and `AuthProvider`,
...
- Protect the views with log in and roles:
...
- Add a custom configuration for routes.

Routes configuration is usually present in `routes.tsx` file, which is generated by Vaadin:
...
Client-side views are protected by default with `protect()` function.
