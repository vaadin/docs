...
Example: you may want to change the log in url, add a specific React route object with `withFileRoutes` function, disable server-side views or add a fallback component with `withFallback`, e.g. 404 page that will be shown, if no client-side view is found for a given url.
