...
The [filename]`auth.ts` helpers from the same page store that user information on login, and provide the [methodname]`isUserInRole()` helper that the access checks below build on:
