...
Browser-callable access control can be considered basic security for signals, since it only allows limited control over the access to signals. However, there are situations that require finer control over signals. For example, you might want to allow anyone to subscribe to a signal, but only certain logged-in users with a specific role to update the value of that signal. This level of control is realized through adding operation validators to the signals.
...
- A `status` with two possible enumerated values (i.e., `ALLOWED` and `REJECTED`); and
- An optional `message` that can be used to provide a reason for the rejection.

The [classname]`ValidationResult` class provides two convenient static methods to create instances of the class:

`allow()`:: Creates an instance with the status set to `ALLOWED`.
`reject(String message)`:: Creates an instance with the status set to `REJECTED` and a message.
    ...
    .withOperationValidator(operation -> { // <2>
        if (operation instanceof ValueOperation<Double> valueOp) { // <3>
            if (Math.abs(valueOp.value()) != 1.0) { // <4>
                return ValidationResult.reject("Only up / down vote by 1 is allowed");
            }
        }
...
[NOTE]
When a validator is defined for a signal instance, it's called for all operations submitted to that signal. The validator should be able to handle all operations that the signal supports. In practice, this means that the [classname]`SignalOperation` parameter of the [methodname]`OperationValidator#validate` method should be checked for the operation type, and cannot be cast to one of operations that a signal supports with checking.
