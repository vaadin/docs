...
The client-side API of the [classname]`ListSignal` provides methods to insert and remove items. The [classname]`ListSignal` is a sequence of [classname]`ValueSignal` entries. Therefore, its API are about how the entries are added to or removed from the list, and how the concurrent operations regarding the structure of the entries is handled.
