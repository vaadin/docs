...
The client-side API of the [classname]`ListSignal` provides methods to insert and remove items. The [classname]`ListSignal` is a sequential organization of [classname]`ValueSignal` entries. Therefor, its API are about how the entries are added to or removed from the list, and how the concurrent operations regarding the structure of the entries is handled.
