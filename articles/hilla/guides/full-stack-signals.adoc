...
Since the `todoItems` signal holds the shared list of tasks, any subscribed client to this signal receives real-time updates when the list changes. As the result, when a client add a new task to the list, all other clients receive the update and the list is re-rendered to reflect the changes. The above example, however, doesn't demonstrate how to remove or update tasks in the list. This is covered in the next section.

[[list-signal-api]]
==== ListSignal API

The client-side APIs of the [classname]`ListSignal` provides methods to insert and remove items. The [classname]`ListSignal` is a sequential organization of [classname]`ValueSignal` entries. Therefor, its APIs are about how the entries are added to or removed from the list, and how the concurrent operations regarding the structure of the entries is handled.

As this is currently under active development, more methods and functionalities are added with each new release. The currently available ones are: [methodname]`inserLast` and [methodname]`remove`. These are described in the following sections.
...
The following example demonstrates how to create a to-do list view that enables concurrent users to add, remove, and update tasks in a shared list (no changes needed on the server-side):
...
As demonstrated in the above example, each entry in the [classname]`ListSignal<T>` is a [classname]`ValueSignal<T>` itself, meaning each value can be updated individually using the available APIs of the `ValueSignal`, and the changes to each individual entry are automatically propagated to all other clients that are subscribed to each entry of the [classname]`ListSignal`. This enables the React rendering process to only render the updated entry, instead of re-rendering the whole list.
