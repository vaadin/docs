...
The client-side APIs of the [classname]`ListSignal` provides methods to insert and remove items. The [classname]`ListSignal` is a sequence of [classname]`ValueSignal` entries. Therefore, its APIs are about how the entries are added to or removed from the list, and how the concurrent operations regarding the structure of the entries is handled.
