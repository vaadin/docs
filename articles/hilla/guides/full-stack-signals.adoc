...
<3> Create a list signal instance with an operation validator that rejects all removal operations. The other two signal instance remains unchanged. The new signal instance is stored in the `userTodoItems` field.
