...
As this is currently under active development, more signal types are added with each new release. Currently available ones are: [classname]`ValueSignal` and [classname]`NumberSignal`. These are described in the following sections.
...
[[list-signal]]
=== [since:com.vaadin:vaadin@V24.5]#ListSignal#

The [classname]`ListSignal<T>` is a full-stack signal that holds a list of values of an arbitrary type. The type should necessarily be a JSON-serializable one that is supported by the Hilla framework.
...
public class TodoService {
    ...
    private final ListSignal<Message> chatChannel =
                        new ListSignal<>(Message.class);

    @Nonnull
    public ListSignal<@Nonnull TodoItem> todoItems() {
        return todoItems;
    }
}
----

and on the client-side code:
...
The above example demonstrates a simple to-do list view that enables concurrent users to add tasks to a shared list. The `todoItems` signal holds the list of tasks, and the client-side code subscribes to this signal to receive real-time updates when the list changes.

The client-side API of the [classname]`ListSignal` provides methods to insert and remove items. In the following section, these APIs are described in detail.

[[list-signal-api]]
==== ListSignal API

As this is currently under active development, more methods and functionalities are added with each new release. The currently available methods are: [methodname]`inserLast` and [methodname]`remove`. These are described in the following sections.

`insertLast(value: T): Operation`:: Inserts a new value at the end of the list...
`remove(item: ValueSignal<T>): Operation`:: Removes the given item from the list...

The following example demonstrates how to create a to-do list view that enables concurrent users to add, remove, and update tasks in a shared list:
...
As demonstarted in the above example, each entry in the [classname]`ListSignal<T>` is a [classname]`ValueSignal<T>` itself, meaning each value can be updated individually, and the changes to that specific entry are automatically propagated to all other clients that are subscribed to that entry of the [classname]`ListSignal`. This enables the React rendering process to only render the updated entry, instead of re-rendering the whole list.
