...
Loading the [classname]`WebComponent` script can take some time, depending on the network, making it prudent to show a loading indicator.

It is possible to listen to the `onload` event for the [classname]`WebComponent` script so the loading element can be removed when the scrip is done loading.

The [methodname]`createWebComponent` takes in a onload callback function as the 3rd parameter.
Also an onerror callback can be set as the 4th parameter, but if not given there will be a error log into the console with the script tag to show which [classname]`WebComponent` script failed to load.
