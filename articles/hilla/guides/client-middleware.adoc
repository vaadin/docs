...
The middleware has access to the call context information, including the service and method names, the supplied parameters, and other client call options.
...
export const MyLogMiddleware: Middleware = async function(
  ...
  console.log(
    `Sending request to service: ${endpoint} ` +
    `method: ${method} ` +
...
export const MyApiDispatcherMiddleware: Middleware = async function(
  ...
  if (context.endpoint === 'ExternalService') {
...
export const MyStubMiddleware: Middleware = async function(
  ...
  if (context.endpoint === 'StubService') {
    //
    return new Response('{}');
  }
