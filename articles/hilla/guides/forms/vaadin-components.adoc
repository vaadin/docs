---
title: Binding Data to Input Fields
page-title: How to integrate Vaadin components into Hilla forms
description: Implementing data binding, type mapping and error messages when binding data with Hilla components.
meta-description: Learn how to enhance Hilla forms by integrating Vaadin components into them.
order: 100
---
:hilla-react:

:vaadin-custom-field: CustomField
:vaadin-text-field: TexField
:vaadin-number-field: NumberField
:vaadin-password-field: PasswordField
:vaadin-text-area: TextArea
:vaadin-checkbox: Checkbox
:vaadin-radio-button: RadioButton
:vaadin-select: Select
:vaadin-combo-box: ComboBox
:vaadin-radio-group: RadioGroup
:vaadin-list-box: ListBox
:vaadin-checkbox-group: CheckboxGroup
:vaadin-multi-select-combo-box: MultiSelectComboBox
:vaadin-date-picker: DatePicker
:vaadin-time-picker: TimePicker
:vaadin-date-time-picker: vaadin-date-time-picker

= [since:dev.hilla:hilla@v2.2]#Binding Data to Input Fields#

include::{root}/articles/hilla/lit/guides/forms/vaadin-components.adoc[tag=content]
