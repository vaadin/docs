...
Modify the Spring Security configuration and use the [methodname]`VaadinStatelessSecurityConfigurer` to set up stateless authentication, as follows:
...
public class SecurityConfig {
    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.with(new VaadinStatelessSecurityConfigurer<>(),
            ...
            .expiresIn(3600) // The JWT lifetime in seconds
        );
        return http.build();
    }
}
