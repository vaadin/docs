...
This token is then included in subsequent service calls.
...
When the server receives an HTTP request targeting a browser-callable service, Hilla first checks whether the accessing user is authorized to invoke that service.
It does this based on the standard role-based access control scheme.
The application developer can specify role-based access rules as annotations for the service class or its individual methods.
For example:
...
In the above example, all methods in the [classname]`MyService` class are denied by default.
...
Services can also be exposed for unauthenticated requests using the `@AnonymousAllowed` annotation.
See <<configuring#, Configuring Security>> for more details and code examples on service security.


== Security Considerations

=== Application Security is Service Security
...
This means that validation and sanitation of data received by browser-callable services should always occur on the server, regardless of the client-side logic.
...
The CSRF token is included in each service call and validated as matching by the server.
