...
The connection state determines whether a user's avatar is shown.
Activation of the connections is handled by a connection context.
By default, [classname]#ComponentConnectionContext# activates the connections when the connected UI components are attached to the UI, and inactivates when they are detached.
