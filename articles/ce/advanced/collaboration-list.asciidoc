...
The `ListInsertOperation` class allows you to prepare a list insert that can then be applied to the list using the `CollaborationList::insert` method.
...
.Static methods of `ListInsertOperation`
...
.Conditional methods of `ListInsertOperation`
