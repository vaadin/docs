...
[IMPORTANT]
Collaboration Engine is completely free for development purposes. However, developers without a commercial Vaadin subscription (Pro, Prime or Enterprise) are limited to enabling real-time collaboration features for no more than 10 end users per month.
Learn how to <<going-to-production#, take an app that uses Collaboration Engine into production>>.
