...
image::images/collaboration-messages-example.png["A Chat with three messages, as well as controls for typing in new messages", width=399]

They enhance the regular `MessageList` and `MessageInput` components by automatically synchronizing the messages for all users connected to the same topic.
The message list only renders the submitted messages, while the message input provides controls for submitting new messages.

The following example shows how to set them up:
...
Please refer to <<{articles}/ds/components/messages#theming, Theming>> in Messages documentation for details on how to style messages based on the theme.
