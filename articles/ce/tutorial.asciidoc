...
Collaboration Engine does not behave correctly if you explicitely configure push to use `PushMode.MANUAL`, as Collaboraiton Engine does not invoke `UI.push()`, and the user interactions would therefore not be pushed out to the other connected users.
