...
`CollaborationMessageList` enables changing the properties of `MessageListItem` after they are generated.
This is done by providing a `MessageConfigurator`.

Here is an example of how to use a message configurator to add a custom style to the current user's messages.
Refer to <<{articles}/ds/components/messages#theming, messages theming documentation>> for details on how style messages based on the theme.
[source,java]
----
...
The message configurator can also be used to change the message's text, for example censoring certain words in messages.
[source,java]
----
