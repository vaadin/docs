...
It's important to note when implementing the `fetchMessages` method, that the returned
stream should include the message(s) sent exactly at the `FetchQuery::getSince` timestamp.
This ensures that all messages are included even when multiple messages have been sent at the same millisecond.
