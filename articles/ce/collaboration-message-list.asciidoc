...
You can set the message persister for a message list as an optional
parameter when providing a topic id; either in the constructor or in `setTopic`:
...
[source,java]
----
include::{root}/src/main/java/com/vaadin/demo/ce/MessageListDocumentation.java[tags=message-list-set-persister,indent=0]
----
