...
Collaboration Engine includes a manager to handle text messages in a topic, for example subscribing to incoming messages and submit new ones programmatically.
...
NOTE: If a [classname]#MessageManager# instance is using the same topic id of a xref:{articles}/ce/components/collaboration-message-list#ce.collaboration-message-list[`CollaborationMessageList`], messages submitted with the manager will also appear on the list.
