...
Collaboration Engine includes a manager to handle text messages in a topic, e.g. subscribing to incoming messages and submit new ones.
The manager can also be configured to persist messages to a custom backend (e.g. a database) so that messages are restored when the application restarts.
