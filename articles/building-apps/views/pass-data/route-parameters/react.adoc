...
Wildcard route parameters are used to match any number of URL segments. To create a view that accepts a wildcard route parameter, use the  character followed by the parameter name when naming the view file. For example, to create a view that accepts a wildcard `path` parameter, name the view `*.tsx`. The directory structure under the `views` looks like this:
