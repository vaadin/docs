...
The example code in this guide does not look like the mock-ups above. It uses simple components and focuses only on the UI pattern: how to structure the layout, represent selection in the URL, and synchronize the two. It does not define data models, persistence, or form/grid configuration. Substitute your own components where indicated.
