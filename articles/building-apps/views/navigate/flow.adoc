...
Open [classname]`GreetingView` and update its route to `say/hello/to/vaadin`. If followed the <<../add-view/flow#try-it,adding views>> tutorial, you've already done this.
...
Now, you'll create a new view that provides multiple ways to navigate to the greeting view. In the [packagename]`com.example.application.greeting.ui.view` package, create a class called `LinksView`:
...
A [classname]`RouterLink` creates a clickable link to another view. Modify [classname]`LinksView` to include a link to the [classname]`GreetingView`:
...
Hover on the "Greetings" link to see that it points to `\http://localhost:8080/say/hello/to/vaadin`. Click the link to navigate to the greeting view, then use the *browser's back button* to return.


=== Navigate Programmatically

Next, you'll add a button that navigates to the greeting view when clicked. Modify [classname]`LinksView` to include a [classname]`Button`:
...
Switch back to the browser. Thanks to *hotswap*, the new [guibutton]*Greetings* button should appear automatically. Click it to navigate to the greeting view.


=== Create an API

To make navigation more reusable and readable, you'll now create a dedicated method for navigating to the greeting view. 

Open [classname]`GreetingsView` and add this method:
