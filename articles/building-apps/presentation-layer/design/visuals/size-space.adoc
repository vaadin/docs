...
Lumo provides a solid foundation with a default base size of 16 pixels. The font size of the `<html>` element is crucial because all sizes and spaces in Vaadin components are relative to the root element's font size. Lumo uses `rem` units, which are multipliers of the root font size.
