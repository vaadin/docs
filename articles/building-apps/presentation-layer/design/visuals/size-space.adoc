...
You can customize these adjustments separately for touch devices and various screen resolutions to ensure an optimal user experience across all displays. It’s recommended to control sizing, spacing, and font sizes through CSS custom properties. The example below demonstrates how to increase element sizes on touch devices while keeping the font size unchanged.
...
When reducing font sizes, it's essential to keep smaller text elements, such as those in badges, legible. A minimum recommended size for readability is 12 pixels. For guidance on adjusting the scaling ratio between font sizes, refer to the <<{articles}/building-apps/presentation-layer/design/visuals/typography#type-scale,type scale>>.
