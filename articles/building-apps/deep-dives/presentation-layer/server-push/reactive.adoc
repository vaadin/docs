...
When building the user interface with either Vaadin Flow or Hilla, you can use reactive streams to allow a background job to update the user interface. This is covered in the <<{articles}/building-apps/deep-dives/application-layer/background-jobs/interaction/reactive#,Producing Reactive Streams>> documentation page.
