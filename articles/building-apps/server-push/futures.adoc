...
If a `CompletableFuture` completes successfully, you can instruct it to perform a specific operation by calling the `thenAccept()` method on it. This method takes a `Consumer` as its input. When the `CompletableFuture` completes, it passes the result to this consumer.
...
Flow has no version of `UI.accessLater()` that works with `Function`. However, since you are probably not interested in returning a result, you can create a helper function that adapts a `Consumer` to a `Function`, such as this:
