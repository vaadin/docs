...
Changes and JavaScript invocations that are scheduled for a UI are kept in memory until a response is written for its client, which needs either a request from the client or an open push connection. A UI whose client has stopped receiving responses -- a laptop that went to sleep, a closed push connection, a discarded browser tab -- therefore accumulates everything a background job keeps scheduling for it, until the session expires. Invocations scheduled for a component that is detached or invisible are held in the same way, until it's attached or visible again.

Flow logs a warning, once per UI, when 1,000 JavaScript invocations are waiting to be sent. The warning names the component and the expression of the most recent invocation and explains why the invocations are being held. To also log the call site of the invocation that triggered it, enable debug logging for `com.vaadin.flow.component.internal.PendingJavaScriptInvocationUtil`; turning that logger off silences the warning.

Keep invocations from piling up in one of three ways:

- If only the latest value is relevant for the client -- a progress value or the current time, for example -- set an element property instead of calling [methodname]`executeJs()`. Only the last value of a property is sent.
- Keep the [classname]`PendingJavaScriptResult` that [methodname]`executeJs()` returns and call [methodname]`cancelExecution()` on it before scheduling the next invocation.
- Stop scheduling updates while the client isn't receiving them. [methodname]`UI.getLastUpdateSentTimestamp()` tells when the updates pending for the UI were last written towards the client, and can be called from a background thread without holding the session lock, so the job can decide whether to update the UI before acquiring it:
...
The timestamp says when the updates were written towards the client, not that the client received them. For a UI that hasn't written a response yet, it's the time the UI was created.
