...
meta-description: Learn how to use virtual threads in a Vaadin Flow user interface by reading this guide.
order: 10
---
...
If you have experience with Swing, you might be tempted to use a `Timer`, or to start a new `Thread`, manually. In Flow, this is not a good idea. Flow applications are multi-user applications, with potentially thousands of concurrent users. If each user creates their own `Timer`, or starts their own `Thread`, you may run out of threads. If that happens, the application crashes.
...
This is the easiest way of starting a new user interface thread. If you are able to use virtual threads, they should be your first choice. If you run into problems, though, switch to the `TaskExecutor`.
...
Before starting a task with `TaskExecutor` and `TaskScheduler`, you should make sure it is actually UI-related and not a background job. To use them properly, you would inject them into your view, and then call them when needed.
...
[CAUTION]
Do not use the `@Async` annotation in Flow views. It turns them into proxies that do not work with Vaadin.
...
[CAUTION]
Do not use the `@Scheduled` annotation in Flow views. It turns them into proxies that do not work with Vaadin.
