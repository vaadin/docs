...
`@NpmPackage` declares which npm packages a component needs, but it doesn't decide the version that a transitive dependency of those packages resolves to. An add-on can pin versions for the whole application by shipping one or more versions files in:

----
src/main/resources/META-INF/VAADIN/versions/
----

Every versions file on the classpath is read and merged, so the platform, the component integrations, and your own add-on each pin the packages they care about. A file groups its entries under a name of your choice, and each entry gives the npm package name and the version to pin it to:
...
An entry can also name the packages that the pinned one replaces, which are then left out of the installation:
...
----
{
  "my-components": {
    "status-badge": {
      "npmName": "@example/status-badge",
      "jsVersion": "2.1.0",
      "exclusions": ["@example/legacy-status-badge"]
    }
  }
}
----

A version that a scanned `@NpmPackage` annotation declares, or that the application has set in its own [filename]`package.json`, takes precedence over a pinned one, so an application can still override what an add-on pins. If two versions files pin different versions of the same package, the newest one is used and a warning is logged.
