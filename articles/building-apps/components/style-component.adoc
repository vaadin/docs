...
The lazy approach is useful for components with significant CSS that isn't needed on every page. For small, often-used components, importing from `styles.css` is simpler. Either way, scope rules to the component's class name so they don't leak. See <<{articles}/styling/stylesheets#,Stylesheets>> for more details.
