...
meta-description: Style custom Vaadin components with CSS class names, theme properties, utility classes, theme variants, and custom CSS properties.
order: 20
---
...
For quick styling without writing CSS, you can apply utility classes from Java.

If your application uses the *Lumo* theme, use the `LumoUtility` constants:
...
If your application uses the *Aura* theme, use https://tailwindcss.com/[Tailwind CSS] classes instead — Lumo utility classes don't work with Aura:

[source,java]
----
public class InfoCard extends Composite<VerticalLayout> {
    public InfoCard(String title, String description) {
        addClassNames("bg-white", "rounded-lg", "p-4", "shadow-sm");
    }
}
----

[NOTE]
Both Lumo utility classes and Tailwind CSS must be explicitly enabled before use. See <</styling/utility-classes#,Utility Classes>> for setup instructions.

Utility classes are convenient for prototyping and one-off components. For reusable components with complex styling, a dedicated CSS class is usually cleaner.
...
*Avoid inline styles for reusable components.* `getStyle().set(...)` is fine for one-off adjustments, but reusable components should use CSS classes. Inline styles are harder to override and maintain.
