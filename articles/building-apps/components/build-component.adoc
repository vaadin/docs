...
Creating components is primarily about structuring code, not reuse. Even one-off components (e.g., a detail panel used only inside one view) are worth extracting — they keep each class focused, give the section a name, and define a clear boundary for its behavior. See <<../ui-basics/compose#,Compose with Components>> for more on decomposition.


== Choosing a Base Class

There are two main approaches: extending an existing component, and using `Composite`.
...
This is straightforward, but every public method of the parent (like `add()`, `removeAll()`, `setText()`) is available to callers. That means external code can modify the component's internal structure. Be especially cautious about extending layout components like `VerticalLayout` — you inherit all layout methods, and callers can rearrange your internals.
