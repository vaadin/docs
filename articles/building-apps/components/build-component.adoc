...
When the teardown only undoes the setup, you can [since:com.vaadin:vaadin@V25.3]#keep both in one place with `whenAttached()`#. The handler runs on every attach, and the `Registration` it returns runs on the matching detach:

[source,java]
----
public StatusBadge(Broadcaster broadcaster) {
    whenAttached(ui -> {
        Consumer<Status> listener = this::refresh;
        broadcaster.subscribe(listener);

        // Returned to whenAttached(), run when the component is detached
        return () -> broadcaster.unsubscribe(listener);
    });
}
----
