...
One-off components (e.g., a detail panel used only inside one view) are also worth extracting. They keep each class focused even if they aren't reused. See <<../ui-basics/compose#,Compose with Components>> for more on decomposition.


== Choosing a Base Class

Vaadin offers three main approaches: extending an HTML element, extending a Vaadin component, and using `Composite`.

=== Extending an HTML Element

Extend `Div`, `Span`, or another HTML element class when your component is a simple container:
...
This is straightforward, but every public method of `Div` (like `add()`, `removeAll()`, `setText()`) is available to callers. That means external code can modify the component's internal structure.

=== Extending a Vaadin Component

Extend a Vaadin component like `VerticalLayout` or `Button` when you want to specialize its behavior:
...
This works well for small specializations. Be cautious about extending layout components like `VerticalLayout` — you inherit all layout methods, and callers can rearrange your internals.
...
Events are the right choice for reusable components. They're type-safe and follow the standard Vaadin pattern. See <</flow/component-internals/events#,Events>> for the full reference.

=== Callbacks

Use callbacks (`Runnable`, `Consumer<T>`) for one-off components where defining an event class would be overkill:
...
*Don't put business logic in components.* Components handle UI concerns — displaying data, collecting input, and forwarding user actions. Business logic (validation rules, calculations, service calls) belongs in <<../business-logic/add-service#,services>> or the view that coordinates them.

*Use constructor parameters for required data; setters for optional data.* If a component can't function without certain data, require it in the constructor. Use setters for data that can change after creation or that has sensible defaults.
