...
This article shows how to wrap a plain JavaScript library — one that doesn't expose a web component or React component — as a Vaadin component. The approach uses `@JsModule` to load a small connector file that initializes the library on a DOM element, and the Element API, `callJsFunction` and `@ClientCallable`, to communicate between server and client. For libraries that offer a custom element, see <<wrap-web-component#,Wrap a Web Component>>. For React components, see <<wrap-react-component#,Wrap a React Component>>.
...
DOM events are simpler for most cases. Use `@ClientCallable` when you need to send structured data or trigger server logic that does not map to an event. See <</flow/component-internals/client-server-rpc#,Remote Procedure Calls>> for the full reference.
