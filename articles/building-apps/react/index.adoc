...
[[enabling]]
== Enabling React Views in a Project
...
The version comes from the `vaadin-bom` dependency management that the project already has, and no other build configuration is needed. Without it, files under [filename]`src/main/frontend/views` are ignored, and no TypeScript clients are generated for services. The <<{articles}/hilla/guides/browser-callable-services#enabling,Browser-Callable Services reference guide>> describes the same dependency from the service side.
