...
Leave the restricted parts out of the returned markup, as in the example above. Don't render them and then hide them with CSS, such as `display: none` or `visibility: hidden`. The content is then still in the DOM, where anyone can reveal it with the browser's developer tools. The same goes for disabling a control through styling instead of the `disabled` property. Either way, the server is what enforces the rules, so protect the services as well.
