...
There's no shortcut for mapping multiple routes to the same view, but you can create an additional view file that returns the target component. For example, the following view targets the root route (`/`):
...
To make the same view available at `/home` and `/main`, create two more view files that return it:
