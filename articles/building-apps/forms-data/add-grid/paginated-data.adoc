...
For a more detailed walk-through of the example code, continue reading below.
...
public interface ItemRepository<Item, Long> extends PagingAndSortingRepository<Item, Long> {
...
[NOTE]
Yes, if you are not using <<../../security/protect-services#,method security>> or any other service layer logic.
Otherwise, it is better to have an <<../../business-logic/add-service#,application service>> between the UI and the repository.
