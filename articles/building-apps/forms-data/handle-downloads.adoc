...
To let users download the contents of a <</components/grid#,Grid>>, generate the file from the grid's items when the download starts. The following example exports the selected rows as CSV, or all rows if nothing is selected. It uses a lambda that receives a `DownloadEvent` and writes directly to the response output stream:

[source,java]
----
grid.setSelectionMode(Grid.SelectionMode.MULTI);

Anchor exportLink = new Anchor(event -> {
    Set<Person> selected = grid.getSelectedItems();
    Stream<Person> people = selected.isEmpty()
            ? grid.getGenericDataView().getItems() // <1>
            : selected.stream();

    event.setFileName("people.csv");
    ...
    try (var writer = new PrintWriter(new OutputStreamWriter(
        ...
        people.forEach(person -> writer.println(String.join(",",
                toCsvValue(person.getFirstName()),
                toCsvValue(person.getLastName()),
                toCsvValue(person.getEmail()))));
    }
}, "Export as CSV");
...
private static String toCsvValue(String value) {
    ...
    // Quote values that contain separators, quotes, or line breaks
...
----
<1> Returns the items in the order they're shown in the grid, including the current sorting and filtering. With a lazy data provider, this fetches all items from the backend, so for large data sets, query the service directly instead.

For more complex files, consider using a CSV or spreadsheet library.
