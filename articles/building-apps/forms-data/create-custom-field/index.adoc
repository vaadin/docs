...
page-title: Create a Custom Field in Vaadin
description: Learn how to create a Custom Field in Vaadin.
meta-description: Guide on how to create a Custom Field in Vaadin.
order: 6
---
...
== Custom Fields in Vaadin

A Custom Field is useful when you want multiple components to behave as a single form field,
or when you have a component that does not support integration with Binder, labels, or helper text.
...
This means you *usually* do not need to add additional listeners to synchronize the field value.
You must ensure that `generateModelValue()` correctly gathers all relevant changes
to compute the field's value.

[WARNING]
Be mindful that automatic tracking of value changes does not apply to all components, for example, `Grid` only provides selection change events.
...
Unlike Vaadin components, these elements do not automatically handle states or styles.
...
For example, if `invalid` state is irrelevant to your custom field, you do not need to add logic for handling that state.
...
Binding a Custom Field works the same way as with built-in components such as `TextField` or `ComboBox`. If you are familiar with <<{articles}/building-apps/forms-data/add-form/fields-and-binding#,binding>> and <<{articles}/building-apps/forms-data/add-form/validation#,form validation>>, the process should feel identical.
...
[IMPORTANT]
If you are extending `AbstractField`, you must implement the `HasValidationProperties` interface and provide elements that use the `invalid` and `errorMessage` properties.
...
* It is recommended that you limit internal validation to built-in validators in Vaadin components.
...
and your own internal elements, it is important to know how to target both effectively.
...
.Lumo theme used in examples below
[NOTE]
The code examples below use component style variants and style properties of the <<{articles}/styling/themes/lumo#,Lumo theme>>. If you are not using Lumo, you need to use other alternatives.
...
[IMPORTANT]
When using `HasStyle` API, prefer <<{articles}/styling/themes/lumo/lumo-style-properties#,Lumo Style Properties>> when working with the Lumo theme, or use your own CSS properties. For example use `"var(--lumo-space-xs)"` instead of `"4px"`. This ensures consistency across your application and makes it easier to adjust global styling later.
...
The exact code for this in `DateTimePicker` is a bit too complex to show as an example, since it is based on a web-component.
...
For single-input cases, you typically do not need to create an additional label.
However, the built-in label should be associated with the input element.

By default, this association does not exist because `CustomField` does not know
...
This step-by-step example shows the creation of a duration field.
...
If you test the component by entering some valid values in the input fields, you find that it works functionally but does not look polished.
...
This Custom Field does not require extensive custom styling.
<<{articles}/styling/themes/lumo/utility-classes#,Lumo Utility Classes>> can be used to quickly address the spacing issues.
...
Here is the updated version of the DurationField:
...
DTO classes are usually separate from the UI code. Place it under `src/main/java/<your package>/data`, for example `src/main/java/com/mydomain/myproject/ui/component`.
...
Here is the updated DurationField with validation included:
...
A common approach used by other Vaadin components is to create a dedicated class that contains all translatable strings.

Start by only including the texts for the components added
and which do not already have a public API for updating them
...
Here is the updated `DurationField` with localization support:
...
Finally, address the accessibility requirements of this Custom Field.
...
You will create a method that does all that on JS side, since it helps to avoid generating separate unique ids for the inputs.
It is not pretty, but it takes care of everything needed.
...
Here is the updated DurationField with accessibility support included:
...
By following these steps, you have seen how to move from a minimal implementation to a robust, production-ready custom field.
