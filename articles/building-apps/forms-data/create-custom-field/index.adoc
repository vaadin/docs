...
The following example shows text as a plain label that turns into a `TextField` when the user double-clicks it, or focuses it and presses Enter. When the text field loses focus, the label shows the new value again. Because the component is a Custom Field, you can bind it with `Binder` like any other field.

```java
public class EditableLabel extends CustomField<String> {
    private final Span label = new Span();
    private final TextField textField = new TextField();

    public EditableLabel() {
        label.getElement().setAttribute("tabindex", "0"); // <1>
        label.getElement().addEventListener("dblclick", e -> startEditing());
        label.getElement().addEventListener("keydown", e -> startEditing())
                .setFilter("event.key === 'Enter'");

        textField.setVisible(false);
        textField.addBlurListener(e -> stopEditing());
...
```
<1> Makes the label reachable with the keyboard.

An empty label has no size, so the user can't double-click it. If the value can be empty, give the label a minimum width or show placeholder text in it.

If the text should be editable at all times, use a regular `TextField` instead of a `contenteditable` element. A text field already supports `Binder`, validation, and accessibility.
...
[NOTE]
The code examples below use component style variants and style properties of the <<{articles}/styling/themes/lumo#,Lumo theme>>. If you're not using Lumo, you'll need to use other alternatives.
