...
First, lets generate a unique ID to set on our custom labels. You might have your own way of generating unique ids, but here
we will rely on `RandomGenerator`. We need to use those to indicate that our input fields are labeled by them.

```java
private static final int ID_GEN_CHAR_COUNT = 5;
private final RandomGenerator rand = RandomGenerator.getDefault();
// ...
public DurationField(DurationFieldI18n i18n) {
    this.i18n = i18n;

    var hoursId = randomId("hours", ID_GEN_CHAR_COUNT);
    var minutesId = randomId("minutes", ID_GEN_CHAR_COUNT);

    hoursLabel = createHoursLabel(hoursId);
    minutesLabel = createMinutesLabel(minutesId);
    hours = createHoursField(hoursId);
    minutes = createMinutesField(minutesId);
    //...
}

private NativeLabel createHoursLabel(String id) {
    var label = new NativeLabel();
    label.setId(id);
    //...
    return label;
...
private NativeLabel createMinutesLabel(String id) {
    var label = new NativeLabel();
    label.setId(id);
    //...
    return label;
...
private IntegerField createHoursField(String ariaLabelledBy) {
    var hours = new IntegerField();
    hours.setAriaLabelledBy(ariaLabelledBy);
    //...
    return hours;
}

private IntegerField createMinutesField(String ariaLabelledBy) {
    var minutes = new IntegerField();
    minutes.setAriaLabelledBy(ariaLabelledBy);
    //...
    return minutes;
}
...
Next, we need to fix the `IntegerField`
