...
There are several approaches available, depending on how much customization is needed. The simplest solution will extend `CustomField` class, while a more complex solution could require extending `AbstractField` or `AbstractSinglePropertyField`.
...
In the following example, we create a custom field that has a "Roll" button and a disabled input element that displays the field value. Every time the "Roll" button is clicked, it generates a new value.
...
public class SingleDiceCustomField extends CustomField<Integer> {
    ...
    protected Integer generateModelValue() {
        ...
        // Verify it is a number, before parsing it to a number
    ...
    protected void setPresentationValue(Integer value) {
        input.setValue(String.valueOf(value)); // add value to input
    }

    private Input createInput() {
        var input = new Input();
        input.setEnabled(false); // Do not allow custom input

        return input;
    }

    private NativeButton createButton() {
        var button = new NativeButton("Roll");
        button.addClickListener(clickEvent -> {
            // Generate a random number, set as value
...
.Best practice
[IMPORTANT]
Use both `vaadin-custom-field` and the custom class in your selector to prevent accidental styling of unrelated components.
