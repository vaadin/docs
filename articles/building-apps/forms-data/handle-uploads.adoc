...
== Showing Uploaded CSV Data in a Grid

Because the success callback can update the UI, you can also show the contents of an uploaded file right away, for example, to let the user review imported data before saving it. The following example reads a CSV file with a header row into a <</components/grid#,Grid>>. Each row is a `String[]`, and the grid gets one column for each header value:
...
Upload upload = new Upload(UploadHandler.inMemory(
        (metadata, data) -> {
            List<String[]> rows = new String(data, StandardCharsets.UTF_8)
                    .lines()
            ...
            for (int i = 0; i < headers.length; i++) {
                int index = i;
                grid.addColumn(row -> index < row.length ? row[index] : "")
                        .setHeader(headers[i]);
            }
            grid.setItems(rows.subList(1, rows.size()));
        }));
upload.setAcceptedFileTypes("text/csv", ".csv");
----

Splitting lines by commas doesn't handle quoted values that contain commas or line breaks. For real-world CSV files, parse the data with a CSV library.
