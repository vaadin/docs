...
[NOTE]
This page assumes you have already <<add-spring-data#,added Spring Data JPA>> to your project and are familiar with JPA concepts. If you are new to JPA, read the https://spring.io/guides/gs/accessing-data-jpa[Accessing Data with JPA] guide first.
...
Spring Data checks the `@Id` field to decide whether an entity is new. If the ID is `null`, the entity is new; otherwise, it is persistent.

[NOTE]
If you assign IDs before persisting, implement the Spring Data `Persistable` interface.

Do not use Spring Data's `AbstractPersistable` base class. Declare the `@Id` field directly in each entity, or create your own base class. This gives you better control over ID generation.
...
----
<1> Use `ProxyUtils.getUserClass` because Hibernate may return proxied entities whose classes do not match directly.
...
This is cleaner but makes non-equality queries harder—`LIKE` queries require strings, not domain primitives. Attribute converters also do not work with primary keys.

Use converters for domain primitives that are only queried by equality and are not identifiers.
...
Once the transaction completes, entities become detached. Further changes are not persisted without an explicit `save`.
...
[CAUTION]
Do not modify managed entities if you do not intend to save the changes. Roll back the transaction to discard modifications.
