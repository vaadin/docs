...
This separation sometimes goes by the name CQRS (Command Query Responsibility Segregation), though you don't need to adopt the full pattern to benefit from the idea. Both the <<jpa#query-classes,JPA>> and <<jooq#query-classes,jOOQ>> documentation pages show how to implement this with dedicated query classes.
...
See <<jpa#,JPA & Spring Data>> for implementation details.
