...
.Disable Hibernate's Automatic DDL Generation
[%collapsible]
====
Open `src/main/resources/application.properties` and remove the line that sets `spring.jpa.hibernate.ddl-auto` to `update`.
====
...
[%collapsible]
====
Run the `TaskServiceTest` integration test in your IDE. The test should pass, and in the log, you should see messages like the following:
