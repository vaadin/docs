...
----
<1> You can check for the latest version on the https://central.sonatype.com/artifact/org.testcontainers/testcontainers-jooq-codegen-maven-plugin[Maven Central Repository].
...
This starter automatically configures jOOQ to use the primary data source of your application. It also configures jOOQ to work with the Spring transaction manager. This means that you can control database transactions in the same way regardless of whether you're using jOOQ or <<jpa#,JPA>>. For more information about managing transactions in Vaadin applications, see the <</building-apps/forms-data/consistency/transactions#,Transactions>> documentation page.
...
If you have <</building-apps/forms-data/consistency/domain-primitives#,domain primitives>> in your entities, you can handle them in different ways, depending on how you choose to map your database records to your entities.
...
jOOQ supports both <</building-apps/forms-data/consistency/optimistic-locking#,optimistic locking>> and <</building-apps/forms-data/consistency/pessimistic-locking#,pessimistic locking>>. These are explained in the following sub-sections.
