...
.Create a Testcontainers Configuration
[%collapsible]
====
In `src/test/java/[application package]`, create a new class `TestcontainersConfiguration`:
...
[%collapsible]
====
In `src/test/java/[application package]`, create a new class `TestApplication`:
...
Next, run the test application's `main()` method. The application should start up, using a PostgreSQL database managed by Testcontainers.
...
[%collapsible]
====
Open `TaskServiceTest` in your IDE and import the new `TestcontainersConfiguration`:
...
Run the test in your IDE. Everything should work as before.
====
