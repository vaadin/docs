...
In the following example, `myFirstMethod()` executes inside its own transaction, regardless of whether it is called directly by a client or by `mySecondMethod()`.
...
You do not have to do anything special to commit a transaction. Spring commits the transaction after the callback returns, unless it has been marked for rollback.
