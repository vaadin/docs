...
meta-description: Learn about ACID transactions from things such as deadlocks to overall management.
order: 10
---
...
Consistency means that the database is in a valid state after each committed transaction. All integrity constraints -- such as unique keys, foreign keys, and check constraints -- are met after each transaction.
...
Check the documentation for your database if you are not familiar with how it handles transaction isolation.
...
`MANDATORY`:: If there is an active transaction, Spring executes the method inside it. Otherwise, Spring throws an exception and does not execute the method.
...
`NOT_SUPPORTED`:: When there is an active transaction, Spring suspends it. The method is then executed without a transaction. Once the method has completed, Spring resumes the earlier one.
