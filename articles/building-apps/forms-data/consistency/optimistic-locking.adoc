...
Both <</building-apps/forms-data/repositories/jpa#,JPA>> and <</building-apps/forms-data/repositories/jooq#,jOOQ>> have built-in support for optimistic locking. It's easy to implement them. However, you'll need both the primary key and the version number whenever you make an update.
