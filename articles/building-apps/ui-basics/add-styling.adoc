...
The `var(--lumo-...)` values are theme custom properties — named values defined by the theme for colors, spacing, typography, and other styles. Using them instead of hardcoded values keeps your styles consistent and makes them adapt to dark mode automatically. Both the <<{articles}/styling/themes/lumo/lumo-style-properties#,Lumo>> and <<{articles}/styling/themes/aura#,Aura>> themes provide custom properties — see their reference pages for the full list.
