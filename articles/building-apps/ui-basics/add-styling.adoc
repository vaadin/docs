...
Values like `var(--lumo-primary-color)` are theme custom properties — named constants provided by the Lumo theme. Using them instead of hardcoded values keeps your styles consistent and makes them adapt to dark mode automatically.
...
Then write the matching CSS rules in your `styles.css` file:
...
The `var(--lumo-...)` values are theme custom properties. They work like constants — using them instead of hardcoded pixel values and colors keeps your styles consistent with the theme and makes them adapt to dark mode automatically. See the <<{articles}/styling/themes/lumo#,Lumo>> reference for the full list.
...
Start with the simplest approach that meets your needs:

*Theme variants* — The first thing to try. If a component has a built-in variant that matches what you need, it's one line of Java and nothing else. Button styles, notification colors, field sizes.

*Inline styles* — Good for one-off adjustments and dynamic styles that change at runtime based on business logic. Conditional coloring, runtime-calculated sizes. Pure Java, no stylesheet needed.

*CSS class names* — Use when you need reusable styles, want to style multiple elements consistently, or need more control than inline styles offer. Custom cards, branded elements, view-specific layouts.
