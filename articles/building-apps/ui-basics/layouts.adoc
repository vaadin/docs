...
[source,java]
----
header.setAlignItems(FlexComponent.Alignment.CENTER);          // center items vertically
footer.setJustifyContentMode(FlexComponent.JustifyContentMode.END); // push items to the right
----

image::images/align-items.svg[Diagram showing START, CENTER, and END alignment of items in a HorizontalLayout]

[NOTE]
Justification only has a visible effect when the layout is larger than its content along the main axis -- so you usually pair `setJustifyContentMode()` with `setWidthFull()` or an explicit width. For the full list of alignment values, see the component reference.
