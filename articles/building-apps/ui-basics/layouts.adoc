...
[source,java]
----
bar.expand(searchField);          // searchField grows; the button keeps its size
...
[NOTE]
Vaadin includes an experimental `layoutComponentImprovements` feature flag (Flow only) that makes `setWidthFull()` / `setHeightFull()` / `setSizeFull()` apply `flex: 1` instead of a literal percentage, avoiding the shrinking trap. It's *off by default* -- don't assume the behavior unless it's been enabled.


[#overflow-and-scrolling]
== Overflow and Scrolling

A flex child won't shrink below its content size by default, so it can overflow and produce unwanted scrollbars. Fix it by setting the minimum size to `0` on the overflowing component:

[source,java]
----
overflowingComponent.setMinHeight("0"); // or setMinWidth("0")
----

`HorizontalLayout` can also wrap onto multiple lines with `setWrap(true)`.

When a region needs to scroll, wrap it in a <</components/scroller#,Scroller>> rather than setting CSS overflow, and leave the inner layout's height undefined so it hugs its content and the `Scroller` can tell when to scroll:

[source,java]
----
var scroller = new Scroller();
scroller.setSizeFull();
var content = new VerticalLayout(); // no explicit height -- hugs content
scroller.setContent(content);
----

A `Scroller` only scrolls when it has a bounded height. Inside a full-height view, that means every flex layout between the `Scroller` and the sized root must be allowed to shrink below its content -- call `setMinHeight("0")` on those layouts, as the copy-paste example does on its content row.
