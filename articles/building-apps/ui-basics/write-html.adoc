...
public class HtmlExampleView extends Div {
    ...
    public HtmlExampleView() {
        ...
        footer.add(new Anchor(
                "https://vaadin.com/docs", "full documentation"));
...
List items can contain any component, not just text:
...
== When to Use HTML Elements vs. Vaadin Components

Use the HTML element classes for text and semantic structure — headings, paragraphs, lists, and links. For arranging components in rows, columns, and grids, use <<layouts#,layout components>> instead. Use Vaadin components (Button, TextField, Grid, Dialog, and others from the <</components#,component library>>) for interactive features like form inputs, data display, and user actions.
