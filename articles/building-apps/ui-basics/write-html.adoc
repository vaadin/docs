...
The [classname]`Html` component can also [since:com.vaadin:vaadin@V25.2]#do the sanitization itself#: the constructors that take a [classname]`SerializableSupplier<Safelist>` clean the content with the supplied jsoup [classname]`Safelist`, for example `new Html(untrustedHtml, Safelist::relaxed)`. Pass a method reference or a lambda that builds the [classname]`Safelist`, rather than capturing a pre-built instance, because jsoup's [classname]`Safelist` isn't serializable. Note that the safelist must permit the root element of the fragment. See <</flow/security/advanced-topics/vulnerabilities#xss,Cross-Site Scripting>> for more information.


== When to Use HTML Elements vs. Components

Use the HTML element classes for text and semantic structure — headings, paragraphs, lists, and links. For arranging components, Vaadin's <<layouts#,layout components>> handle the most common patterns. If you are comfortable with HTML and CSS, you can also use `Div` with CSS flexbox or grid for more custom layouts — see <<add-styling#,Add Styling>> for how to apply CSS. Use Vaadin components (Button, Text Field, Grid, Dialog, and others from the <</components#,component library>>) for interactive features like form inputs, data display, and user actions.
