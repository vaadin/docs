...
Put the test class in the view's package, because the examples read package-private view fields.
...
Most signal-driven UI is wired up with the [methodname]`bind*` family rather than explicit effects. From a test's perspective, each binding is a different property to assert on after a signal changes:
