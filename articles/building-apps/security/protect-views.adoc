...
=== Leaving Out, Hiding, and Disabling Components
...
When the component has to exist -- because other code refers to it, or because you change its state while the view is open -- hide it with [methodname]`setVisible(false)`, or disable it with [methodname]`setEnabled(false)`:
...
salaryField.setEnabled(authenticationContext.hasRole("ADMIN")); // <2>
----
<1> Users without the `ADMIN` role don't see the button at all.
<2> Users without the `ADMIN` role see the salary, but can't change it.

*Both methods are enforced on the server*, not only in the browser. Flow stops sending updates to an invisible component, and ignores events and property changes that arrive from a component that's invisible or disabled. A user who makes the element visible or enabled again with the browser's developer tools therefore still can't trigger its listeners.
...
Setting either state on a container applies it to the children as well, so you can hide or disable a whole section of a view with a single call. For the full semantics of both states, see <<{articles}/flow/component-internals/visibility#,Component Visibility>> and <<{articles}/flow/component-internals/enabled-state#,Component Enabled State>>.

.Don't Use CSS to Restrict Access
[WARNING]
Never restrict access with CSS -- such as `display: none`, `visibility: hidden`, or `pointer-events: none` -- or by setting the `hidden` or `disabled` attribute directly on the element. These only change how the component looks and behaves in the browser. On the server, the component is still visible and enabled, and it keeps accepting events, so anyone can restore it from the browser's developer tools and use it. Use CSS for styling. Use [methodname]`setVisible()` and [methodname]`setEnabled()` for access.


=== Hiding Isn't a Security Boundary

Leaving out, hiding, and disabling components decides *what users can reach through the UI*. It doesn't decide what they can do to your data. A bug in the view, a component you forget to hide, or a later refactoring can all expose an action you meant to restrict.
