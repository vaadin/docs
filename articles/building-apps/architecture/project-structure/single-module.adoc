...
With this, the `build-frontend` goal runs only when the `production` profile is active. The `vaadin-dev` dependency is declared as `<optional>true</optional>` in the main dependencies, so `spring-boot-maven-plugin` automatically excludes it from the production JAR.
