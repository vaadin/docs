...
To update the user interface, you have to add special completion stages that execute after the `CompletableFuture` completes. For more information about how to add these, see the <</building-apps/architecture/server-push/futures#,Consuming Futures>> documentation page.
