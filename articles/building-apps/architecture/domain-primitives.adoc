...
Third, a domain primitive validates all of its input data in the constructor. Because it's also immutable, this means that it's guaranteed always to be valid. For more information about validation, see the <</building-apps/architecture/consistency/validation#,Validation>> documentation page.
