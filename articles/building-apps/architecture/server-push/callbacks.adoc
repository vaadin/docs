...
When building the user interface with Vaadin Flow, the easiest way of allowing a background job to update the user interface is through callbacks. This is explained in more detail in the <</building-apps/architecture/background-jobs/interaction/callbacks#,Callbacks>> documentation page.
