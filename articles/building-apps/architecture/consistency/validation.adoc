...
The most important validation takes place at the application layer boundary. This is typically inside an <</building-apps/architecture/application-services#,application service>>, but it could also be in some other system component that communicates with an external system.
...
For more information about domain primitives, see the <</building-apps/architecture/domain-primitives#,Domain Primitives>> documentation page.
