...
This guide shows how to connect a Large Language Model (LLM) into a Vaadin application using Spring AI, Spring Boot, and the <<{articles}/flow/ai-components#,AI Components>>. You'll build a minimal chat UI with **MessageList** and **MessageInput**, stream responses token-by-token, and keep a conversational tone in the dialog with the AI -- all without writing boilerplate wiring code.
...
The <<{articles}/flow/ai-components#,AI Orchestrator>> connects your UI components to the LLM. It handles message display, token streaming, conversation memory, and UI updates automatically. You don't need to write a separate service class -- the orchestrator manages the Spring AI integration directly.
...
* Add **file attachments** with `UploadManager` via <<{articles}/flow/ai-components#file-attachments,`withFileReceiver()`>>.
* Support **tool calls** via <<{articles}/flow/ai-components#tool-calling,`withTools()`>>.
* **Persist conversation history** via <<{articles}/flow/ai-components#conversation-history,`ResponseCompleteListener`>>.
