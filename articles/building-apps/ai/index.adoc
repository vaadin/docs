...
* use the xref:{articles}/flow/ai-components#[AI Components] to connect LLM providers to Vaadin UI components with minimal boilerplate,
...
[TIP]
The <<{articles}/flow/ai-components#,AI Components>> eliminate the boilerplate of wiring UI components to LLM frameworks. The [classname]`AIOrchestrator` handles streaming, conversation history, file attachments, and tool calling behind a simple builder API. See the <<{articles}/flow/ai-components#,component documentation>> for the full API reference.
