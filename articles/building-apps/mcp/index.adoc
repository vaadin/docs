...
[TIP]
====
Documentation lookup is one half of what an agent needs. The other half is knowing whether the code it wrote actually runs. The <<{articles}/flow/configuration/live-reload/dev-loop-cli#,Dev Loop CLI>> gives an agent a single command that makes its edits live in the running application and reports the result as an exit code, along with a set of skills that teach it how to use them. The two are designed to be used together.
====
