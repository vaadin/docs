...
Vaadin's hosted server is the one to point your tools at unless your organization can't send documentation queries to a service outside its own network. For those cases the same server ships as a container image you run yourself -- it needs no network at runtime, it can record every tool call to an audit log, and it reports nothing back to Vaadin. See <<{articles}/building-apps/agentic-development/mcp/self-hosting/#,Self-Hosting the MCP Server>>.
