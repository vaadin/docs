...
* *Tool calls are reported to Amplitude.* That's how Vaadin sees which tools earn their place. The question itself is never sent: an event carries the tool name, how long the call took, and the question's *length*.
* *No record is kept of individual queries.* The server can append an audit log of every call it answers, and the hosted deployment leaves that switched off, so no such log exists.
...
Vaadin's hosted server is the one to point your tools at unless your organization can't send documentation queries to a service outside its own network. For those cases the same server ships as a container image you run yourself: it needs no network at runtime, it can record every tool call to an audit log, and it reports nothing back to Vaadin.

It really is the same server. There's no internal build for `mcp.vaadin.com` and a separate public one for customers -- the bytes that pass the release smoke test are the bytes both run, so a self-hosted instance answers from the same code and the same documentation corpus. Only the configuration above differs. See <<{articles}/building-apps/agentic-development/mcp/self-hosting/#,Self-Hosting the MCP Server>>.
