...
The easiest way to set up Claude Code for Vaadin development is the <<{articles}/building-apps/agentic-development/agent-skills#,Vaadin Skills>> plugin, which you install from the https://github.com/vaadin/agent-marketplace[Vaadin Agent Marketplace]. The plugin installs both the Vaadin MCP server and a set of Vaadin development skills. When you use the plugin, you don't need to configure the MCP server manually as described below.
...
Alternatively, run `/plugin` and select [guilabel]*vaadin-skills* from the marketplace browser.
...
[[replacing-an-earlier-marketplace]]
=== Replacing an Earlier Marketplace
