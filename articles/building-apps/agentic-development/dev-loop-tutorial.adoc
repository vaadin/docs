...
Open https://start.vaadin.com[Vaadin Start] and keep the sample view. The dev loop [since:com.vaadin:vaadin@V25.3]#needs a recent enough platform version#, so expand *Configure Project Settings* and check which version the project uses. Download the project, extract it, and open it in your IDE.
...
Claude Code already has a marketplace named `vaadin-marketplace` with a different source, such as the deprecated `vaadin/claude-plugin`. Remove it, add the marketplace again, and reinstall the plugin, which the removal disabled:
+
[source,terminal]
----
