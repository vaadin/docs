...
Open https://start.vaadin.com[Vaadin Start] and keep the sample view. The dev loop needs Vaadin 25.3 or newer, so expand *Configure Project Settings* and check that the Vaadin version is at least that. Download the project, extract it, and open it in your IDE.
...
Check two things in [filename]`pom.xml` before going further. First, that the version you picked came through:
