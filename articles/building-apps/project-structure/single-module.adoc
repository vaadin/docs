...
[CAUTION]
In a single-module project, all of the code is in the same place. There are no system safeguards to prevent breaking these constraints -- intentionally or otherwise. The risk of this happening grows with the codebase or as the number of people involved increases. This leads to code being an unmaintainable: known as a http://www.laputan.org/mud/mud.html#BigBallOfMud[Big Ball of Mud]. When it seems this may be appraoching, consider adding https://www.archunit.org/[ArchUnit] tests to your project, or converting to a <<multi-module#,multi-module>> project.
...
The code structure of a Vaadin application should resemble its <<{articles}/building-apps/architecture/components#,architecture>>. This is illustrated here by a fictional, _data driven_, Vaadin Flow application. This application has a _Views_ component, a _Services_ component, and an _Entities_ component.
