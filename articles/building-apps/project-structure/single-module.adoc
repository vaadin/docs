...
The _Views_ component is the user interface of the application. It communicates with the _Services_ component. Since the views are built with <<{articles}/flow#,Flow>>, Spring is able to directly inject instances of the services into the views, and no remote invocation is needed.
