...
Put the service interfaces and DTOs in the `services-api` module, and add it as a dependency to the `views` module. The `services-api` module shouldn't depend on the `entities` module. Otherwise, the `entities` become a transitive dependency of `views` -- something to avoid in this case.
