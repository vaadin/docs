...
All callbacks have to be thread-safe since they're called from the background thread, but owned and created by the user interface. For more information about how to implement these callbacks, see the <</building-apps/server-push/callbacks#,Implementing Callbacks>> documentation page.
