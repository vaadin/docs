...
When you cancel a `CompletableFuture`, it completes with a `CompletionException` caused by a `CancellationException`. However, the job continues to run silently in the background until it is finished. If you want to notify the job itself that it has been cancelled, and it should stop running at the next suitable moment, you have to make some changes.
