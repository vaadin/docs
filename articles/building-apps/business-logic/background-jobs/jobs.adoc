...
In code, a job is a Spring bean, annotated with the `@Service` annotation. It contains one or more methods that execute the job in the calling thread when called.
...
Fixing the bug is easy, but users will not want to wait for the next batch run to occur. Instead, you can add a button to the user interface that allows them to trigger invoice generation for an individual order.
