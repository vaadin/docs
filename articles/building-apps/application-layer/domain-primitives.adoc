...
Third, a domain primitive validates all of its input data in the constructor. Because it's also immutable, this means that it's guaranteed to always be valid. For more information about validation, see the <<{articles}/building-apps/application-layer/consistency/validation#,Validation>> documentation page.
