...
var result = customerRepository.findBySpecification(
    CustomerSpecification.byName("ACME")
        .and(CustomerSpecification.byCountry(Country.US)
            .or(CustomerSpecificaiton.byCountry(Country.FI))
        ), 
...
The challenge with this approach is that it is difficult, but not impossible, to build specification objects that are not coupled to the technology used to implement the repository. However, most business applications do not change their databases or have to support multiple repository implementations. Since the repositories are already a leaky abstraction, you can just as well embrace it and make the specifications implementation specific. After all, the specifications themselves still hide the implementation from the code that uses them. 
...
To address this issue, you should move all query methods that don't return entities to their own _query objects_. Query objects read from the same data source as the repositories, but are decoupled from them. After moving the query method from the example above to its own query object, you end up with something like this:
