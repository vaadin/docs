...
Compared to <<callbacks#,callbacks>>, this approach is easier to implement in the application layer, but more difficult to implement in the presentation layer. You should only use it if you've used `CompletableFuture` previously, and you need other features that it offers, like chaining completion stages together.
