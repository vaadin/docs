...
<2> Contains all files for 'my-theme'.
...
`style.css` is being treated as the starting point for loading other non-component-specific stylesheets.
...
. Create a new empty maven project (using either your preferred IDE's wizard or through Maven CLI) and make sure no unused java classes or resources exist in the project, then continue with the next steps.
. Update the pom.xml
