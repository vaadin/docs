...
Create style sheets as JavaScript modules (`.js` files) for each of the Vaadin elements in the `/frontend/themes/myTheme/` folder.

For example, modify  the `vaadin-button` component by adding custom rules to the original Lumo rules in `frontend/themes/myTheme/vaadin-button.js`:
