...
(Correspondingly, if you have used a JS module for your custom chart styles, you need to remove the import of the default theme as well as it's `include` in the `<style>` element. It's also recommended to refactor the module into a regular stylesheet.)
