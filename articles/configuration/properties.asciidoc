...
UIs that are open on the client side send a regular heartbeat to the server to tell they are still alive, even though there is no ongoing user interaction.
When the server doesn't receive a valid heartbeat for a given UI, it eventually removes that UI from the session.
...
The sync ID is used to handle situations when the client sends a message to a connector that has recently been removed on the server.
