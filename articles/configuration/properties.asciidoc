...
|`ci.build`
|When set to `true`, the build installs the exact npm package versions defined in the package lock file. In practice `npm ci` is run instead of `npm install`, or if pnpm is used the install command is run with `--frozen-lockfile` parameter.
|`false`
