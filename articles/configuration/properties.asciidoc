...
|assert
|When production mode is enabled, the Vaadin session lock check is done according to this setting. By default, the check is only done if assertions are also enabled: this is done to avoid the small performance impact of continuously checking the lock status. Alternative values are: 'log' to log a warning, or 'throw' to fail with an IllegalStateException. Since Vaadin 24.4.
