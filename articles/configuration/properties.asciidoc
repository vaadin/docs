...
The interval is expressed in seconds and the default is 300 (5 minutes).
...
This option is only supported if webpack is used as the front-end build tool and not if the application uses Vite, which is the default.
