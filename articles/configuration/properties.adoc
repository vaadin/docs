...
|`null`
|Defines the hosts allowed to access Vaadin development tools. A comma-separated list of allowed hosts should be provided as the value. `?` and `*` wildcards can be used. For example, `192.168.1.*,172.17.?.*`. Loopback addresses are always allowed regardless of the value set here.
