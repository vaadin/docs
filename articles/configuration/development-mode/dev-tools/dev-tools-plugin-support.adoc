...
The client side element is rendered inside its own tab in the dev tools window.
...
Java service provider needs to have the fully qualified name for the plugin, `.src/main/resources/META-INF/services/com.vaadin.base.devserver.DevToolsMessageHandler`
...
This gives the `DevToolsInterface` that is used to interact with the dev tools.
