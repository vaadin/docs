...
Vaadin Flow automatically serves the required resources for a PWA when you use the `@PWA` annotation in the <<{articles}/advanced/modifying-the-bootstrap-page#,Application Shell>>.
...
Even if your PWA currently works without HTTPS in some environments (for example, Android), this is likely to change and PWAs that don't support HTTPS will probably malfunction in the future.
