...
Defaults to `false`.
For production, the front-end is bundled and optimized, as described in <<../production#,Deploying to Production>>.
...
Usually needed to create an optimised production bundle and to load components sources to the browser on demand, i.e. once one opens a route where these components are used.
Defaults to `false`.
