...
`eagerServerLoad`::
Whether or not to insert the initial User Interface Definition Language (UIDL) object in the bootstrap [filename]`index.html`.
Defaults to `false`.
...
Whether or not we're running in legacy V14 bootstrap mode.
Defaults to `false`.
