...
In this part of this tutorial, explains how to get a Vaadin Flow application to utilize Web Push. It'll enable Web Push for a completed Customer Relationship Management (CRM) application so that users can receive notifications -- even when they're not in the application.
...
Also, Safari needs the web push notification features enabled. To do this, go to Settings &rarr; Safari &rarr; Advanced &rarr; Experimental Features. There you can enable `Notifications` and `Push API`.
====
