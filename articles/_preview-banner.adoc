...
ifdef::ticket-url[]
:ticket-link: You can report bugs and request additional features on {ticket-url}[GitHub,window=_blank].
endif::[]
ifndef::ticket-url[]
:ticket-link:
endif::[]

:forum-link: https://vaadin.com/forum/[provide feedback in the Forum,window=_blank]
...
ifdef::feature-flag[]
This is a preview version of {preview-feature}. You need to enable it with the {feature-flag-docs-link} `{feature-flag}`. Preview versions may lack some planned features, and breaking changes may be introduced in any Vaadin version. We encourage you to try it out and {forum-link} so that we can make it stable. {ticket-link}
endif::[]
