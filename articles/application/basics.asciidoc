...
A Hilla application is written in TypeScript and runs in a single HTML page.
...
Views in Hilla applications are programmed in _reactive_ rather than in imperative manner, as you would with Hilla Flow, jQuery, and so forth.
...
The following example is given in both imperative (Flow/Java) and reactive (Hilla/TypeScript):

[.example.show-code]
--

In imperative UI programming, like with Hilla Flow, there are two states: the data model and the UI.
...
To have a typical main view with menu navigation and multiple sub-views, you can use the https://vaadin.com/docs/ds/components/app-layout[App Layout] component.

The entry point of a Hilla application is the _application shell_, which consists of the following files:
...
To navigate between views, you need to define routes (see <<../routing/defining#,Defining Routes>> for more details).

If the application has server-side views implemented with Hilla Flow, the routing configuration starts a server session and loads routes to the server-side views.
See <<starting#, Starting the Application>> on how to run a Hilla application initially stateless and then start a session from TypeScript code.

Most of an application consists of _views_, as described next.

== Adding a View

Views in Hilla are custom HTML elements that extend [classname]#LitElement#, which handles rendering the component state reactively.
...
<head>
  ...
  <title>TypeScript Support in Hilla</title>
...
Hilla applications have some prerequisites.
...
Hilla provides a type-safe and secure way to access data from backend in frontend views by using generated TypeScript code.
