...
Please refer to the <<../security#, Security>> for configuring endpoint access.
...
In Hilla applications, the `index.ts` (or, optionally, `index.js`) file is also a module.
...
Hilla has this path alias in <<basics#Configuring TypeScript Compiler,the default TypeScript compiler configuration>> (`tsconfig.json`); webpack configuration file (`webpack.generated.js`) respects the `tsconfig` aliases by default.
...
You can change the folder by providing the path for the generator in the `generatedFrontendDirectory` property for Hilla Maven plugin.
