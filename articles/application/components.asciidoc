...
You can then use the https://vaadin.com/docs/ds/components/text-field[Text Field] component (the `<vaadin-text-field>` element) in your view's [methodname]#render()# method:
...
In the preceding example, the https://vaadin.com/docs/ds/components/combo-box[Combo Box] and https://vaadin.com/docs/ds/components/checkbox[Checkbox] components with the IDs `greeting` and `custom` respectively are mapped to typed class variables of the same names via the `@query` decorator.
...
|`visible`
|Visibility; set it to `false` to hide the element.
|===
