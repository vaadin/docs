...
Creating a new Hilla project with link:https://start.vaadin.com/[Vaadin Start] (or link:https://vaadin.com/labs/cli[Hilla CLI]) already takes care of setting up the basics for you by providing a MobX store (named `AppState`) and `View` and `Layout` helper base classes for your application views and layouts.
