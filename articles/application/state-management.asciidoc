...
When using Lit, you need to extend the `MobxLitElement` class instead of `LitElement`.
...
Creating a new Hilla project with the Vaadin CLI already takes care of setting up the basics for you by providing a MobX store (named `AppState`) and `View` and `Layout` helper base classes for your application views and layouts.
