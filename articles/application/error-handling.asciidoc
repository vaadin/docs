...
As a special case, if the server-side exception is an instance of [classname]#com.vaadin.fusion.exception.EndpointException# or a subclass, the server instead responds with https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/400[400 Bad Request] and the exception type and message passed to the [classname]#EndpointException# in Java are available in the [classname]#EndpointError# instance via the `type` and `message` attributes.
...
When a request is cancelled programmatically, the endpoint call promise will resolve with an `AbortError`. Actually this only
happens when you explicitly cancel request as explained in <<../advanced/cancelling-endpoint-requests#,this article>>.
