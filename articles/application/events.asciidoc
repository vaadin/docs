...
For registering event listeners in Hilla application views, declare them in your Lit templates using `@event` bindings with view class methods.
...
See the API references for those in the https://vaadin.com/docs/ds/components[Vaadin components] documentation.
...
When making forms in Hilla applications, consider using the <<../data-binding/binder#,Form Binder>>.
