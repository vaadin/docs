...
_(Note: this is an internal element whose styling isn't supported)_.
...
Custom margins applied to components in these layouts are now applied _in addition to_ the spacing, instead of _overriding_ the spacing.
Although the best approach is to refactor all custom margins applied to elements inside layouts with spacing, a quick fix is to subtract `var(--lumo-space-m)` from affected margins (`margin-top` for Vertical Layouts and `margin-left` for Horizontal Layouts):
[source,css,role="before"]
----
.some-layout-child {
  margin-left: 50px;
...
It's now a child of a new `vaadin-select-value-button` internal component, and is easiest to access as a regular child element of Select:
[source,css,role="before"]
----
...
Custom themes applied to the loading indicator still work in Vaadin 23 (as they apply to the loading state of the new indicator).
