...
While importing styles using `@CssImport` still works, the recommended way to apply CSS to Vaadin applications is using <<{articles}/styling/custom-theme#,the theme folder format>> introduced in Vaadin 19.
...
9. See <<{articles}/styling/custom-theme#, the theme folder format documentation>> for more details on the new format.
