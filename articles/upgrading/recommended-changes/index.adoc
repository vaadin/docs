...
This new model requires several annotations typically placed on the `MainLayout` / `MainView` class to be moved to a class implementing the `AppShellConfigurator` interface. For example, the `PWA` annotation:
...
- [methodname]`setDataProvider()` is now deprecated and it is recommended to use the overloaded [methodname]`setItems()` methods.
<<../../flow/binding-data/data-provider#lazy-data-binding-using-callbacks, Read more>> about the `dataview` API which was origininally link:https://vaadin.com/releases/vaadin-17[introduced] with Vaadin 17.
...
== Replace JsModule Stylesheets With CssImports
.Click and read if you have styles imported with [annotationname]`@JsModule` annotations.
[%collapsible]
====

Loading stylesheets with the [annotationname]`@JsModule` annotation in Flow is deprecated.
JS-wrapped stylesheets should be refactored to regular CSS files and loaded with @CssImport, or, preferably, using <<custom-theme-folder,the new theme folder format>> recommended for styling applications since Vaadin 19.
...
Replace all [annotationname]`@JsModule` annotations with [annotationname]`@CssImport` annotations for each stylesheet.
...
While importing styles using `@CssImport` still works, the recommended way to apply CSS to Vaadin applications is using <<{articles}/ds/customization/custom-theme#,the theme folder format>> introduced in Vaadin 19.

.Does not work with Material theme
[NOTE]
...
3. Create a stylesheet called `styles.css` inside the folder (or, if you already have one, move it there).
...
5. Move all Vaadin component stylesheets, that have a `themeFor` parameter (or, if refactoring from JsModules, a `theme-for` attribute in the `dom-module`) into the `components` folder and rename them to correspond to the `themeFor` value (the component's HTML element name)
+
----
/* Before */
...
6. Move other stylesheets next to `styles.css`
+
----
frontend
└── themes
...
8. Apply the theme using the `@Theme` annotation, using the theme folder's name as parameter
+
[source,java]
----
@Theme("my-theme")
...
9. See <<{articles}/ds/customization/custom-theme#, the theme folder format documentation>> for more details on the new format.