In the latest Vaadin versions, the <<{articles}/flow/templates/polymer#, [classname]`PolymerTemplate`>> class is deprecated and it is recommended instead to use a <<{articles}/flow/templates#, [classname]`LitTemplate`>>.
