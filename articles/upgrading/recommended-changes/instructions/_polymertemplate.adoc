In the latest Vaadin versions the <<{articles}/create-ui/templates/polymer#, [classname]`PolymerTemplate`>> class is deprecated and using a <<{articles}/create-ui/templates#, [classname]`LitTemplate`>> is recommended instead.
