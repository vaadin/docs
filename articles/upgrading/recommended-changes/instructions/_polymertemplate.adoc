In the latest Vaadin versions the <<{articles}/templates/polymer#, [classname]`PolymerTemplate`>> class is deprecated and using a <<{articles}/templates#, [classname]`LitTemplate`>> is recommended instead.
...
[NOTE]
If you use [annotationname]`@Id` for <<{articles}/templates/components#, binding to template components>>, note that the package name should be changed from `com.vaadin.flow.component.polymertemplate.Id` to `com.vaadin.flow.component.template.Id` for [classname]`LitTemplate`.
...
See the [annotationname]`@ClientCallable` <<{articles}/element-api/client-server-rpc#clientcallable-annotation, documentation>> for more details.
...
With Vaadin 23, it is recommended to handle server-to-client data communication with the Vaadin <<{articles}/element-api/properties-attributes#, Element APIs>> instead.
...
For this, you can use a <<{articles}/element-api/client-server-rpc#clientcallable-annotation, [annotationname]`@ClientCallable`>> method.
