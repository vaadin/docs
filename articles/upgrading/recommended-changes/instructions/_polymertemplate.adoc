In the latest Vaadin versions, the <<{articles}/create-ui/templates/polymer#, [classname]`PolymerTemplate`>> class is deprecated and it's recommended instead to use a <<{articles}/create-ui/templates#, [classname]`LitTemplate`>>.
...
In general, the concept with [classname]`PolymerTemplate` and [classname]`LitTemplate` is mostly the same, but there are some differences that need to be taken into account when migrating.

[discrete]
=== Replace EventHandler with ClientCallable

The [annotationname]`@EventHandler` annotation used with [classname]`PolymerTemplate` can't be used with [classname]`LitTemplate`.
...
Consider the following content in the [classname]`LitElement` based view:
