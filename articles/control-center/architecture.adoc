...
Control Center is a Kubernetes-native platform for deploying, managing, and monitoring Vaadin applications. It simplifies production operations by combining an opinionated https://kubernetes.io/docs/concepts/extend-kubernetes/operator/[Kubernetes Operator,window=read-later] with a Vaadin-based management UI.
...
The platform integrates with https://www.keycloak.org/[Keycloak,window=read-later] for authentication and authorization, and configures the application with access to a https://www.postgresql.org/[PostgreSQL,window=read-later] database for persistence when needed.
...
External access is routed through a central https://kubernetes.github.io/ingress-nginx/[Ingress NGINX,window=read-later] load balancer.
...
Control Center introduces a Kubernetes https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/[custom resource definition (CRD),window=read-later] named `App`.This resource abstracts away the complexity of deploying and configuring a production-ready Vaadin application.
