...
It is important to xref:../getting-started/local-environment#windows[update] the `hosts` file on the Windows system with the domain you used during the Control Center installation, in this case with the `control.local` and `auth.local` domains:
