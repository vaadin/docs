...
Control Center is installed and managed using https://helm.sh/[Helm,window=read-later]. By default, installing or upgrading without specifying a version will use the latest available stable release.

== Installing or Upgrading to a Specific Version

To install or upgrade to a specific version of Control Center, use the `--version` flag with the `helm install` or `helm upgrade` command. The version must match a https://hub.docker.com/repository/docker/vaadin/control-center/tags[published chart version,window=read-later].
