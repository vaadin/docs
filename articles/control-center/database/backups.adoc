...
WARNING: To prevent the sensitive values of the secrets to be *stored in the shell's history*, write down the values in files *without an extra newline character* at the end of the text and pass those files to `kubectl` to save their content as the secret's values.
