...
Control Center can automatically provision a PostgreSQL database for a deployed application by connecting it to a managed Postgres cluster. The database is automatically created, and credentials are injected into the application environment.

To enable this, set the `spec.postgres.cluster` field in the App manifest with the name of an existing Postgres cluster.

To list the available Postgres clusters in the namespace:

.Terminal
[source,shell]
----
kubectl get clusters -n vaadin
----
...
control-center-postgres   30m   1           1       Cluster in healthy state   control-center-postgres-1
----

[IMPORTANT]
====
The Postgres cluster must exist in the **same namespace** as the application. When Control Center is installed, it provisions a default cluster in its own namespace.
====
...
spec:
  ...
  version: "1.0.0"
  postgres:
    cluster: control-center-postgres
----
