...
[TIP]
====
It's common to use the app version to tag a release build, for example `my-app:1.0`.
Another common tag is `latest` to mark the most recent release, but using this tag will force Kubernetes to always pull the image from the registry: deploying an application with the `latest` tag requires the image to be available in the registry.
====
