...
- [.dollarsign]*$* *Multi-Factor Authentication*: Enable MFA to require the user to verify their identity with an additional factor, such as an authenticator application.
- [.dollarsign]*$* *Password-less authentication*: Allow the user to authenticate using a passkey instead of their password. When enabled, the user must register a passkey on their next login.
...
=== Authenticating with Passkeys [.dollarsign]*$*
