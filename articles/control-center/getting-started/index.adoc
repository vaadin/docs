...
Via your cloud provider's dashboard, create two DNS records. Point them to your cluster's public IP address, which are obtained from your cloud provider. For example, supposing your domain is `mydomain.com`, you'll create `control.mydomain.com` and `auth.mydomain.com`, both pointing to the cluster's external IP.
...
You'd replace `mydomain.com` here with your domain, and replace email address value with your own. The email is used to create the initial user account in Control Center.
...
This ingress configuration assumes that you have already a certificate in your production environment. To create the `control-center-tls` secret for the certificate, you should execute this from the command-line:
...
Once deployed, copy the temporary password for the initial user. Run the following to retrieve it:
