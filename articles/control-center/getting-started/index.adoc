...
[NOTE]
Granting administrator privileges enables automatic installation of required dependencies. If you choose not to grant these privileges, though, you'll need to manually install the necessary resources. See the <<production-installation#,Production Installation>> section for details on how to do this.
...
[IMPORTANT]
Since this does not grant administrator privileges, you'll need to manually install the required dependencies. See the <<production-installation#,Production Installation>> section for details on how to do this.
...
[source,bash]
----
kubectl create secret tls control-center-tls --cert=cert.pem --key=key.pem
----
