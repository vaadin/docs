...
To support backups in a local development environment, volume snapshot functionality must be available.Minikube provides built-in StorageClass and VolumeSnapshotClass implementations that are compatible with Control Center and can be easily enabled using addons.

Enable both required addons with:
