...
description: Setting up your development environment similarly as Vaadin employees makes it easier to review and accept code contributions.
order: 10
---
...
This affects the Flow project where `flow-client` has `flow-server` sources as a resource.

The fix this, comment out the `flow-server` resource for the `flow-client/pom.xml` (https://github.com/vaadin/flow/blob/master/flow-client/pom.xml#L166-L168[source]) during development.
