---
title: Style Guide
page-title: Vaadin documentation style guide for contributors
description: Writing style guidelines for documentation writers.
meta-description: Follow these guidelines to contribute to Vaadin documentation with consistency and clarity.
order: 20
---
...
Avoid using explicit Vaadin version numbers in text (e.g., _Vaadin 25_). Instead, use something like `[role="since:com.vaadin:vaadin@V25.1"]` for parts of the documentation that apply since a specific version.

[example.recommend]
====
`Using the integrated content areas in *+++[since:com.vaadin:vaadin@V25.2]#Tab Sheet#+++* is the easiest way to switch between the different content for each tab.`
====

[example.avoid]
====
`Using the integrated content areas in Tab Sheet *(available in Vaadin 25.2 and later)* is the easiest way to switch between the different content for each tab.`
====
...
A badge or a note is only meaningful within the major version it was written for: readers don't need to be told that something arrived in a minor release of an earlier major version. Drop these references when a branch is forked for a new major version. The Check Stale Version References workflow fails on any that are left behind. Where a reference to an earlier major version is deliberate, such as a compatibility matrix, put `// stale-version-ok` on the line above it.
