---
title: Word List
page-title: Word list for contributing to Vaadin docs
description: List of words and phrases to avoid, and word usage advice for the Vaadin documentation.
meta-description: Follow the approved terminology and style conventions when contributing to Vaadin documentation.
order: 40
---
...
The adjective is _frontend_. +
For example:

[example]
====
_Processing takes place at the *frontend*._ +
_**Frontend** processing is kept to a minimum._
====
...
=== (s), (es)
Don't append _(s)_ or _(es)_ to a singular word to indicate that the item in question may or may not be plural. Instead, either use the plural word only, or explicitly give both forms. +
For example:
