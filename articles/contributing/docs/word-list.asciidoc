...
=== "aka"
This is an abbreviation of _also known as_. It has its origins in police usage and, hence, conveys an inappropriate tone for software documentation.

Instead, use parentheses or the word _or_. For example:

[example.recommend]
The company specializes in software as a service *(SaaS)*.
[example.avoid]
The company specializes in software as a service, *aka SaaS*.

[example.recommend]
Each instantiated object of the class has a separate copy, *or instance*.
[example.avoid]
Each instantiated object of the class has a separate copy, *aka instance*.
...
Instead, use _deny list_ or _block list_. +
...
=== "click"
_Click_ is both a verb and a noun. Avoid using _click on_. +
For example:
...
=== "e.g."
See <<./styleguide#latin-abbreviations,Lating Abbreviations>>.
