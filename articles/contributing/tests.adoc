...
++++
<style>
[class^="PageHeader-module--descriptionContainer--ba197"] {display: none;}
</style>
++++
...
Debugging can be made in a several ways. One way is to navigate to a test module and run `mvnDebug jetty:run` and starting "Remote JVM Debug" configuration. This is usually available in IDEs.

In IntelliJ IDEA, you can run Jetty plugin in debug mode. For example, you'd navigate to a particular test module in "Maven" panel under "Flow Tests" node. Then choose "Plugins" &rarr; "jetty", and then right-click on "jetty:run" and select "Debug '[module-name]' ...".
