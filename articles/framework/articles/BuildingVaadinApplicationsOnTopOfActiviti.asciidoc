...
https://www.activiti.org[Activiti] BPM engine can be used together with
...
https://tomcat.apache.org[Tomcat 7]. It uses an embedded in-memory
https://www.h2database.com[H2 database] for storing data, which means
that all your data will be lost when the server is restarted.

https://www.eclipse.org/downloads/packages/release/helios/sr2/eclipse-ide-java-ee-developers[Eclipse
...
https://en.wikipedia.org/wiki/Model-view-presenter[Model-View-Presenter]
