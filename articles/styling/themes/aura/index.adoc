...
Every read-only property is marked with the [badge]*Read-only* badge on the Aura reference pages, and its description names the properties to set instead. A property without the badge is customizable. Some read-only color properties also carry a [badge]*light-dark()* badge, which means their value is a CSS `light-dark()` function with a separate value for each color scheme.
====
