...
Applies a separate color scheme to the content area of App Layout. The App Layout drawer and navbar use the global color-scheme property. Has no effect if App Layout is not used. The default is `inherit`, which follows the global `color-scheme`.

`--aura-notification-color-scheme`::
Applies a separate color scheme to all Notifications. The default is `inherit`, which follows the global `color-scheme`.
...
Because both of those values are computed, every property with this badge also carries the xref:./#read-only-properties[Read-only,role=badge] badge. Don't assign values to them. Instead, set the properties they're computed from -- usually the `-light` and `-dark` suffixed pair, or the `--aura-contrast-level` property. The description of each read-only property below names what to set instead.
...
[.property-table]
`--aura-app-background`::
The main background image, used for the `<html>` element and the App Layout content area. By default this is a computed gradient, based on the `--aura-background-color` property, which becomes more prominent with more saturated background colors. Unlike the other computed colors on this page, this property is meant to be overridden: you can set it to a solid color, an image, or your own gradient.

`--aura-background-color` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]::
The primary background color that defines the overall tone of the application. This color is used to compute <<#text-color,text>>, <<#border-color,border>>, and <<#surface-color,surface>> colors. Automatically assigned the value of the `-light` or `-dark` suffixed property, depending on the active color scheme. Set `--aura-background-color-light` and `--aura-background-color-dark` instead of this property.

`--aura-background-color-light`::
The primary background color of the application for the light color scheme. The default is `oklch(0.95 0.005 248)`, a very light, slightly blue-tinted gray.

`--aura-background-color-dark`::
The primary background color of the application for the dark color scheme. The default is `oklch(0.2 0.01 260)`, a very dark, slightly blue-tinted gray.

`--vaadin-background-container` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]::
Background for “containers”, buttons, toolbars, notifications, and other highlighted content areas. By default computed from `--aura-background-color` and `--aura-contrast-level`. Use this color if you want a darker background in the light color scheme and a lighter color in the dark color scheme. Set `--aura-background-color-light`, `--aura-background-color-dark`, or `--aura-contrast-level` instead of this property.

`--vaadin-background-container-strong` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]::
A more prominent variant of the `--vaadin-background-container` color, computed from the same properties. Set `--aura-background-color-light`, `--aura-background-color-dark`, or `--aura-contrast-level` instead of this property.
...
[.property-table]
`--vaadin-text-color` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]::
The main text color used in the UI. Defaults to the <<#neutral-color,neutral color>>. Set `--aura-neutral-light` and `--aura-neutral-dark` instead of this property, or adjust `--aura-contrast-level`.

`--vaadin-text-color-secondary` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]::
A secondary text color, with at least 4.5:1 contrast ratio with the main background color. Computed from `--vaadin-text-color` and `--aura-contrast-level` by default. Set `--aura-neutral-light`, `--aura-neutral-dark`, or `--aura-contrast-level` instead of this property.

`--vaadin-text-color-disabled` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]::
Text color for disabled text. Computed from `--vaadin-text-color` and `--aura-contrast-level` by default. Set `--aura-neutral-light`, `--aura-neutral-dark`, or `--aura-contrast-level` instead of this property.

`--aura-accent-text-color` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]::
A text color derived from the accent color, with at least 4.5:1 contrast ratio with `--aura-background-color`. Set `--aura-accent-text-color-light` and `--aura-accent-text-color-dark` instead of this property.
...
[.property-table]
`--aura-accent-border-color` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]::
A border color that is slightly tinted with the <<#accent-color,accent color>>, designed to be paired together with the `--aura-accent-surface` color. Computed from the accent color, the border color, and the surface and contrast levels. Set `--aura-accent-color-light`, `--aura-accent-color-dark`, `--aura-contrast-level`, or `--aura-surface-level` instead of this property.

`--vaadin-border-color` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]::
A high-contrast border color. By default computed from `--aura-background-color` and `--aura-contrast-level` in Aura. Set `--aura-contrast-level` instead of this property, or change the background color through its `-light` and `-dark` properties.

`--vaadin-border-color-secondary` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]::
A low-contrast border color. By default computed from `--aura-background-color` and `--aura-contrast-level` in Aura. Set `--aura-contrast-level` instead of this property, or change the background color through its `-light` and `-dark` properties.
...
[.property-table]
`--aura-neutral` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]::
Automatically assigned the value of the `-light` or `-dark` suffixed property, depending on the active color scheme. Set `--aura-neutral-light` and `--aura-neutral-dark` instead of this property.

`--aura-neutral-light`::
A dark gray by default, computed from `--aura-background-color-light` and `--aura-contrast-level`.

`--aura-neutral-dark`::
White by default, computed from `--aura-background-color-dark` and `--aura-contrast-level`.
...
Each color has a corresponding text color property that provides appropriate <<#contrast,contrast>> against the background. The generic colors in the left column are customizable. The text colors in the right column are read-only: each is computed from the generic color beside it and `--aura-contrast-level`, so set one of those two instead.

[%header]
|===
|Generic Color | Default Value | Text Color
|`--aura-red` | `oklch(0.59 0.2 25)` | `--aura-red-text` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]
|`--aura-orange` | `oklch(0.61 0.35 87)` | `--aura-orange-text` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]
|`--aura-yellow` | `oklch(0.89 0.3 98)` | `--aura-yellow-text` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]
|`--aura-green` | `oklch(0.6 0.2 155)` | `--aura-green-text` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]
|`--aura-blue` | `oklch(0.55 0.2 264)` | `--aura-blue-text` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]
|`--aura-purple` | `oklch(0.58 0.22 290)` | `--aura-purple-text` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]
|===
...
[.property-table]
`--aura-accent-color` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]::
Automatically assigned the value of the `-light` or `-dark` suffixed property, depending on the active color scheme. Set `--aura-accent-color-light` and `--aura-accent-color-dark` instead of this property.
...
`--aura-accent-contrast-color` xref:./#read-only-properties[Read-only,role=badge] xref:#light-dark-function[light-dark(),role="badge light-dark"]::
A color that provides strong contrast against the `--aura-accent-color`, for example, button label color when the button background uses the accent color. The default contrast color is either white or a semi-transparent black, depending on the lightness of the accent color. Set `--aura-accent-contrast-color-light` and `--aura-accent-contrast-color-dark` instead of this property.

`--aura-accent-contrast-color-light`::
The accent contrast color for the light color scheme. Computed from `--aura-accent-color-light` by default: white for a dark accent color, a near-black for a light one.

`--aura-accent-contrast-color-dark`::
The accent contrast color for the dark color scheme. Computed from `--aura-accent-color-dark` by default: white for a dark accent color, a near-black for a light one.
...
A computed color that you can apply to an element to give it elevation. The color is computed based on the `--aura-background-color` property. The final value of the computed color depends on the effective values of the `--aura-surface-level` and `--aura-surface-opacity` properties. Set those two instead of this property.

`--aura-surface-color-solid` xref:./#read-only-properties[Read-only,role=badge]::
An opaque version of the effective surface color. Essentially the same as if `--aura-surface-opacity` was `1`. Useful for situations when a semi-transparent color is not suitable (e.g., when a surface needs to fully cover something). Set `--aura-surface-level` instead of this property.

`--aura-accent-surface` xref:./#read-only-properties[Read-only,role=badge]::
A surface color that is tinted with the effective <<#accent-color,accent color>>. To change the tint, apply one of the <<#accent-color-class-names,accent color class names>> or set the accent color for the element, rather than setting this property.
