...
.Avoid Using The `theme="dark"` Attribute Globally
[CAUTION]
====
Avoid using `ui.getElement().getThemeList().add("dark")` to switch to the dark color scheme globally.

[methodname]`getThemeList()` is the generic theme name API that every component has, and it's the right tool for variants such as `small` or `contrast`. 

When using the [propertyname]`ColorScheme.Value.DARK` value specifically, Vaadin adds the `theme="dark"` attribute on the `<html>` element for backwards compatibility. You should not rely on it going forward.

In tests, assert on [methodname]`Page.getColorScheme()` rather than on the `theme` attribute.
