...
Vaadin supports link:https://tailwindcss.com/[Tailwind CSS,window=_blank], and the Lumo theme has its own set of <<{articles}/components/themes/lumo/utility-classes#,utility classes>>. These two collections are mutually exclusive and cannot be used simultaneously.
...
To enable Tailwind, enable the `tailwindCss` <<{articles}/flow/configuration/feature-flags#,feature flag>> by adding the following line to `src/main/resources/vaadin-featureflags.properties`:
...
The Lumo theme includes its own set of utility classes called <<{articles}/components/themes/lumo/utility-classes#,Lumo Utility Classes>>. These only work in combination with the Lumo theme, but not with the Aura theme. To load the Lumo Utility Classes, import the respective stylesheet with [annotationname]`@StyleSheet`.
...
A complete list of Lumo Utility Classes is available on its <<{articles}/components/themes/lumo/utility-classes#,reference page>>.
