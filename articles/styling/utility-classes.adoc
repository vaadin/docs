...
// TODO: Link "Lumo Utility Classes" to reference article
The Lumo theme includes its own set of utility classes called Lumo Utility Classes. These only work in combination with the Lumo theme, but not with the Aura theme. To load the Lumo Utility Classes, import the respective stylesheet with [classname]`@StyleSheet` or `@import`.
