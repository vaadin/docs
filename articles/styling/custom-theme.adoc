...
It can be packaged as a dependency for reuse in multiple applications, as described in <<{articles}/components/ds-resources/customization/custom-theme-packaging#, Packaging a Theme for Reuse>>.
...
See <<{articles}/components/ds-resources/customization/style-scopes#, Style Scopes>> for details on global vs local CSS.
...
As the internal styling of Vaadin components is <<{articles}/components/ds-resources/customization/style-scopes#, isolated from the global CSS inside the component's shadow DOM>>, the easiest way to customize their styles is through the CSS properties available in the built-in themes (see Foundation section for details), and by injecting custom CSS directly into the shadow DOM of the components by placing them in the components sub-folder.
...
See <<{articles}/components/ds-resources/customization/styling-components#, Styling Components>> for details on writing CSS for Vaadin components.
