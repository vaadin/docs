...
. If the theme uses npm assets, as described in <<{articles}/styles/custom-theme/custom-theme-configuration#stylesheets, Style Sheets From npm Packages>>, add a [filename]#Dependencies.java# class (1) with the corresponding [classname]`@NpmPackage` annotations:
+
----
[project root]
└── src
