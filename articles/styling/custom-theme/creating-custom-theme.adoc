...
When Flow UIs are <<{articles}/integrations/embedding#, embedded>> into other web pages, some styles need to be in the root of the parent page instead of the Shadow DOM wrapper in the embedded UI. You can load the styles into the parent page by placing them in a stylesheet called `document.css`.
...
To avoid copy-pasting styles into two places, move them to a separate style sheet and use `@import` to include them in both [filename]`styles.css` and [filename]`document.css`.
