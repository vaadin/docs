...
CSS selectors in a global style sheet can't affect those elements.
...
If you want the same result as with the global style sheet, you need to use the `:host` selector to match the element which is the “host” for this shadow DOM or style scope:
...
- https://vaadin.com/blog/css-encapsulation-with-shadow-dom[CSS Encapsulation with Shadow DOM]
endif::web[]
