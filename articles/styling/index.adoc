...
[NOTE]
The Vaadin 24 way of loading styles through the [classname]`@Theme` annotation and a theme folder in `frontend/themes` has been deprecated in Vaadin 25, but is still supported. Please see the link:/docs/v24/styling[Vaadin 24 Styling documentation] for details on this usage.


== Stylesheets

// TODO: Link "Stylesheets" to article
Stylesheets, placed in the `src/main/resources/META-INF/resources` folder, are loaded with the [classname]`@StyleSheet` annotation.

In most cases you should apply [classname]`@StyleSheet` annotations to the main application class that implements the [classname]`@AppShellConfigurator` interface. This ensures that the CSS in it is applied globally, up-front, when the application UI is loaded into the browser.
...
You can split your CSS over multiple stylesheets, and load them either with separate [classname]`@StyleSheet` annotations, or load additional stylesheets using the CSS `@import` rule.
...
// TODO: Link "Stylesheets" to article
More details are provided in the Stylesheets section.

== Themes

// TODO: Link to Lumo and Aura reference pages
By default, Vaadin components are rendered with their minimalistic built-in base styles, which are a good starting point if you want to customize the look and feel extensively. Vaadin also ships with two themes, Aura and Lumo, that provide a more polished look and feel for Vaadin components.
...
Vaadin themes can be loaded with the [classname]`@StyleSheet` annotation or with a CSS `@import` rule in your own stylesheet. Note that themes should be imported before any other styles in your application.
...
Both themes provide a comprehensive set of style properties (CSS custom properties) that make them easily customizable without complex CSS selectors. These properties are listed on the Aura and Lumo reference pages.
...
Vaadin components also provide their own component-specific style properties for customizing their styles. These properties are listed on the Styling sub-pages in the component documentation (see e.g. <<{articles}/components/text-field/styling#,TextField>>), along with the most common CSS selectors you can use to target the parts and states of components with your own CSS.
...
// TODO: Link "theme" to themes article
...
. *Variant*: check if the component provides a style variant that matches (at least partially) what you need;
// TODO: Link "theme" to theme props section in component styling page
// TODO: Link "component style properties" to component props section in component styling page
. *Style Properties*: customize the component using theme and/or component style properties;
// TODO: Link "write your own CSS" to write CSS section in component styling docs
...
Utility classes can be used to apply styles to HTML elements (like `<div>` and `<span>`), and (with some limitations) to Vaadin components. Vaadin supports Tailwind CSS, and the Lumo theme has its own set of Lumo Utility Classes.
