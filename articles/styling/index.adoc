...
Inline CSS can also be applied to individual elements and components throught the [methodname]`getStyle()` Java API. These should be used sparingly to avoid hard-to-maintain spaghetti code, but can be useful e.g. for applying styles dynamically based on business logic.
...
Inline styles applied through the [methodname]`getStyle()` API are mainly useful for styling HTML elements and the Horizontal and Vertical Layout components. The `getStyle()` API is rarely effective for styling other Vaadin components is limited, due to the complex nested structure of most components. Exceptions to this rule include margin, padding, and <<styling-components#component-style-properties,component style properties>>.
