...
In most project configurations, <<{articles}/styling/stylesheets#, stylesheets>> are placed in the `src/main/resources/META-INF/resources` folder. In non-Spring projects compiled into WAR files they are placed in the `src/main/webapp` folder. In both cases they are loaded with the [annotationname]`@StyleSheet` annotation.
