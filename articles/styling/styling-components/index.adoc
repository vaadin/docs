...
Style property values can also be applied to any container element, such as <<{articles}/components/basic-layouts#, layouts>> or <<{articles}/create-ui/standard-html#, HTML elements>> like ``div``s, typically by applying CSS class names to them. All Vaadin components, as well as other child elements, will automatically inherit style property values applied to their parent elements:
...
* <<parts-and-states#, Details on the different types of styleable parts and states in Vaadin components>>
