...
By using the `~=` operator in the `[theme~="dark"]` selector it will match even when multiple theme variants are defined, for example, `<body theme="plain dark">`. Just using the `=` operator is slightly more performant if you don't need to match all instances.
