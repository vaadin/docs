...
If you're using the Lumo Dark theme variant, customize colors after the `[theme~="dark"]` selector, i.e., like this:
...
By using the `~=` operator in the `[theme~="dark"]` selector it will match the color when multiple dark theme variants are defined. For example, if you use `<html theme="dark">` on a HTML element somewhere in the global style scope of your application, the color change will apply to it as well.
This helps you achieve a unified appearance between the different components in your application.
Just using the `=` operator here is slightly more performant, but it won't match the color on other instances.
