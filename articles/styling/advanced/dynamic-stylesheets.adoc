...
This is done using the `addStyleSheet` method on the [classname]`Page` class, which takes a URL parameter. The URL can point either to a stylesheet served by the application itself, located in the resource folder `src/main/resources/META-INF/resources` (or in `src/main/webapp` for non-Spring apps compiled into WARs), or to an external URL.
