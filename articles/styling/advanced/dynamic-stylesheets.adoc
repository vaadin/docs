...
UI.getCurrentOrThrow().getPage().addStyleSheet("dynamic-styles.css");
/* Style sheet loaded from an external URL */
UI.getCurrentOrThrow().getPage().addStyleSheet("https://example.com/styles.css");
----
...
Registration registration = UI.getCurrentOrThrow().getPage().addStyleSheet("dynamic-styles.css");
...
public class ThemeChanger extends Select<String> {
    public ThemeChanger() {
        ...
        addValueChangeListener(event -> switchTheme(
                event.getSource().getUI().orElse(UI.getCurrentOrThrow()),
                event.getValue()));
    }
