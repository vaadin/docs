...
.@CssImport still Supported but Not Recommended
[NOTE]
In some older versions of Vaadin, stylesheets were loaded using `@CssImport` and `@Stylesheet` annotations -- and in very old versions using the `@HtmlImport` annotation. While `@CssImport` and `@Stylesheet` still work, they're no longer recommended as the primary way to load stylesheets.
