...
[NOTE]
====
The loading of shadow DOM stylesheets from the `components` sub-folder is not compatible with the <<../configuration/development-mode#precompiled-bundle, pre-compiled frontend bundle>> introduced in Vaadin 24. In order to use this mechanism for loading shadow DOM styles, the pre-compiled bundled must be <<../configuration/development-mode#disable-precompiled-bundle, disabled>>.
====
