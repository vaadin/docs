...
[NOTE]
====
Although the project may contain multiple theme folders, only one can be applied to the UI. There is no way dynamically to switch theme folders run-time. However, there are ways to <<advanced/runtime-theme-switching#, switch between variants>> of the same theme, <<advanced/loading-styles-dynamically#, load additional styles dynamically>> on top of the theme, and create <<advanced/parent-and-sub-themes#, base-themes inherited by sub-themes>>.
====
