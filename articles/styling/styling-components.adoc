...
Aura style properties are prefixed with `--aura-` while Lumo style properties are prefixed with `--lumo-`. The theme style properties are listed on the <<{articles}/components/themes/aura#, Aura>> and <<{articles}/components/themes/lumo#, Lumo>> reference pages.
...
Some components like Grid provide APIs for applying <<#shadow-parts, custom shadow part names>> to their internal elements, instead of class names.

The same approach can be used to scope styles to a particular view or <<{articles}/styling/styling-elements#, other UI element>>.
