...
For example, we might want to style the Button component differently in our application, depending on whether it's a “Signup” button or a “Delete account!” button.
...
The fact that they exist in the DOM independently from their parent component means that a CSS classname that's given to the parent element isn't propagated down to the sub-component.
