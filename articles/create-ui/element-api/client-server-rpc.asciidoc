...
It is worth noting that if the component with the [annotationname]`@ClientCallable` method is underneath a modal dialog or component, it is considered _inert_, that means that it is not available for interaction, including RPC calls.
If you want the [annotationname]`@ClientCallable` method to be available also when the component is _inert_, you need to annotate it with the [annotationname]`@AllowInert` annotation.
Consult the <<{articles}/advanced/server-side-modality#,Server-Side Modality documentation>> for more information.

*Example*: Using the `@AllowInert` annotation to allow calls to an inert component.
[source,java]
----
@ClientCallable
@AllowInert
