...
A client-side template cannot render child templates created using the [classname]`Component` or [classname]`Element` API, because it uses the shadow DOM, which renders the *shadow tree*, instead of the element's children, which are in the light DOM.
...
----
* For example, a `<label slot="title">Main header</label>` element will now _not_ be positioned in the default "No content given!" slot, but rather in the `<h1><slot name="title">...` slot.
