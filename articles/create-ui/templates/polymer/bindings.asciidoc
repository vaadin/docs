...
Double square brackets (`[[ ]]`) mark a one-way binding.
...
Polymer can't bind certain common native element properties directly, because the binding causes issues on one or more browsers.
...
* Double curly brackets (`{{ }}`) mark two-way bindings &ndash; both server-to-client and client-to-server data flow.

* Double square brackets (`[[ ]]`) mark one-way bindings &ndash; only server-to-client data flow.
