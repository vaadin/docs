...
[NOTE]
Since Vaadin 24.3, click shortcut resets focus on client side on an active element, e.g. `<input>`, by default before executing the shortcut action, e.g. to properly submit the input's value when it's focused and shortcut is triggered. This behavior can be disabled by calling https://vaadin.com/api/platform/com/vaadin/flow/component/ShortcutRegistration.html[[methodname]`setResetFocusOnActiveElement(boolean)`] on the shortcut registration.
