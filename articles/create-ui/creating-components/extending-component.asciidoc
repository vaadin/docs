...
These are alternative approaches that are mutually exclusive.
...
This example creates a `NumericField` component by extending the `TextField` component.
...
See <<{articles}/styling/custom-theme/styling-components#,Styling Components>> for more information.
