...
Mixins also keep your code clean and simple. For example, let's compare setting a component's width. Here's what you'd have to do without mixins: 

----
getElement().getStyle().set("width", "300px")
----

That's not too complicatged. Here's what's required after implementing the [interfacename]`HasSize` interface: 
