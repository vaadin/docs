...
Implements [interfacename]`HasElement` mixin.
...
Implements [interfacename]`HasElement` and [interfacename]`HasEnabled` mixins.
...
Vaadin Flow provides many additional useful mixin interfaces. [interfacename]`HasElement` is low level API that is used by most other mixins.

* [interfacename]`HasElement`: Marker interface for any class that's based on an [classname]`Element`.

Following list has mixins depending directly on a root element. All implements [interfacename]`HasElement`:
...
* [interfacename]`HasValueAndElement`: Same as [interfacename]`HasValue` and also implements [interfacename]`HasElement` and [interfacename]`HasEnabled`.
...
* [interfacename]`HasDataProvider<T>`: For listing components that use a data provider to display data. Implements also [interfacename]`HasItems`.
