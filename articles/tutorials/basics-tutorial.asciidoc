...
Below the [methodname]#render()# method in the [classname]#TodoView# class, add a [methodname]#connectedCallback()# https://lit.dev/docs/components/lifecycle/[lifecycle callback] to initialize the view when it's attached to the DOM.
...
Start your server with the `./mvnw` command if you don't already have it running.
