...
Define a JPA _entity_ class for the data model, by creating a new `Todo.java` file in `src/main/java/com/example/application` with the following content:
...
**The first time you run the application, it may take up to a few minutes** as Hilla downloads all dependencies and builds a frontend bundle.
The following builds do not download dependencies and builds are much faster.
...
When you define an `@Endpoint`, Hilla creates the needed REST-like endpoints, secures them, and generates TypeScript interfaces for all the used data types and public methods.
...
Create a new `TodoEndpoint.java` file in `src/main/java/com/example/application`:
...
You can learn the basics of Lit in the <<../application/lit#, Lit basics article>>.

Next, create a view for adding and viewing todo items.
...
<2> A Hilla `Binder` is used to handle the form state for creating new Todos.
...
Define some paddng on the view. 
Web Components are have a default `display` value of `inline`, which is rarely what you want. 
...
Define a `render()` method that returns a `html` template literal inside the class.
...
Below the `render()` method in the `TodoView` class, add a `connectedCallback()` https://lit.dev/docs/components/lifecycle/[lifecycle callback] to initialize the view when it is attached to the DOM.
...
Below the connectedCallback(), add another method to handle the creation of new `Todo`.
...
If you get stuck or need help, please reach out to the Hilla community:

- https://vaadin.com/forum[Hilla Forum]
- https://discord.gg/vaadin[Hilla Community Discord]
