...
For instance, do not store a separate `contactCount` value, but compute it instead, based on the contacts array.
...
It is best to keep business and application state separate for easier testing and reuse.
...
This way, MobX does not create proxy objects for all objects.
Also, you do not need the [methodname]#initFromServer()# method to be turned into an action.
...
If you do not have the development server running, start it with the `mvn` command from the command line.
