...
This can be done with the command `npx web-push generate-vapid-keys`, which gives output looking something like
...
Vaadin provides the `@PWA` annotations, that automatically generate the required PWA resources. Add the `@PWA` annotation on [classname]`Application.java` as follows:
...
In cases where there exists a subscription on the client for the application, but it has been lost for the server, it can be gotten from the service worker.
...
Sending the message to all registered subscriptions using the `notifyAll()` method:
...
For using `sendNotification` the correct user subscription is needed and the message needs to be in the format:
