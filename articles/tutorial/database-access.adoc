...
In this chapter, you connect the view to the backend to display and update data.
...
The starter you downloaded contains the _entities_ and _repositories_ you need, along with some sample data loaded using `src/main/resources/data.sql` file.
...
Instead of accessing the database directly from the view, you create a Spring Service.
The service class handles the application's business logic and, in larger applications, it often transforms database entities into Data-Transfer Objects (DTO) for views.
In this tutorial, you create a single service that provides all the methods you need.
...
Here, you check that you aren't trying to save a `null` object.
...
You don't need to implement the method.
Spring Data provides the implementation based on the query.
