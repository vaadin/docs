...
description: Learn to set up a parent layout and navigate among child views.
---
...
<6> Sets [methodname]`setHighlightCondition(HighlightConditions.sameLocation())` to avoid highlighting the link for partial route matches. Technically, every route starts with an empty route, so without this, it would always appear as active, even though the user isn't on the view.
...
In the next chapter, you'll secure the application by adding a log-in screen.
