...
<2> Instead of styling the layout with raw CSS, we are using some <<{articles}/styling/lumo/utility-classes#,Lumo Utility Classes>> shipped with the default theme.
...
<5> Calls the service to get all companies, then creates a [classname]`DataSeriesItem` for each, containing the company name and employee count. The method don't exist at this point, we'll be adding it next.
...
----
<1> The Formula is a Hibernate feature that allows to specify SQL snippets to fetch special fields. Here we get the count of employees without needing to fetch all the employees. Note that in a bigger application you probably want to do this in some alternative way as all Company entity loads now execute an additional SQL query, even though the employeeCount is only needed in this DashboardView.
