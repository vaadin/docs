...
description: Learn how to compose reusable components with a basic layout and components.
---
...
This chapter covers creating a new component, as well as importing and using a custom component.


== Components Using Composition

Vaadin Flow is a component-based framework. In the previous pages of this tutotorial, you worked with several components, like `Grid`, `TextField`, and `VerticalLayout`. However, the real power of the component-based architecture is in the ability to create your own components.
...
The form component you will create will have text fields for the first and last name, an email field, and two select fields: one to select the company, and another to select the contact status.
...
----
<1> `ContactForm` extends `FormLayout`: a responsive layout that shows form fields in one or two columns, depending on viewport width.
...
The next step is to add the form you just created to the main view. To do this, amend `ListView` as follows:
...
----
<1> Creates a field for the form so you have access to it from other methods later.
...
<5> Initialize the form with empty company and status lists: you'll add these in the next chapter.
