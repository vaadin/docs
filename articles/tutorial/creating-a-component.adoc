...
description: Shows how to compose reusable components with a basic layout and components.
---
...
This page covers creating a new component, as well as importing and using a custom component.


== Components Using Composition

Vaadin Flow is a component-based framework. On the previous pages of this tutotorial, you worked with several components, like `Grid`, `TextField`, and `VerticalLayout`. However, the real power of the component-based architecture is in the ability to create your own components.
...
The form component you'll create will have text fields for the first and last name, an email field, and two select fields: one to select the company, and another to select the contact status.
...
----
<1> Creates a field for the form so later you will have access to it from other methods.
...
<5> Initialize the form with empty company and status lists: you'll add these on the next page.
