...
In this chapter, you learn core Vaadin concepts and scaffold the first view of the Custom Relationship Management (CRM) application.
...
Vaadin includes over <<{articles}/components#,40 UI components>> to help you build applications faster.
...
<10> Adding some class names to components makes it easier to style the app later using CSS.
...
You need to restart the server in a only couple of cases; these are highlighted in the instructions.

You should now see the empty view structure in the browser window.
In the next chapter, you build a component for the form that's used for editing contacts.
