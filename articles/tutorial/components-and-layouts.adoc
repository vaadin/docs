...
In this chapter, you learn core Vaadin concepts and scaffold the first view of the Custom Relationship Management (CRM) application. It covers Vaadin component basics and constructing a view with the Vaadin Flow Java API.


== Basic Elements

Before getting into the details of creating a view, it's important to understand the basic elements  that will be used.
...
Vaadin components are custom HTML elements that are registered with the browser. They're based on W3C web component standards. The components have light and dark themes, that can be customized with CSS variables to fit your brand.
...
Layouts determine how components display in the browser window. The most common layout components are `HorizontalLayout`, `VerticalLayout`, and `Div`. The first two set the content orientation as horizontal or vertical, whereas `Div` lets you control the positioning with CSS.

`HorizontalLayout` and `VerticalLayout` provide methods to align items on both the primary and the cross axis. For example, if you want all components, regardless of their height, to be aligned with the bottom of a `HorizontalLayout`, you can set the default alignment to `Alignment.END`.  
...
One unique Vaadin Flow feature is that you can build web applications entirely in Java, removing you from having to write common HTML. This higher level of abstraction makes development more productive and debugging easier.

Vaadin does supports HTML templates and customizing the code that runs in the browser. However, you don't usually have to worry about this.


== The Contact List View

The first view is the Contact list view. You can see how it looks in the screenshot. It lists all of the contacts. Users can search, add, edit, and delete contacts in this view.

You focus initially only on the list view. You'll add a layout containing the header and sidebar later in the "Navigation and Parent Layouts" chapter.

In this and the next chapter, you will create the required layouts and components for the view. Then, in the chapter that follows, you will create a service class for accessing the backend and populating the view with data.
...
Below are comments about the code above, with the numbers associated with the circled numbers at the end of various lines of code.
...
<8> Configure the search field to fire value-change events only when the user stops typing. This way you avoid unnecessary database calls, but the listener is still fired without user leaving the focus from the field.
...
You can keep the server running throughout this tutorial. You only need to restart the server in a couple of cases. These are highlighted in the instructions.

You should now see the empty view structure in the browser window. In the next chapter, you will build a component for the form that's used for editing contacts.
