...
public class ListView extends VerticalLayout { // <1>
    ...
    private HorizontalLayout getToolbar() {
        ...
        Button addContactButton = new Button("Add contact"); // <9>

        var toolbar = new HorizontalLayout(filterText, addContactButton); // <10>
        toolbar.addClassName("toolbar"); // <11>
        return toolbar;
    }
}
----
...
<9> Add a button to the toolbar. NOTE: This button will be used later on in the tutorial for handling view state.
<10> The toolbar uses a `HorizontalLayout` to place the `TextField` and `Button` next to each other.
<11> Adding some class names to components makes it easier to style the application later using CSS.
