...
The [classname]`ContactForm` API is designed to be reusable; it's configurable through properties and it fires the necessary events. Thus far, you've passed a list of companies, the status, and the contact to the form. However, you need the application to listen for the events to complete the integration.
