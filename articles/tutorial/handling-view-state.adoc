...
description: Learn how to communicate among components in Vaadin Flow.
---

= Passing Data & Events among Vaadin Components

In the previous chapter, you created a reusable form component to edit contacts. In this chapter, you'll connect it to the rest of the view and manage the view state.
...
Next, you'll build the application. You should be able to select contacts in the grid and see them in the form. However, none of the buttons will work yet.
...
To make the layout responsive and usable on both mobile and desktop, you need to add CSS. To do this, add the following CSS to [filename]`frontend/themes/flowcrmtutorial/styles.css`:
...
The CSS media query hides the grid and the toolbar when you're editing contacts on a narrow screen.
...
The [classname]`ContactForm` API is designed to be reusable; it's configurable through properties and it fires the necessary events. So far, you have passed a list of companies, the status, and the contact to the form. However, you need the application to listen for the events to complete the integration.
...
----
<1> The save event listener calls [methodname]`saveContact()`. It does a few things:
.. Uses `contactService` to save the contact in the event to the database;
.. Updates the list; and
.. Closes the editor.
<2> The delete event listener calls [methodname]`deleteContact()`. In the process, it also does a few things:
.. Uses `contactService` to delete the contact from the database;
.. Updates the list; and
.. Closes the editor.
<3> The close event listener closes the editor.

Build the application and verify that you're now able to select, add, update, and delete contacts.
