...
description: Shows how to use Binder to bind a data object to input fields and validate input.
---

= Vaadin Forms: Data Binding & Validation

On the <<./creating-a-component#,Create a Component>> page, you created the input fields and buttons that you need for editing contacts. On this tutorial page, you will bind those inputs to a `Contact` object to create a fully functional form with validation.

This page covers creating a Vaadin Binder, binding input fields, and field validation.
...
<2> Calls `binder.readBean()` to bind the values from the contact to the UI fields. The `readBean()` copies the values from the contact to an internal model. That way you don't overwrite values if you cancel editing.
...
There are a few events you can fire: `SaveEvent`; `DeleteEvent`; and `CloseEvent`. To define new events, add the following code at the end of the `ContactForm` class:
...
On the next page of this tutorial, you'll connect the form to the list view to complete the first view.
