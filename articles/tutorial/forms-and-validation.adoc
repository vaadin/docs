...
In the <<./creating-a-component#,Creating a Component>> chapter, you created the input fields and buttons that you need for editing contacts. In this chapter, you will bind those inputs to a `Contact` object to create a fully functional form with validation.

This chapter covers creating a Vaadin Binder, binding input fields, and field validation.
...
There are a few events you can fire: `SaveEvent`; `DeleteEvent`; and `CloseEvent`.

To define new events, add the following code at the end of the `ContactForm` class:
...
Build the project and verify that it compiles. However, there won't be any visible changes yet.

In the next chapter, you'll connect the form to the list view to complete the first view.
