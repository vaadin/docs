...
All Vaadin Pro tools and components are free for students through the https://education.github.com/pack[GitHub Student Developer Pack]. If you need OSS alternative for TestBench, you can approach similar results also with plain https://www.selenium.dev[Selenium WebDriver] or https://playwright.dev[Playwright].
====
...
It can be used alone if you orchestrate starting and stopping your server, with for example a Maven exections in `pre-integration-test` and `posot-integration-test` and execute the actual test in integration test phase.
In this example, we'll use the same @SpringBootTest annotation we used in the previous phase to ensure we have a running server during the browser test execution.
...
----
<1> This annotation specifies the test to be run on the local machine and using Firefox. 
The default is Chrome, which is fine for most users, thus we have commented out this line
<2> The super class `BrowserTestBase` provides handy helper methods and configures TestBench for us automatically.
<3> The `openBrowser` method is annotated to be executed before each actual tests.
In this test case we always open the localhost at the random port SpringBootTest has set up for us.
The browser should be automatically redirected to the login screen.
<4> BrowserTest annotation is an special version of the better known Test annotation, that will be handy if you decide to extend your end-to-end tests to cover multiple browsers at some point.
