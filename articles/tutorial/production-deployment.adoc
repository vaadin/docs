...
[TIP]
From the Cloud provider's point of view, Vaadin is a standard Java Web application. You can deploy your application onto almost any cloud platform, in many different ways. Read the <<{articles}/production/cloud-providers#, Cloud Deployment tutorials>> for more options.
...
[TIP]
If you're not located in North America, you will get a better experience with Azure by choosing a region close by. For example, a Europe based developer can do this with `az config set defaults.location=westeurope` or pick the location per application. Pick a location that already https://azure.microsoft.com/explore/global-infrastructure/products-by-region/?products=container-apps&regions=all[supports Container Apps].

Docker image is a basic building block used by many modern hosting solutions to run applications.
The example project contains a simple ready made `Dockerfile` that essentially describes how the jar file build in the previous step should be run.
...
Note that the horizontal scaling doesn't work yet with Vaadin applications -- session affinity for Container Apps ingress is currently in development. However, you can configure the node to be a larger one to scale up your application.
...
If you had any problems or were confused by any part, please write to us at Vaadin on Twitter https://twitter.com/vaadin[@vaadin] or https://discord.gg/vaadin[Vaadin's Discord chat] server.
