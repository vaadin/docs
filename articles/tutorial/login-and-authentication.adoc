...
public class SecurityConfig extends VaadinWebSecurity { // <2>
    ...
    public InMemoryUserDetailsManager userDetailsService() {
        return new CrmInMemoryUserDetailsManager(); // <5>
    }
}
----
<1> Enable Spring Security.
<2> Extend from [classname]`VaadinWebSecurity` to configure Spring Security for Vaadin.
<3> Allow public access to the image directory.
<4> Allow access to the login view.
...
Verify that you cannot access http://localhost:8080/dashboard without being logged in.
You can log in with:
...
In the next chapter, you will learn how to turn it into a PWA to make it installable on mobile and desktop.
