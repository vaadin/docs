...
order: 100
page-title: Setting up Authentication & Login in Vaadin Flow
...
In this chapter, you'll secure the Customer Relationship Management (CRM) application by setting up Spring Security and adding a login screen to limit access to logged-in users.
...
Stop and restart the server to pick up the new Maven dependencies. You should now be able to log in and out of the app. Verify that you can't access http://localhost:8080/dashboard without being logged in. You can log in with the username, `user`, and the password, `userpass`.

image::images/login/logout-button.png[Log out button on page]

You have now built a full-stack CRM application with navigation and authentication. In the next chapter, you'll learn how to turn it into a PWA to make it installable on mobile and desktop platforms.
