...
GitHub runs Vale when you make a pull request in the Vaadin Docs repository, so you don't necessarily need it, but running it locally shortens your feedback loop.
...
It's only recommended for small fixes, as the preview process is slow.
...
* _Style sheet_ for the documentation site isn't applied and hence emphasis
and other custom formatting isn't shown in the preview
* _Conditional directives_ don't work, because attributes aren't defined,
and hence text intended only for web isn't shown at all in the preview
* _Macros_ don't work, for example menu selection macros
* _Cross-references_ don't point to correct paths in the preview
...
See the https://docs.errata.ai/vale/about[Vale documentation] for instructions on installing it on macOS, Windows, or Linux.
...
[discussion-id]`1D5FF345-19F2-4F7B-836F-DA92F3CCD3F6`
