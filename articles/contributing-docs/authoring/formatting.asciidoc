...
AsciiDoc doesn't care about line breaks, only paragraph breaks.
...
Deleting or commenting out a sentence is also easier.
...
It's also a good idea to keep sentences short, so that they fit one line each. +
...
For pages in the same folder or product, it's most convenient to use relative paths.
...
This is recommended in task lists with screenshots, and in other cases where a caption isn't needed and an image frame might be distracting.
...
An image caption shouldn't be in title case.

References to images use the caption text in the reference.
However, it's used differently in Vaadin Docs and the print edition.
...
Make sure the white background doesn't get transparent when taking screenshots.

_Don't use transparency in images_::
...
Text in diagrams shouldn't be much smaller than this.
If text is less than half of this, it's unreadable.
+
...
The full listing doesn't show the lines marked with `// hidden-source-line` when you toggle *Expand code*.
...
The TypeScript example doesn't need to be functional, just contain the needed components.
...
[discussion-id]`688226A1-B5C3-45EB-9505-19D10956360A`
