...
// Many examples use 'I' or 'me', and 'we' or 'our'
...
== Abbreviations
Don't terminate abbreviations with a period if the last letter of the unabbreviated form is included in the abbreviation.
For example:
...
Don't use apostrophe _s_ (_`'s_) for this purpose. +
For example:
...
[example]
====
_**Unfortunately,** we can't reproduce the bug._ +
...
[[grammar.apostrophe]]
== Apostrophe (`')
The apostrophe has a specific function: to show when one or more letters have been omitted.
For that reason, it's used in contracted forms of verbs.

However, to show the function of the apostrophe in contractions in general, here are some examples:
...
+++[+++A little illogical, but it's the accepted contraction.+++]+++
...
The other standard use of the apostrophe in English is to signify possession.
...
* in the few irregular cases where the plural form doesn't end in _s_ (for example, _men_, _women_, _children_, _sheep_), add _`'s_, as for a single noun
...
Notice that the phrase that's in apposition is delimited by commas.
...
It's likely that there is only one _Susan Rae_ in this context, but there are, no doubt, many company employees. In other words, _Susan Rae_ isn't another name for _company employee_.

Hence, _Susan Rae_ is *not* in apposition with _company employee_ and, for that reason, isn't delimited by commas here.

Let's look at an example that's more relevant to the context of technical documentation. +
...
Here, _parameter_ and _duration_ don't mean the same thing.
For this reason, _duration_ isn't in apposition with _parameters_, so it's *not* delimited by commas.

== Articles
Missing and misused articles (_a_, _an_, _the_) are a very common problem, especially for speakers of languages which don't have articles, such as Finnish, Russian, and Japanese.
...
* https://www.grammarly.com/handbook/grammar/articles/[Grammarly Handbook] about the use of articles (in English)

* https://englantia.wordpress.com/grammar/yksikko-monikko-ja-artikkelit/[Yksikkö, monikko ja artikkelit] (in Finnish)

* https://materiaalit.internetix.fi/fi/kielet/englanti11/e1_lesson2.htm[Articles] (in Finnish)
...
However, contractions aren't generally appropriate in formal contexts, such as academic works and legal documents.
...
We view some other entities as existing in bulk, as amounts of something, rather than many separate objects that we can count. +
For example, we apply the label _air_ to a contiguous body of gaseous substances. It doesn't usually make sense to talk about _one air_ or _two airs_. +
The same applies to entities such as _sugar_, _water_, and aluminum. +
We call this category of entities _uncountable_.

From the point of view of grammar, it's important to consider whether some entity is countable. +
...
Uncountable nouns, again by definition, can't be plural.

However, there is a small trap here. +
Many nouns can't be categorically defined as either countable or uncountable; it depends on their meaning in the given context.
...
====
However, _time_ is countable when it means an occasion or a period. +
For example:
...
We know because *we assume that the next noun phrase after the participle clause denotes the person or thing that the participle refers to*.

Look at these similar sentences:

====
Being corrupt, the file was rejected. +++[+++Since the next noun phrase after the participle phrase is _the file_, it's clear that it's the file that's corrupt.+++]+++
...
Usually with such poorly chosen word order, we can probably guess at the intended meaning.
...
By contrast, in a _passive_ sentence, the subject of the verb is *not* the _agent_; that is, it isn't the person or thing that performs the verb. +
...
In a _passive_ sentence, the _agent_ is denoted using the preposition _by_. +
...
- because we don't know who or what the agent is
- because it's unimportant who or what the agent is.
...
Unfortunately, this may lead us to wonder who _they_ (or _people_) are, even though in this case it's unimportant. +
...
Accordingly, avoid using the _passive_ when it's possible to express the same idea elegantly and simply in _active voice_.
...
*it's neater to say:* _I asked Linda Johnson, who works in the same office as me_.
====
...
It gives us essential information to identify the person or thing that was mentioned.
...
+++[+++The relative clause is essential to understand which bug is being talked about.+++]+++
...
[example]
====
It's necessary to *fully* understand the process before starting. +++[+++Instead of, for example, _to understand the process fully_.+++]+++
...
Clearly, we need to prioritize simplicity, clarity, and accuracy always, even if it means we have to compromise on elegance.
...
In English, we generally use a present or present perfect tense in this type of clause, although it refers to a future time.
The rest of the sentence may use any appropriate future form, or an imperative (instruction) form.
For example:
...
[discussion-id]`8BD95586-0E8D-4D5A-99B9-583629D4750D`
