...
== Em Dash ("&mdash;")
In U.S. English, a pair of _em_ dashes (so called because they have the same width as the letter _M_) are used to indicate a fragment of text in parenthesis. The pair of _em_ dashes imply a more significant break in the structure of the sentence than one marked by a comma or brackets.
...
In cases where items in the list themselves contain commas, it's better to use semi-colons as item separators, in order to avoid confusion. +
For example:
