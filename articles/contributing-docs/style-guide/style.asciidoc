...
== Ampersand ("&amp;")
Don't use the ampersand character in place of _and_ in general text, unless space is limited.
...
+++[+++This could mean that the text field has both a caption and an input box; or that the text field has a box that's for both the caption and for input.+++]+++
...
+++[+++It's unclear whether the adjective _nested_ applies to both _field_ and _layout_, or only to _field_.+++]+++
...
== Exclamation Mark ("!")
Avoid using exclamation marks in technical documentation, unless it's as part of some code syntax.
...
== Jargon
Avoid using jargon.
Always try to use inclusive language.

== Latin Abbreviations

Don't use the following Latin abbreviations, but rather write them in English:

pass:[<!-- vale Vaadin.NoteThat = NO -->]

e.g.::
Rather use expression such as _such as_, _for example_, or _for instance_.
+
Note that _for example_ https://www.myenglishteacher.net/forexample.html[always requires] surrounding commas, while _such as_ only requires preceding comma when it's used in the beginning of a restrictive clause.
+
[example]
====
...
etc.::
This abbreviation is sometimes fine to use, but you are still encouraged to use expressions such as _and so forth_. If used, it should be preceded by comma and followed by period.
+
[example]
====
...
Write out a number if it's an approximation, rather than an accurate figure.
For example:
...
Don't use _1st_, _2nd_, _3rd_, etc.
...
== Plurals
Don't use _s_ in parentheses to indicate that there may be one or more of something.
== For example
...
Instead, choose an alternative wording, even if it's slightly longer.
For example:
...
== Plus
Don't use _plus_ as a synonym for _and_.
...
_the office door_
====
Notice that, in the last group of examples, the noun that's used as a descriptor is always singular, even if the word it governs is plural.
For example:
...
[[style.proper_nouns]]
== Proper Nouns
A _proper noun_ is a word or phrase that's the name of something or somebody; for example, _Chrysler Building_, _Czech Republic_, _Indian Ocean_, _Pink Floyd_, _Windows 11_.

It's distinct from a _common noun_, which is the "standard" type of noun that identifies the nature of something or somebody; for example, a _building_, a _republic_, an _ocean_, a _band_, an _operating system_.
...
If you aren't sure of the correct form, an internet search is usually enough to find it.

pass:[<!-- vale Vaadin.ThereIs = YES -->]

== Scanning
_Scanning_ is the process by which a reader quickly overviews the text to evaluate the content.
The reader does this to find out whether it really does contain the information that they are looking for.
...
While the technical details are taken care of by the framework, it's important to understand how to design the application so that the server responds to client requests in a secure way.
====
...
One reason is that it detracts from the professional style that we're trying to convey with our documentation.
...
Don't use the slash character to write fractions, such as _1/2_ or _3/4_, as these may be liable to misinterpretation.
...
Clearly, we always need to prioritize simplicity, clarity, and accuracy, even if it means we have to compromise on elegance.
...
It's very common to use a compound expression with a numeric value and units as a descriptive phrase.
...
== Vaadin Versions
Don't use _Vaadin 14_ or other Vaadin version numbers in text.
Instead, use the `[role="since:com.vaadin:vaadin@V19"]` tag to indicate version numbers.

== X
Don't use the character _x_ as a multiplication sign.
Instead, use the multiplication symbol &times; (`\&times;` in AsciiDoc).


[discussion-id]`3F7413BE-4CA8-4D13-A48F-813A00DCB6DB`
