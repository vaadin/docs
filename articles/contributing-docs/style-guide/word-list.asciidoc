...
// Many examples use 'I' or 'me', and 'we'
...
=== "above"
Don't use _above_ to indicate the position of an item on a page.
...
It's better to use a clearer term, such as _platform-independent_.
...
=== "America"
Don't use _America_ or _American_ as synonyms for _United States_ or _US_.
...
=== "approximately"
Write this in full; don't use [line-through]#_approx_#.
...
It isn't correct to say, _It looks like it`'s going to rain_, although this is a very common grammatical error.
...
Here, the noun phrase _rain_ doesn't contain a finite verb.
...
Instead, _like_ "expects" to be followed by a noun phrase (that is a phrase that has the grammatical function of a noun). +
For example:
...
=== "below"
Don't use _below_ to indicate the position of an item on the screen, as different devices and browsers may position elements differently.
...
=== "blacklist"
Don't use the term _blacklist_.
...
_**Right-clicking** on the drop-down presents the user with more options._
====
...
_Several command-line options are available._ +
+++[+++Adjective phrase+++]+++
====
...
It isn't correct to use _either_ where there is a choice between more than two things. +
...
=== "hang"
Don't use _hang_ to refer to a situation when a program stops responding. +
Instead, write _stops responding_.
...
[example]
====
_[line-through]#Hopefully#, the new version is released at the beginning of July._
====
...
*or* +
_It's hoped that the new version is released at the beginning of July._
====
...
Don't use _outdent_, as it's often unclear what exactly this means in a given situation.
...
_It's_ can mean _it is_ or _it has_; the context always tells us which meaning is intended.
...
+++[+++_It's_ must mean _it has_, since _It's been&hellip;_ makes equally little sense.+++]+++
====
...
+++[+++The verb _persist_ doesn't take an object.+++]+++
====
...
=== "post"
Don't use _post_ as a synonym for _after_. +
Instead, use _after_.
...
Don't use _hit_ in this context.
...
=== "(s)", "(es)"
Don't append _(s)_ or _(es)_ to a singular word in order to indicate that the item in question may or may not be plural. +
For example:
...
=== "ship"
Don't use _ship_ to refer to software releases. +
...
However, in non-defining relative clauses (see <<grammar#grammar.relative_clauses,relative clauses>>), we can't use _that_. +
For example:
...
[example]
====
_**[line-through]#While#** it's possible to write code using an ordinary text editor, using an IDE has significant advantages._ +
_**Although** it's possible to write code using an ordinary text editor, using an IDE has significant advantages._
