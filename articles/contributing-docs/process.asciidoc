...
* A writing process is typically just as iterative as a software development process.
 ...
 It's therefore good to have some time between revisions, so that you can forget what you have written and then reread it like someone else would.
...
This doesn't imply a reversed waterfall model, but just that you write the documentation for the code you are about to write first.
...
API Documentation gives a more concrete and bottom-up approach. Nevertheless, for some features, such as where a class _is_ a feature (you can think of a server-side Vaadin component), there isn't much difference in the viewpoint of the class documentation and its description in the user manual.
