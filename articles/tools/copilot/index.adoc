...
|User is asked before first operation if AI usage is allowed. That can be changed later using Vaadin Activation button image:images/activation-button.png[Activation,26] Settings > AI Usage.

|Command Window prompt submitted
|Prompt, AI model request and response, 30 days retention
|In https://vaadin.com/profile[Profile Settings] on your Vaadin.com account

|User activity
|Telemetry
|Change `vaadin.devmode.usageStatistics.enabled` system property value.

|Copilot error happened
