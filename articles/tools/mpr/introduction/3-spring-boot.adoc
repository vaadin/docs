...
When porting the UI to a flow component, you lose the ability to use UI methods, such as `setErrorHandler`. You can still access those methods by using `UI.getCurrentOrThrow()`. The method `setContent` isn't supported, though. You should use instead the `add` method from your Flow layout.
