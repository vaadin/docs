---
title: Running a Vaadin Legacy CDI Application with MPR & Flow
order: 7
layout: page
---
...
You will need to remove any version from `com.vaadin:vaadin-cdi` since the compatible version for it is managed by the `vaadin-bom`. Instead, the `pom.xml` file should look like this:
