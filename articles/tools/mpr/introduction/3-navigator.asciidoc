...
the `Navigator` we navigate to a corresponding Flow view if available.
...
Then the `navigator.addView("away", AwayView.class);` configuration in the previous
...
Now, there is no need to setup a `Navigator` and the View still receives a `ViewChangeEvent`
