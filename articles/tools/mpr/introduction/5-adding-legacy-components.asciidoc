...
We highly recommend that the new views added to the application follow Flow's routing system. In Flow, "Views" are called "RouteTargets" and are managed by primarily by the `@Route` annotation. For details on the differences in navigation between Flow and previous Vaadin versions, see the <<{articles}/upgrading#,Upgrading Guide>>.
