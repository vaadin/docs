...
You're also able to use CDI beans, e.g. `SessionScoped` beans, in both V14+
...
4. Add the `vaadin-cdi` dependency. The version isn't needed as it's defined
by the `vaadin-bom`.
+
[source,xml]
----
...
one project. To navigate from Vaadin 7 part to Vaadin 14 part you can
