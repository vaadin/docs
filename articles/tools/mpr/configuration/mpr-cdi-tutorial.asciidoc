...
step-by-step guide on how to adopt this approach.
...
<dependency>
    ...
    <!--
        At the moment mpr-cdi-v7 is in pre-release stage and you need to use
...
+
[NOTE]
Since, a pre-release version of a Vaadin product is being used you need to add
the `vaadin-prereleases` repository to your `pom.xml`, like this:
+
[source,xml]
----
<repositories>
...
----
+
At the moment `mpr-cdi-v7` is in pre-release stage and you may need to use e.g.
version `1.0.0.alpha1`.

4. Add the `vaadin-cdi` dependency. The versions is not needed as it is defined
by the `vaadin-bom`.
+
[source,xml]
----
