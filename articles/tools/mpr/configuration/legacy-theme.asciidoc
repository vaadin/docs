...
[NOTE]
Legacy theme is applied only when the view have legacy components. Thus it is
recommended that, CSS which is applied to non-legacy components, is not being included
in the legacy theme.
