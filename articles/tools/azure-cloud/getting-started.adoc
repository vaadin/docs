...
page-title: Getting started with Azure for Vaadin
description: Step-by-step guide showing how to use the Azure Cloud Kit to deploy an application to Azure Cloud.
meta-description: Learn how to set up and deploy Vaadin applications on Azure cloud platform effectively.
order: 10
---
...
To run the script, open a terminal window and navigate to the folder where you extracted the Azure Cloud Kit. Then run the following command:
...
To get the ingress IP address, execute the following from the command line:
