...
Assuming you're satisfied with the output from `terraform plan`, you can run `terraform` with the `apply` option. This deploys your application to Azure. It'll create all of the resources that are defined in the [filename]`variables.tf` and [filename]`settings.tf` files.
