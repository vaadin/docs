...
[classname]`UIUnitTest` base class is able to get the instantiated view, but child components may not be always accessible directly.
...
To overcome this limitations, [classname]`UIUnitTest` provides a query functionality, that lets you scan the UI and filter for the components you need to interact with in test methods.
Query object can be obtained by calling the [methodname]`select()` method, providing the type of the component you are searching for.
You can also restrict search scope to the children of current view, using the [methodname]`selectFromCurrentView()` method.
...
Example of terminal operators are [methodname]`first()`, [methodname]`last()`, [methodname]`atIndex()`, [methodname]`all()` and [methodname]`id()`.

.Using Query API to get component wrappers
[source,java]
----
// Get a wrapper for the first TextField in the UI
TextFieldWrap nameField_ = select(TextField.class).first();

// Get a wrapper for the first TextField in the current view
TextFieldWrap nameField_ = selectFromCurrentView(TextField.class).first();

// Get a wrapper for the first TextField with the given label
TextFieldWrap nameField_ = selectFromCurrentView(TextField.class)
...
TextFieldWrap<TextField, String> textField = selectFromCurrentView(TextField.class)
...
Sometimes you may need to query for components nested inside the UI, in a hierarchy composed by many different types of components.
To simplify such situations, the query object offers methods to chain a new query starting on a found component, so that complex query can be created in a fluent way.
The [methodname]`thenOn()` method and its variants, for example [methodname]`thenOnFirst()`, provides you a new query object for the given component type, setting the search scope to the component selected from current query.
...
TextFieldWrap<TextField, String> button = selectFromCurrentView(VerticalLayout.class)
