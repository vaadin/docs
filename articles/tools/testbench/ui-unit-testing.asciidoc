...
In a [classname]`UIUnitTest` class, wrapping a component can be done with `wrap(component)` or `wrap(Wrapper.class, component)`.

- `wrap(component)` will return a component specific wrapper if one can be determined for the given component or the [classname]`ComponentWrap` generic wrapper.
