...
For information about testing with Spring or Spring Boot, consult the <<{articles}/tools/testbench/ui-unit-testing/spring-tests#, Spring specific documentation>>.
...
In a [classname]`UIUnitTest` class, obtaining a tester for a component can be done with [methodname]`test(component)` or [methodname]`test(Tester.class, component)`.
