...
If the annotation is used without providing [methodname]`classes()` and [methodname]`packages()`, it acts a shortcut for using the test class package

Using the annotation without providing [methodname]`classes()` and [methodname]`packages()`, acts as a shortcut for restricting scan to current test class package and sub-packages.
