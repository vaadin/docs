...
Authentication details are available before creating the UI instance and navigating to the default root, so redirects to login view does not happen, when simulating logged in users.
