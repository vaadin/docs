...
AppSec Kit isn't enabled in production mode. Therefore, you'll need to start your application in development mode -- which is the mode in which applications start by default. You don't need to enable this, explicitly.
...
You should configure https://vaadin.com/docs/v14/flow/advanced/tutorial-push-configuration[server push] for a `View` so you'll be notified when new vulnerabilities are found. You can do this by adding the `@Push` annotation to a View class like so:
