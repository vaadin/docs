...
All Vaadin UI components and listeners are already marked as serializable. So if a class extends or implements one of them, it's not necessary to add explicitly the `Serializable` implementation statement. However, it's necessary to do so on non-UI components that may be referenced in the UI graph.
