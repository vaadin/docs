...
Restrictions can be imposed by configuring `vaadin.serialization.transients.include-packages` and `vaadin.serialization.exclude-packages` properties, to limit the inspection to classes whose packages matches the configured rules.
