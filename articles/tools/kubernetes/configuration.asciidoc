...
When a session is serialized, Kubernetes Kit inspects classes for transient fields that can be injected during deserialization (see <<{articles}/tools/kubernetes/session-replication#,session replication>>).
