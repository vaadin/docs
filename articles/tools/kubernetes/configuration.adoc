...
| Enables or disables the Kit autoconfiguration
| `true`
...
The `service-name` and `service-port` properties defines the endpoint of the Hazelcast service deployment within a cluster. The `service-port` defaults to `0`.
