...
page-title: How to debug Kubernetes Kit Session Replication | Vaadin
...
meta-description: Learn how to debug session replication in Kubernetes deployments with Vaadin's Kubernetes Kit Session Replication Debug tool.
order: 110
---
...
[source,java]
----
<source-info group="Spring (no Spring Boot)"></source-info>
@Configuration
class AppConfig {
...
<source-info group="Plain Java"></source-info>
@WebListener
public class SerializationDebugToolFilterRegistration implements ServletContextListener {
    @Override
    public void contextInitialized(ServletContextEvent sce) {
        var registration = sce.getServletContext()
            .addFilter(SerializationDebugRequestHandler.Filter.class.getName(),
                       SerializationDebugRequestHandler.Filter.class);
        registration.setAsyncSupported(true);
