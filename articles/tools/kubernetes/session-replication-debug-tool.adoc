...
* a Vaadin [classname]`RequestHandler` that evaluates if the HTTP request produces UI changes and prepares the tester execution and the result notification on the UI notification, if server push is enabled;
...
In addition, the HTTP filter must be registered, either as a Spring bean or with a [classname]`ServletContextListener`.
