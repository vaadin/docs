...
The debug tool is composed by an HTTP session replication tester, engaged by a Vaadin [classname]`RequestHandler`, which tries to serialize and deserialize the HTTP session for every user action on the browser that triggers a server request. It works only on `development mode` and only if session serialization is enabled by setting the following configuration property:
...
It indicates that the method is [methodname]`measurePageLoadPerformance` in [classname]`DashboardView`.
