...
* a Vaadin [classname]`RequestHandler` that evaluates if the HTTP request produces UI changes and prepares the tester execution and the result notification on the UI notification, if PUSH feature is enabled
* an HTTP filter that runs the tester after the HTTP request has been processed

The debug tool works only in `development mode` and only if the following preconditions matches:

* `vaadin.devmode.sessionSerialization.enabled` application configuration property is set to true
* `sun.io.serialization.extendedDebugInfo` system property is set to true
* JVM is started with `--add-opens java.base/java.io=ALL-UNNAMED` flag

The `extendedDebugInfo` system property and the `--add-opens` instruction are required to offer as much detailed information as possible.
...
.Spring Boot Maven Plugin CLI
[source,terminal]
----
mvn -Dspring-boot.run.jvmArguments="-Dsun.io.serialization.extendedDebugInfo=true --add-opens java.base/java.io=ALL-UNNAMED" spring-boot:run
----

.JAR execution
[source,terminal]
----
java -Dsun.io.serialization.extendedDebugInfo=true --add-opens java.base/java.io=ALL-UNNAMED -jar target/myapp.jar
----
--
...
The debug tool is installed automatically on Spring Boot projects. Otherwise, it can be installed manually by configuring the provided [classname]`VaadinInitListener`, either by adding an entry in `META-INF/services/VaadinServiceInitListener` or by defining a bean if using Vaadin Spring add-on.
In addition, also the HTTP filter must be registered, either as a Spring bean or with a [classname]`ServletContextListener`.
...
.Install on plain Java web project
--
.Service Loader entry in META-INF/services/VaadinServiceInitListener
[source,text]
----
...
For views directly or indirectly referencing unserializable objects, the classes causing issues are reported. Double check those classes, make them [classname]`Serializable` and then navigate to the view again to see if the issues are solved or if other problems have arisen.
