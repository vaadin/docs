...
The Design System Publisher is currently based on Gatsby which is known to have certain issues on Windows.
If you are using Windows, see https://www.gatsbyjs.com/docs/how-to/local-development/gatsby-on-windows/[Gatsby on Windows] for more information.

It's generally recommended to use a Linux or macOS environment for development.
On Windows, you can use https://learn.microsoft.com/en-us/windows/wsl/install[Windows Subsystem for Linux (WSL)] to run the Design System Publisher.
