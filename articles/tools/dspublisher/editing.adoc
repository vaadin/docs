...
The _Expand code_ button in the example reveals all the lines of the source code. This can be useful for making it easier for readers to see the most relevant part.
...
Alternatively, you can use the `DOCS_IMPORT_EXAMPLE_RESOURCES="true"` <<configuration#options,configuration option>> to load all front-end resources up front. Then it's sufficient to include the Java code example without an additional, or hidden TypeScript example. Add it as before the start-up or build command:
