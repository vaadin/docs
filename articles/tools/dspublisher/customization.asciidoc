...
A <<{articles}/styling/custom-theme#,custom application theme>> can also be applied, which affects the theme of rendered UI examples within the documentation pages.
...
See <<{articles}/styling/custom-theme#,Applying a Custom Application Theme>> for instructions how to apply a theme which is used in <<ui-examples#,UI examples>>.
