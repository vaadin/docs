...
Whenever changes to a TypeScript example are saved, Publisher rebuilds the example and automatically refresh the page after a couple of seconds.
...
The comment must be written in the format defined for the example's syntax highlighting.
...
Snippets must be enabled in the `include::[]` directive in the page content, by adding the `tags=snippet` parameter in the brackets following the file path:
...
Parts of the source code can be entirely omitted from the rendered code examples by appending a specifically formatted comment after the line.
...
[source,html]
----
include::{root}/frontend/demo/component/inputfields/input-field-label.ts[render,tags=snippet,group=TypeScript]
----

[source,java]
----
include::{root}/src/main/java/com/vaadin/demo/component/inputfields/InputFieldLabel.java[render,tags=snippet,group=Java]
----
--

This is done by wrapping multiple source code blocks into an `[.example]` block and specifying a name for each group using the `[group=...]` attribute:
...
Each group can contain multiple files by providing multiple source code includes with the same group parameter.
