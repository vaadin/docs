...
Upon successful authentication, the provider redirects the user back to the protected view. To access the authenticated user's information (such as name, email and roles), Flow provides the [classname]`AuthenticationContext` bean that can be used as a view constructor argument. Then you can get the authenticated user with [methodname]`getAuthenticatedUser(Class<U> userType)`. It returns an [classname]`Optional` object containing the user instance if the session has an authenticated user or is empty otherwise.
...
By default, the SSO Kit authenticated user will be an instance of [classname]`OidcUser`. If your application defines a custom user service to provide your own user type, you can use that type as the `userType` parameter.
