...
There are a few pre-requisites to using Keyclock with SSO Kit. First, you will need to get access to <<../#,SSO Kit or a trial license>>. You'll then need to clone the https://github.com/bwajtr/vaadin-sso-kit-keycloak-demo[demo application from GitHub], and you'll need to https://docs.docker.com/get-docker/[install Docker].
...
You will then be redirected to the User details page.
...
First, select `user` in the [guilabel]*Users* menu and then select the [guilabel]*Credentials* tab to create a new password. Choose [guilabel]*Set password* and just enter `user` as the password for `user`.
...
When you're finished, select [guilabel]*Save* to save and create the client. You will then be redirected to the [guilabel]*Client details* page.
...
To connect Keyclock to a Vaadin applicaiton, navigate to the [guilabel]*Credentials tab* in [guilabel]*Client details* and copy the [guilabel]*Client secret* to your clipboard.
...
For best practices, you should now test user authentiation. To do this, first, when your application is running, open http://localhost:8080 in your browser.
