...
Create a Vaadin project as described in <<{articles}/getting-started#, Getting Started>>, and add the Hilla Spring Boot starter to it as described in <<{articles}/hilla/guides/endpoints#enabling, Enabling Endpoints in a Vaadin Project>>. Without that dependency, no TypeScript clients are generated for browser-callable services, and the React views under [filename]`src/main/frontend/views` are ignored.

.Examples Refer to Starter Files
[NOTE]
The steps below refer to files from a starter project as examples: an endpoint named [classname]`HelloWorldEndpoint`, an About view, and a [filename]`MainLayout`. Apply them to the endpoints and views your own project has.
...
Add the `sso-kit-starter-hilla` module -- the flavor of the starter that comes with the endpoints and the client-side context that React views use -- to the [filename]`pom.xml` file of a Vaadin application like so:
...
SSO Kit registers its own endpoints as Spring beans, so TypeScript clients are generated for them together with your application's endpoints. No endpoint generator configuration is needed.
