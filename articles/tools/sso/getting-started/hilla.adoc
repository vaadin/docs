...
= [deprecated:com.vaadin:vaadin@V25.3]#Getting Started with SSO Kit (Hilla)#
:hilla-react:
...
Create a Vaadin project as described in <<{articles}/getting-started#, Getting Started>>, and add the Hilla Spring Boot starter to it as described in <<{articles}/hilla/guides/browser-callable-services#enabling, Enabling Browser-Callable Services in a Vaadin Project>>. Without that dependency, no TypeScript clients are generated for browser-callable services, and the React views under [filename]`src/main/frontend/views` are ignored.

.Examples Refer to Starter Files
[NOTE]
The steps below refer to files from a starter project as examples: a browser-callable service named [classname]`HelloWorldEndpoint`, an About view, and a [filename]`MainLayout`. Apply them to the services and views your own project has.
...
Add the `sso-kit-starter-hilla` module -- the flavor of the starter that comes with the services and the client-side context that React views use -- to the [filename]`pom.xml` file of a Vaadin application like so:
...
To try this feature, replace the `@AnonymousAllowed` annotation in [filename]`HelloWorldEndpoint.java` with `@PermitAll`. When you do this, unauthenticated users won't be able to access all service methods. You could also apply the same annotation at the method level for more fine-grained control.
