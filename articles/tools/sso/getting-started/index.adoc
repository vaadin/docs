...
[NOTE]
====
For React views and the services behind them, use the Hilla flavor of the starter instead. It provides the same auto-configuration, plus the endpoints and the client-side context that React views use, and its configuration properties are in the `hilla.sso` namespace instead of `vaadin.sso`:
...
The version of both starters comes from the `vaadin-bom` dependency management that the project already has.
...
The Hilla starter registers SSO Kit's own endpoints as Spring beans, so TypeScript clients are generated for them without any endpoint generator configuration.

.Older Project Layout
[NOTE]
The client-side steps in this section were written for a project where the routes are listed in a [filename]`frontend/routes.tsx` file and the application shell is in [filename]`frontend/App.tsx`. A project that uses file-based routing -- views under [filename]`src/main/frontend/views`, as described in <<{articles}/building-apps/react/add-view#,Add a React View>> -- has neither file, so these steps need to be adapted to it.
