...
[WARNING]
The kit's built-in Back-Channel Logout implementation, activated with `vaadin.sso.back-channel-logout=true`, is deprecated since SSO Kit 3.1. Don't enable it: Spring Security's filters are already active by default and cover the same feature.
