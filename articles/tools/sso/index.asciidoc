...
These properties can be added to your `application.properties` file where you define the provider URL and the client registration details, such as credentials and scope.
...
If there is no view defined for this route, Spring will auto-generate a page with links to each of the configured providers login forms.
...
[TIP]
====
Some providers support custom theme for their login pages, find out more in <<{articles}/tools/sso/theming, Theming>>.
====
...
To access the authenticated user's information (such as name, e-mail and roles), SSO Kit provides the [classname]`AuthenticationContext` bean that can be used as a view constructor argument.
Then you can get the authenticated user with the [methodname]`getAuthenticatedUser()`, which will return a [classname]`Optional` object containing the user instance if the session has an authenticated user or empty otherwise.
...
.Configuring the logout redirect route
[example]
--
.application.properties
[source,properties]
...
==== Back-Channel Logout
Back-Channel Logout is a feature that enables the provider to terminate user sessions from outside the application, for example from the provider's user dashboard or from another application.
