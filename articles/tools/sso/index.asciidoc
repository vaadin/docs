...
SSO Kit provides a `SingleSignOnConfiguration` bean to auto-configure Vaadin and Spring to
...
auto-configuration class by setting the `auto-configure` property to `false` and provide your own configuration class:
...
This configuration enables login for the identity providers defined in the
application configuration and instructs the application to accept
requests for the login route, which can be configured setting the `login-route` property:
...
You can set which views require authentication annotating them as described in https://vaadin.com/docs/latest/security/enabling-security#annotating-the-view-classes:[Annotating the View Classes].
For example:
...
SSO Kit provides two methods for logging out the user, the `RP-Initiated Logout` (https://openid.net/specs/openid-connect-rpinitiated-1_0.html:[spec]) and the `Back-Channel Logout` (https://openid.net/specs/openid-connect-backchannel-1_0.html:[spec]).

==== RP-Initiated Logout

RP-Initiated Logout enables the Relying Party to request that an End-User be logged out by the OpenID Provider by redirecting the End-User's user agent to the OpenID Provider's logout endpoint.

The `AuthenticationContext` bean provides the `AuthenciationContext::logout()` method to terminate both the local user session and the provider's session:
...
After a successful logout the user is redirected to the application's redirect route. We can set this by `logout-redirect-route` property:
