...
order: 10
description: Details on what is the observability kit agent and its use.
---
...
As an industry-wide standard, it's https://opentelemetry.io/vendors/[supported by a number of tools and vendors^]. We have tested and support <<{articles}/tools/observability/integrations/index.asciidoc#,integrations>> with a specific selection of vendors. Still, the agent should work with any vendor that supports the OpenTelemetry standard.

[NOTE]
OpenTelemetry is still a relatively new standard. This means support from specific vendors may still be limited or incomplete.
...
This provides a plenty of framework-specific details that can be used to identify issues, track performance or observe user behavior.
...
It also adds span attributes that can be used to filter traces or spans, or to inspect additional details about an operation.

The following sections list the most relevant traces and spans created by the agent, as well as their attributes.
...
|`http.host`
|Set on all traces and root spans, and contains the host name that triggered the request.
Can be used to filter traces by host name.

|`http.route`
|Set on all traces and root spans, and contains either a view's route template, excluding any actual parameter values, or a file path for file and stream requests.
Can be used to filter traces for specific views.

|`http.target`
|Set on all traces and root spans, and contains either a view's actual route, including parameter values, or a file path for file and stream requests.
Can be used to check which parameters were provided to a view through its route.

|`vaadin.flow.version`
|Set on all traces and root spans, and contains the Flow version used by the application.

|`vaadin.request.type`
|Set on all traces and root spans, and contains the type of Flow request made against the application.
...
|Histogram
|Samples observations, like individual request durations, and distributes them into buckets. Each bucket counts the number of observations that fall into a specific value range.
Histograms are typically used to calculate quantiles.
They also provide a total sum of all observed values and the total count of observations. This allows calculations of averages.
...
Since the Observability Kit Agent is based on the OpenTelemetry Java agent, it also includes a number of non Vaadin-specific instrumentation that can be helpful in monitoring an application. For example, it can include database requests using the JPA API, or outgoing HTTP requests.
