...
The default instrumentation of the Observability Kit agent provides a reasonable amount of data out-of-the-box. However, it cannot cover all of the specifics of an application.
...
Custom instrumentation allows the execution of specific pieces of code to be recorded as spans. These will automatically be added to the current trace, and will provide additional details, such as attributes and errors.
...
To create a span manually, acquire a tracer instance from the OpenTelemetry API, and then start and end a span around the code that should be traced. An example of how that might look is shown below:
