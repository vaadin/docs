...
Instrumentation can be configured in the [filename]`agent.properties` file that is used with the Java agent.
...
It is also the only one required.
...
The default instrumentation is disabled because a single-page Vaadin application logging all requests was not helpful and generated plenty of data.
