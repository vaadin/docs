...
This creates a trace for every request that's related to a UI (e.g., `/products/:productId`), using the current view's route template as name.


=== Static File Requests

This creates a trace for all static files that are served (e.g., `/static/image.png`.), using the path as name.
...
|The route to where it was navigated.
...
Frontend traces provide spans with data related to operations and events initiated by the frontend application -- within the browser. Frontend Observability provides several instrumentation modules, which are detailed below.
