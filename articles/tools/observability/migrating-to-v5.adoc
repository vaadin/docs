...
The opt-in features are UI state size, database monitoring, and the two detail toggles that would add sensitive or high-cardinality data to what leaves the application.
...
|(none)
|`vaadin.client.*`
|Browser-observed timing (bootstrap, navigation, UIDL requests and rendering, Web Vitals), plus browser errors and connection-state transitions.
...
|New, opt-in per-route database metrics.
...
Version 5 emits spans for the request lifecycle, navigation, RPC, service-executor tasks, data provider count and fetch queries, and -- when database monitoring is on -- JDBC queries, through the Observation API.
Spans that would have been unbounded as timer tags, such as the invoked component and event, are attached as span-only attributes.
...
[NOTE]
Version 5 also retains the interactions that failed or ran over the UX budget -- and the data provider queries behind a slow component -- and serves them, grouped and ready to act on, from an Actuator endpoint.
This has no version 4 equivalent, and it's the fastest way from a user report to a line of code.
