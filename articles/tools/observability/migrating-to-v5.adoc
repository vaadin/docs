...
|(no equivalent)
|`vaadin.observability.sessions`, `.uis`, `.navigation`, `.requests`, `.errors`, `.resync`, `.database`, `.insights`
...
Version 5 emits spans for the core request lifecycle -- request handling, navigation, RPC, service-executor tasks, and, when database monitoring is on, JDBC queries -- through the Observation API.
...
[NOTE]
Version 5 also retains the interactions that failed or ran over the UX budget and serves them, grouped and ready to act on, from an Actuator endpoint.
