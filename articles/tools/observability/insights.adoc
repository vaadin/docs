...
In development mode the same insights also appear in the <<copilot-panel#,Copilot panel>>, which announces new ones as they appear, and their replay steps <<#dev-replay,say what the user had filled in>>.
...
It opens on these findings rather than on the numbers -- the endpoint's own payload, ranked with errors first -- and keeps the live `vaadin.*` meters below them.
It also announces a finding it hasn't seen before in the Copilot log, so you learn that something broke without having the panel open.

See the <<copilot-panel#,Copilot Panel>> page for the whole panel: how findings are ranked and read, how to set one aside, what the meter table shows, and what the panel needs to be there.
Nothing about it exists in production, where neither Copilot nor the dev-tools connection does.
