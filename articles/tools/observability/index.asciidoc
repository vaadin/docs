...
The core of this add-on is the <<advanced/observability-agent#,Observability Kit agent>>, which is a Java agent that is run together with a Vaadin Flow application to automatically collect telemetry data.
This includes:
...
|Logs
|The agent instruments popular logging frameworks, such as log4j or logback, to collect logs and correlate them with traces.
|===

Check out the next chapter to get started with using the add-on: <<getting-started#,Quick Start Tutorial>>.
