...
=== Account Setup
Start by setting up a Datadog account. Open the https://www.datadoghq.com/[Datadog website^] and click on `Free Trial`. Select the appropriate region for storing data. Then fill out the remaining details and follow their instructions.

In order to complete the sign-up process, you must install one of their agents and have it send data.
...
Download the binary from the https://github.com/open-telemetry/opentelemetry-collector-releases/releases/latest[latest release on GitHub^]. Make sure to download the `otelcol_contrib_` variant. It includes support for Datadog and the correct variant for your operating system. Then run the collector using the following command:
[source,Shell]
----
...
As a result, we provide the following work-arounds for collecting log data, and correlating them with the traces collected by the Observability Kit agent.
...
This class checks if the `trace_id` and `span_id` attributes have been set by the agent for this log event. It writes corresponding Datadog trace and span IDs to the JSON output.
...
To configure the Observability Kit agent, create an `agent.properties` file with the following content:
...
By default, the OTLP exporters assume that the OpenTelemetry collector is running locally. Therefore, there is no need to configure endpoints.
...
To get an idea of what kind of tags can be filtered, click on a trace. This brings up a detailed view with all tags recorded for the span.
