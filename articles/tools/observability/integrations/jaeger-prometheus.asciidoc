...
[IMPORTANT]
This setup is intended for local testing only. It is not production ready.

== Download and Run Jaeger

// tag::jaeger-setup[]

https://www.jaegertracing.io/download/[Download the latest version of Jaeger^] (tar.gz file) for your platform.
...
Download the latest version of Prometheus for your platform https://prometheus.io/download/[here^] (tar.gz or zip file).
Extract the contents of the downloaded archive.
...
On the right, there will now be a graph of timeline versus duration.
From here you can easily see events that are causing delays (i.e., larger circles), or errors (i.e., red circles).
Clicking one of the circles will open the <<#trace-detail,trace detail page>>.
...
If any of the spans have an error, it will be marked by a red exclamation (!) icon.
...
[NOTE]
If you don't know the metric name, you can use the metrics explorer (i.e., the Earth icon next to `Execute`).
