...
From there you can see events that are causing delays, indicated by larger circles, or errors, indicated by red circles.
...
If any of the spans has an error, it is be marked by a red exclamation (!) icon.
...
If you do not know the metric name, you can use the metrics explorer, by clicking the Earth icon next to [guibutton]*Execute*.
