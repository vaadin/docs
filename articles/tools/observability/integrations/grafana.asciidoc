...
Please refer to the Grafana documentation on how properly to configure and secure a self-hosted setup.
...
This command starts a set of services for collecting and displaying data from the Observability Kit agent:
...
To view traces, open the http://localhost:3000/explore[Explore view^], and select `Tempo` as the data source.
...
To view metrics, open the http://localhost:3000/explore[Explore view^], and select `Prometheus` as the data source.
Select a metric from the `Metric` dropdown (e.g., `process_runtime_jvm_memory_usage` to view JVM memory usage).
