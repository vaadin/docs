...
tab-title: New Relic
order: 10
description: Instructions for using New Relic
---
...
To avoid losing data, messages can be truncated automatically using the property `otel.attribute.value.length.limit` (see below).
...
From the view `All entities`, select under `Services - OpenTelemetry`, your service name: `vaadin` if you didn't specify one.

Now you will have a menu where you can look through the telemetry data from your application.
...
We provide a pre-configured dashboard for New Relic that contains several metrics, such as request throughput, error rate and JVM stats.
