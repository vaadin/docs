...
The findings need [propertyname]`vaadin.observability.insights`, which is on by default, together with the instrumentation that feeds each kind:
...
The panel says that insights aren't being collected only when nothing is watching at all: when `insights` is off, or when both `errors` and `requests` are.
That's the case worth distinguishing from an empty list that reads like "nothing is wrong".
Switching off only `data` or only `client` leaves the panel collecting, and simply drops that kind of finding.
