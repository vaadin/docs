...
The user may notice a temporarily offline notification or the loading indicator spawn, but browser page is not reload and his work can continue without losing any data.

Speaking about HTTP session, this means that attributes must be constantly stored in a shared/distributed storage, so that newly created HTTP sessions may filled with the most recent user data.


== Vaadin Kubernetes Kit Session Replication

Vaadin Kubernetes Kit helps with session replication, by transfer HTTP session data on a backed session storage (Hazelcast or Redis) at every request.
...
Objects that for any reasons cannot be serializable (e.g. third party classes) must be defined `transient`, so they are skipped by the serialization process.
Of course, after deserialization transient fields are null.

For Spring application, Vaadin Kubernetes Kit is also capable to overcome this limitation.
Transient fields referencing Spring beans are detected during serialization and metadata is stored along the session attributes.
...
A Vaadin application is composed by UI objects that acts as root nodes of graphs of components composition that represents the web page the user interact with in through the browser.
UIs are collected into [classname]`VaadinSession` that is stored in the HTTP session, to preserve the server side state across request.
...
When writing a component make sure that all its non transient members implements [classname]`Serializable`.

All Vaadin UI components and listeners are already marked as serializable, so if extending or implementing them it is not necessary to explicitly add the `Serializable` implementation statement.

However, it is mandatory to do that on non UI components that may be referenced in the UI graph.

For example, if a view is holding a reference to a DTO or a configuration object, it must be marked as serializable, and all its member must be serializable.
...
=== Transient Fields for Not Serializable Objects
