...
Second, we need to add getters for the save, delete, and close buttons so that we can use those from inside the [classname]`MainView` class to listen to events inside the form.
...
The Spring framework injects it here.
...
The Binder automatically adds [classname]`BeanValidator` which validates beans using Java Specification Requests (JSR) 303 specification.
...
This can occur, for example, if the user tries to save a contact with a blank email field.
<14> Clears the form
<15> Populates the form with the provided contact.


That's all. Now if we rerun the application, we are able to see the form populated with the contact that was selected from the grid.
