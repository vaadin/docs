...
Vaadin supports many pre-built <<{articles}/components#, layout components>>
...
NOTE: When Lumo theme is in use, the margin, padding and spacing properties accept t-shirt sizes which translate to the corresponding <<{articles}/components/ds-resources/foundation/size-space#space, spacing properties>>. Otherwise, the property values map directly to the corresponding CSS properties.
