...
Chances are Eclipse is running on an older version of Java. Install *Java 11 or newer*. Note that you can have multiple versions installed, so *make sure Eclipse uses the correct one.*
