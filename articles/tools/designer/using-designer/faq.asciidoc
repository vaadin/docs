...
Vaadin Designer layouting behaviour matches that of the components - it's a good idea to familiarize yourself with the appropriate component documentation.
...
By default, some distros don't have the correct libraries installed that are required by Chromium. Check the logs and install the appropriate libraries. For example, if you see these error messages:
