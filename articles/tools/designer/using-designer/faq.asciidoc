...
Yes. A maven multi-modules project can also have different Vaadin versions (from Vaadin 8 to the latest version) in seperate sub-modules.
...
----
{
  "--node.modules.project.name": "(module B's name)"
}
----
