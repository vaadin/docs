...
[[designer.layout]]
= [deprecated:com.vaadin:vaadin@V25.0]#Laying Out Views#
...
[NOTE]
When the Lumo theme is in use, the margin, padding and spacing properties accept t-shirt sizes which translate to the corresponding <<{articles}/styling/themes/lumo/lumo-style-properties/size-space#space, spacing properties>>. Otherwise, the property values map directly to the corresponding CSS properties.
