...
Vaadin Designer supports theming the same way as Flow (see <<{articles}/theming/creating-a-custom-theme, Custom Theme>> for more information).
When a design is opened, Vaadin Designer:

 * Modifies `package.json` file to add utility libraries. These changes should be commited to your version control system.
 ...
 * Static resources (images, fonts, ...) of the currently using theme are resolved.

When a style sheet under a custom theme is changed, Designer is automatically reloaded to apply the changes.

[NOTE]
When switching the theme under @Theme, all Designer visual editors need to be re-opened to apply the new theme. Also in Preview mode, the browser isn't auto reloaded when styles are changed, you will need to refresh the browser manually.
