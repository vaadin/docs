...
If you need to use Collaboration Engine powered features for more than 20 users / month, request a commercial license file.
...
A https://vaadin.com/collaboration#pricing[starting package] for 199 EUR / 500 users / month is available without the need for any other Vaadin commercial subscription.
...
This directory is used by Collaboration Engine to automatically store usage data to keep track of users for each month.
You should also put your `ce-license.json` file in the same directory for Collaboration Engine to find it.
...
Collaboration Engine can fire the following types of license events a maximum of once each during the license's lifecycle:
...
NOTE: Collaboration Engine throws an exception in production mode if a configuration hasn't been defined.
...
Collaboration Engine counts how many users use its features during each calendar month.
...
Your application will continue to work, but `CollaborationAvatarGroup` will show only the user's own avatar, and forms built with `CollaborationBinder` will not show edits made by other users.
...
Users from 501 through 520 can use the application, but it works as if Collaboration Engine wasn't in use.
...
Collaboration Engine only counts those users towards the quota whose `UserInfo` objects are passed to collaborative features.
