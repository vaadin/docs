...
[NOTE]
If you explicitly configure push to use `PushMode.MANUAL`, Collaboration Engine doesn't work, as it doesn't manually invoke `UI.push()` to send the user interactions to the other connected users.
