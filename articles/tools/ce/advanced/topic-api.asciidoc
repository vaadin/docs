...
This tutorial shows how to use the low-level Collaboration Engine APIs, by updating and subscribing to a topic map.
...
Since the example doesn't (yet) register any own listeners that need cleanup, it can just return `null`.
