...
Using Collaboration Engine with its default settings in an application running in a clustered environment would result in users being able to collaborate only with others connected to the same application instance (i.e., on the same node).
