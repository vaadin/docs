...
[IMPORTANT]
The Collaboration Engine backend for clustering support is an experimental feature. This means that its behavior, API, as well as its look and feel might change.
You may enable it with a <<Feature Flag>>.

Using Collaboration Engine with its default settings in an application running in a clustered environment would result in users being able to collaborate only with others connected to the same application instance, that is, on the same node.
