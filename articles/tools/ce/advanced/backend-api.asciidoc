...
To use the Backend API, it must be explicitly enabled with a feature flag.
Feature flag can be activated with one of the following methods:
...
3. Enable the [guilabel]`Collaboration Engine backend for clustering support` feature
