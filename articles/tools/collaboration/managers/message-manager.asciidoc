...
When implementing the [methodname]`fetchMessages()` method, it is important that the returned stream includes the messages sent exactly at the time that you can get with [methodname]`FetchQuery.getSince()`.
