...
Collaboration Kit includes a message manager to handle text messages in a topic.
...
<2> The [methodname]`setMessageHandler()` method takes a callback that is invoked when a new message is submitted.
...
NOTE: If a [classname]`MessageManager` instance is using the same topic ID of a xref:../components/collaboration-message-list#ce.collaboration-message-list[`CollaborationMessageList`], messages submitted with the manager also appears on the list.


[[ce.message-manager.persister]]
== Persisting Messages

The data in Collaboration Kit is by default stored only in the application memory.
This applies also to messages handled by [classname]`MessageManager`, which means that all the messages are lost when the server is restarted.
...
The fetched messages are stored in Collaboration Kit's memory and shared among other managers connected to the same topic, avoiding redundant back-end queries.
...
Then, the manager makes a query to the persister to fetch all the messages with a timestamp that's equal or more recent than the last message in memory.
The new messages are again stored in the Collaboration Kit's memory, updating all the manager instances that are connected to the same topic.

When implementing the [methodname]`fetchMessages()` method, it's important that the returned stream includes the messages sent exactly at the time that you can get with [methodname]`FetchQuery.getSince()`.
