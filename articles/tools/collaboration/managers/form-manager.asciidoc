...
Collaboration Kit includes a manager to set property values and field highlighting in a form, and to react to changes to them. It provides a simple way to create a custom form component with collaborative features.
...
<1> Creating a [classname]`FormManager` takes a component to bind the manager connection context to the UI, an instance of [classname]`UserInfo` that represents the current local user, and the topic identifier to connect to.
