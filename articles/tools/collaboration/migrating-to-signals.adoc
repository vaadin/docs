...
Tracked in https://github.com/vaadin/collaboration-kit/issues/147[collaboration-kit#147].
...
Shared signals have neither. Every constructor and conversion takes a plain [classname]`Class`, so the element type has nowhere to go.

The way this fails is worth knowing, because the obvious test passes. `new SharedValueSignal<Set<Role>>(Set.class)` doesn't compile, since `Set.class` is a `Class<Set>`. The constructor that takes an initial value does compile, and silently stores the erased runtime class:
...
The most faithful workaround keeps the stored JSON unchanged -- a bare array -- by holding a [classname]`JsonNode` and converting where a [classname]`TypeReference` is available:
