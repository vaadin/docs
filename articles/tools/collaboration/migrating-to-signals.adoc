...
= [since:com.vaadin:vaadin@V25.3]#Migrating from Collaboration Kit to Signals#
...
Collaboration Kit is deprecated and isn't included in Vaadin 26, so this migration is a prerequisite for that upgrade rather than an optional modernization. Vaadin tracks bringing the remaining Collaboration Kit features into Flow in https://github.com/vaadin/collaboration-kit/issues/138[collaboration-kit#138]. Most of the gaps below are open work rather than deliberate omissions, and each one links to its tracking issue where one exists.
...
Shared signals live in one JVM. Collaboration Kit doesn't, and that single difference is the one thing that can stop a migration:

*Clustering and session serialization*:: Collaboration Kit has an experimental <<advanced/backend-api#,`Backend` API>> that shares topic data between nodes, and it documents which of its classes are safe to keep in the HTTP session, which is what makes session replication work. Shared signals support neither. Serializing one fails fast rather than producing broken state, precisely because the state can't leave the node it was created on.

These are two symptoms of the same limitation and are expected to be solved together, since both come down to moving signal state out of a single JVM. Work on it is tracked in https://github.com/vaadin/platform/issues/8703[platform#8703], which covers the cluster event log and session serialization in one design. If the application needs either, add a reaction there -- the issue is prioritized partly on how many people ask for it -- and stay on Collaboration Kit until it lands.

<<#gaps,Gaps and Cases That Can't Be Migrated>> covers the reasoning and the observable symptoms in detail. That section also lists the features that *can* be migrated but only by rebuilding behavior Collaboration Kit provides out of the box, and the smaller API-level differences worth knowing before starting.
...
| Cleanup returned from [methodname]`whenAttached()`
...
Tie the pair to the lifetime of the view with [methodname]`whenAttached()`. The handler runs on every attach, and the [classname]`Registration` it returns runs on the matching detach:
...
Written by hand with a detach listener alone, this is a mistake that's easy to make and hard to see. A view that's detached and attached again -- navigating back to a retained view, a component moved between layouts, a dialog reopened -- then releases more times than it retains. The count reaches zero while the view is still open, the state is dropped, and the next user to open the same document gets a fresh [classname]`DocumentState` and silently stops sharing anything with them. A `@PreserveOnRefresh` view surviving a reload gets the opposite: it is re-attached without an intervening detach, so a hand-written pair retains twice and never releases. [methodname]`whenAttached()` handles both, clearing any live cleanup before running the handler again.
...
[methodname]`whenAttached()` is the correct construction.
====
...
A multi-select field needs its element type named, which is what [methodname]`CollaborationBinder::forField` used the two-class `forField(field, Set.class, Role.class)` form for. Pass a Jackson [classname]`TypeReference` instead:
...
The overload exists on [classname]`SharedValueSignal`, [classname]`SharedListSignal`, and [classname]`SharedMapSignal`, and on [methodname]`SharedNodeSignal.asValue()`, [methodname]`asList()`, and [methodname]`asMap()`. Use the [classname]`Class` form for everything else -- it's shorter, and only a parameterized type needs the type reference.
...
.Clean Up on Detach
[IMPORTANT]
Editor entries need the same cleanup as presence entries. A user who navigates away while a field is focused gets no `vaadin-highlight-hide`, so run [methodname]`clearEditor()` for the local user from the cleanup of a [methodname]`whenAttached()` scope.
...
Tracked in https://github.com/vaadin/platform/issues/8703[platform#8703]. If the application needs either of the two symptoms below, react on that issue and stay on Collaboration Kit until it lands -- there's no workaround that keeps the benefits of migrating.

Both come from the same place. A shared signal owns a local tree, and its state can't leave the node it was created on. Clustering and session serialization are what that costs you, and the design in platform#8703 addresses them together: a cluster event log for the first, and a serialization strategy for the second.
...
The API is designed with clustering in mind -- the tree, the command log, and the confirmation model are all in place -- and platform#8703 describes the missing piece as a strongly consistent event log per signal identifier, backed by whatever clustering technology the application already runs.
...
Writing every change through a shared database and polling it back reproduces neither the latency nor the transactional guarantees, and gives up the reason to use signals in the first place.
...
The exception is a deliberate placeholder rather than a final design decision: a signal with listeners from other sessions would drag those sessions into the serialized graph, and the deserialized copy would be stale. Failing loudly was chosen over either.
...
* *No scope on the write.* [methodname]`whenAttached()` ties an entry to a component correctly, including the awkward re-attach cases, but the application still writes that scope itself for every kind of entry. Nothing on the signal API says "this entry belongs to this UI or session".
...
| Presence tracking
| Build it
| A [methodname]`whenAttached()` scope over a list signal
...
* https://github.com/vaadin/platform/issues/8703[platform#8703] -- clustering and session serialization for shared signals. React there if you need it.
