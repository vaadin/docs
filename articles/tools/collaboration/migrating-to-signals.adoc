...
Clustering and session serialization support for shared signals are being built next. Both are specified together in https://github.com/vaadin/platform/issues/8703[platform#8703], because they're the same problem: a shared signal's state currently can't leave the node it was created on. Until that lands, an application that needs either should stay on Collaboration Kit, which supports both.

That limitation is the one thing that can stop a migration outright, and it shows up as two symptoms.

*Clustering*:: Collaboration Kit's [classname]`Backend` SPI, behind the `collaborationEngineBackend` <<{articles}/flow/configuration/feature-flags#,feature flag>>, replicates an ordered event log between nodes. Shared signals have no such SPI: every signal created through a public constructor owns a local tree, and the constructor documentation says outright that it doesn't support clustering.
+
...
+
An application that implemented a custom [classname]`Backend` has nothing to port it to, including the [interfacename]`MembershipListener` and [classname]`MembershipEvent` callbacks it used to clean up after a node that disappeared.

*Session serialization*:: Collaboration Kit <<advanced/serialization#,documents>> which of its classes are safe to keep in the HTTP session, and that split is what lets <<{articles}/tools/kubernetes#,Kubernetes Kit>> replicate sessions. Serializing a shared signal instead throws [classname]`NotSerializableException`, deliberately: a signal with listeners from other sessions would drag those sessions into the serialized graph, and the deserialized copy would be stale.
+
This reaches further than it first appears. A signal in a view field is reachable from the session, and so is one captured by the lambda behind any `bind*()` call, because the binding is stored on the component. Either makes the session graph unserializable. Not affected: the registry bean, since an application-scoped bean isn't session state, and <<{articles}/flow/ui-state/local-signals#,local signals>>, which are per-user.

platform#8703 covers the cluster event log for the first and the serialization strategy for the second. Adding a reaction there is worth doing if you need it, since the issue is prioritized partly on how many people ask.
...
Everything else in Collaboration Kit can be migrated. <<#gaps,What You Have to Build Yourself>> covers the parts that cost real work, and the smaller API-level differences worth knowing before starting.
...
[TIP]
====
Collaboration Kit marks its entry points [annotationname]`@Deprecated` for removal: [classname]`CollaborationEngine`, [classname]`CollaborationEngineConfiguration`, [classname]`CollaborationBinder`, [classname]`CollaborationBinderUtil`, [classname]`CollaborationAvatarGroup`, [classname]`CollaborationMessageList`, [classname]`CollaborationMessageInput`, [classname]`FormManager`, [classname]`MessageManager`, and [classname]`PresenceManager`.

Compiling against a version that carries those annotations turns the migration into a work list. Only the entry points are marked, so you get one warning per feature to migrate rather than one per declaration -- the supporting types, such as [classname]`UserInfo`, [classname]`TopicConnection`, and the maps and lists, are deliberately left alone.
...
The mapping in the previous sections covers the common cases. This section is the inventory of what migrates only at the cost of rebuilding something, and where the two APIs differ in ways that are easy to trip over. For the one thing that can stop a migration outright, see <<#when-not-to-migrate-yet,When Not to Migrate Yet>>.
...
Tracked in https://github.com/vaadin/flow/issues/25867[flow#25867].
...
Tracked in https://github.com/vaadin/flow/issues/25865[flow#25865].
...
Tracked in https://github.com/vaadin/collaboration-kit/issues/146[collaboration-kit#146].
...
Tracked in https://github.com/vaadin/flow/issues/25864[flow#25864].
...
Tracked in https://github.com/vaadin/flow/issues/25868[flow#25868].
...
Tracked in https://github.com/vaadin/flow/issues/25866[flow#25866].
...
Use this to confirm the migration covers everything before removing the Collaboration Kit dependency. *Direct* means the signals API does the same job; *Build it* means the behavior is reachable but you write it; *Missing* means there's no equivalent. <<#gaps,What You Have to Build Yourself>> explains each of the last two, and <<#when-not-to-migrate-yet,When Not to Migrate Yet>> covers the single *Missing* row that can stop a migration.
...
* https://github.com/vaadin/platform/issues/8703[platform#8703] -- clustering and session serialization for shared signals, being built next. React there if you need it.
* https://github.com/vaadin/flow/issues/23868[flow#23868] -- a collaborative binder in Flow, built on signals.
* https://github.com/vaadin/flow/issues/25865[flow#25865] -- resolving a shared signal by identifier, and discarding unused state.
* https://github.com/vaadin/flow/issues/25867[flow#25867] -- tying the lifetime of a shared signal entry to a UI or session.
* https://github.com/vaadin/flow/issues/25864[flow#25864] -- keeping shared state consistent with a database.
* https://github.com/vaadin/flow/issues/25868[flow#25868] -- access to the previous value when a value changes.
* https://github.com/vaadin/flow/issues/25866[flow#25866] -- an emptiness condition for list transactions.
* https://github.com/vaadin/flow/issues/23659[flow#23659] -- binding items of a data component to a list signal.
* https://github.com/vaadin/collaboration-kit/issues/146[collaboration-kit#146] -- presence tracking and user color allocation.

.Where This Guide Falls Short
[TIP]
If something in your application doesn't map onto anything here, say so in whichever issue above comes closest, or open a new one against https://github.com/vaadin/flow/issues[Flow] describing the Collaboration Kit feature you're replacing and what you tried. The gaps listed in this guide were found that way, and several were closed the same way. A concrete use case is worth more than a feature request: it's what decides whether a gap gets filled and how.
