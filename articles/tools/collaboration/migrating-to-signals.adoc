...
Collaboration Kit is superseded by signals: its entry points are deprecated for removal, and it isn't included in Vaadin 26. This migration is a prerequisite for that upgrade rather than an optional modernization. Vaadin tracks bringing the remaining Collaboration Kit features into Flow in https://github.com/vaadin/collaboration-kit/issues/138[collaboration-kit#138]. Most of the gaps below are open work rather than deliberate omissions, and each one links to its tracking issue where one exists.
...
Shared signals live in one JVM. Collaboration Kit doesn't, and that single difference is the one thing that can stop a migration outright. It shows up as two symptoms.
...
Both come down to moving signal state out of a single JVM, and they're expected to be solved together. Work on it is tracked in https://github.com/vaadin/platform/issues/8703[platform#8703], which covers the cluster event log and the serialization strategy in one design. If the application needs either, add a reaction there -- the issue is prioritized partly on how many people ask for it -- and stay on Collaboration Kit until it lands.

Writing every change through a shared database and polling it back is not a workaround worth recommending: it reproduces neither the latency nor the transactional guarantees, and it gives up the reason to use signals in the first place.
