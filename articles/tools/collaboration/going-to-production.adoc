---
title: Production 
page-title: How to deploy Vaadin Collaboration Kit to production
description: Preparing a Vaadin application for production that uses Collaboration Kit.
meta-description: Learn how to deploy Vaadin Collaboration Kit to production. Explore best practices for secure setup, licensing, and optimizing real-time collaboration features.
order: 999
---


[[ce.production]]
= [deprecated:com.vaadin:vaadin@V24.5]#Configuring Collaboration Kit for Production#

.[since:com.vaadin:vaadin@V24.5]#Collaboration Kit is Now Free#
[NOTE]
Collaboration Kit features are free for all users without a license. Licensing and user quota configuration isn't required.

Enabling production mode for a Vaadin application isn't difficult: it's described in <<{articles}/flow/production#, Deploying to Production>>. However, if your application uses Collaboration Kit, it requires some extra steps to prepare it for production.
...
----
include::./MyVaadinInitListener.java[tags=init-listener;configuration;event-handler;event-handler-ref;!*,indent=0]
----
...
Another way to provide the license file storage location is to pass the data directory as a parameter on server startup. This method assumes that you already have a production-ready build available, and that you want to run the package on the server. Read <<{articles}/flow/production#, Deploying to Production>> to learn more about building your application for production.
...
The directory should be both readable and writable by the user running the Vaadin application. See <<{articles}/flow/configuration/properties#,Configuration Properties>> for more information.
...
[source,java]
----
include::./MyVaadinInitListener.java[tags=event-handler;!event-handler-ref,indent=0]
----
...
[source,java]
----
include::./MyVaadinInitListener.java[tags=send-email,indent=0]
----
