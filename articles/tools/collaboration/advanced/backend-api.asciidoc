...
Since our implementation only depends on a single Hazelcast [classname]`IList`, it's very easy to implement the [methodname]`openEventLog` method and we do so by returning a new instance of the [classname]`HazelcastEventLog` with the list named after the `logId` parameter.
...
For our Hazelcast implementation, we return the local member identifier.
