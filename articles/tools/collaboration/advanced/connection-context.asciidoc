...
----
<1> The asynchronous function uses Spring's [classname]`@Async` annotation. It needs to receive the [classname]`CollaborationEngine` as a parameter, as [methodname]`CollaborationEngine.getInstance()` would throw an exception when running in a background thread. For example, the following would fail:
+
[source,java]
----
