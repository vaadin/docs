...
A [classname]`SystemConnectionContext` is currently available, in addition to the [classname]`ComponentConnectionContext`, which is the default that's used implicitly.
...
[classname]`SystemConnectionContext` is a connection context that's always active.
