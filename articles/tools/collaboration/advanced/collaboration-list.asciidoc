...
When you have gotten the list instance, you can insert values at the end of the list with the [methodname]`insertLast()` method.
...
|`insertFirst()`
| static
| This value is inserted at the beginning of the list.

| `insertLast()`
| static
| This value is inserted at the end of the list.

| `insertBefore()`
| static
| The given value is inserted immediately before a specified key.

| `insertAfter()`
| static
| The value is inserted immediately after a specified key.

| `insertBetween()`
| static
| This is shorthand for `insertAfter(value, keyBefore).ifNext(keyBefore, keyAfter)`.

| `ifFirst(key)`
| conditional
| This indicates the specified `key` must be the first in the list.

| `ifLast(key)`
| conditional
| The specified `key` must be the last in the list.
