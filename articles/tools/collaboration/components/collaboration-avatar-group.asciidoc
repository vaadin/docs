...
the user's avatar is added to the group for all users.
...
[NOTE]
The `User` and `UserService` classes aren't available in Collaboration Kit.
...
After running the application and opening the view, you should see your own avatar.
...
an automatically generated abbreviation is displayed in the avatar.
...
The name is also presented in the overflow drop-down;
when all the avatars don't fit into the group,
the last avatar, which has text such as "+3", can be clicked to show
the overflowing avatars and names in a drop-down.
...
Setting an image isn't a replacement for the `name` property.
When an image is provided, `name` is still displayed in a tooltip on hover
and in the overflow drop-down.
...
that `StreamResource` isn't serializable as JSON,
so it can't be included in `UserInfo` and sent back and forth
through Collaboration Kit.
...
different topics.
Think about a form view that's used for editing different entities,
...
Setting `null` topic disables the connection to Collaboration Kit.
