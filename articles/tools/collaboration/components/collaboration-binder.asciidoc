...
Next, we'll cover the special cases where you need to do a little bit more than bind a property to a field.

=== Non-Primitive Value Types

Collaboration Kit supports only a limited set of primitive-like value types,
...
In this example, the `Person` bean that we're editing, has a reference to the person's
...
In this example, we're binding an enum property of the bean to a `Checkbox`,
so the value type `boolean` needs to be provided:
