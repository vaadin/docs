...
== Feature Limitations
Collaboration Kit is production-ready and stable, however the following features are still under development, and are not currently available:
...
* Collaboration between multiple nodes or pods in a cluster of application servers is not supported.
  A future version will enable running Collaboration Kit as a standalone server or a cloud deployment that can be used from multiple application servers.
