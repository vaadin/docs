...
page-title: Vaadin Collaboration Kit
description: Deprecated. Add real-time collaboration features to your Vaadin application.
meta-description: Vaadin Collaboration Kit is deprecated and isn't included in Vaadin 26. Use signals for real-time collaboration features instead.
order: 60
...
.Collaboration Kit Is Deprecated
[IMPORTANT]
Collaboration Kit is deprecated and isn't included in Vaadin 26. Use <<{articles}/flow/ui-state/shared-signals#,shared signals>> instead: they're built into Vaadin Flow, need no extra dependency, and cover the same real-time, multi-user state. For a feature-by-feature mapping, and for the cases that don't have a signals equivalent yet, see <<migrating-to-signals#,Migrating from Collaboration Kit to Signals>>. Plan the migration before the upgrade rather than during it.

Collaboration Kit is open source, licensed under the https://www.apache.org/licenses/LICENSE-2.0[Apache License 2.0].
...
Vaadin Flow has built-in <<{articles}/flow/ui-state/shared-signals#,shared signals>> that cover most of what Collaboration Kit does, without an extra dependency. Because Collaboration Kit isn't included in Vaadin 26, an application that uses it has to move to signals before upgrading.

The two can run side by side, in the same application and even in the same view, so the move can be made one view or one feature at a time rather than in a single step.

See <<migrating-to-signals#,Migrating from Collaboration Kit to Signals>> for a feature-by-feature mapping, the code for each of the high-level use cases, and the cases that don't have a signals equivalent yet.
