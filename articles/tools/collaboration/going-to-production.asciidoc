...
page-title: Production Configuration
...
If the number of users in a month exceeds 10 times the quota during the grace period, or if it exceeds the licensed quota after the grace period ends, the collaborative features are disabled for the users in excess of the base quota. Your application will continue to work, but `CollaborationAvatarGroup` will show each user only their avatar, and forms built with `CollaborationBinder` won't show edits made by other users. The users who are registered within the allowed quota, however, will have collaborative features available throughout the month.
...
You can change your quota at any time. When you replace a license file with a new one, Collaboration Kit resets the grace period. If you exceed your new quota after that, you will receive a new 30-day grace period.
...
Collaboration Kit counts for the quota only those users whose `UserInfo` objects are passed to collaborative features. That means that you can limit usage to a subset of your users. There are two ways to do this:
