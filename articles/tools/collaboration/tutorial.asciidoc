...
[NOTE]
If you explicitly configure push to use `PushMode.MANUAL`, Collaboration Kit doesn't work, as it doesn't manually invoke `UI.push()` to send the user interactions to the other connected users.
Collaboration Kit works with any of the transport modes for push.
...
This tutorial uses an id based on the UI instance instead of a user-based id.
...
A topic can be compared to a chat room - it has an id that's unique in the context of the application.
...
In this example, there is only one topic to work on, so you can use any hard-coded topic id.

If the application was expanded so that there would be a view for editing person entities, you'd need to have unique topic ids for each entity.
...
To enable collaboration with the text field and date picker components, use a class called `CollaborationBinder`.
...
To initialize a collaboration binder, you need to provide the type that is edited, as well as the local user's information.
After initializing, use the regular binder methods to bind the person object's name property to the text field component, and the date of birth property to the date picker component.

Finally, set the topic to connect to (the same as for `CollaborationAvatarGroup`) and a supplier for the initial bean value that populates the fields when the first user connects to the topic.
The supplier could load the editable item from a backend, but in this example it populates the fields with an empty `Person` object.
