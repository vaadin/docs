...
[NOTE]
If you explicitly configure push to use `PushMode.MANUAL`, Collaboration Kit does not work, as it doesn't manually invoke `UI.push()` to send the user interactions to the other connected users.
