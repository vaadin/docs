...
To initialize a collaboration binder, we need to provide the type that is edited, as well as the local user's information.
...
Finally, we set the topic to connect to (the same as for `CollaborationAvatarGroup`) and a supplier for the initial bean value that populates the fields when the first user connects to the topic.
