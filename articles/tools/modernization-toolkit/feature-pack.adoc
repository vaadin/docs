...
In addition to various layouts and components, Vaadin Feature Pack also supports blocking dialogs. Typical desktop applications wait for the user's response over a modal dialog and proceed execution after that. This is not the case with web applications in general. Accepting a user response while the serving thread is waiting at the server side requires Virtual Threads from Java 21. This is a separate artifact that you need to refer explicitly in order to use it and not included in Feature Pack components by default.
