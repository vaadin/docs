...
The Analyzer will prompt for two options: a local summary analysis (output in the console), or it can send a request for free detailed report to Vaadin. In both cases The Analyzer connects to the Vaadin servers to download the most up-to-date coverage data.
