...
When a Swing application embedded via `SwingBridge` fails to start — for example because the configured main class cannot be loaded, the AWT toolkit cannot initialize, or no `JFrame` becomes visible within the launch timeout of 5 seconds — the canvas area is replaced with an in-page error view that shows:
