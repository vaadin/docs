...
[NOTE]
====
Server push must be enabled with the [annotationname]`@Push` annotation on your application shell class — without it the view stays empty, because the Swing frame updates never reach the browser. The skeleton starter has the annotation in place; for a from-scratch project, see <<installation-from-scratch#swing-bridge.installation-from-scratch.push, Enable Server Push>>.
====
