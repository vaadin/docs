...
meta-description: Bring your Swing JAR into a SwingBridge project, choose a JAR-loading strategy, and wrap the application in a Vaadin Flow view.
order: 3
---
...
. *Create a Vaadin Flow view* that wraps the Swing application's `main` class.

The instructions apply equally whether you started from the skeleton starter (<<getting-started#, Getting Started>>) or set up the project from scratch (<<installation#, Installation>>).
...
To use a custom directory, pass `-Dapplibs.dir=<path>` on the command line or configure it under `<systemPropertyVariables>` in your `pom.xml` (see <<installation#, Installation>>).
