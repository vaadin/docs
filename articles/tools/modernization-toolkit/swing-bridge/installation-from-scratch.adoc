...
[NOTE]
====
Set `java.version`, not `maven.compiler.source`. The Spring Boot parent derives `maven.compiler.release` from `java.version`, and `release` overrides `source` and `target` — so a pom that only sets `maven.compiler.source` compiles to the parent's default bytecode level while appearing to target Java 21.
====
...
The trailing `-agentlib:jdwp=…` flag enables remote debugging on port 5005, bound to the loopback interface so that only a debugger on the same machine can attach. Use `address=*:5005` only when you need to attach from another host, and never on a machine reachable from outside. Remove it (or change `suspend=n` to `suspend=y`) to suit your needs — see <<running-and-debugging#swing-bridge.running-and-debugging.remote-debug, Remote Debugging>>.
