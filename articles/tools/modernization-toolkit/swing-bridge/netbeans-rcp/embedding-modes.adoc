...
`embedTopComponent(preferredId)` switches the instance to render a single `TopComponent` chrome-less — no menu bar, toolbar, status bar, or docking, only the view's content. It is a per-instance, fluent switch on the same component and returns the bridge for chaining:
...
| Switches this instance to render the named `TopComponent` chrome-less. Per-instance; call before the component attaches. Returns `this`.
