...
To satisfy organizations with strict security policies on installing tools in their network, the Modernization Toolkit Analyzer for Maven is limited in a few ways.

First, the Analyzer doesn't change your code. It writes all output to the console, not the disk.

The Analyzer connects to Vaadin servers to retrieve data files, but it doesn't send any information to Vaadin. You have full discretion over what about your application or the Analyzer report you might discuss with Vaadin. The Analyzer tool doesn't have a feature for transmitting information. Therefore, you can't use it intentionally or accidentally to send information.
...
The most common pattern for Swing applications is to search for `java.awt;javax.swing` However, there are many other libraries that are popular in Swing applications like JGoodies or JDesktop. To analyze a Swing application, for a more complete pattern, you might use, `javax.swing;java.awt;com.jgoodies;org.jdesktop.layout`.
