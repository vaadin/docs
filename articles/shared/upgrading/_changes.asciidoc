...
Fusion frontend code is moved from `@vaadin/flow-frontend` to `@vaadin/fusion-frontend`.
It means that the Fusion module imports need to be updated correspondingly.

An example of changing the import of Fusion `EndpointError` from
[source,typescript]
----
...
This means that you need to add explicit security annotations the endpoints you want to make accessible (either at the class level or the method level).
...
See how the use the flag in <<../guide/configuration/index#, Configuration Properties>>.
