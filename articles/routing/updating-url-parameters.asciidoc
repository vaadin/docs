...
description: To preserve deep linking developer must sometimes maintain the Route parameters and Query parameters manually
order: 25
layout: page
---
...
Route parameters, Route templates and Query Parameter are most often defined in a link pointing to a specific view with the desired state and when navigating to view the Router calls the view's [methodname]`setParameter` method.
