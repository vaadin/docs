...
The optional may be empty if the target view is not immediately available for some reason. This may happen for example because of security constraints or if the navigation was cancelled or posponed in navigation lifecycle events TODO link.
