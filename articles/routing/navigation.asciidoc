...
If deep linking is required, the target view must <<route-parameters#maintaining-route-parameters-without-navigation,maintain enough details about the UI state in the URL>> or developer must use Route Parameters to pass the data instead of the direct Java API.
