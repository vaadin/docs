...
[NOTE]
Since Flow 23.2 new constructor signatures have been introduced. Using ambiguous constructor calls will cause build errors. For example `new RouterLink(null, viewClass)` can refer to both `public RouterLink(String text, Class<? extends Component> navigationTarget)` and `public RouterLink(Router router,
