...
.Using ambiguous constructor calls will cause build errors
[CAUTION]
For example, [methodname]`new RouterLink(null, viewClass)` can refer to both [methodname]`public RouterLink(String text, Class<? extends Component> navigationTarget)` and [methodname]`public RouterLink(Router router,
