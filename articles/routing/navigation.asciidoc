...
Note that, with this code, Vaadin does not have the data to maintain a deep link to edit the very same user. 
In many use cases, this is fine, but if you expect to share links to the target view, the target view must maintain enough details in the URL and deserialize the parameters, as described in <<route-parameters#maintaining-route-parameters-without-navigation, Maintaining Route Parameters without Navigation>>, or use Route Parameters to pass the data.
...
If your target view implements HasRouteParameters, you can submit trivial data to the target view as RouteParameters. Compared to directly interacting with the target views Java API, this method automatically maintains the URL for deep linking. However, you can't pass more complex objects as route parameters as such.
