...
_See the <<{articles}/flow/routing#, corresponding article for the Flow/Java API>>._
...
Routing in Java views is described in <<{articles}/flow/routing#, `Defining Routes [in Java]`>>.
...
The server-side Java Component API (Flow) in Hilla provides a client module that acts as a bridge between a client router and server routes.
It is also possible to integrate server-side Hilla with any JavaScript-based routing solution.
...
It is automatically included into any Hilla application and can be imported into any TypeScript file as `import { Flow } from '@vaadin/flow-frontend';`.
