...
In certain cases, a developer might want to maintain the parameters in the browser URL, such as if the edited entity in a form changes via an instance method instead of via passing URL parameters.
In this case, the developer can use the [classname]`History` object to maintain the [classname]`RouteParameters`.

*Example*: A view that updates the deep linking parameters based on a parameter given in a public API.
