...
The [classname]`RouteConfiguration` class allows you to configure routes to a specific scope.
You can configure routes to:

* all users using the *application* scope, or
* only certain active users using the *session* scope.

You can access the scope using the [methodname]`forSessionScope()` and [methodname]`forApplicationScope()` static methods.
...
A route set for the user can override a route with the same path from the application scope.
...
The main route URL is returned by the [methodname]`getUrl()` methods in [classname]`RouteConfiguration`.
Any additional registered route is considered to be as a route alias.
