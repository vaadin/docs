...
This returns a `404` HTTP response and displays the text specified in the parameter to [methodname]`setText()`. Exceptions are matched first by exception cause, then by exception super type.
...
You can use one of the [methodname]`rerouteToError()` method overloads. You need only add the exception class to the target, and a custom error message, where necessary.
