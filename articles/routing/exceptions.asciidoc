...
Exception targets work in the same way as regular navigation targets, except that they don't typically have a specific `@Route`, because they're shown for arbitrary URLs.
...
If using a custom [interfacename]`ErrorHandler`, see the  <<{articles}/advanced/custom-error-handler#, Showing Error Parameter Views For Non Navigation Exceptions>> page on how to use this feature.
