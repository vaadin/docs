...
Vaadin shows the registered error views if an exception occurs outside routing and rerouting. An example this is when a button click event is fired during a remote procedure call (i.e., RPC) on a server. 
