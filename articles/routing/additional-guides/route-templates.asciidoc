...
It is, therefore, recommended to use route templates only if your use case is not achievable using <<../route-parameters#, route parameters>>.
...
A route template is a sequence of segments `/*seg_1*/*seg_2*/.../*seg_n*`, where each segment is either a fixed segment (a string that does not start with `:`) or a parameter segment 
according to the syntax rule given in the next paragraph.
...
* `_parameter_name_` `parameter_name` is the name of the parameter whose value to retrieve when a URL matching the template is resolved on the server. It must be prefixed by a colon (`:`).
...
The final route template is the result of the value composition from the `@Route`, `@RouteAlias` and `@RoutePrefix` annotations as explained in <<{articles}/routing/layout#,Router Layouts and Nested Router Targets>>.
