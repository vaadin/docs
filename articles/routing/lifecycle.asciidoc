...
.Event call order
[NOTE]
`BeforeLeaveEvent` is first sent to manually registered `BeforeLeaveListener` instances and then to `BeforeLeaveObserver` instances found connected to the UI.

.Listener order
[NOTE]
`BeforeLeaveListener` instances are called in the order they were registered. `BeforeLeaveObserver` instances are called in descendant order staring from the UI.
...
.Event call order
[NOTE]
`BeforeEnterEvent` is first sent to the target component if it implements `HasUrlParameter`, then to manually registered `BeforeEnterListener` instances and lastly to `BeforeEnterObserver` instances found connected to the UI.
